...
class WriteOperationE2ESmokeTests(unittest.TestCase):
    ...
    def test_relation_preview_sampling_repeats_only_preview_and_withdraw_uses_last_identity(
        ...
        def request_fn(
            ...
            if url.endswith(write_operation_e2e_smoke.CONFIRM_PREVIEW_PATH):
    ...
    def test_reversible_scenario_matches_registered_shape_consumers_and_bounded_rows(self) -> None:
        ...
        for checkpoint in (*scenarios[0].checkpoints, scenarios[0].recovery_checkpoint):
            ...
            self.assertEqual(preview.path, f"/api/workbench/actions/{checkpoint.name}/preview")
    ...
    def test_reversible_scenario_rejects_four_scopes_for_one_affected_page(self) -> None:
        ...
        with TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "scenario.json"
            path.write_text(json.dumps([scenario]), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "at most three exact scope probes"):
                write_operation_e2e_smoke.load_scenarios(path, http_target_ms=1000)
    ...
    def test_inactive_test_owned_workbench_preimage_does_not_run_recovery(self) -> None:
        with TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "scenario.json"
            path.write_text(
                json.dumps([_raw_bank_oa_invoice_scenario("bank-oa-invoice", "inactive-preimage")]),
                encoding="utf-8",
            )
            scenario = write_operation_e2e_smoke.load_scenarios(path, http_target_ms=1000)[0]
        ...
        def request_fn(
            ...
            if "page-audit" in url:
                status_code = 200
                payload = _system_audit_payload("system-audit:inactive-preimage")
            elif method == "POST" and url.endswith(write_operation_e2e_smoke.WITHDRAW_PREVIEW_PATH):
                status_code = 400
                payload = {"error": "workbench_relation_not_found"}
            else:
                raise AssertionError(f"unexpected read-only preimage request: {method} {url}")
            return http_slo_probe.HttpProbeResponse(
    ...
    def test_unknown_test_owned_workbench_preimage_fails_closed_before_mutation(self) -> None:
        with TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "scenario.json"
            path.write_text(
                json.dumps([_raw_bank_oa_invoice_scenario("bank-oa-invoice", "unknown-preimage")]),
                encoding="utf-8",
            )
            scenario = write_operation_e2e_smoke.load_scenarios(path, http_target_ms=1000)[0]
        ...
        def request_fn(
            ...
            if "page-audit" in url:
                status_code = 200
                payload = _system_audit_payload("system-audit:unknown-preimage")
            elif method == "POST" and url.endswith(write_operation_e2e_smoke.WITHDRAW_PREVIEW_PATH):
                status_code = 409
                payload = {"error": "workbench_relation_version_conflict"}
            else:
                raise AssertionError(f"unexpected read-only preimage request: {method} {url}")
            return http_slo_probe.HttpProbeResponse(
                status_code=status_code,
