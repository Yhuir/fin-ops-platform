...
class NoOaBankBatchApplicationServiceTests(unittest.TestCase):
    ...
    def test_list_batches_reads_current_canonical_facts_without_refresh_state(self) -> None:
        ...
        first = service.list_batches_payload({"month": ["2026-03"], "bucket": ["unsubmitted"]})
        rows[0]["debit_amount"] = "2.00"
        rows[0]["amount"] = "2.00"
        second = service.list_batches_payload({"month": ["2026-03"], "bucket": ["unsubmitted"]})

        self.assertEqual(first["summary"]["total_amount"], "1.00")
        self.assertEqual(second["summary"]["total_amount"], "2.00")
        self.assertNotIn("read_model_status", second)
        self.assertNotIn("refresh_enqueued", second)
        self.assertNotIn("refresh_reason", second)
