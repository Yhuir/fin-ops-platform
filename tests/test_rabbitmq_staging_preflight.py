...
class RabbitMqStagingPreflightTests(unittest.TestCase):
    ...
    def test_preflight_runs_real_checks_with_runtime_env_fallbacks(self) -> None:
        ...
        self.assertNotIn("workbench.read_model.refresh", dispatcher_command)
        self.assertNotIn("bank_detail.read_model.refresh", dispatcher_command)
        ...
        self.assertNotIn("output_invoice_collection.read_model.refresh", dispatcher_command)
        self.assertNotIn("oa_pending_payment.read_model.refresh", dispatcher_command)
        self.assertNotIn("bank_account_balance.read_model.refresh", dispatcher_command)
        self.assertNotIn("cost_statistics.read_model.refresh", dispatcher_command)
        self.assertNotIn("tax_offset.read_model.refresh", dispatcher_command)
