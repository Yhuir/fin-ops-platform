...
class _CommandService:
    ...
    def update_relation_metadata_for_case_id(self, **kwargs: object) -> dict[str, object]:
        ...
        for relation in self.relations:
            ...
            amount_check = kwargs.get("amount_check")
            if isinstance(amount_check, dict):
                relation["amount_check"] = deepcopy(amount_check)
...
def _run(
    ...
    *,
    ...
    canonical_rows: dict[str, dict[str, object]] | None = None,
    refreshes: list[dict[str, object]] | None = None,
) -> dict[str, object]:
    output = io.StringIO()
    ...
    with (
        ...
        patch.object(
            ...
            return_value=canonical_rows or {},
        ),
        patch.object(
            repair_ops,
            "refresh_after_workbench_requirement_repair",
            side_effect=lambda _app, months, **kwargs: (
                refreshes.append({"months": list(months), **deepcopy(kwargs)})
                or {
                    "months": list(months),
                    "workbench_relation_enqueued": True,
                    "workbench_enqueued": True,
                    "matching_dirty_months": list(months),
                }
            ),
        ),
        patch.object(
...
class WorkbenchRelationRequirementRepairOpsTests(unittest.TestCase):
    @staticmethod
    def _reapply_relation() -> dict[str, object]:
        tag_code = "expense:engineering_service:personnel_insurance"
        return {
            "case_id": "case-income-no-oa",
            "status": "active",
            "relation_mode": "manual_confirmed",
            "month_scope": "2026-05",
            "row_ids": ["bank-income", "invoice-blue", "invoice-red", "invoice-final"],
            "row_types": ["bank", "invoice", "invoice", "invoice"],
            "special_metadata": {
                "paired_requirement_source": "bank_transaction_paired_policy",
                "paired_requirement_tag_codes": [tag_code],
                "paired_requirement_tag_code": tag_code,
                "paired_requirement_version": 6,
                "requires_oa": True,
                "requires_invoice": True,
                "unrelated": "preserved",
            },
            "amount_check": {"status": "matched", "direction": "unknown"},
            "created_at": "2026-07-20T10:00:00+08:00",
            "updated_at": "2026-07-21T10:00:00+08:00",
        }

    @staticmethod
    def _reapply_canonical_rows() -> dict[str, dict[str, object]]:
        return {
            "bank-income": {
                "id": "bank-income",
                "type": "bank",
                "credit_amount": "182400.00",
            },
            "invoice-blue": {
                "id": "invoice-blue",
                "type": "invoice",
                "invoice_type": "销项发票",
                "total_with_tax": "203400.00",
            },
            "invoice-red": {
                "id": "invoice-red",
                "type": "invoice",
    ...
    def test_explicit_rule_reapply_recalculates_canonical_amount_and_refreshes_exact_scope(self) -> None:
        command = _CommandService([self._reapply_relation()])
        canonical_rows = self._reapply_canonical_rows()
        rules = _rules(requires_oa=False, requires_invoice=True)
        ...
        dry_run = _run(command, args, rules=rules, canonical_rows=canonical_rows)

        self.assertTrue(dry_run["explicit_rule_reapply"])
        self.assertEqual(dry_run["target_relation_count"], 1)
        self.assertEqual(dry_run["repair_reason_counts"], {"explicit_rule_reapply": 1})
        ...
        relation = command.relations[0]
        self.assertFalse(relation["special_metadata"]["requires_oa"])
        self.assertTrue(relation["special_metadata"]["requires_invoice"])
        self.assertEqual(relation["special_metadata"]["paired_requirement_version"], 7)
        self.assertEqual(relation["special_metadata"]["unrelated"], "preserved")
        self.assertEqual(
            relation["amount_check"],
            {
                ...
                "direction": "receipt",
                "oa_total": None,
                "bank_total": "182400.00",
                "invoice_total": "182400.00",
                "oa_amount": None,
                "bank_amount": "182400.00",
                "amount_delta": "0.00",
                "mismatch_fields": [],
                "requires_note": False,
            },
    ...
    def test_explicit_rule_reapply_rollback_restores_metadata_and_amount_preimage(self) -> None:
        ...
        rollback = _run(
            command,
            ["--rollback", "--expected-fingerprint", fingerprint],
        )

        self.assertEqual(rollback["written_relation_count"], 1)
        self.assertTrue(rollback["explicit_rule_reapply"])
        self.assertEqual(command.relations[0]["special_metadata"], original["special_metadata"])
        self.assertEqual(command.relations[0]["amount_check"], original["amount_check"])

    def test_explicit_rule_reapply_fails_closed_on_rule_or_canonical_amount_drift(self) -> None:
        command = _CommandService([self._reapply_relation()])
        canonical_rows = self._reapply_canonical_rows()
        canonical_rows["bank-income"]["credit_amount"] = "182399.99"
        args = [
            "--dry-run",
            "--reapply-case-id",
            "case-income-no-oa",
        ...
        with self.assertRaisesRegex(RuntimeError, "amount check is not exact"):
            _run(
                command,
                args,
                rules=_rules(requires_oa=False, requires_invoice=True),
        ...
        with self.assertRaisesRegex(RuntimeError, "rule version changed"):
            _run(
                command,
                args,
                rules={**_rules(requires_oa=False, requires_invoice=True), "version": 8},
                canonical_rows=self._reapply_canonical_rows(),
            )
