...
class PrometheusMetricsTests(unittest.TestCase):
    def test_render_prometheus_metrics_exports_runtime_api_and_worker_metrics(self) -> None:
        ...
        self.assertNotIn("finops_read_model_dirty_scopes", rendered)
        self.assertNotIn("finops_stale_dirty_scope_count", rendered)
        self.assertNotIn("finops_read_model_refresh_duration_ms", rendered)
        self.assertNotIn("finops_read_model_refresh_by_key", rendered)
        ...
        self.assertIn(
            'finops_worker_heartbeat_lag_seconds{status="available",worker_instance="oa-sync",worker_kind="oa-sync"} 2.5',
            rendered,
        )
