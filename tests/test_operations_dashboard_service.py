...
class OperationsDashboardServiceTests(unittest.TestCase):
    ...
    def test_runtime_repository_bounds_read_model_duration_history_query(self) -> None:
        ...
        self.assertIn("ranked_refresh_events as", normalized_sql)
        ...
        self.assertIn("row_number() over ( partition by event_type order by updated_at desc )", normalized_sql)
        self.assertIn("where runtime_metric_rank <= %s", normalized_sql)
        self.assertIn("'-infinity'::timestamptz", normalized_sql)
        self.assertNotIn("cross join lateral", normalized_sql)
