...
class SettingsAccessControlPreflightTests(unittest.TestCase):
    ...
    def test_multiple_cleanup_targets_are_canonicalized_by_hash_not_raw_id(self) -> None:
        ...
        cleanup = report["oa"]["menu_binding_cleanup"]
        target_hashes = cleanup["target_hashes"]
        self.assertEqual(cleanup["target_count"], 2)
        self.assertEqual(target_hashes, sorted(target_hashes))
        self.assertEqual(cleanup["rollback_target_hashes"], target_hashes)
        salt = hashlib.sha256(b"settings-access-control-v1\0main-security-test").hexdigest()
        self.assertGreater(
