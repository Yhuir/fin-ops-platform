...
class HttpSloProbeTests(unittest.TestCase):
    ...
    def test_default_probes_cover_page_domains_and_known_slow_endpoints(self) -> None:
        ...
        self.assertIn("page_size=10", probe_paths["workbench_groups_all_paired"])
        ...
        self.assertIn("view=bank_account", probe_paths["cost_statistics_explorer_all"])
    ...
    def test_gzip_json_response_reports_compressed_response_size(self) -> None:
        payload = gzip.compress(b'{"rows":[]}')
    ...
    def test_probe_scores_only_http_contract_and_latency(self) -> None:
        responses = [{"rows": []}, {"rows": [{"id": "row-1"}]}]
