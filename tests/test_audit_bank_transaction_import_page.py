...
class BankTransactionImportPageAuditTests(unittest.TestCase):
    ...
    def test_unified_repository_dispatch_adds_registered_contract(self) -> None:
        ...
        self.assertEqual(report["audit_contract"]["contract_revision"], "page-audit-contract.v29")
    ...
    def test_historical_weak_key_survives_v3_migration_without_rewriting_import_history(self) -> None:
        from copy import deepcopy
        ...
        transaction["bank_serial_no"] = "SERIAL-1"
        identity = BankTransactionIdentityService().identity_for_mapping(transaction)
        transaction.update(source_unique_key=identity.identity_key, data_fingerprint=identity.suspected_key)
        connection.rows[0].update(source_unique_key=identity.suspected_key, data_fingerprint="")
        connection.rows[0]["raw_payload"]["normalized_payload"].update(
            source_unique_key=identity.suspected_key, data_fingerprint="")
        ...
        for changes in [{"source_unique_key": identity.identity_key + "wrong"},
                        {"amount": "99.99"}, {"bank_serial_no": "OTHER"}]:
    ...
    def test_controlled_replay_can_reference_canonical_transaction_by_strict_statement_position(self) -> None:
        connection = FakeConnection()
        audit = bank_transaction_import_page_audit._zero_audit_counts()
        audit.update(
            {
        ...
        row_payload = connection.rows[0]["raw_payload"]["normalized_payload"]
        connection.rows[0].update(
            {
                "source_unique_key": "bank-v3:62220001:enterprise_serial_no:NEW:abc123",
        ...
        connection.transactions[0].update(
            {
                "batch_id": "older-batch",
                "source_unique_key": "bank-v2:62220001:bank_serial_no:OLD",
        ...
        connection.transactions[0].update(
            {
                ...
                "currency": "CNY",
                "raw_payload": {
                    "normalized_payload": {"balance": "899.10", "currency": "CNY"}
                },
            }
        )
        ...
        connection.transactions[0].update(
            {
                ...
                "balance": "900.00",
                "currency": None,
                "raw_payload": {"normalized_payload": {"balance": "900.00"}},
            }
        )
    ...
    def test_controlled_replay_accepts_parser_default_cny_for_missing_legacy_currency(
        self,
    ) -> None:
        connection = FakeConnection()
        audit = bank_transaction_import_page_audit._zero_audit_counts()
    ...
    def test_controlled_replay_missing_currency_does_not_match_explicit_currency(self) -> None:
        connection = FakeConnection()
        connection.rows[0].update(
            {
                "decision": "duplicate_skipped",
