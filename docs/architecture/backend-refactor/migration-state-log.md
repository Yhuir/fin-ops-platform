...
- 统一验证并合入 PF-P189 的完整文档 diff。
- 合入目标是 `dev`，不是 `main`。
- 只允许重构文档和状态机变更。
- 不修改 production code，不修改 tests，不执行 Traffic Gate，不部署，不访问生产或真实外部服务。
...
- 在当前功能分支完成 scope 和 verification。
- 精确 stage 允许文件并提交 MG 状态更新。
- 切换到 `dev`。
- 确认 `dev` 与 `origin/dev` 对齐，且工作树干净。
- merge `codex/dev-branch-rebaseline-p189` 到 `dev`。
- 在 `dev` 上重跑同组 verification。
- verification 通过后 push `origin/dev`。
- 不 push `origin/main`。
...
- 只做 Turnover Ledger 在 local runtime support 抽离完成后的 seam rebaseline。
- 不修改 production code、tests、schema、UoW 语义或部署配置。

#### 执行摘要

- 已确认 Turnover Ledger 当前已经完成的三组结构性切片：
  - request-boundary extraction
  - local runtime support extraction
  - primary write builder / legacy fallback / local adapter 前置收口
- 已确认继续做“纯 helper 拆分”的收益开始下降。
- 已确认当前剩余的高价值问题不再是 `server.py` 局部 wiring，而是写路径一致性边界：
  - local path 仍使用 `TurnoverLedgerLocalDirtyOutboxWriter`
  - postgres path 与 local path 的 dirty/outbox 语义不完全同构
  - `tag selection`、`relation extra`、`bank row tags`、`confirm`、`withdraw` 仍存在 local / postgres 双路径一致性契约需要重新盘点
- 已确认下一条不应直接做 module completion gate，而应先做 write consistency / dirty-outbox rebaseline。
...
- `git ls-files --others --exclude-standard`：Pass
- `git diff --check`：Pass
- CodeGraph / source seam audit：Pass

#### 下一条 Prompt 上下文

- 下一条应生成并审查：
  - `PF-P168 - Turnover Ledger Write Consistency / Dirty-Outbox Rebaseline`
- PF-P168 仍应是 docs-only / discovery，不得直接改事务语义。
...
- 只做 Turnover Ledger 当前写一致性 / dirty-outbox / 双路径契约 rebaseline。
- 不修改 production code、tests、schema、UoW 语义或部署配置。

#### 执行摘要

- 已确认当前剩余高价值 seam 主要集中在 `TurnoverLedgerWriteFacade` / `TurnoverLedgerWriteUnitOfWork` / dirty-outbox writer 的 local vs postgres 双路径差异。
- 已确认五条写路径都已统一走 `TurnoverLedgerWriteUnitOfWork`，但一致性能力并不完全一致：
  - `relation extra`
    - 有 `expected_versions`
    - 有 durable/in-memory idempotency key + request fingerprint
    - postgres path 使用 `TurnoverLedgerDirtyOutboxWriter`
    - local path 使用 `TurnoverLedgerLocalDirtyOutboxWriter`
  - `withdraw`
    - 有 `expected_versions`
    - 无 durable idempotency
    - postgres/local 同样存在 dirty-outbox 双路径差异
  - `confirm`
    - 无 `expected_versions`
    - 无 durable idempotency
  - `bank row tags`
    - 无 `expected_versions`
    - 无 durable idempotency
    - refresh_requests 同时覆盖 `bank_detail` / `workbench` / `turnover_ledger`
  - `tag selection`
    - 无 `expected_versions`
    - 无 durable idempotency
- 已确认所有 primary write builder 仍在 local path 中注入：
  - `TurnoverLedgerLocalDirtyOutboxWriter`
  - `stale_precondition_port=SimpleNamespace(assert_current=lambda **_kwargs: None)`
  这意味着 local path 当前只保证“best-effort + rollback contract”，不具备和 postgres path 完全对等的 stale precondition enforcement。
- 已确认当前更高收益的下一步，不是立刻改事务语义，而是先用 characterization tests 锁定：
  - 哪些路径必须继续允许 local/postgres 差异
  - 哪些路径已经应该收敛到一致 contract
  - relation extra / withdraw 的 `expected_versions` contract
  - confirm / bank row tags / tag selection 仍缺失的 precondition / idempotency 事实边界
- 结论：
  - 下一条最小高价值切片应是 `PF-P169 - Turnover Ledger Write Consistency Characterization Tests`
...
- `git diff --check`：Pass
- CodeGraph / source seam audit：Pass
- dirty-outbox / expected_versions / idempotency source audit：Pass

#### 下一条 Prompt 上下文

- 下一条应生成并审查：
  - `PF-P169 - Turnover Ledger Write Consistency Characterization Tests`
- PF-P169 只补测试，不得直接修改 UoW 或 dirty-outbox 语义。
...
- 只补 Turnover Ledger 写一致性 / dirty-outbox / 双路径 contract 的 characterization tests。
- 不修改 production code、schema、UoW 语义或部署配置。

#### 执行摘要

- 已锁定 `relation extra` 当前已保证的写一致性 contract：
  - `expected_versions`
  - durable/in-memory idempotency key + request fingerprint
  - `turnover_relation_extra_changed` dirty-outbox refresh contract
- 已锁定 `withdraw` 当前已保证的写一致性 contract：
  - `expected_versions`
  - `turnover_relation_changed` dirty-outbox refresh contract
  - 当前仍无 durable idempotency
- 已锁定当前明确缺失、后续要收敛的 contract：
  - `confirm` 当前仍无 stale precondition / durable idempotency
  - `bank row tags` 当前仍无 stale precondition / durable idempotency
  - `tag selection` 当前仍无 stale precondition / durable idempotency
- 已锁定所有 primary write builder 当前仍保留的双路径差异：
  - local path 继续使用 `TurnoverLedgerLocalDirtyOutboxWriter`
  - local path 继续注入 no-op `stale_precondition_port`
- 结论：
  - 下一条最小高价值实现切片应先处理 `withdraw` 的 stale precondition integration
  - 不应在同一刀里顺手迁移 `confirm`、`bank row tags`、`tag selection`
...
- `git diff --check`：Pass
- `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_api -v`：Pass（114 tests）
...
- 只处理 Turnover Ledger `withdraw` 写路径的 stale precondition integration。
- 不迁移 `confirm`、`bank row tags`、`tag selection` 或 `relation extra`。
- 不修改 schema、deploy、Traffic Gate 或真实外部服务配置。

#### 执行摘要

- 新增 `TurnoverLedgerRelationStalePreconditionPort`，复用 request boundary 已生成的 `expected_versions["relation:{relation_id}"]`。
- `TurnoverLedgerWithdrawPrimaryWriteFacadeBuilder` 不再给 withdraw UoW 注入 no-op stale precondition，而是注入 relation stale precondition port。
- 新增 `TurnoverLedgerWritePreconditionError`，由 withdraw handler 映射为 HTTP `409` 和 `turnover_relation_conflict`。
- 新增测试锁定 stale withdraw 在 mutation / audit / dirty-outbox refresh 前被拒绝。
- 保持 `confirm`、`bank row tags`、`tag selection` 当前无 stale precondition 的 baseline 不变。
...
- 下一条建议生成并审查：
  - `PF-P171 - Turnover Ledger Confirm Stale Precondition Discovery and Characterization`
- 原因：
  - `withdraw` 已具备 expected_versions 契约并完成 app wiring。
  - `confirm` 当前仍无 expected relation version 输入契约，不能直接做 integration；应先盘点 API / frontend / relation detail / source_version 可用事实，并用 characterization tests 锁定现状。
...
- 只处理 Turnover Ledger `confirm` stale precondition 的 discovery / characterization。
- 只允许新增或调整测试与重构文档。
- 不接入 confirm stale precondition port，不修改 UoW 语义，不修改 schema / deploy / Traffic Gate。

#### 审查结论

- PF-P171 是 PF-P170 后合理的下一步：
  - `withdraw` 已经具备 `expected_versions` request boundary 并完成 stale precondition wiring；
  - `confirm` 目前没有 request-level `expected_versions` 输入契约；
  - 直接实现 confirm stale guard 会引入未定义 API 合约，因此必须先用 tests 锁定当前行为和缺口。

#### 验收标准

- 测试必须证明 confirm handler 当前不会从请求 payload 透传 `expected_versions` 到 write command。
- 测试必须证明 confirm primary builder 当前仍使用 no-op stale precondition port。
- 文档必须记录 confirm stale precondition 的后续实现前置条件。

#### 执行摘要

- 新增 characterization test：
  - `test_confirm_request_body_expected_versions_are_currently_not_forwarded_to_write_command`
- 该测试使用真实 `TurnoverLedgerWriteFacade` + recording UoW override，证明即使 confirm request body 携带 `expected_versions`，最终 `TurnoverLedgerWriteCommand.expected_versions` 仍为空。
- 复用 builder guard：
  - `test_turnover_ledger_primary_write_builders_still_use_noop_local_stale_precondition_ports`
- 结论：
  - confirm 不能直接接 stale precondition integration；
  - 后续必须先定义 confirm expected_versions API/contract，再进入实现。
...
- PF-P170 + PF-P171 已形成一个可合并的 stale precondition 小切片：
  - PF-P170：withdraw stale precondition integration；
  - PF-P171：confirm expected_versions 缺口 characterization。
- 下一步应生成并执行：
  - `PF-P171-MG - Turnover Ledger Stale Precondition Cumulative Merge Gate`
...
- 只处理 PF-P170 + PF-P171 的 cumulative MG。
- 覆盖 withdraw stale precondition integration 与 confirm expected_versions characterization。
- 不新增业务实现，不执行 Traffic Gate，不修改 deploy/schema/生产配置。

#### 审查结论

- 这是当前合理的收口点：
  - 已有一个 production integration（withdraw stale precondition）；
  - 已有一个 test-only gap characterization（confirm expected_versions）；
  - 继续进入 confirm integration 前应先把当前稳定切片合入 `main`。

#### 验收标准

- diff 只能包含 PF-P170/PF-P171 允许文件。
- 必须检查 untracked files，禁止临时文件混入。
- 必须精准 `git add`，禁止 `git add .` / `git add -A`。
- 必须在功能分支提交，在 `main` 合并后重跑验证，通过后才允许 `git push origin main`。
...
- PF-P172 应先设计 confirm expected_versions API/contract，不应直接实现 stale precondition integration。
...
- docs-only planning。
- 只盘点 confirm expected_versions API / frontend / backend / tests 事实。
- 不修改 production code、frontend code、schema、deploy 或 Traffic Gate。

#### 执行摘要

- 已确认后端 confirm handler 只读取 `bank_row_ids` / `note`，不会读取 `expected_versions`。
- 已确认 `TurnoverLedgerConfirmRequestBoundaryFacade.confirm_relation_from_request(...)` 没有 `expected_versions` 参数。
- 已确认 `TurnoverLedgerWriteFacade.confirm_relation(...)` 当前不接受 `expected_versions`。
- 已确认前端 `ConfirmTurnoverRelationRequest` 只有 `bankRowIds` / `note` / `signal`。
- 已确认 `confirmTurnoverRelation(...)` 只提交 `bank_row_ids` 和可选 `note`。
- 已确认页面调用 `confirmTurnoverRelation({ bankRowIds: selectedRow.bankRowIds })`，没有 relation identity/version 输入。
- 已确认 PF-P171 已用测试锁定：confirm request body 的 `expected_versions` 当前不会进入 command 层。

#### 目标 Contract 建议

- 先给前端/后端增加兼容字段：
  - frontend request：`expectedVersions?: Record<string, unknown>`
  - HTTP payload：`expected_versions?: Record<string, unknown>`
  - backend request boundary：透传为 `expected_versions`
  - facade command：`expected_versions`
- 因 confirm 是基于 bank rows 创建/替换 relation，而不是基于单个 existing relation id，后续 key 设计不能机械复用 withdraw 的 `relation:{id}`。
- 最小可行 contract 应先使用 bank row level precondition：
  - `turnover_bank_row:{transaction_id}` 或等价稳定 key；
  - stale port 验证这些 bank rows 仍未被其它 syncable relation 占用，且状态/版本与提交时一致。
- 下一步必须先写 contract tests，不直接实现。
...
- PF-P173 应只补 target / compatibility tests：
  - frontend API payload can carry expectedVersions；
  - backend handler can accept `expected_versions` without breaking legacy payload；
  - facade command can carry expected_versions；
  - no production stale guard implementation yet。
...
- 只补 confirm expected_versions 的 target / compatibility tests。
- 允许修改：
  - `tests/test_turnover_ledger_api.py`
...
- PF-P173 是 PF-P172 后合理的测试切片。
- 当前 confirm contract 尚未实现，目标行为必须用 `unittest.expectedFailure` 或等价机制保留，不得让默认 CI 失败。
- 不应在本轮实现 confirm stale guard。

#### 执行摘要

- 新增 API-level target test：
  - `test_target_confirm_request_expected_versions_reach_write_command`
- 新增 facade/UoW-level target test：
  - `test_target_confirm_relation_facade_passes_expected_versions_before_repository`
- 两个 target tests 均标记为 `unittest.expectedFailure`，表达后续转绿目标且保持默认 CI 绿色。
- 保留 PF-P171 当前行为 characterization test，不删除、不放宽。
...
- PF-P174 应只实现 `expected_versions` 从 HTTP request -> request boundary -> write facade -> command 的透传，并将 PF-P173 的两个 expectedFailure tests 转绿。
- PF-P174 不应接入真实 confirm stale precondition port；真实 stale guard wiring 应在下一条 prompt 处理。
...
- 只实现 confirm `expected_versions` 透传：
  - HTTP request body
  - confirm request boundary
  - `TurnoverLedgerWriteFacade.confirm_relation(...)`
  - `TurnoverLedgerWriteCommand.expected_versions`
- 允许修改：
  - `backend/src/fin_ops_platform/app/server.py`
  - `backend/src/fin_ops_platform/services/turnover_ledger_write_adapters.py`
  ...
  - `tests/test_turnover_ledger_uow_contract.py`
  - 后端重构文档
- 不接入真实 confirm stale precondition port，不修改 schema/deploy/Traffic Gate。

#### 审查结论

- PF-P174 是 PF-P173 后合理的最小实现：
  - 只让 expected_versions 到达 command；
  - UoW 既有 stale precondition seam 会在测试 fake port 中生效；
  - app builder 仍保持 no-op confirm stale port，真实 guard wiring 留给后续 prompt。

#### 执行结果

- 已移除 PF-P173 两个 target tests 的 `unittest.expectedFailure` 并确认 RED：
  - API target 失败于 `command.expected_versions` 仍为空。
  - UoW/facade target 失败于 `confirm_relation(...)` 尚不接受 `expected_versions`。
- 已实现 confirm `expected_versions` 透传：
  ...
  - legacy confirm fallback / test recorder 保持兼容，未携带 `expected_versions` 时保持 `{}`。
- 未接入真实 confirm stale precondition port，未修改 schema/deploy/Traffic Gate。

#### 验证

- RED before implementation：
  - `tests.test_turnover_ledger_api.TurnoverLedgerApiTests.test_target_confirm_request_expected_versions_reach_write_command`：Fail as expected。
  - `tests.test_turnover_ledger_uow_contract.TurnoverLedgerUoWContractTests.test_target_confirm_relation_facade_passes_expected_versions_before_repository`：Fail as expected。
- `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_api.TurnoverLedgerApiTests.test_target_confirm_request_expected_versions_reach_write_command tests.test_turnover_ledger_api.TurnoverLedgerApiTests.test_confirm_request_body_without_expected_versions_keeps_empty_write_command_versions -v`：Pass。
- `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_uow_contract.TurnoverLedgerUoWContractTests.test_target_confirm_relation_facade_passes_expected_versions_before_repository -v`：Pass。
...
- 生成并审查 `PF-P175 - Turnover Ledger Confirm Stale Precondition Port Wiring`。
- PF-P175 只允许将 confirm expected_versions 接入真实 stale precondition port，不迁移 tags、relation extra、withdraw 或其它写路径。
...
- 只处理 Turnover Ledger confirm stale precondition integration。
- 允许新增 confirm 专用 bank-row stale precondition port，用于识别 `turnover_bank_row:<transaction_id>` expected version。
- 允许让 `_turnover_bank_transaction_rows()` 暴露已有 bank category version 字段，作为 confirm stale precondition 的版本事实源。
- 允许更新 confirm builder guard tests 和 API stale conflict tests。
- 不迁移 withdraw、bank row tags、tag selection、relation extra。
- 不修改 schema/deploy/Traffic Gate/外部服务配置。

#### 审查结论

- confirm 不能直接复用 withdraw 的 `TurnoverLedgerRelationStalePreconditionPort`，因为 confirm 写路径基于 bank rows 创建 relation，没有 relation id。
- PF-P175 应使用 bank-row 版本 key，例如 `turnover_bank_row:<transaction_id>`，并通过现有 bank rows provider / category version 数据校验。
- 本轮仍不实现 durable idempotency，不扩大到其它 Turnover write path。

#### 执行结果

- 新增 RED tests：
  - `test_confirm_stale_bank_row_precondition_rejects_before_mutation_or_refresh`
  - `test_turnover_ledger_confirm_builder_uses_bank_row_stale_precondition_port`
- 新增 `TurnoverLedgerBankRowStalePreconditionPort`：
  - 只识别 `turnover_bank_row:<transaction_id>` expected version。
  - 使用 bank rows provider 中的 `category_version` / `manual_category_version` / `version` 比较。
  - mismatch 或 missing row 返回 `TurnoverLedgerWritePreconditionError(error_code="turnover_relation_conflict")`。
- `TurnoverLedgerConfirmPrimaryWriteFacadeBuilder.build()` 已从 no-op stale precondition 改为 bank-row stale precondition port。
- `_turnover_bank_transaction_rows()` 暴露已有 category version，不新增查询或 schema。
- confirm handler 已将 `TurnoverLedgerWritePreconditionError` 映射为 409 JSON。
- 未迁移 withdraw、bank row tags、tag selection、relation extra；未实现 durable idempotency。

#### 验证

- RED before implementation：
  - PF-P175 target tests 当前实现下失败（confirm 返回 200；builder 仍 no-op）。
...
- 只处理 Turnover Ledger tag selection update 的 legacy fallback。
- 将 handler 中的 direct settings update、read model clear、refresh enqueue 迁入显式 legacy fallback facade/adapter。
...
- 只处理 Turnover Ledger relation extra update 的 legacy fallback。
- 将 handler 中的 direct relation extra update、best-effort persist、read model clear、refresh enqueue 迁入显式 fallback facade/adapter。
- 不处理 bank row tags、confirm、withdraw。
...
- 在 `turnover_ledger_write_adapters.py` 新增 `TurnoverLedgerRelationExtraLegacyFallbackFacade`。
- `_turnover_ledger_relation_extra_write_facade()` 在 primary UoW facade 不可用时返回 legacy fallback adapter。
- `_handle_api_turnover_ledger_relation_extra_update(...)` 已删除 `facade is None` direct route update / persist / clear / enqueue 分支。
- 保留 handler 中的 expected_versions/idempotency precheck；本轮未改变 stale conflict、idempotency conflict、validation 或 response payload 行为。
- 更新 relation extra tests：
  - 新增 handler thinness 静态门禁。
  - 将 fallback success/queue failure 测试改为 unsupported postgres queue API 触发 fallback adapter。
...
- Branch scope：
  ...
  - `git log --oneline main..HEAD`：只包含 PF-P120/PF-P121/PF-P122/PF-P123/PF-P124/PF-P124-MG 相关提交。
- Branch verification：
  - `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_api -v`：Pass，63 tests。
  - `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_uow_contract -v`：Pass，56 tests。
  - `python3 -m compileall backend/src/fin_ops_platform/app/server.py backend/src/fin_ops_platform/services/turnover_ledger_write_adapters.py`：Pass。
- Merge：
  - 已将 `codex/turnover-ledger-remaining-boundary-p120` 合入 `main`。
- Main verification：
  - `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_api -v`：Pass，63 tests。
  - `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_uow_contract -v`：Pass，56 tests。
  - `python3 -m compileall backend/src/fin_ops_platform/app/server.py backend/src/fin_ops_platform/services/turnover_ledger_write_adapters.py`：Pass。
...
PF-P130 已完成 bank row tags legacy fallback facade extraction：

- 新增 `TurnoverLedgerBankRowTagsLegacyFallbackFacade`，封装 category apply、category snapshot save、relation rebuild 和 after-mutation。
- `_turnover_ledger_bank_row_tags_write_facade(...)` 在 primary facade 不可用时返回显式 legacy fallback adapter。
- `_handle_api_turnover_ledger_bank_row_tags_batch(...)` 不再直接调用 `apply_turnover_updates(...)`、`save_bank_transaction_categories(...)`、`rebuild_from_bank_rows(...)` 或 `_after_turnover_relation_mutation(...)`。
...
### PF-P136 - Turnover Ledger Remaining Write Path Rebaseline After Tag Selection Extraction

状态：`verified`

#### 范围

- 基于 PF-P135-MG 之后的最新 `main`，重新盘点 Turnover Ledger 剩余写路径。
- 只做 discovery/planning 和文档回写。
- 不修改 production code，不修改 tests，不进入 MG。
...
- 最新 Remaining Write Path Matrix。
- PF-P135 之后已稳定部分与剩余部分的边界。
- 下一条最小实现 prompt，只能推荐一条。
...
- `git ls-files --others --exclude-standard`
- `git diff --check`
- `rg -n "PF-P136|Remaining Write Path Rebaseline After Tag Selection Extraction|relation extra local adapter" docs/architecture/backend-refactor/migration-state-log.md docs/architecture/backend-refactor/refactor-prompts.md docs/architecture/backend-refactor/turnover-ledger-write-uow-plan.md`

#### 执行摘要

- 已确认 PF-P135 后，tag selection local path 已不再是最高优先级剩余 seam。
- 已重新扫描 remaining write paths：
  - `relation extra` local path 仍由 `server.py` 通过 `save_snapshot=lambda ...` 组装 `TurnoverLedgerLocalRelationExtraConnection`；
  - `confirm` / `withdraw` local path 仍由 `server.py` 组装 relation snapshot/save/rebuild 依赖；
  - `bank row tags` local path 仍包含 category snapshot、relation snapshot 与跨 Bankdetail/relation 的多依赖组装。
- 已确认新的最小剩余 seam 是 `relation extra local adapter`：
  - 只涉及 extra snapshot save/replace；
  - 不涉及 relation rebuild；
  - 不跨 Bankdetail；
  - 风险显著低于 confirm / withdraw / bank row tags。

#### Remaining Write Path Matrix

| 写路径 | 当前状态 | 已完成切片 | 剩余问题 | 推荐优先级 |
| --- | --- | --- | --- | --- |
| relation extra | primary UoW、idempotency、explicit fallback、local extra repository 已稳定 | stale/idempotency/local extra adapter/handler-thinness/queue failure tests 已完成 | local connection 仍由 `server.py` 通过 `save_snapshot=lambda ...` 组装 | P1 |
| confirm relation | primary UoW + explicit legacy fallback + invalidation seam 已稳定 | stale guard、duplicate submit、fallback adapter、UoW skeleton 已完成 | local relation connection/repository 仍由 `server.py` 组装，包含 rebuild 依赖 | P2 |
| withdraw relation | primary UoW + explicit legacy fallback + invalidation seam 已稳定 | stale guard、duplicate submit、fallback adapter、UoW skeleton 已完成 | local relation connection/repository 仍由 `server.py` 组装，仍有 relation snapshot save seam | P3 |
| bank row tags batch | explicit fallback facade + invalidation seam 已稳定 | local bankdetail port / local connection / fallback cleanup 已完成大半 | 仍跨 category snapshot、relation snapshot、Bankdetail 与 relation rebuild，复杂度最高 | P4 |

#### 下一条 Prompt 上下文

PF-P136 结论：下一条最小实现 prompt 应为 `PF-P137 - Turnover Ledger Relation Extra Local Adapter Extraction`。这一步只应抽离 `relation extra` local path 的 snapshot/save 组装，不得顺手改 confirm/withdraw/bank row tags。

### PF-P137 - Turnover Ledger Relation Extra Local Adapter Extraction

状态：`verified`

#### 范围

- 只抽离 relation extra local path 的 local adapter seam。
- 把 `server.py` 中 relation extra local path 的 snapshot/save 组装尽量收束到 adapter module。
...
- 已新增 `TurnoverLedgerLocalRelationExtraAdapterSet`，将 relation extra local path 的 snapshot/save/repository 组装从 `server.py` 移入 adapter module。
- `server.py` 的 `_turnover_ledger_relation_extra_write_facade(...)` 不再直接内联 `save_snapshot=lambda ...`。
- 新增 source-level guard test，锁定 relation extra write facade 不再内联 local snapshot/save closure。
- 过程中发现一个真实语义回归风险：local extra save 必须保持“持久化失败仅记 warning，不改变成功响应路径”的旧合同；已在 adapter 内复用这一行为，而不是修改测试。
- 原有 relation extra 行为保持不变：stale、idempotency、queue failure rollback、fallback facade、no direct clear 全部继续通过。
