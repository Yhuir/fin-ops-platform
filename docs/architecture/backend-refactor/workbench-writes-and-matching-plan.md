...
| OA sync polling / dirty rebuild | Removed; `FIN_OPS_OA_POLLING_ENABLED` is no longer a supported trigger | No HTTP-process polling/rebuild thread | OA source changes must enqueue durable `oa.sync` and be handled by runtime worker/projection sync | Runtime worker lease/drain contract | Old HTTP-process polling/hot rebuild path was removed because it could overlap Workbench writes and bypass durable run/outbox facts. |
