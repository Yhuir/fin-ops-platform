...
## PF-P123 Tag Selection Legacy Fallback Facade Extraction

状态：`verified`

目标：

- 只处理 tag selection update fallback。
- 将 direct settings update、read model clear、refresh enqueue 从 handler 移入显式 legacy fallback facade/adapter。
- 让 handler 只做 HTTP mapping、session/body parsing、调用 facade-like object 和 response packaging。
...
- 新增 `TurnoverLedgerTagSelectionLegacyFallbackFacade`。
- tag selection facade construction 在 primary UoW facade 不可用时返回 legacy fallback adapter。
- tag selection handler 删除 `facade is None` direct mutation/clear/enqueue 分支。
- tests 增加 handler thinness 静态门禁，并验证 unsupported postgres queue API fallback adapter 保持 legacy update/clear/enqueue 行为。
...
- 只处理 relation extra fallback，不处理 bank row tags、confirm、withdraw。

## PF-P124 Relation Extra Legacy Fallback Facade Extraction

状态：`verified`

目标：

- 只处理 relation extra update fallback。
- 将 direct route update、best-effort persist、read model clear、refresh enqueue 从 handler 移入显式 legacy fallback facade/adapter。
- 保留 expected_versions/idempotency precheck，不在本轮迁移。

边界：

- 允许修改 `server.py`、`turnover_ledger_write_adapters.py`、Turnover Ledger tests 和 backend-refactor 文档。
- 不处理 tag selection、bank row tags、confirm、withdraw。
- 不改变 response payload、validation、idempotency conflict 或 stale conflict 行为。
...
- 新增 `TurnoverLedgerRelationExtraLegacyFallbackFacade`。
- relation extra facade construction 在 primary UoW facade 不可用时返回 legacy fallback adapter。
- relation extra handler 删除 `facade is None` direct route update / persist / clear / enqueue 分支。
- 保留 expected_versions/idempotency precheck。
- tests 增加 handler thinness 静态门禁，并验证 unsupported postgres queue API fallback adapter 保持 legacy update/persist/clear/enqueue 行为。
...
- 统一覆盖 PF-P120 到 PF-P124 完整 diff。

## PF-P124-MG Fallback Facade Cumulative Merge Gate

状态：`verified`

范围：

- 统一验证并合入 PF-P120 到 PF-P124。
- 覆盖 fallback rebaseline、fallback tests、cleanup planning、tag selection fallback adapter、relation extra fallback adapter。
...
- Branch scope：Pass，diff 只包含允许文件。
- Branch verification：
  - `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_api -v`：Pass，63 tests。
  - `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_uow_contract -v`：Pass，56 tests。
  - `python3 -m compileall backend/src/fin_ops_platform/app/server.py backend/src/fin_ops_platform/services/turnover_ledger_write_adapters.py`：Pass。
...
- 只处理 withdraw relation fallback。
- 将 route withdraw、after-mutation 从 handler 移入显式 fallback facade/adapter。
...
- 允许修改 `server.py`、`turnover_ledger_write_adapters.py`、Turnover Ledger tests 和 backend-refactor 文档。
- 不改变 queue reason、response payload、validation error behavior。
- 不处理 confirm、tag selection、relation extra 或 bank row tags。
...
- `TurnoverLedgerWithdrawLegacyFallbackFacade` 已迁入 `turnover_ledger_write_adapters.py`。
- `_turnover_ledger_withdraw_write_facade()` 在 primary UoW facade 不可用时返回 withdraw fallback adapter。
- `_handle_api_turnover_ledger_withdraw(...)` 不再直接调用 route withdraw、不再直接调用 `_after_turnover_relation_mutation(...)`。
- 新增 withdraw handler-thinness guard，并将 withdraw fallback characterization tests 改为通过 unsupported postgres queue API 触发 fallback adapter。
- 验证通过：
...
- 统一覆盖 PF-P125、PF-P126、PF-P127；不得直接进入 bank row tags。
...
- 只合并 relation mutation fallback planning、confirm fallback adapter、withdraw fallback adapter。
- 不处理 bank row tags，不新增业务实现。
...
- PF-P128/PF-P129/PF-P130 已作为 bank row tags fallback cleanup cumulative slice 合入 `main`。
...
- bank row tags handler 不再直接执行 fallback category apply/save、relation rebuild、after-mutation。
...
- 统一覆盖 PF-P128/PF-P129/PF-P130。
- 只合入 bank row tags fallback cleanup discovery、tests 和 adapter extraction。
- 不执行 Traffic Gate。
...
- 已确认 relation mutation invalidation 主链切片已完成，不应立即重复返工。
- 已确认 relation extra primary path 与 local extra adapter 处于稳定状态。
- 已确认 tag selection local path 仍是最小剩余 seam：风险小、测试完整、仍有 `server.py` 闭包组装问题。

Remaining write path matrix：

| 写路径 | 当前状态 | 剩余问题 | 优先级 |
| --- | --- | --- | --- |
| tag selection | primary UoW + fallback 已稳定 | local path 闭包组装仍在 `server.py` | P1 |
| relation extra | primary UoW + local extra adapter 已稳定 | legacy best-effort helper 仍在，但优先级次于 tag selection | P2 |
| confirm relation | fallback facade + invalidation adapter 已稳定 | local relation wiring 仍在 `server.py`，但复杂度高 | P3 |
| withdraw relation | fallback facade + invalidation adapter 已稳定 | local relation wiring 仍在 `server.py`，并受 stale/duplicate 约束 | P4 |
| bank row tags batch | fallback facade + invalidation adapter 已稳定 | 跨 Bankdetail/Workbench，复杂度最高 | P5 |

下一条最小 prompt：

- `PF-P135 - Turnover Ledger Tag Selection Local Adapter Extraction`
- 理由：这是当前最小、最稳定、最不易引入跨模块回归的剩余切片。
...
- 继续减少 `server.py` 中 tag selection local path 的 Application-capturing 闭包。
- 把 local adapter 组装进一步收束到 adapter module。

边界：

- 只处理 tag selection local path。
- 不改 relation extra、confirm、withdraw、bank row tags。
- 不进入 MG。
...
## PF-P148 Primary Write Builder Rebaseline After Legacy Fallback Cleanup
...
- 在 legacy fallback group 收口后，重新盘点 Turnover Ledger primary write builder 剩余 seam。
- 为后续 characterization / extraction 建立新的事实源。

结论矩阵：

1. 五个 `_turnover_ledger_*_write_facade(...)` 仍在 `server.py` 中各自组装 `TurnoverLedgerWriteUnitOfWork`
   - relation extra
   - bank row tags
   - confirm
   - withdraw
...
2. 共享重复点：
   - `state_store` / `queue_repository` availability check
   - `storage_backend == "postgres"` 分支判断
   ...
   - 多个 unused port 仍以 `SimpleNamespace()` 占位

3. 当前不宜直接进入统一 builder extraction 的原因：
   - 一次性收束 5 条写路径 builder，scope 过大；
   - placeholder/unused port contract 还没有机械测试门禁；
   - relation extra 还带有 idempotency store 分支，复杂度高于其它路径。

下一步：

- 生成并审查 `PF-P149 - Turnover Ledger Primary Write Builder Characterization Tests`。

## PF-P149 Primary Write Builder Characterization Tests
...
- 用 source-level characterization tests 锁定 Turnover Ledger primary write builder 当前 contract。
- 在 extraction 前保护 `server.py` 中仍然存在的 builder duplication 事实。
...
- 当前 builder duplication 已被机械锁定。
- 下一条最小实现切片应先处理 `tag selection primary write builder`，而不是直接统一所有路径。

下一步：

- 生成并审查 `PF-P150 - Turnover Ledger Tag Selection Primary Write Builder Extraction`。
...
- 把 tag selection primary write builder 从 `server.py` 收到 adapter module。
- 在不改变 runtime behavior 的前提下，为后续 primary builder 组抽离建立第一条真实实现样板。

执行结果：

- `turnover_ledger_write_adapters.py` 新增 `TurnoverLedgerTagSelectionPrimaryWriteFacadeBuilder`。
- `_turnover_ledger_tag_selection_write_facade(...)` 不再直接在 `server.py` 中构造 `TurnoverLedgerWriteUnitOfWork(...)`。
- PostgreSQL path 仍走 `TurnoverLedgerTagSelectionSettingsAdapter + TurnoverLedgerDirtyOutboxWriter`。
- local path 仍走 `TurnoverLedgerLocalTagSelectionAdapterSet + TurnoverLedgerLocalDirtyOutboxWriter`。
- 保留了 legacy fallback 行为：builder 不可用时仍返回 `_turnover_ledger_tag_selection_legacy_fallback_facade()`。
...
- tag selection 已成为第一条离开 `server.py` inline UoW builder 的 primary write path。
- 剩余四条 primary write path 里，withdraw 是下一条最小实现切片。

下一步：

- 生成并审查 `PF-P151 - Turnover Ledger Withdraw Primary Write Builder Extraction`。
...
- 把 withdraw primary write builder 从 `server.py` 收到 adapter module。
- 在不改变 withdraw runtime behavior 的前提下，继续缩小 primary builder 组的 server-side duplication。

执行结果：

- `turnover_ledger_write_adapters.py` 新增 `TurnoverLedgerWithdrawPrimaryWriteFacadeBuilder`。
- `_turnover_ledger_withdraw_write_facade(...)` 不再直接在 `server.py` 中构造 `TurnoverLedgerWriteUnitOfWork(...)`。
- PostgreSQL path 仍走 `TurnoverLedgerRelationWritePort + TurnoverLedgerDirtyOutboxWriter`。
- local path 仍走 `TurnoverLedgerLocalWithdrawRelationAdapterSet + TurnoverLedgerLocalDirtyOutboxWriter`。
- builder 不可用时仍回退到 `_turnover_ledger_withdraw_legacy_fallback_facade()`。
...
- withdraw 已成为第二条离开 `server.py` inline UoW builder 的 primary write path。
- 现在最小的下一刀是 confirm primary builder；bank row tags 和 relation extra 仍更重。

下一步：

- 生成并审查 `PF-P152 - Turnover Ledger Confirm Primary Write Builder Extraction`。
...
- 把 confirm primary write builder 从 `server.py` 收到 adapter module。
- 在不改变 confirm runtime behavior 的前提下，继续缩小 primary write builder 组的 server-side duplication。

执行结果：

- `turnover_ledger_write_adapters.py` 新增 `TurnoverLedgerConfirmPrimaryWriteFacadeBuilder`。
- `_turnover_ledger_confirm_write_facade(...)` 不再直接在 `server.py` 中构造 `TurnoverLedgerWriteUnitOfWork(...)`。
- PostgreSQL path 仍走 `TurnoverLedgerRelationWritePort + TurnoverLedgerDirtyOutboxWriter`。
- local path 仍走 `TurnoverLedgerLocalConfirmRelationAdapterSet + TurnoverLedgerLocalDirtyOutboxWriter`。
- builder 不可用时仍回退到 `_turnover_ledger_confirm_legacy_fallback_facade()`。
...
- confirm 已成为第三条离开 `server.py` inline UoW builder 的 primary write path。
- 剩余两条里，bank row tags 是下一条更小的 seam，relation extra 仍然最重。

下一步：

- 生成并审查 `PF-P153 - Turnover Ledger Bank Row Tags Primary Write Builder Extraction`。
...
- 把 bank row tags primary write builder 从 `server.py` 收到 adapter module。
- 在不改变 bank row tags runtime behavior 的前提下，继续缩小 primary write builder 组的 server-side duplication。

执行结果：

- `turnover_ledger_write_adapters.py` 新增 `TurnoverLedgerBankRowTagsPrimaryWriteFacadeBuilder`。
- `_turnover_ledger_bank_row_tags_write_facade(...)` 不再直接在 `server.py` 中构造 `TurnoverLedgerWriteUnitOfWork(...)`。
- PostgreSQL path 仍走 `TurnoverLedgerBankdetailWritePort + TurnoverLedgerDirtyOutboxWriter`。
- local path 仍走 `TurnoverLedgerLocalBankRowTagsAdapterSet + TurnoverLedgerLocalDirtyOutboxWriter`。
- builder 不可用时仍回退到 `_turnover_ledger_bank_row_tags_legacy_fallback_facade()`。
...
- bank row tags 已成为第四条离开 `server.py` inline UoW builder 的 primary write path。
- 现在只剩 relation extra 最后一条 seam；完成它后即可进入 primary builder 组的 cumulative MG。

下一步：

- 生成并审查 `PF-P154 - Turnover Ledger Relation Extra Primary Write Builder Extraction`。
...
- 把 relation extra primary write builder 从 `server.py` 收到 adapter module。
- 完成 Turnover Ledger primary write builder 组的最后一条 seam 收尾。

执行结果：

- `turnover_ledger_write_adapters.py` 新增 `TurnoverLedgerRelationExtraPrimaryWriteFacadeBuilder`。
- `_turnover_ledger_relation_extra_write_facade(...)` 不再直接在 `server.py` 中构造 `TurnoverLedgerWriteUnitOfWork(...)`。
- PostgreSQL path 仍走：
  - `TurnoverLedgerExtraRepositoryAdapter`
  - `TurnoverLedgerDirtyOutboxWriter`
  - workbench write idempotency store
- local path 仍走：
  - `TurnoverLedgerLocalRelationExtraAdapterSet`
  - `TurnoverLedgerLocalDirtyOutboxWriter`
  - in-memory idempotency store
- relation extra idempotency store 的 handler 内联组装已收进 builder / helper boundary。

测试更新：

- 新增：
  - `test_turnover_ledger_relation_extra_primary_write_facade_uses_builder_boundary`
- 组级 characterization 已翻面为：
...
- Turnover Ledger 五条 primary write path 已全部离开 `server.py` 的 inline UoW builder 组装。
- 现在应进入这组切片的 cumulative MG，而不是继续扩大实现范围。
...
## PF-P155 Write UoW Post-Builder Rebaseline
...
- 在 primary write builder 组完成后，重新盘点 Turnover Ledger 剩余写路径 seam。
- 确认下一阶段应该治理的是 write handler / facade / UoW deeper boundary，而不是继续围绕 builder 组扩张。

执行结果：

- 已确认：
  - 五条 primary write path 已全部离开 `server.py` 的 inline UoW builder。
  - 当前剩余问题主要集中在 handler 仍持有的 orchestration，而非 builder。
- 剩余 seam：
  - relation extra handler 仍持有部分 stale precheck、idempotency key extraction、HTTP conflict mapping
  - withdraw handler 仍持有 relation detail read、`affected_months` 计算、`expected_versions` 组装
  - confirm handler 仍持有 `affected_months` 计算，但尚未建立与 withdraw 对称的 stale contract
  - bank row tags handler 仍持有 target validation 和 `affected_months` 计算
  - `Application` 仍保留 local snapshot replace helpers、relation extra idempotency helper methods 和 legacy fallback facades

结论：

- 下一阶段最小切片应先锁 handler-owned contract，再决定是否下沉到 facade/UoW。
- 现在不应该优先抽 local transaction shim，也不应该先删 fallback。

下一步：

- 生成并审查 `PF-P156 - Turnover Ledger Write Handler Boundary Characterization Tests`。

## PF-P156 Write Handler Boundary Characterization Tests
...
- 锁定 Turnover Ledger 剩余 write handler 的真实 orchestration contract。
- 在进入 handler boundary extraction 前，先保护 relation extra、confirm、withdraw、bank row tags 的当前行为。

执行结果：

- 已补齐并验证 relation extra / confirm / withdraw / bank row tags 的 handler characterization tests。
- 锁定内容包括：
  - relation extra：`expected_versions`、`idempotency_key`、stale conflict mapping
  - confirm：`affected_months` 计算与 response 注入
  - withdraw：relation precheck、`expected_versions`、`affected_months`
  - bank row tags：target validation、`affected_months`、invalidation flags
- 额外补充 override path characterization：
  - relation extra override 透传 `expected_versions` / `idempotency_key`
  - bank row tags override 透传 `affected_months` 并保留 response flags
...
- 生成并审查 `PF-P157 - Turnover Ledger Confirm Handler Boundary Extraction`。

## PF-P157 Confirm Handler Boundary Extraction
...
- 只抽离 confirm handler 当前仍持有的 orchestration 边界。
- 把 `affected_months` 解析/注入从 `server.py` confirm handler 挪到显式 boundary 对象。

执行结果：

- 新增 `TurnoverLedgerConfirmRequestBoundaryFacade`。
- `Application._handle_api_turnover_ledger_confirm(...)` 现在只保留：
  - auth/session
  - JSON 校验
  - request boundary 调用
  - error mapping
- 从 handler 抽走：
  - `bank_row_ids` 规范化
  - `affected_months` 计算
  - `result["affected_months"]` 注入
- confirm override / legacy fallback / primary facade 路径全部保持兼容。
...
- confirm 是剩余 handler seam 中最轻的一条，已平稳收掉。
- 下一条应该进入 `withdraw`，因为它仍持有更重但边界明确的 request orchestration。

下一步：

- 生成并审查 `PF-P158 - Turnover Ledger Withdraw Handler Boundary Extraction`。

## PF-P158 Withdraw Handler Boundary Extraction
...
- 只抽离 withdraw handler 仍持有的 request orchestration。
- 让 handler 回到 HTTP mapping 边界，不直接处理 relation precheck / expected_versions / `affected_months`。

执行结果：

- 新增 `TurnoverLedgerWithdrawRequestBoundaryFacade`。
- 新增 `TurnoverLedgerWithdrawRequestBoundaryError`。
- 从 `Application._handle_api_turnover_ledger_withdraw(...)` 抽走：
  - relation detail 读取
  - manual-only / already-withdrawn precheck
  - `expected_versions` 组装
  - `affected_months` 计算
  - `affected_months` response 注入
- handler 现在只做 session、body、boundary 调用与 HTTP error mapping。
- override / legacy fallback / primary facade 路径保持兼容。
...
- withdraw handler seam 已成功下沉到显式 boundary。
- 当前最自然的下一条是 `bank row tags`，因为它仍持有 target validation、`affected_months` 和 invalidation flags。

下一步：

- 生成并审查 `PF-P159 - Turnover Ledger Bank Row Tags Handler Boundary Extraction`。

## PF-P159 Bank Row Tags Handler Boundary Extraction
...
- 只抽离 bank row tags handler 当前仍持有的 request orchestration。
- 把 target validation、`affected_months` 和 response invalidation flags 收到显式 boundary 对象。

执行结果：

- 新增 `TurnoverLedgerBankRowTagsRequestBoundaryFacade`。
- `Application._handle_api_turnover_ledger_bank_row_tags_batch(...)` 已不再直接持有：
  - target validation
  - `affected_months`
  - invalidation flags 注入
- 新边界对象保留了 `facade is None -> legacy fallback facade` 的兼容链。
...
## PF-P166-MG Local Runtime Support Cumulative Merge Gate
...
- `PF-P164 + PF-P165 + PF-P166` 已通过 cumulative MG。
- 已确认本组 diff 只包含：
  - local runtime support 生产代码改动
  - `tests/test_turnover_ledger_api.py`
  - backend-refactor 文档回写
- 已 merge 到 `main`，并在 `main` 上完成复验：
  - `git diff --check`：Pass
...
## PF-P179 Withdraw Durable Idempotency Contract Tests
...
- confirm 已完成 stale precondition + durable idempotency。
- withdraw 已完成 expected_versions、stale precondition 和 transaction-bound dirty/outbox UoW。
- withdraw 当前仍缺 durable idempotency；重复提交目前有 legacy duplicate rejection characterization，但没有 stable replay/conflict contract。

目标：

- 新增 withdraw durable idempotency 目标测试。
- 使用 `unittest.expectedFailure` 保留目标合同，不在本轮实现。
- 为后续 PF-P180 实现切片提供最小、安全的 RED 契约。

目标契约：

- `TurnoverLedgerWriteFacade.withdraw_relation(..., idempotency_key=...)` 未来应写入 `TurnoverLedgerWriteCommand.idempotency_key` 和稳定 request fingerprint。
- HTTP withdraw same idempotency key + same payload 应 replay first response，不重复 relation mutation / refresh enqueue。
- HTTP withdraw same idempotency key + different payload 应返回 conflict，不重复 relation mutation / refresh enqueue。

边界：

- 不修改 production code。
- 不实现 idempotency store、HTTP mapping 或 UoW behavior。
- 不迁移 confirm、bank row tags、tag selection、relation extra。
- 不修改 schema、deploy、Traffic Gate 或真实外部服务配置。

验证计划：

- `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_api -v`：预期 Pass，包含新增 expected failures。
- `PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_uow_contract -v`：预期 Pass，包含新增 expected failures。
- `git diff --check`。
- `git status --short --branch`。
- `git ls-files --others --exclude-standard`。
...
- 移除 expectedFailure 并转绿目标测试。
- 只实现 withdraw durable idempotency，不迁移其它写路径。

## PF-P180 Withdraw Durable Idempotency Integration
...
- 只实现 withdraw durable idempotency。
- 不迁移 confirm、bank row tags、tag selection、relation extra。
- 不修改 schema、deploy、Traffic Gate 或真实外部服务配置。

执行结果：

- PF-P179 三个 withdraw durable idempotency target tests 已从 `unittest.expectedFailure` 转为普通通过。
- `TurnoverLedgerWriteFacade.withdraw_relation(..., idempotency_key=...)` 将 idempotency key 和 stable request fingerprint 写入 command。
- action name 使用 `turnover_relation_withdraw`。
...
- Withdraw request boundary 和 HTTP handler 支持 body `idempotency_key` / `idempotencyKey`。
- Withdraw primary builder 接入现有 Workbench idempotency store boundary；local path 使用 in-memory store 保持测试/开发语义。
- Withdraw handler 复用 Workbench idempotency conflict/in-progress/failed 409 mapping。
