...
| ---: | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `POST /api/background-jobs/{job_id}/acknowledge` | pending_contract | platform-ops | medium | `job.worker_tasks` 只提供核心状态；acknowledged 是 UI/通知层状态，不能写入核心 task status。 | 待新增 `job.task_notifications` 或 `job.worker_task_acknowledgements`。 | `acknowledge_background_job(job_id, actor, reason)`。 | `background_job.acknowledged`。 | none。 | none；只影响通知已读状态。 | `background_job.ack:{job_id}:{actor}`。 | `can_access_app`，仅 owner 或 admin 可 ack。 | 删除 ack 记录或写 `acknowledged=false` 补偿记录。 | fixture 使用已存在 worker task 和同 actor ack，校验旧 envelope 与新通知状态。 | yes |
| 2 | `POST /api/background-jobs/{job_id}/retry` | pending_contract | platform-ops | medium | `job.worker_tasks`、`job.dead_letters`；只允许 retry `failed`/`dead_lettered` 且任务类型有重放协议。 | 现有 `job.worker_tasks`、`job.outbox_events`、`job.dead_letters` 足够；若要保存 retry lineage，需新增 `superseded_by_task_id`。 | `retry_worker_task(job_id, actor, reason, replay_mode)`。 | `background_job.retry_requested`。 | `worker_task.retry_requested` 或按原 task_type 发布重放 command。 | 由 task payload 的 `affected_months/affected_scopes` 决定。 | `background_job.retry:{job_id}:{actor}:{reason_hash}`。 | owner 或 admin；system task 仅 admin。 | 新任务创建后不可删除旧事实；失败时可取消新 task。 | fixture 覆盖 failed task retry 生成新 task/outbox，不重放真实外部副作用。 | no |
| 3 | `POST /api/workbench/settings` | pending_contract | platform-ops | high | 当前 settings 读取为默认投影；真实设置 facts 尚未冻结。 | 待新增 `app.settings_profiles`、`app.settings_events` 或等价 settings facts。 | `save_workbench_settings(actor, expected_version, patch)`。 | `settings.updated`。 | `settings.changed`。 | workbench、cost_statistics、search 按配置影响范围失效。 | `settings.save:{actor}:{expected_version}:{patch_hash}`。 | `can_admin_access`。 | 用 previous snapshot 写反向 patch；保留审计。 | fixture 使用最小设置 patch，校验 version、audit、失效事件。 | yes |
| 4 | `POST /api/workbench/settings/projects/sync` | pending_contract | platform-ops | high | 项目主数据源未冻结；不得从 OA 源库实时读取。 | 待新增 `app.project_profiles`、`staging.project_sync_rows`。 | `request_project_sync(actor, scope, source_watermark)`。 | `project_sync.requested`。 | `project.sync_requested`。 | cost_statistics、search、workbench project filter 失效。 | `project.sync:{scope}:{source_watermark}`。 | `can_admin_access`。 | 取消 queued task；已完成同步通过新同步批次补偿。 | fixture 使用 staging project rows，不连 OA 源。 | yes |
| 5 | `POST /api/workbench/settings/projects` | pending_contract | platform-ops | high | 项目设置 facts 未冻结；只能写 PostgreSQL project profile。 | 待新增 `app.project_profiles`、`app.project_profile_events`。 | `upsert_project_profile(actor, project_id, expected_version, patch)`。 | `project_profile.upserted`。 | `project_profile.changed`。 | cost_statistics、search、workbench project filter。 | `project.profile.upsert:{project_id}:{expected_version}:{patch_hash}`。 | `can_admin_access`。 | 写反向 patch 或新版本恢复。 | fixture 覆盖 create/update、重复 key 幂等。 | yes |
| 6 | `DELETE /api/workbench/settings/projects/{project_id}` | pending_contract | platform-ops | high | 项目设置 facts 未冻结；删除必须是软删除/停用。 | 待新增 `app.project_profiles`、`app.project_profile_events`。 | `deactivate_project_profile(actor, project_id, expected_version)`。 | `project_profile.deactivated`。 | `project_profile.changed`。 | cost_statistics、search、workbench project filter。 | `project.profile.delete:{project_id}:{expected_version}`。 | `can_admin_access`。 | 重新激活前一版本。 | fixture 校验软删除后列表隐藏、历史仍可追溯。 | yes |
| 7 | `POST /api/workbench/settings/data-reset/jobs` | pending_contract | platform-ops | high | 数据重置是 destructive admin job；必须先有 backup/approval evidence。 | 现有 `job.worker_tasks`、`job.outbox_events`，需新增 `app.data_reset_requests` 保存审批和 scope。 | `create_data_reset_job(actor, scope, approval_id, backup_evidence_id)`。 | `data_reset.requested`。 | `data_reset.requested`。 | 由 reset scope 决定；完成后触发 read model rebuild。 | `data_reset.request:{scope}:{approval_id}`。 | `can_admin_access` 且二次确认。 | queued 可取消；running 后只允许按 runbook restore/补偿。 | fixture 只能用 isolated staging，校验不触发生产事实删除。 | yes |
| 8 | `POST /api/workbench/settings/data-reset` | pending_contract | platform-ops | high | 立即执行 data reset 禁止直接迁移；必须改为 job command。 | 同上。 | `execute_data_reset_job(task_id)`，仅 worker 使用。 | `data_reset.executed`。 | `read_model.rebuild_requested`。 | all affected scopes。 | `data_reset.execute:{task_id}:{approval_id}`。 | admin + approved maintenance context。 | 只能 restore backup 或 replay import；不可物理删除无审计。 | fixture 应证明 API 返回 409/202，真实执行仅在 isolated staging。 | yes |
| 9 | `GET|POST /projects` | blocked_fact_source | platform-ops | high | legacy project route 的项目事实源未冻结；不得用 OA 或 Python state 猜测。 | 待新增 `app.project_profiles`。 | GET none；POST `create_project_profile`。 | POST: `project_profile.created`。 | POST: `project_profile.changed`。 | cost_statistics、search、workbench project filter。 | POST: `project.create:{external_project_id}:{payload_hash}`。 | GET `can_access_app`；POST `can_admin_access`。 | POST 软删除或状态回退。 | fixture 先冻结 project profile rows，再覆盖 list/create。 | yes |
| 10 | `GET /projects/{project_id}` | blocked_fact_source | platform-ops | high | project detail facts 未冻结。 | 待新增 `app.project_profiles`。 | none。 | none。 | none。 | none。 | none。 | `can_access_app`。 | none。 | fixture 使用单条 project profile detail。 | yes |
| 11 | `POST /projects/assign` | blocked_fact_source | platform-ops | high | assignment 目标对象和事实表未冻结。 | 待新增 assignment fact，或复用 `app.workbench_row_overrides` 的 `project_assignment` 类型但需字段确认。 | `assign_project(actor, source_object, project_id, expected_version)`。 | `project.assigned`。 | `project_assignment.changed`。 | workbench、search、cost_statistics。 | `project.assign:{object_type}:{object_id}:{project_id}:{expected_version}`。 | `can_mutate_data`。 | 写反向 assignment event 恢复旧项目。 | fixture 覆盖 bank/invoice/OA 三类对象；未知对象保持 blocked。 | yes |
| 12 | `GET /ledgers` | blocked_fact_source | platform-ops | high | ledger facts/read model 未冻结。 | 待新增 `app.ledgers` 或由 `app.reconciliation_cases`/`app.turnover_relations` 派生的 read model。 | none。 | none。 | none。 | none。 | none。 | `can_access_app`。 | none。 | fixture 先冻结 ledger projection 口径，再验 list。 | yes |
| 13 | `GET /ledgers/{ledger_id}` | blocked_fact_source | platform-ops | high | ledger detail source 未冻结。 | 同上。 | none。 | none。 | none。 | none。 | none。 | `can_access_app`。 | none。 | fixture 覆盖 detail、line items、audit timeline。 | yes |
| 14 | `POST /ledgers/{ledger_id}/status` | blocked_fact_source | platform-ops | high | ledger status mutation facts 未冻结。 | 待新增 `app.ledger_events` 或状态列。 | `change_ledger_status(actor, ledger_id, expected_version, status)`。 | `ledger.status_changed`。 | `ledger.changed`。 | ledger read model、search。 | `ledger.status:{ledger_id}:{expected_version}:{status}`。 | `can_mutate_data`。 | 写状态回退 event。 | fixture 覆盖合法状态流转和重复提交。 | yes |
| 15 | `GET /reminders` | blocked_fact_source | platform-ops | high | reminders 可能来自 jobs、ledger due dates 或 legacy state，事实源未冻结。 | 待新增 `app.reminders` 或 read model。 | none。 | none。 | none。 | none。 | none。 | `can_access_app`。 | none。 | fixture 先冻结 reminder source/type/status。 | yes |
| 16 | `POST /reminders/run` | blocked_fact_source | platform-ops | high | reminder execution 是 job-producing route，任务 payload 未冻结。 | 待新增 `app.reminder_runs`；现有 `job.worker_tasks/outbox_events`。 | `request_reminder_run(actor, scope, run_at)`。 | `reminder.run_requested`。 | `reminder.run_requested`。 | reminders read model。 | `reminder.run:{scope}:{run_at}`。 | `can_mutate_data` 或 admin，按提醒范围确定。 | 取消 queued task；完成后以新 run 补偿。 | fixture 覆盖创建 task/outbox，不发送真实通知。 | yes |
