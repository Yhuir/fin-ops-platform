...
状态：PF-P004 `verified`；PF-P005 `verified`；PF-P005-MG `planned`
...
- Groups refresh status stale/refreshing 时不会读取旧 Redis JSON payload，但当前实现仍会把 fresh DB payload 写入 Redis。PF-P006 需要判断这个写缓存行为是否符合 freshness 设计。
