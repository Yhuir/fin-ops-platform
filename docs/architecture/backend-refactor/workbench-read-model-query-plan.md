...
| `GET /api/workbench/groups` | `_handle_api_workbench_groups` | `_workbench_sql_read_repository.get_workbench_groups_page(...)` | `month`、`zone`、`page`、`page_size`、`status`、`source_kind`、区域级 `search`、`detail_level`、filters、sort | groups page、row counts、pagination、`read_model_status` | 只有 refresh status fresh 时使用 Redis；Redis key 包含 scope/version/search/filter/detail_level 维度；miss/stale enqueue refresh 并返回 `202 refreshing` 或 stale payload | `api.ts::fetchWorkbenchGroupsPage`、`types.ts::ApiWorkbenchGroupsPayload` | `frontend-used`：groups、pagination、row counts、group rows summary。区域搜索按 active structured group rows 命中并返回完整组；旧 `search_mode/search_by_pane` 已删除。`backend-only`：source_versions、active_generation_id、read_model_version 当前前端类型未显式承接。 | `test_workbench_sql_runtime.py::test_workbench_groups_api_uses_sql_groups_contract`；Redis 版本 key、区域搜索/filters/sort 组合契约已有回归 |
...
- Redis key：groups page cache 必须包含 scope、active generation/source version、detail_level、区域 search、filter、sort 等维度；旧三栏搜索 key 由 schema/version 变更淘汰。
