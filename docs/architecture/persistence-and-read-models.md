...
- `job.*` 保存后台任务、outbox 和三个保留 read model 的 dirty scope。
- `read_model.*` 只承载已登记的共享 read model；历史页面 projection 表不属于当前运行时合同。
...
三个 read model 都使用月份 shard；`all` 只做 fan-out command，不是可查询 projection。
...
- 已退役页面的 service、repository、projector、refresh producer、worker handler、registry/env、前端
  polling/status DTO 和页面专属运维工具必须保持删除。
...
- 发布前必须通过 registry 精确集合、退休事件/表访问负向扫描、三个保留 read model 的 freshness/worker
