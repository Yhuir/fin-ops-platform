...
| 流水规则批量处理 | 计划 `app.bank_flow_rule_batches`、`app.bank_flow_rule_batch_events`、规则 family `bank_flow_rule_batch_tag_requirements` | `bank-flow-rule-batches` | `BankFlowRuleBatchApplicationService`、明确 UoW、受控 rebaseline service | bank flow rule batch application/query ports | bank_flow_rule_batch、workbench_relation、workbench、bank_detail、turnover_ledger、search refresh | 旧 `selected_tag_codes`、shared state-store broad snapshot 或调用方直接改 batch/relation 状态。 |
| 免 OA 批次 | `app.no_oa_bank_batches`、`app.no_oa_bank_batch_events` | `no-oa-bank-batches` legacy owner | `NoOaBankBatchApplicationService`、明确 UoW、受控 legacy repair/rebaseline adapter | no-OA application/query ports、rebaseline dry-run | no_oa_bank_batch、workbench_relation、turnover_ledger、search refresh | shared state-store broad snapshot 写入、调用方直接改 batch 状态、或把旧 `selected_tag_codes` 迁移为新规则事实。 |
