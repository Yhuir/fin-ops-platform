...
| 银行流水 | `app.bank_transactions` | `imports-bank-transactions` + `bank-details` | 银行导入确认、import job、受控分类上下文更新 | bank transaction repository/query ports | bank_detail、workbench、turnover、no-OA、search refresh | 页面从 snapshot 加载全量流水后自行改写状态。 |
| 银行分类和标签 | `app.bank_transaction_categories`、`app.bank_transaction_category_events`、`app.bank_transaction_category_confirmations` | `bank-details` | bank detail category/rule/confirmation services | bank detail read/query ports、tag read facade | bank_detail、turnover_ledger、no_oa_bank_batch、search、workbench refresh | turnover/no-OA 直接写银行分类表。 |
...
| 免 OA 批次 | `app.no_oa_bank_batches`、`app.no_oa_bank_batch_events` | `no-oa-bank-batches` | `NoOaBankBatchApplicationService`、明确 UoW | no-OA application/query ports | no_oa_bank_batch、workbench_relation、turnover_ledger、search refresh | shared state-store broad snapshot 写入或调用方直接改 batch 状态。 |
