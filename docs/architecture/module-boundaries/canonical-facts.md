...
| OA 待付款准入/支付状态快照 | `app.oa_pending_payment_admissions`、`app.oa_pending_payment_status_snapshots`、`app.oa_sync_watermarks` 中 `oa_pending_payment_source:<tenant>:<scope>` | `oa-integration` / `oa-pending-payments` / `runtime-workers` | OA sync 的 `commit_authoritative_snapshot` 权威 replace/delete；relation-driven reconcile worker 的 `record_payment_statuses` 幂等增量同步；有 active outflow 写 `已支付`，无 active outflow 写 `待支付` | OA PG-only projector、Page Audit、`oa-sync` payment reconcile handler | 页面下一次 canonical GET 直接读取 | 页面直写或直读 Mongo/MySQL；金额相等作为 paid gate；失败状态被自动覆盖；外部写成功后不更新 PG snapshot；其它模块直接写这些表。 |
