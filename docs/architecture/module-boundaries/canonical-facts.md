...
| 流水规则批量处理 / Bank Transaction Paired Policy | `app.bank_flow_rule_batches`、`app.bank_flow_rule_batch_events`、`read_model.bank_flow_rule_batch_rows`、`app_settings.bank_flow_rule_batch_tag_rules` | `bank-flow-rule-batches` | `BankFlowRuleBatchApplicationService`、Bank Transaction Paired Policy rule writer、明确 UoW | bank flow rule batch application/query ports、policy payload | 规则资格变化只输出受影响月份 bank_flow_rule_batch refresh；导入/分类/自动标签规则变化经 owner lifecycle/UoW 输出本 scope；批次 relation 写入按 relation owner 合同输出跨页 refresh | 旧 `selected_tag_codes`、把所有 active tag 当未提交资格、原始规则版本污染 source version、规则保存 `all` refresh、旧 no-OA 历史重算、规则保存追溯改写 existing relation、shared state-store broad snapshot 或调用方直接改 batch/relation 状态。 |
