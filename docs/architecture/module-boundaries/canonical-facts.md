...
| OA 子项补充凭证 | `app.workbench_oa_supporting_documents`、`app.workbench_oa_supporting_document_bundles` | `reconciliation-workbench` | 凭证 service/repository 统一原子保存、审计和 matching 通知 | 凭证 GET/gallery/content、Workbench bounded hydration | 单次 canonical 回读与原 matching scope | 复制金额到每份文件、按 OA 猜未知金额、写正式发票或税额。 |
