...
| 银行流水 | `app.bank_transactions` | `imports-bank-transactions` + `bank-details` | 银行导入确认、import job、受控分类上下文更新 | bank transaction repository/query ports | bank_detail、bank_flow_rule_batch、workbench、turnover、no-OA、search refresh | 页面从 snapshot 加载全量流水后自行改写状态。 |
| 银行分类和标签 | `app.bank_transaction_categories`、`app.bank_transaction_category_events`、`app.bank_transaction_category_confirmations` | `bank-details` | bank detail category/rule/confirmation services | bank detail read/query ports、tag read facade | bank_detail、bank_flow_rule_batch、turnover_ledger、no_oa_bank_batch、search、workbench refresh | turnover/no-OA/流水规则批量处理直接写银行分类表。 |
...
| 流水规则批量处理 / Bank Transaction Paired Policy | `app.bank_flow_rule_batches`、`app.bank_flow_rule_batch_events`、`app_settings.bank_flow_rule_batch_tag_rules`；共享输入为银行/分类事实和 `app.workbench_pair_relations` active rows | `bank-flow-rule-batches` | `BankFlowRuleBatchApplicationService`、Bank Transaction Paired Policy rule writer、relation command、明确 UoW/delta writer | `BankFlowRuleBatchCanonicalQueryRepository`、policy payload | 页面在同一 repeatable-read snapshot 输出 summary/page/detail；规则资格变化只返回信息性 affected months；relation 写入按 relation owner 合同影响关联台 | `read_model.bank_flow_rule_batch_rows` 页面读取、Workbench relation projection、旧 `selected_tag_codes`、所有 active tag 默认合格、规则保存 refresh/enqueue、旧 no-OA fallback、追溯改写 existing relation、shared broad snapshot 或调用方直接改 batch/relation 状态。 |
| 免 OA 批次 | `app.no_oa_bank_batches`、`app.no_oa_bank_batch_events` | `no-oa-bank-batches` legacy owner | `NoOaBankBatchApplicationService`、明确 UoW、受控 legacy repair adapter | no-OA application/query ports | no_oa_bank_batch、workbench_relation、turnover_ledger、search refresh | shared state-store broad snapshot 写入、调用方直接改 batch 状态、或把旧 `selected_tag_codes` 迁移为新规则事实。 |
...
| OA 待付款银行关系 | `app.oa_pending_payment_bank_relations`、`app.bank_transaction_relation_claims`、`app.oa_pending_payment_bank_relation_events` | `oa-pending-payments` | OA pending payment relation service | OA pending payment read/query ports | oa_pending_payment、bank_detail、turnover_ledger、workbench_relation refresh | 其它模块直接 claim 银行流水关系。 |
