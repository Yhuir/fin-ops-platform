...
- 状态：PSCIP-L4 closed。
- 适用范围：当前 15 个 App Status read model；`bank_flow_rule_batch` 已登记为独立可执行 manifest/worker。流水规则批量处理对外使用 `bank_flow_rule_batch` operation target，freshness/readiness/outbox/worker 直接使用 `bank_flow_rule_batch` scope/event；mutation 和 refresh persistence 走 bank-flow 命名 IO，PostgreSQL 运行时查询 `read_model.bank_flow_rule_batch_rows`。no-OA legacy 继续使用 `read_model.no_oa_bank_batch_rows`。
...
- 2026-07-02 复核：生产 active release 在验证过程中从 `HEAD-ef1a13cd-20260702120002` 切换到 `HEAD-0811061c-20260702121133`，当前证据以 `HEAD-0811061c-20260702121133` 为准。该 release 的 `read_model_slo_smoke --apply --critical-only --target-ms 5000` grouped run 为 14/16 pass，`invoice_lifecycle:2026-01` 超过 5s、`tax_offset:2026-01` 在 grouped run 超时；随后 targeted rerun 两个 scope 均 pass，max enqueue-to-fresh `2297.862ms`，post-check `/health/ready` ready、scope contract clean、dirty/outbox/readiness 收敛、required worker 无 missing/stale/mismatch。该结果证明 direct read model 当前可收敛，但 grouped-run 长尾必须保留为性能风险。write-operation audit 发现 Workbench relation confirm/withdraw fan-out 最慢 34.844s；本地已关闭 confirm/withdraw UoW 的 repository 隐式 fan-out，并改为 UoW 显式 target planner。Admin Token authenticated HTTP/SSE、真实写操作矩阵和部署后 retest 证据未闭合时，不得声明当前 release 的 full external PSCIP-L4 或高性能全域闭环。
- 残余风险：Search 历史单次 grouped-run 高延迟样本需继续观察；Workbench groups admin smoke 的 `400` 是 probe shape 问题，不是 stale-as-fresh 证据；1s 高性能目标仍为 open。
...
| `cost_statistics` | `cost_statistics` | `partitioned_scoped_parent_rollup` | `queryable_parent_aggregate` | active/all month + parent aggregate；月份 shard 只消费 Workbench active generation，父 scope 从成本统计物化 shard 聚合；允许父聚合查询语义 | `cost-statistics` | `CostStatisticsQueryService` | `CostStatisticsReadModelRepositoryPort` | `cost_statistics_api_session` | `tests/test_cost_statistics_sql_runtime.py` |
...
- Workbench confirm/withdraw 的事务内 writer owner 是 `WorkbenchWriteUnitOfWork`；`Application._workbench_uow_repository_factory(...)` 必须注入 `PostgresWorkbenchRelationRepository(transaction, enqueue_refreshes=False)`。新增 downstream read model 必须扩展 `refresh_metadata.downstream_scope_types` / target planner 和测试，禁止把旧 repository hidden fan-out 放回生产主链路。
