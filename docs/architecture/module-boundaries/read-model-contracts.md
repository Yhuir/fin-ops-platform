...
- 状态：PSCIP-L4 closed。
- 适用范围：当前 15 个 App Status read model；`bank_flow_rule_batch` 已登记为独立可执行 manifest/worker。流水规则批量处理对外使用 `bank_flow_rule_batch` operation target，freshness/readiness/outbox/worker 直接使用 `bank_flow_rule_batch` scope/event；mutation 和 refresh persistence 走 bank-flow 命名 IO。共享物理存储期内，保存、删除和 snapshot merge 必须以 `relation_mode` 隔离 no-OA 与 bank-flow rows。
