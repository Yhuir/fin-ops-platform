...
- 2026-07-02 复核：生产 active release 在验证过程中从 `HEAD-ef1a13cd-20260702120002` 切换到多个性能切片 release；当前最新证据以 `pscip-l4-workbench-insert-5f530d1b5` 为准。该 release `/health/ready` ready，required worker missing/stale/mismatch 为 `0/0/0`，scope contract default `ok=true`、invalid read model scope count `0`。最新 `read_model_slo_smoke --apply --critical-only --target-ms 5000` grouped run 为 14/16 pass，`turnover_ledger:all` `5591.378ms`、`bank_flow_rule_batch:2026-02` `5445.482ms` fail；targeted retry 两者分别 `993.910ms`、`455.961ms` pass，说明存在 grouped-run 尾延迟。Workbench 1s targeted smoke 仍 fail，最新样本 `1485.007ms`；72h `write_operation_slo_audit` 仍显示 Workbench relation confirm/withdraw cross-page 历史长尾，按新 release 激活时间过滤后 confirm/withdraw/no-OA withdraw 缺当前 release 真实写样本。该结果证明 read model runtime 可收敛，不能证明 full external PSCIP-L4 或高性能全域闭环。
