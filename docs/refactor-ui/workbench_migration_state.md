...
从远端最新 `refactor-ui` 继续执行 `P-WB006-css-containment-cleanup`。该 prompt 只清理 workbench `.Mui*` CSS selectors 和迁移后的 project class styles，不处理 legacy test provider 或依赖。
