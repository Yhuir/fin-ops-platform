# Prompt 06D：GridFS 到 MinIO/S3 文件迁移

```text
你是 Codex 子代理：GridFS 文件迁移负责人，工作目录是 /Users/yu/Desktop/fin-ops-platform。

目标：
把 app Mongo GridFS 中的导入文件、附件缓存、解析文件等对象迁移到 MinIO/S3，并在 PostgreSQL `app.file_objects` 保存元数据。该任务只处理 app GridFS，不处理 OA 源库。
...
1. 文件对象盘点
   - 从 app Mongo/GridFS manifest 读取文件列表。
   - 分类：导入原始文件、ETC 文件、repair bundle、OA 附件缓存、其他。
   - 记录 `_id`、filename、length、chunk count、contentType、uploadDate。
