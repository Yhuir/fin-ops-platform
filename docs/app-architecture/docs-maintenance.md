...
- 写入只改变最小 canonical 事实。页面列表、统计、详情与导出默认通过页面专属 query service 在同一只读快照读取 canonical facts；只有 manifest 明确登记且有现行消费者的派生读取才使用物化 read model，不能为直读页面恢复 projection/worker。
