...
- 周期性 `oa.sync` 必须是 change-driven：completed OA、status、admission 与上一 snapshot 一致时只记录 run/watermark，不更新 projection/status/admission 的业务时间戳，也不 fan-out Workbench/OA/成本等页面 refresh。若无业务变化却持续出现页面 dirty/outbox 或 `app.oa_applications.updated_at` 漂移，按同步 owner 回归处理，禁止通过扩大 worker 数量掩盖写放大。
