...
- 页面读取 `project|bank|expense_type` 的月份或 `active:all` / `all:all` 时，query owner不能只看 parent readiness。它用 canonical/set-based Workbench month proofs与 active generations找出 exact stale Workbench scopes；上游 stale 时只 ensure这些 Workbench months并返回`202`，由页面已有的有界自动轮询继续。依赖匹配后的轮询才让 Cost gate比较 concrete child的 Workbench/Bank Detail versions、latest dirty与 parent `source_shards`并enqueue exact stale child，避免Cost任务在依赖未完成时反复defer/claim和争用I/O；child成功后沿 month→parent收敛。`time|bank_tag` 使用独立 Bank Detail profile，只检查/ensure Bank Detail exact month scopes并直接读取其 rows，不入队 Workbench或Cost。global statistics可独立 non-fresh，不能反向阻塞这两个视图。
