...
- 页面读取 `project|bank|expense_type` 的月份或 `active:all` / `all:all` 时，query owner不能只看 parent readiness。它用 canonical/set-based Workbench month proofs与 active generations找出 exact stale Workbench scopes，并在同一次页面访问分别 ensure这些 Workbench months与同 project scope的 Cost children；Cost child可并发入队，但worker必须在 Workbench匹配前defer且零payload/publish I/O，因此不需要第二次页面访问。依赖匹配后，Cost gate比较 concrete child的 Workbench/Bank Detail versions、latest dirty与 parent `source_shards`；child成功后沿 month→parent收敛。`time|bank_tag` 使用独立 Bank Detail profile，只检查/ensure Bank Detail exact month scopes并直接读取其 rows，不入队 Workbench或Cost。global statistics可独立 non-fresh，不能反向阻塞这两个视图。
