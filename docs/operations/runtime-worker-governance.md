...
- 页面读取 `project|bank|expense_type` 的月份或 `active:all` / `all:all` 时，query owner不能只看 parent readiness。它用 canonical/set-based Workbench month proofs与 active generations找出 exact stale Workbench scopes；上游 stale 时先 ensure这些 Workbench months，并在同一次访问中只 stage当前请求的唯一 Cost scope。Cost worker在任何 payload I/O 前用 manifest dependency fail closed/defer，精确补投缺失 child，依赖匹配后才发布；页面有界轮询只观察同一 scope，不负责串行发现下一阶段。child成功后沿 month→parent收敛。`time|bank_tag` 使用独立 Bank Detail profile，只检查/ensure Bank Detail exact month scopes并直接读取其 rows，不入队 Workbench或Cost。global statistics可独立 non-fresh，不能反向阻塞这两个视图。
