...
- 页面读取 `project|bank|expense_type` 的 `active:all` / `all:all` 时，query owner不能只看 parent readiness。它用 set-based canonical Workbench month proofs与 active generations找出 exact stale Workbench scopes；upstream全部 fresh后，Cost gate比较每个 concrete child 的 Workbench/Bank Detail versions、latest dirty与 parent `source_shards`，只 enqueue exact stale Cost children。child成功后沿 month→parent收敛。`time|bank_tag` 使用独立 Bank Detail profile，只检查/ensure Bank Detail exact month scopes并直接读取其 rows，不入队 Workbench或Cost。global statistics可独立 non-fresh，不能反向阻塞这两个视图。
