...
- `read_model.workbench_generations` 的终态历史 generation 应受 retention 控制。建议告警阈值：总 generation 数超过 300、`read_model.workbench_*` 总大小超过 10GB、根分区可用空间低于 20GB 或 `pg_wal` 异常增长。timer 每天 03:35 运行并带最多 15 分钟随机延迟；先检查 `systemctl status finops-prune-workbench-generations.timer finops-prune-workbench-generations.service`、service journal、`/var/log/fin-ops/workbench-generation-prune-YYYYMMDD.log` 和 worker 是否持续重复发布同一 scope。日志 policy 应显示 `keep_recent=1 keep_days=0 limit=500 delete_batch_size=1 statement_timeout_seconds=60`（或 operator override）。若后续小批超时，已提交批次保留、当前 service 失败，下一次 timer 会从剩余 `superseded|failed` 候选继续；不得因此手工删除 active/building generation。清理完成后的 ready health 检查失败会让 helper/service 非零退出；此时先按日志确认清理报告和已提交批次，再处理 API 健康问题，不能把 service failure 当作 generation 删除事务自动回滚。
