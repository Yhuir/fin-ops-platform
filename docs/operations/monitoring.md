...
- OA 同步状态。
- 唯一保留 read model `workbench_relation` 的 dirty scopes。
- 关联台 direct API 的 endpoint/SQL/connection-pool latency、payload、timeout 和错误率。
- 后台任务状态。
...
- Workbench direct endpoint 出现 timeout、5xx、连接池等待、temp spill 或 SQL 扫描行数异常增长。
- 当前 release 出现任何 `workbench.read_model.refresh` enqueue/claim/publish、page Redis get/set、`/api/workbench/refresh-status` 调用或 `read_model.workbench_*` runtime I/O；这些均表示退休链路被重新接入。
...
- Redis `redis_miss_count` 快速增长且登记 read model 的 PostgreSQL 热读压力同步升高；关联台 direct API 不应产生 page Redis hit/miss。
- 数据重置任务异常结束。
- `workbench_relation` 返回 `read_model_unavailable` 表示共享 SQL read repository 未配置或初始化失败；关联台 direct repository 失败应返回 `workbench_query_unavailable`，两者都禁止回落旧 snapshot/generation。
...
## Workbench Direct SQL 卫生

- 先在源表下推 month/status/visibility，再选择 touched active relations；relation array GIN 只能做候选剪枝，最终必须按 `(row_type,row_id)` 与 ordinality 精确复核。
- initial 在一个 read-only snapshot 内共享一次 group spine；pagination 使用 query-bound keyset cursor，禁止 `OFFSET`、per-group correlated scans、N+1 hydration 和 raw JSON 全表搜索。
- summary 列表只 hydrate 当前页，禁止 `detail_fields`、raw payload、OCR/附件全文进入列表响应。
- 索引只在隔离数据库 `EXPLAIN (ANALYZE, BUFFERS)` 证明必要后增加；生产只执行 bounded authenticated GET 和 plain `EXPLAIN`。不得为追求直读延迟重新建立 page projection/cache。
- 历史 `read_model.workbench_*` 索引与表只作离线回滚证据；不得恢复或用于当前查询计划。

## 日志要求

- 日志应包含请求路径、用户、动作、耗时和错误摘要。
- worker 日志应包含 `queue_event_id`、`event_type`、`attempts`、`trace_id` 和 `source_version`。
- Workbench direct API 指标按 `endpoint` 聚合 HTTP duration、connection acquire、SQL execute/fetch、database query count、payload bytes、timeout 和 error 的 p50/p95/p99。
- 退休链路零值指标单独监控 page event enqueue/claim/publish、page table I/O、page Redis get/set 与 refresh-status 请求；任一非零都应排查代码或旧浏览器/代理调用。首次 direct cutover 的 release gate 会在候选 T+0 请求前启动观测，在 T+300 后比较 event/scope total+watermark、`pg_stat_statements` calls/rows 与旧表累计 stats；任一动态观测面不可用或 stats reset 均阻断并进入既有 rollback。Redis page cache 不使用全局 `MONITOR`：release evidence 只声明绑定 candidate commit 的 retired source owner 缺席和 direct read source 无 Redis helper 调用 marker，这是机械 source/runtime-owner proof，不宣称 Redis 动态 delta。
...
- `/health/ready` 只计算当前 blocker：PostgreSQL、release/runtime guard、required worker heartbeat，以及 critical `workbench_relation` 的 current-effective failed/dead-letter、readiness 缺失/失败和超过 300 秒的 dirty scope。关联台 direct API 由 endpoint/SLO 与 page canonical audit 证明，不登记 read-model readiness。普通短暂 pending/processing、非 critical backlog 和历史已覆盖 failure 不阻断。
...
正确而页面分组错误，应修复 direct group-spine SQL 或 canonical DTO assembler 后重新部署。禁止通过刷新旧
projection、rehydrate generation 或手改 `read_model.*` 掩盖错误。

如果 relation facts 发生变更，只通过既有 runtime queue/backfill 入口刷新仍登记的
`workbench_relation` scope并标记 matching 的精确月份；关联台和其它 direct-canonical 页面由下一次 API
...
- `workbench_relation` 的 read-model metrics，以及 Workbench direct endpoint 的 API/SQL/pool/error metrics；不得继续导出当前 page Workbench generation 指标。
...
该工具默认 dry-run，只选择唯一保留 read model `workbench_relation` 中已有 fresh readiness 的 direct scope；显式加
...
- 工作台 combined initial/groups/filter-options/settings、银行明细账户/流水/规则、待找发票 rows/filter-options/rules、进项发票使用
...
- 登记 read-model API 可接受 `200` 或 `202`，并必须记录自己的 `read_model_status`、`cache_status` 和 `refresh_enqueued`。Workbench direct API 只接受业务成功 `200`，不返回这些字段，只以 HTTP/result contract 和延迟判定。
...
App Health 和 OA sync 保持各自有界 polling；Workbench page freshness polling 已删除。生产验证必须同时采样 `/api/app-health`、`/api/oa-sync/status`、`/health/ready` 和 Workbench combined initial/groups/facet；返回 HTML fallback、认证失败、错误状态、超时或零样本都不能算通过，并确认 `/api/workbench/refresh-status` 不再进入当前请求链。
...
不表示当前 registry 或 required worker 重新接线这些 scope。当前唯一允许的 read-model 集合为 `workbench_relation`；历史 `workbench.read_model.refresh` 也只作为 forbidden 污染签名。
...
## Workbench Direct 读 API 验证

生产和 staging 使用以下分层合同：

- `/api/workbench?month=...`：唯一首屏入口，在一个 `REPEATABLE READ READ ONLY` snapshot 中返回 exact summary 与 paired/unpaired 各 50 个 compact groups。
- `/api/workbench/groups?...&detail_level=summary&cursor=...`：单区搜索、筛选和 keyset pagination；cursor 与 scope/zone/sort/filter/search 绑定。
- `/api/workbench/filter-options?...&cursor=...`：惰性 facet，移除目标列自身条件，其它条件继续生效。
- `/api/workbench/groups/detail?...&detail_key=...` 与 `/api/workbench/rows/:id?row_type=...`：用户动作触发的有界完整详情。
- `exception_bucket=active|processed`：服务端 set-based anomaly 筛选和 exact count；浏览器不得 drain 全量 full-detail pages。
- 列表响应禁止 `detail_fields`、raw/source payload、OCR 或附件全文；GET 禁止 enqueue、Redis page cache、generation/freshness I/O。

实时加载判读：

- `workbench_query_unavailable`：检查 PostgreSQL connection pool、statement timeout、query plan 和 direct repository 初始化；不得回落旧 generation。
- 写已提交但 reread 失败：页面提示“写已提交、重新读取失败”，不要重试 mutation；恢复数据库读取后执行 normal GET。
- OA 同步中：继续查看独立 `/api/oa-sync/status` 与 OA worker；它不是 Workbench page freshness。
- OA 附件正式发票缺失：检查 canonical `app.invoices` promotion/source link 与 OA attachment identity bridge，使用 OA owner 的受控 sync/repair 入口；不得使用已退休的 Workbench generation rehydrate 修 canonical facts。

验证顺序：

1. 隔离 PostgreSQL 对 candidate、hydration、search/filter/facet/detail 执行 `EXPLAIN (ANALYZE, BUFFERS)`，记录 scanned/returned rows、buffers、temp spill 和 query count。
2. 生产只运行 authenticated bounded GET 与 plain `EXPLAIN`；分别记录 combined initial、groups、facet、detail、exception 的 p50/p95/p99、payload、errors、pool acquire 和 SQL execute/fetch。
3. 验证 direct hard contract：错误为 0、专项 P95 不超过 1000ms、P99 不超过 2000ms；P50 600ms、P95 800ms、P99 1200ms 是优化目标，不伪装为已保证结果。
4. 在混合端点巡检中确认其它页面、`workbench_relation` 和 matching 正常，并证明 retired page event/cache/table/status I/O 为零。
5. 若真实 plan 证明缺索引，只增加最小、可解释索引；若重写后仍不达 SLO，应明确放宽 exact/all/search 产品合同或重新评估物化结构，禁止暗加 cache/双链。
