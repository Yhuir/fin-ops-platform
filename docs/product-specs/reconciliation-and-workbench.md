...
- “录入发票”只打开一个右侧抽屉，默认直接显示复用发票导入页的多张发票编辑器；编辑器同时支持 `JPG/PNG/PDF` 上传识别和人工校正。提交只以发票强身份创建新 canonical invoice，或把已存在的唯一 canonical invoice 显式关联到当前 OA 子付款项；疑似重复、多个候选或身份不足必须整批拒绝，禁止按金额猜测。`补充凭证` 是次级模式，仅保存当前 OA 子付款项的报销证明材料，可预览/删除且不进入统一发票池、不创建正式关系成员。有效凭证满足对应明细的资料要求，隐藏该项缺票提示和“录入发票”；仅凭证覆盖的金额不参与正式发票金额比较，OA—银行金额仍按真实金额比较。存在正式发票时仍核验其金额，不能把上传图片表述为真实发票金额一致。删除最后一份有效凭证后恢复实际资料状态。凭证在对应明细同行的发票栏全宽显示文件名，可点击预览并管理。发票确认、`oa_expense_item_invoice` 来源边和正式关系扩展必须在同一事务成败；显式来源边是当前展示归属，旧附件来源继续保留为审计证据。禁止抽屉套抽屉、第二发票池和先入池后关系失败的半成品。
...
- 异常抽屉的最外层“未配对异常 / 已配对异常”切换与标题同行；其下才是“金额异常 / 仅资料异常”两个互斥视图。金额异常视图按 `OA = 流水`、`OA = 发票`、`流水 = 发票`、`三项互异` 四个父组展示上述七种金额分类及唯一关系数，默认选中固定顺序中的首个非零分类；这些子分类不属于仅资料异常。仅资料异常只包含“没有金额异常、但至少存在一种附件异常”的关系。一个关系同时存在金额与资料异常时只归入其唯一金额分类，资料异常继续作为该关系的附属 Chip 展示，不得再次出现在“仅资料异常”或其它金额分类中；每个数量都按唯一关系计数，而不是按异常 item 数计数。
