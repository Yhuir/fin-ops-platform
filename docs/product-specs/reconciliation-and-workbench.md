...
- active paired relation 同时满足“无 OA、至少一条且全部为收入流水、至少一张且全部为销项发票、CNY、付款方和发票号完整”时，OA 栏显示“待补收据 / 打印收据”。点击直接打开浏览器打印流程，服务端按付款方、交易日期和币种生成 A5 横向双联 PDF：收据金额取收入流水合计，逐张列出销项发票；多流水、多发票均保留，发票合计与收入合计不一致不阻断。相同 relation 内容指纹复用不可变 PDF，关系事实变化生成新快照；该派生收据不进入统一发票池、不成为关系成员、不改变三栏统计或配对状态，只记录“已生成”和“已请求打印”，不得伪称已经实际打印。
