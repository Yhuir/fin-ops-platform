...
- OA 附件发票优先通过显式 `source_expense_item_id` 与付款明细对齐；只有单张发票价税合计与单个付款项按分精确相等且双方唯一时才进入同一展示带。缺少显式来源时，允许在同一完整关联组、方向已知且金额双方唯一时做纯视觉精确金额兜底；重复金额、金额不一致、一对多、多对一、金额组合、项目名或顺序推断一律保留在残余展示带。该视觉同行不创建或修改正式关系。每个付款项的“申请事由”继续显示来源“费用内容”和“费用说明”。
