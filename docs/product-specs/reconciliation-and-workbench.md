...
- 发票来源与发票归属是两个独立合同。`source_kinds[]` 只用于展示同一 canonical 发票已经具有的来源证据，允许同时显示“OA附件 + 导入记录 + 明细归属”；`source_kind` 继续作为单值结构兼容字段，不能覆盖复数来源。只有 `source_expense_item_ids[]` 决定发票与 OA 子付款项同行。OA 附件来源和显式明细归属必须在后续 Excel 导入中保留；未知来源不得默认伪装为“人工导入”。
