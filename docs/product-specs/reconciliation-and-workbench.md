...
- 关系在 OA、流水、综合核对金额与方向都可比较时归为七种互斥整组结果：`OA 流水一致，票多`、`OA 流水一致，票少`、`OA 发票一致，付多`、`OA 发票一致，付少`、`发票流水一致，OA 提少了`、`发票流水一致，OA 提多了`、`三项不一致`。综合核对金额为去重正式发票加已确认的凭证实际金额，未知金额不补值。正式发票的局部 OA—发票差异保留既有定位规则；凭证子项已知差额在无整组分类时单独归为“明细金额不一致”，不能被其它子项反向差额抵消。
...
- “录入发票”打开单一右侧抽屉，默认复用发票导入页的多张发票编辑器，支持 `JPG/PNG/PDF` 上传识别和人工校正。提交以强身份创建或复用唯一 canonical invoice 并关联当前 OA 子项；身份不足或冲突整批拒绝。次级“补充凭证”模式保存多份文件和一个总金额，可预览、编辑和清空，不进入统一发票池或正式关系成员。有效文件且金额明确才满足凭证资料要求；凭证实际金额参与核对，OA—银行继续按真实金额比较。同项正式发票与凭证金额合并核验；凭证金额只表示凭证自身合计，不含已有发票。保存时记录当前发票依据，后续发票身份或金额变化要求重新确认凭证金额。清空文件后金额为空并恢复实际资料状态。凭证在对应明细同行显示文件和总金额。发票确认、来源边和关系扩展同事务成败；OA 附件来源优先，人工录入来源不能覆盖它。不另设外部“选择已有发票”按钮，录入仍可复用已有同一张发票。
...
每个子付款项可上传多份凭证，但只填写一次总金额；非空文件组保存必须有明确非负、两位小数金额。正式发票与凭证金额按同项合计，凭证多份文件只计一次；发票依据变化后要求重新确认凭证金额，防止同笔支出被重复计额。凭证是核对依据，不是税务发票；未知金额不按 OA 或文件名推断。文件集合与金额一次保存，清空后不计额。主表和异常抽屉共用三栏与凭证操作，外部已有发票选择按钮移除，录入仍可复用已有同一张发票。整组七种金额分类之外，凭证子项差额被总额抵消或缺少整组可比条件时使用“明细金额不一致”；有凭证无金额提示待填写。进行中 OA 不进入已配对。

### 子付款项录入与部分补齐（2026-09-21）

- 一个子付款项可绑定多张发票，已有票时录入只追加，不覆盖此前票；同一 canonical 发票只计一次。缺票入口为“录入发票”；已有票或凭证且已知合计小于 OA 子项金额时，在左上角显示“+”，点击打开同一录入抽屉。相等、超额或金额未知时不显示追加入口。主关联台与异常处理共用子项布局和目标定位。
- 取消普通行的“选择已有发票”和“更改归属”；OA 附件解析必须携带原子项来源，人工补录必须携带明确目标。没有来源或存在歧义的历史人工票不能通过隐藏异常假装已经归属。
- 同一正式关联内，单独归属于某项的已有发票按实际金额扣减，剩余金额唯一时才补齐后到票；例：71 元已有 23 元，唯一剩余的 23+25 元补齐后为三张。来源标签保持真实，金额推断不得升级为 OA 附件来源。共享票仍按连通分量核对，不能扣到每个子项中。
