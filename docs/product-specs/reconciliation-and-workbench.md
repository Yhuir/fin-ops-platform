...
- 日常报销子付款项声明已上传附件，但该 item 没有任何已解析且显式绑定的发票时，产生 `oa_invoice_attachment_missing`。发票栏在对应 item 行显示一个不可选择、不可写入 canonical 数据的 `OA发票附件缺失` 占位 chip；它只属于展示和异常处理，不是发票事实。
