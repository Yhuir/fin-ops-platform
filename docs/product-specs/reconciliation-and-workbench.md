...
- active relation 同时满足“无 OA、至少一条且全部为收入流水、至少一张且全部为销项发票、CNY、付款方和发票号完整”时，无论当前位于 `paired` 还是 `unpaired`，OA 栏都只显示一个“编辑并打印收据”动作；无 active relation 的 singleton、支出流水、进项发票或混合关系不显示。点击先打开单一右侧编辑抽屉，按 `银行收据!A1:J12` 的 A5 横向一联版式预览，允许编辑付款单位、日期、摘要、金额、备注、主管和经手人；每张收据按付款方、交易日期和币种分组，收入流水合计是不可变核对金额，明细合计必须与其一致后才能打印。红票只有备注精确包含 `被红冲蓝字数电发票号码：<20位号码>` 且蓝票唯一时才冲销：完全冲销的红蓝票均不进明细，部分冲销只保留蓝票净额；模糊、缺失、歧义或超额冲销必须显式提示并经人工核对确认，不得猜测。提交打印时服务端重读 active relation、校验关系版本、来源指纹和编辑后明细；相同编辑内容复用不可变 PDF，关系、来源或编辑内容变化生成新快照。该派生收据不进入统一发票池、不成为关系成员、不改变三栏统计或配对状态，只记录“已生成”和“已请求打印”，不得伪称已经实际打印。
