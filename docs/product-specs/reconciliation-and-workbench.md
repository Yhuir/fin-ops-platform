...
- 关系在 OA、流水、综合核对金额与方向都可比较时归为七种互斥整组结果：`OA 流水一致，票多`、`OA 流水一致，票少`、`OA 发票一致，付多`、`OA 发票一致，付少`、`发票流水一致，OA 提少了`、`发票流水一致，OA 提多了`、`三项不一致`。综合核对金额为去重正式发票加未被正式发票覆盖的凭证实际金额，未知金额不补值。正式发票的局部 OA—发票差异保留既有定位规则；凭证子项已知差额在无整组分类时单独归为“明细金额不一致”，不能被其它子项反向差额抵消。
...
- “录入发票”打开单一右侧抽屉，默认复用发票导入页的多张发票编辑器，支持 `JPG/PNG/PDF` 上传识别和人工校正。提交以强身份创建或复用唯一 canonical invoice 并关联当前 OA 子项；身份不足或冲突整批拒绝。次级“补充凭证”模式保存多份文件和一个总金额，可预览、编辑和清空，不进入统一发票池或正式关系成员。有效文件且金额明确才满足凭证资料要求；凭证实际金额参与核对，OA—银行继续按真实金额比较。有效正式发票来源覆盖该子项时优先核验正式发票，凭证保留但不重复计额。清空文件后金额为空并恢复实际资料状态。凭证在对应明细同行显示文件和总金额。发票确认、来源边和关系扩展同事务成败；OA 附件来源优先，人工录入来源不能覆盖它。不另设外部“选择已有发票”按钮，录入仍可复用已有同一张发票。
...
- active relation 同时满足“无 OA、至少一条且全部为收入流水、至少一张且全部为销项发票、CNY、规范化付款方唯一、付款方和发票号完整”时，无论当前位于 `paired` 还是 `unpaired`，OA 栏都只显示一个“编辑并打印收据”动作；无 active relation 的 singleton、多个付款方、支出流水、进项发票或混合关系不显示。点击先打开单一右侧编辑抽屉，按 `银行收据!A1:J12` 的 A5 横向一联版式预览，允许编辑付款单位、日期、摘要、金额、备注、主管和经手人；一个 relation 固定生成一张收据，多条收入流水全部合计且默认使用最新交易日期，不按每笔流水日期拆分，也不按发票购买方猜测归组。收入流水合计是不可变核对金额，明细合计必须与其一致后才能打印。红票只有备注精确包含 `被红冲蓝字数电发票号码：<20位号码>` 且蓝票唯一时才冲销：完全冲销的红蓝票均不进明细，部分冲销只保留蓝票净额；模糊、缺失、歧义或超额冲销必须显式提示并经人工核对确认，不得猜测。提交打印时服务端重读 active relation、校验关系版本、来源指纹和编辑后明细；相同编辑内容复用不可变 PDF，关系、来源或编辑内容变化生成新快照。该派生收据不进入统一发票池、不成为关系成员、不改变三栏统计或配对状态，只记录“已生成”和“已请求打印”，不得伪称已经实际打印。
...
- 异常抽屉的最外层“未配对异常 / 已配对异常”切换与标题同行；其下才是“金额异常 / 仅资料异常”两个互斥视图。金额异常视图按 `OA = 流水`、`OA = 发票`、`流水 = 发票`、`三项互异` 四个父组展示七种整组金额分类，并由“费用明细”父组展示“明细金额不一致”，各自按唯一关系计数，默认选中固定顺序中的首个非零分类；这些子分类不属于仅资料异常。仅资料异常只包含“没有金额异常、但至少存在一种附件异常”的关系。一个关系同时存在金额与资料异常时只归入其唯一金额分类，资料异常继续作为该关系的附属 Chip 展示，不得再次出现在“仅资料异常”或其它金额分类中；每个数量都按唯一关系计数，而不是按异常 item 数计数。
- 金额分类是服务端事实筛选，不是付款、退款、补票或 OA 草稿的自动执行指令。当前抽屉继续只负责异常审阅与分区；“发票附件未解析”可复用既有“录入发票”闭环，“发票附件缺失”不新增复杂处置。不得因为分类名称自动创建 OA、付款或退款。
- 任一异常默认阻断关系进入已配对区，并进入抽屉“未配对异常”。分类与当前证据指纹完全由服务器计算；用户只选择“接受异常并进入已配对”或“留在未配对”，不得提交或覆盖分类、证据项或 actor。
- “接受异常并进入已配对”只在 relation 其余完整性条件满足时成功；感叹号和原异常 Chip 保留并进入“已配对异常”，Popover 另行显示审阅人、时间和说明，不给 Chip 添加伪状态前缀。“撤回到未配对”写入留在未配对决定，并让主表完整关系同步回到未配对区。两种动作完成后，当前抽屉 bucket、异常类型与金额子分类保持不变，只从当前列表移除已迁移关系；用户需要查看目标 bucket 时再显式切换。两种动作均不修改正式关系、成员、canonical 金额或附件事实。
...
ETC 批次已提交且 OA 来源唯一确认后，OA 与 ETC 汇总发票先形成正式关系，无需等待银行或审批完成。来源依据是显式 OA owner、既存结构化批次标识，或本批上传附件的完整精确路径集合；不能用同金额/备注猜来源。无银行时同组待补全，银行先到/后到均复用同一关系。OA 应付金额用于核对流水，发票保留真实金额及差额。支付申请使用收款人而非申请人，缺明确银行业务引用时要求同币种/支出方向、正确账户（双方有值时）、30 天内同收款人和唯一金额闭环。重复候选留待人工确认。撤回银行保留 OA＋ETC 来源，人工拒绝不被自动重建；审批进行中即使三项齐全也保持在未配对区，正式关联及同行展示继续保留；审批完成且其余完整性与异常条件满足后才进入已配对，成本仍按原审批规则处理。

## 子付款项补充凭证总金额（2026-09-21）

每个子付款项可上传多份凭证，但只填写一次总金额；非空文件组保存必须有明确非负、两位小数金额。正式发票优先，凭证不与覆盖同一子项的正式发票重复计额。凭证是核对依据，不是税务发票；未知金额不按 OA 或文件名推断。文件集合与金额一次保存，清空后不计额。主表和异常抽屉共用三栏与凭证操作，外部已有发票选择按钮移除，录入仍可复用已有同一张发票。整组七种金额分类之外，凭证子项差额被总额抵消或缺少整组可比条件时使用“明细金额不一致”；有凭证无金额提示待填写。进行中 OA 不进入已配对。
