...
- active 异常在发票来源 chip 下一行显示 `金额不一致`，并持续进入统一“异常处理”右侧抽屉的“进行中的异常”。抽屉默认按关系组折叠为一行，只显示 OA、银行流水和发票各自的成员数与总金额；展开后才显示完整三栏明细，同一时间最多展开一组。
- full-access/admin 可以忽略 active 异常；忽略不修改正式关系和 canonical 金额，主表 chip 改为 `已忽略：金额不一致`，并进入同一抽屉的“已忽略的异常”。用户可通过关系组右侧的“撤回忽略”恢复，恢复后重新进入进行中；read-export 只可查看，不显示写按钮。
