...
银行明细页面直接从 PostgreSQL canonical facts 展示银行原始字段、有效标签和关系状态：

- 原始银行字段必须保留可追溯性。
- 标签结果来自统一分类/策略，不由页面临时判定。
- 写成功后只重新读取一次当前 canonical query，不维护页面 read model、轮询或跨页面通知。
...
- `source=manual, manual_assignment=true` 的人工分类是持久覆盖，优先于系统自动分类；再次人工选择时原子替换旧分类/确认事实，清除人工覆盖后才恢复当前自动规则结果。
- `待分类`、`待确认` 和自动标签“撤销”后的重新分类菜单都允许选择系统 `内部往来款`；自动标签“撤销”本身不落空事实，只有保存新标签才提交覆盖。
- 规则变更不追溯覆盖人工分类；受影响页面按各自 canonical/read-model 边界读取最新事实。
...
- 外部往来闭环通常由用户在外部往来款管理页手动选择同一往来组内真实流水确认；可选择多笔流水，但必须同时包含收入和支出，且收支合计差额为 `0.00`。关联台在用户同时选择 OA 与完整闭环流水时，也可复用同一个 canonical Turnover 校验器直接确认：全部银行成员必须有明确的外部往来 role/action/family/counterparty、属于同一业务语义且本金与结算差额为零。缺少这些结构化证明时不得从文本或金额猜测为闭环。
...
- 对已确认且 `relation_mode=turnover_manual_closure` 的零差额闭环，关联台不得用通用银行净额 `支出-收入=0` 去与同组 OA 付款本金比较；应使用闭环的支出本金侧，因此同额借入/归还或支出/回款本身不产生 `OA流水金额不一致`。这一规则只对外部往来页或关联台经 canonical Turnover 校验器写出的 closure mode 生效，不扫描摘要、备注或人工标签文本，也不豁免真实本金差异。
