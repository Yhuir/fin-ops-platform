...
- 对已确认且 `relation_mode=turnover_manual_closure` 的零差额闭环，关联台不得用通用银行净额 `支出-收入=0` 去与同组 OA 付款本金比较；应使用闭环的支出本金侧，因此同额借入/归还或支出/回款本身不产生 `OA流水金额不一致`。这一规则只对外部往来模块写出的 canonical closure mode 生效，不扫描摘要、备注或人工标签文本，也不豁免真实本金差异。
