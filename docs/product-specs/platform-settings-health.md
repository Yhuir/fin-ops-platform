...
- `/settings` 的“访问账户”是唯一人工 ACL 输入，使用独立的 005-only ACL API；普通设置保存与 ACL 加载/保存完全分离。005 为其他 OA 账号勾选页面集合，删除账号或页面集合为空表示 `denied`。
...
- `finops:app:view` 只定位 OA 菜单；OA 仅接收 canonical ACL 到 `finops_app_user` / `finops_admin` 两类专用角色的严格投影。历史分层角色和非专用菜单绑定只由 migration/preflight 识别，不属于页面写入或运行时授权。
