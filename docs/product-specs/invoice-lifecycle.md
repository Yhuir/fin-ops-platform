...
- 付款状态只有“已支付/待支付”。active linked relation 中存在 canonical 支出流水就驱动“已支付”；没有 active 支出流水就驱动“待支付”。金额差异只作为 relation reason，不阻断支付状态同步，也不产生“待核对/支付多了/多条OA合并支付”等第三状态。缺失银行事实或只有收入边仍不能证明已支付。
- 支付状态不再由用户逐行写回。正式 relation 创建、扩展、撤回或恢复时登记 durable reconcile event，由既有 `oa-sync` worker 根据最新 topology 幂等写 MySQL `pay_status` 并更新 PG payment snapshot。是否回退不判断历史写回归属；`pay_status=2` 不自动覆盖，缺 flow id 明确失败。
