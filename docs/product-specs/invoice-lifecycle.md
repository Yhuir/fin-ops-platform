...
- 付款状态只有“已支付/未支付”。active linked relation 中存在 canonical 支出流水就驱动“已支付”；金额差异只作为 relation reason，不阻断支付状态同步，也不产生“待核对/支付多了/多条OA合并支付”等第三状态。缺失银行事实或只有收入边仍不能证明已支付。
- 支付状态不再由用户逐行写回。正式 relation 创建、扩展、撤回或恢复时登记 durable reconcile event，由既有 `oa-sync` worker 根据最新 topology 幂等写 MySQL `pay_status` 并更新 PG payment snapshot。撤回时只恢复本 App 曾改为 paid 的 flow；外部原有 paid 不回退，`pay_status=2` 不自动覆盖，缺 flow id 明确失败。
