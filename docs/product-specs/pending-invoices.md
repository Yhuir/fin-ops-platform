...
`待找发票` 是按银行流水查看发票获取状态的工作台。页面原地使用现有左侧菜单 `待找发票` 和路由 `/pending-invoices`，支持 `全部流水`、`支出流水`、`收入流水` 三个视图。支出视图判断是否已经取得进项发票、是否无需开票、是否流水可替票、是否需要补票或选择已有发票；收入视图判断是否已关联销项发票、是否无需开票或是否为现金收入。

本页面不新增平行菜单，不替代关联工作台写模型。它读取和写入既有事实源：银行流水、进项发票、OA 投影、`workbench_pair_relations`、银行明细标签和 `pending_invoice_tag_groups`。

页面应明确展示流水方向边界：顶部三段切换按钮显示全部、支出、收入数量。`全部流水` 只做统一查看和导出，不显示规则设置；规则设置按支出/收入方向独立维护。

## 首版范围

- 主表使用 MUI Table，不使用 DataGrid。
- 主表为四个大区：`流水`/`支出流水`/`收入流水`、`发票获取状态`、`发票`/`进项发票`/`销项发票`、`OA`。
- 一行等于一条银行流水。
- 多发票、多 OA、多次支付只展示 primary 摘要和 `+N`，完整关系进入右侧抽屉。
- `Sheet1` 的入口在 `发票获取状态` 列，用于查看历史支付、已付合计、待付金额，并选择已有发票建立关系。
- `待找发票规则设置` 抽屉在支出视图允许编辑 `流水代替发票`、`无需开票`；在收入视图允许编辑 `无需开票`、`现金收入`。支出 `需要开票`、收入 `待开发票` 都由后端按 active 银行明细自动标签补集派生并只读展示。
...
| 大区 | 小列 |
| --- | --- |
| 流水 | 对方 / 时间、金额 / 银行账户、摘要 / 凭证 |
| 发票获取状态 | 状态 / 依据 / 主操作 |
| 发票 | 发票号码 / 开票日期、销方/购方 / 识别号、金额 / 差额 |
| OA | 申请人 / 类型、项目 / 详情 |
...
规则事实源来自银行明细 `自动标签规则`。支出规则持久化在 `pending_invoice_tag_groups`，只保存 `流水代替发票`、`无需开票` 两组用户选择；收入规则持久化在 `pending_output_invoice_tag_groups`，只保存 `无需开票`、`现金收入` 两组用户选择。支出 `需要开票` 和收入 `待开发票` 不是用户可编辑事实，而是当前自动标签规则接口中的 `system_rule + active_rules` 减去对应两组后的补集。

- 用户只能在当前方向可编辑规则组中选择已有 active 银行明细自动标签规则。
- 同一方向内，同一标签不能同时归入两个可编辑规则组和补集组；补集由后端派生，前端和后端都不得把同一 code 重复展示或匹配到多个分类。
