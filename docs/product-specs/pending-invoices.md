...
规则事实源仍是设置中的 `pending_invoice_tag_groups` 和银行明细自动标签规则 `bank_transaction_tags`，不新增第二套规则表。`pending_invoice_tag_groups` 只持久化 `流水代替发票`、`无需开票` 两组用户选择；`需要开票` 不是用户可编辑事实，而是所有 active 银行明细自动标签减去这两组后的补集。

- 用户只能在 `流水代替发票`、`无需开票` 中选择已有 active 银行明细标签。
