...
- `POST /api/etc/business-batches/{id}/invoice-pdf/repair` 是管理员专用的 submitted 历史附件恢复入口。multipart 请求包含一个或多个原始 ZIP、当前 `expectedVersion` 和非空 `reason`；只允许补回该批次已有发票当前不可读的 PDF/XML，且原始内容 SHA-256 必须等于既有 metadata。成功递增版本并写审计；重复执行不递增版本并返回零修复。该接口不得新增发票、改变批次成员、OA、relation 或 submitted 状态。
