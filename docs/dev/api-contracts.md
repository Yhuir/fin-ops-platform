...
- `GET /api/cost-statistics/manual-allocations/{relation_case_id}`：定向返回 units、bank_events、allocations、source_allocations、金额合计、pending_reasons、oa_cost_tag_overrides、units[].lock_oa_amount、units[].outside_cost_amount、allows_partial、waiting_oa_ids、unallocated_amount、version、source_fingerprint、can_save。单元 cost_eligible 反映当前审批资格，混合任务的来源若有明确归属则附 allowed_unit_ids。银行证据包含 transaction_id、event_kind、amount、trade_time、counterparty_name、tags、bank_account_label、bank_tag_code、bank_tag_primary_label/sub_label；没有旧 summary。
