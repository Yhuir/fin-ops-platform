...
> 2026-07-11 当前合同纠正：`tax_offset` 不消费 Workbench/红蓝票 relation。旧段落中的 “tax offset relation fan-out / relation 影响进项计划行 / tax/cost downstream” 描述已废止；当前证据是 `workbench-relations-tax-offset-isolation.spec.ts` 与 repository queue 隔离测试。发票/ETC/税务认证 canonical 写入导致的 tax refresh 不受影响。
...
本轮新增 bank-flow Spec-first Browser 覆盖：`bank-flow-rule-batches-flow` 中 `read_model_status=stale -> fresh` 会保持可见 rows、不显示普通空态并自动重读；流水规则标签保存会提交 `PUT /api/bank-flow-rule-batches/tag-rules`，等待 `bank_flow_rule_batch:all` operation barrier fresh 并重读列表；selected-row submit 成功并等待 operation barrier 后继续进入成本统计，等待 `/api/cost-statistics/explorer` 返回 `read_model_status=fresh`，并验证流水规则手续费成本项目、费用类型、银行流水和银行名称出现在项目/流水视图；随后回流水规则批量处理完成撤回和历史只读断言。该 flow 在标签保存、submit、成本统计 fan-out、withdraw 和 history 成功后都会检查页面没有残留操作失败/同步失败/read model 失败等可见错误提示。legacy no-OA 当前没有页面级 Browser 入口，API/read model/worker 回归由 `tests/test_no_oa_bank_batch*.py` 和相关 Workbench tests 覆盖；真实 worker drain 仍走 `infra-smoke` / staging gate。
