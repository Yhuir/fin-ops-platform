...
本轮 bank-flow Spec-first Browser 覆盖：`bank-flow-rule-batches-flow` 中 `read_model_status=stale -> fresh` 会保持可见 rows、不显示普通空态并自动重读；流水规则标签保存提交 `PUT /api/bank-flow-rule-batches/tag-rules` 后先显示成功并关闭抽屉，再由后台等待精确 `bank_flow_rule_batch:YYYY-MM` operation barrier 和重读列表，不再使用或阻塞于 `all` scope。组件测试额外锁定未提交 rail 只显示 OA/发票双 false 标签、完整 summary batch/row count 不受分页影响、submitted/history 保留实际历史标签。selected-row submit 成功后继续进入成本统计并验证流水规则手续费成本项目、费用类型、银行流水和银行名称；随后完成撤回和历史只读断言。legacy no-OA 当前没有页面级 Browser 入口，API/read model/worker 回归由 `tests/test_no_oa_bank_batch*.py` 和相关 Workbench tests 覆盖；真实 worker drain 仍走 `infra-smoke` / staging gate。
