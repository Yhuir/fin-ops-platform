...
- `available_tags`：待找发票规则抽屉唯一可用标签全集，来源等同于 `/api/bank-details/auto-tag-rules` 的 `system_rule + active_rules`，每项至少包含 `code`、`label`、`status`、`output_primary_label`、`output_sub_label`。`output_third_label` 不再作为自动标签规则配置返回或保存。
