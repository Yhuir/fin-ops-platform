...
日期：2026-09-23。状态：历史实施记录。下述独立提示行和解释文字方案已由[恢复异常 icon](workbench-anomaly-icon-restoration.md)替代；金额业务事实及当时验证记录保留。不使用 GSD。
...
本文保留历史实施证据。凭证列表的“本项差额”与“与同项发票合并核对”说明现已移除，仅显示凭证金额；后台金额判断及现有异常入口保持。当前合同见[关联台边界](../modules/reconciliation-workbench/boundary-io.md#2026-09-24-凭证列表仅显示金额)。
