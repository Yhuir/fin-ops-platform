...
**本轮执行：** §14字段、查询及事务增量已实施、合并main并正式发布，实际结果与性能例外见实施§16。保留既有route/service/repository和cash.*十表，仅追加0168三列迁移；不新增数据库账号、缓存、worker或通用架构层。§1–13保留历史合同，冲突处以§14为准，不并行保留旧规则。
...
状态：**统一UI与闭环增量已上线，最新版本和实测见实施§16。** 旧页性能例外和未测项分别报告，规格不等于所有场景验收通过；本次不把普通Page Audit纳入现金事实。
...
| 项目 | 实施动作 | 当前结论 |
| --- | --- | --- |
| OA原始字段/编码/字典/分页/源权限 | 真实上线现金API读完118项目和10阶段，浏览器checkbox核实 | 生产设置尚未保存，未写回OA；不是静态状态映射 |
| PG部署版本/迁移序号/角色权限/连接预算 | PG16实际旧代码在0166/0167各5项兼容通过；同账号十表DML实测，已发布 | 保留迁移owner职责和既有正式发布检查；不执行已取消的cash专用角色SQL拒绝 |
| 现金函数/接口/金额规则 | 17列投影已实现，真PG HTTP闭环、组件/浏览器分层验证 | 合成浏览器写入不等于真实浏览器到PG写入全链；精确范围见实施计划§10.13–10.14 |
| 原App共享ACL/审计/日志/健康出口 | 本地共享边界回归、生产旧16页只读、Nginx三入口日志探针通过 | 保留普通保护；生产受限角色验证按用户要求不执行 |
| 性能 | 生产现金100样本/接口、旧页前后对照、混合负载和只读浏览器测量已完成 | 生产现金为空；旧工作台4并发p95仍1124.80ms；真实写入浏览器耗时未测，不保证零影响 |
...
- 查询的编码query部分上限3500 UTF-8字节，同时保留单列50/全部100项数量上限；长ID可能先达到字节上限。前端应用前和现金HTTP边界一致检查，超出明确400/输入提示，不截断、不自动换POST或另一地址。生产极限请求验证发现原设计6000会先触发既有请求行限制而返回HTML，按原计划收紧到3500，为路径/协议保留空间；不放宽全局Gunicorn/Nginx安全配置。最长合法3500及3501拒绝结果见实施§14。
