...
- 普通 runtime worker 的 read model handler 由 `ReadModelReadinessReporter` 统一包装；新增 read model refresh handler 不能绕过这个 wrapper。`all` fanout 只表示已拆分 scope，不写 `fresh`，真实 scope 完成后才写 readiness。
