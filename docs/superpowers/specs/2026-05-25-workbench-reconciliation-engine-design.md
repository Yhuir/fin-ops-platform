...
- 如果无法唯一确定，普通三方冲突相关行保持 `open` 并输出 blocker；收入 `bank_invoice` 歧义按 2026-05-26 细化设计输出一个结构化 `open` 冲突组，保留银行流水、候选发票和 evidence 上下文。
- 配对规则采用唯一性优先：多个同级候选冲突时不按分数强行选择，全部保持 `open`。
- 自由匹配优先看金额，再看跨来源文本重复证据。
- 自由匹配覆盖支出方向的 OA/银行/发票确定性关系，以及收入方向的银行-销项发票确定性两方关系；收入侧没有 OA，不形成 OA/银行/发票三方关系。
- 自由匹配字段范围：
  - OA：申请人、项目名称、申请事由。
  - 流水：对方户名、摘要、备注。
  - 支出发票：销方名称。
...
- 不把特殊业务类型并入自由匹配规则。
- 不用无解释分数最高原则替代唯一性原则。收入 `bank_invoice` 的同金额多发票场景例外：只允许在主体证据成立且存在唯一最强补强证据候选时自动选择一张，否则保持结构化 `open`。
- 不把疑似候选展示成同一行。
- 不在首版引入新的外部队列基础设施。
- 不把 Redis 作为匹配正确性的依赖。
...
自由匹配处理普通支出 OA、银行流出、进项/供应商发票之间的关系，也处理收入银行流水与销项发票之间的 `bank_invoice` 两方关系。收入业务没有 OA 作为三项桥接对象，因此收入侧不形成 OA/银行/发票三方关系。
...
唯一性优先级高于打分。同一 OA、流水或发票如果能形成多个同级候选，普通自由匹配保持 `open`。收入 `bank_invoice` 同金额多候选需要先比较主体证据和摘要/备注补强证据；只有最高证据候选唯一时才自动选择，否则输出结构化 `open` blocker。
