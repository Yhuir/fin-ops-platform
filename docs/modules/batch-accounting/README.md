...
- 银行流水、OA 行和已有关联关系来自后端批量账务查询边界；前端只消费业务 rows、summary、relations 和 pagination。
- `GET /api/batch-accounting` 前端合同返回 `summary`、`bank_rows`、`oa_rows`、`relations_by_bank_row_id` 和 `pagination`。显式传入 `page/page_size`、`bank_page/bank_page_size` 或 `oa_page/oa_page_size` 时，后端只裁剪对应列表并返回 `pagination`；不传分页参数时保持业务 rows/summary 兼容。
...
- 前端提交/撤回成功后直接重新加载批量账务 payload，并发送 `workbenchRelationUpdated` 作为同浏览器会话刷新提示；事实源以后端 canonical relation write、relation outbox、真实后台任务和 direct API 重读为准。

## 当前边界

- 前端不得展示 `workbench_relation` 旧投影状态、scope 或 刷新入队 诊断；GET 失败显示普通页面错误，mutation legacy 同步状态 错误转成中性“关系状态正在更新”文案。
- `GET /api/batch-accounting` 后端读取 canonical relation context / direct payload，不把 facade 的 legacy 同步状态/status/scope 字段透传给页面。
- `POST /api/batch-accounting/submit` 和 `POST /api/batch-accounting/{relation_id}/withdraw` 必须走 command service，并基于 canonical relation、idempotency、owner 状态、权限/session 和 DB 可写性校验；不允许在缺少 command service 时静默写旧 pair service，也不能把普通 relation distribution 追赶中作为默认写阻断条件。
- 前端 submit/withdraw 必须接入 `GlobalOperationOverlayProvider`。写 API 成功后直接重新加载批量账务 payload；overlay 关闭只能表示当前页面重读完成，不能用本地事件或前端临时列表移动替代 canonical 写入结果。
- 批量账务关系变化会影响关联台、银行明细、成本统计、Search direct payload、进项/销项/OA 待付款等依赖关系或 invoice lifecycle 的页面。
- 写后收敛证据是 canonical relation facts、direct API 重读、relation outbox 和真实后台任务，不是前端事件或 page-level 旧投影 refresh。
- 批量账务 GET 必须保持只读；不能在列表读取路径执行 legacy relation repair。
- 批量账务显式分页的 `page_size` 上限为 200，超限必须返回 `invalid_paging`，不能为了首屏性能静默全量返回或把 旧 relation distribution 伪装成当前事实。

## 影响面清单

| 改动点 | 必查影响 |
| --- | --- |
| 页面筛选、bucket、选择、差额说明、提交/撤回 | `BatchAccountingPage.test.tsx` 的 loading/empty/error/筛选/提交/撤回/事件回归 |
...
| 后端 relation 查询迁移 | `BatchAccountingService`、`WorkbenchRelationReadFacade`、后续 direct query repository |
| Outbox/lifecycle event | `DerivedDataLifecycleService`、runtime worker registry、下游页面 direct API 回归 |
...
- API contract、DTO shape、错误字段、权限校验或状态值变化。
- 业务状态、UI 状态、worker 状态或状态流转变化。
