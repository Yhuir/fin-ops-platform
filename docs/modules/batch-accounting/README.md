...
- 未提交 bucket 的 read path 必须把 Workbench 输入先收窄为批量账务银行候选和日常报销 OA 候选；银行候选只按结构化 `counterparty_name` 读取并使用既有复合索引，不保留 payload counterparty fallback；OA 类型只通过 `coalesce(apply_type) || ' ' || coalesce(expense_type)` 的稳定表达式筛选，并由 migration 0112 的 batch-only 部分 trigram 索引覆盖；OA 附件只按这些 OA IDs 读取。候选 relation 必须调用 `get_batch_accounting_by_row_ids` 专用 bulk freshness I/O；`submitted_count` 使用一次年度 bulk proof + count，不能进入通用逐 scope lookup 或加载完整 submitted relation DTO。
