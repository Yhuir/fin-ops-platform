...
本文件定义 `/batch-accounting` 页面在真实浏览器中的业务验收合同。测试必须保护批量账务提交、撤回、relation freshness barrier、分页首屏、non-fresh 诊断、权限/写边界和下游 relation fan-out，而不是保护当前组件实现细节。

## 模块目标

批量账务页面把符合条件的银行流水与日常报销 OA 行人工确认成 `batch_accounting` Workbench relation，并支持撤回。页面不拥有独立事实源；读侧必须消费 Workbench payload 与 `workbench_relation` read model，写侧必须通过 `WorkbenchRelationCommandService` 和 operation barrier 等待关系读模型收敛。
...
| `BATCH-E2E-002` | 金额一致提交 relation freshness closure | P0 | 用户选择一个合法银行流水和至少一个 OA 行，差额为 0 时可提交；请求必须写 `batch_accounting` relation，成功后等待 `workbench_relation` operation barrier fresh，再重读并进入已提交 bucket。 |
...
| `BATCH-E2E-010` | 真实基础设施 worker drain | P1 | submit/withdraw 后，真实 PostgreSQL/RabbitMQ/Redis/systemd workbench-relation、search、cost 和下游 worker 最终 drain 到 fresh；该项必须在 staging/runtime smoke 验证。 |
