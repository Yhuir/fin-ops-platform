...
| 页面只读 Audit | `PageBusinessAuditIcon` / AppHealth operations API | admin-only 调用 `page-audit?domain=batch_accounting`；active batch-accounting relation 是 canonical expected-set，canonical `relation_mode` 必须与 relation group payload 的 `relation_mode` 比较，不能误与表示 OA/银行组合形态的 `relation_kind` 比较；special metadata 与全部成员 edge 必须双向等价，并在只读一致性快照中执行；与批量提交/撤回 command 隔离，不触发 relation 或 read model 写入 |
