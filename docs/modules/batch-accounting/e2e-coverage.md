...
| `BATCH-E2E-002` | `covered` | `web/e2e/batch-accounting-flow.spec.ts`、`web/src/test/BatchAccountingPage.test.tsx`、`tests/test_batch_accounting_api.py`、`tests/test_workbench_v2_api.py` | Browser 覆盖选择银行流水与两条 OA、差额 0、提交、成功反馈、`POST /api/batch-accounting/submit`、`workbench_relation` operation barrier、列表重读、已提交 bucket 和成功后无可见错误残留；后端覆盖 command service relation 写入、idempotency/version、rollback 和 current invoice rows。 |
...
| `BATCH-E2E-004` | `covered` | `web/e2e/batch-accounting-flow.spec.ts`、`web/src/test/BatchAccountingPage.test.tsx`、`tests/test_batch_accounting_api.py` | Browser 覆盖已提交 bucket、撤回 dialog、原因输入、withdraw endpoint、operation barrier、已提交计数归零、未提交 bucket 恢复、零浏览器错误和成功后无可见错误残留；后端覆盖非 batch relation 拒绝、撤回原因、cancel current relation、history 和旧 snapshot restore 删除。 |
...
| `BATCH-E2E-007` | `covered` | `web/e2e/batch-accounting-flow.spec.ts`、`web/e2e/workbench-relation-fanout.spec.ts`、`web/src/test/domainEvents.test.ts`、`tests/test_derived_data_lifecycle_service.py`、workbench relation tests | Browser 覆盖 submit/withdraw 后 relation barrier 和当前页恢复；关联台/银行明细/成本统计/search 等更广 fan-out 由 Workbench relation、成本统计和各页面 Spec-first 覆盖继续保护。 |
