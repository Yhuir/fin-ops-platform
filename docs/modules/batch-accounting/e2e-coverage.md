...
| `BATCH-E2E-007` | `covered` | `web/e2e/batch-accounting-flow.spec.ts`、`web/e2e/workbench-relation-fanout.spec.ts`、`web/src/test/PageRouteHost.test.tsx`、`tests/test_platform_runtime_boundary_guards.py`、workbench relation tests | Browser 覆盖 submit/withdraw 后当前页 GET；relation tests/guards 覆盖零写后 fan-out，另一个已打开消费者、focus/visibility 与旧业务事件均不自动 GET，用户访问时独立收敛。 |
