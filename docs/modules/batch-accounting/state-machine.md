...
- 进入或重入页面默认银行范围为全部；当前访问内主动选择年份保持有效。切换 bucket、银行年份或服务端分页时重置不再可见的选择。
...
- GET 支持 `bank_year=all`，POST 仍必须使用所选银行 canonical 日期对应的具体年份。三日期均缺失的银行在全部列表可见但不可选择提交；伪造年份提交返回 400。
- 已提交缺日期关系保留可见与撤回资格；其 DTO 年份为 null，不从历史 metadata 补造。
