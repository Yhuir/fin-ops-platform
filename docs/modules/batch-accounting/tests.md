...
| Operation overlay | 提交/撤回写操作成功后直接 reload 页面并关闭 overlay；失败时不假装成功，成功后不能残留操作失败/同步失败等可见错误提示，且页面不得调用 旧操作屏障 | `web/src/test/BatchAccountingPage.test.tsx`、`web/src/test/GlobalOperationOverlayContext.test.tsx`、`web/e2e/batch-accounting-flow.spec.ts` |
| API contract | `GET /api/batch-accounting`、`POST /api/batch-accounting/submit`、`POST /api/batch-accounting/{relation_id}/withdraw` 的状态码、错误码和 DTO shape；后端和前端 mapper 都不暴露 同步状态 字段 | `tests/test_batch_accounting_api.py`、`web/src/test/BatchAccountingApi.test.ts` |
...
| Relation context | canonical relation context、row id 去重、linked/unlinked projection、legacy 同步状态/status 字段不透传 | `tests/test_workbench_relation_read_facade.py`、`tests/test_batch_accounting_api.py` |
| Removed relation worker guard | removed workbench relation worker registry、canonical relation direct reload/background job guard、App Status domain/job 映射 | `tests/test_runtime_worker_registry.py`、`tests/test_app_status_overview_service.py` |
...
| unsubmitted 列表按 bank/oa 年份独立筛选，且页面 API 不透出 relation 同步状态 字段 | covered | `test_unsubmitted_list_uses_direct_workbench_loader`、`test_unsubmitted_list_uses_independent_bank_and_oa_years` |
...
| relation context 不污染批量账务 API shape；写操作继续走 canonical command service | covered | `test_unsubmitted_list_omits_relation_同步状态_fields`、`test_submitted_list_omits_relation_同步状态_fields`、`test_unsubmitted_list_still_requests_relation_context_without_exposing_同步状态`、`test_submitted_list_still_requests_relation_context_without_exposing_同步状态`、`test_submit_uses_canonical_command_service_with_relation_context` |
...
| 前端提交/撤回后显示全屏 operation overlay，直接 reload 并释放，且不调用 旧操作屏障 | covered | `BatchAccountingPage.test.tsx`、`web/e2e/batch-accounting-flow.spec.ts` |
...
| 3. API contract tests | 适用 | `tests/test_batch_accounting_api.py`、`web/src/test/BatchAccountingApi.test.ts` | 覆盖 GET/submit/withdraw 的成功 shape、显式分页 `pagination` / `invalid_paging`、错误码、summary/relations/mutation result；前端 mapper 不暴露 同步状态 字段。 |
| 4. 旧投影/cache/background job tests | 适用 | `tests/test_workbench_relation_read_facade.py`、`tests/test_runtime_worker_registry.py`、`tests/test_app_status_overview_service.py`、`tests/test_batch_accounting_api.py`、`tests/test_platform_runtime_boundary_guards.py` | 共享 relation context 由 direct relation tests 保护；批量账务测试保护 legacy status 字段不进入页面 API payload，并防止恢复 page-level 旧投影 refresh/worker。 |
| 5. Frontend component and interaction tests | 适用 | `web/src/test/BatchAccountingPage.test.tsx`、`web/src/test/GlobalOperationOverlayContext.test.tsx`、`web/e2e/batch-accounting-flow.spec.ts` | 覆盖 loading/empty/error、首屏 GET 暂时失败后的错误态/刷新恢复、筛选、搜索、选择、提交、撤回、operation overlay、CSS/组件契约、窄桌面左侧 rail 可读性和侧栏入口；Browser e2e 覆盖真实 Chromium 中首屏 503 后手动刷新恢复、窄桌面左侧 header 不挤压、未提交 bucket 选择银行/OA、金额归零、提交反馈、已提交 bucket 展示、撤回弹窗、撤回后回到未提交状态，并在恢复/成功反馈后检查没有操作失败/同步失败等可见错误残留。 |
| 6. End-to-end business-flow integration tests | 适用 | `tests/test_batch_accounting_api.py`、`tests/test_workbench_v2_api.py`、`web/src/test/domainEvents.test.ts`、`web/e2e/batch-accounting-flow.spec.ts` | 覆盖 submit -> canonical relation -> submitted list / Workbench direct payload、withdraw -> snapshot restore、前端刷新事件；Browser e2e 额外证明 submit/withdraw 后不调用 旧操作屏障、会重新读取并恢复对应 bucket，且严格捕获 console/page/request/dialog 隐藏错误和成功后的可见错误残留。真实后台任务/direct API 收敛仍是 external-risk。 |
| 7. Existing feature regression tests | 适用 | `tests/test_batch_accounting_api.py`、`tests/test_workbench_v2_api.py`、`web/src/test/BatchAccountingPage.test.tsx`、`web/e2e/batch-accounting-flow.spec.ts` | 覆盖旧 case_id collision、非 batch relation 不被覆盖、GET 不执行 legacy repair、页面不暴露 direct payload 同步状态；Browser e2e 防止提交/撤回后的 summary、bucket、OA 表格状态和“成功但报错提示仍显示”的回归。 |
...
| Relation direct payload unavailable | missing/stale 不进入批量账务 API payload；submit/withdraw 默认由 canonical write safety、owner 状态、权限/session、DB 可写性决定 | covered |
...
| Workbench page-level 旧投影 persist regression | submit/withdraw route owner 不得接收或调用 `schedule_read_model_persist`；写后只发 lifecycle/direct reload | covered |
...
| Write-to-reload closure | submit/withdraw 后不靠前端事件假装完成，必须直接重新加载页面，且不得调用 旧操作屏障 | covered |
...
2. 批量账务 direct 列表返回 -> 选择银行流水 -> 选择 OA 行 -> 金额一致提交 -> 直接 reload -> `workbenchRelationUpdated` -> submitted bucket 展示关系；`web/e2e/batch-accounting-flow.spec.ts` 在真实 Chromium 中覆盖该 happy path，并断言 旧操作屏障 未被调用且成功后没有可见错误残留。
3. 金额不一致 -> 空说明被拒 -> 填写说明提交 -> 关联台/银行明细/成本统计下游通过 canonical relation facts/direct API 看到关系标签。
4. submitted bucket -> 填写撤回原因 -> 撤回 -> 直接 reload -> 原银行/OA 行回到可处理状态；`web/e2e/batch-accounting-flow.spec.ts` 覆盖真实 Chromium 撤回弹窗、原因输入、确认撤回、回到未提交 bucket、零 旧操作屏障 请求和成功后无可见错误残留。
5. legacy backend 关系读侧诊断字段进入内部 payload -> 批量账务 API 不透传 旧投影状态/scope；后端 direct relation/facade guard 继续保护，直到 legacy facade 完全删除。
6. 显式 `page/page_size` -> `bank_rows` / `oa_rows` 有界返回、summary 仍保留完整命中数量，超限页大小 fail closed。
7. submit/withdraw -> 全屏 overlay -> 重新加载当前 bucket -> overlay 释放，且不调用 旧操作屏障。
...
Nightly CI 通过 `scripts/verify.sh all` 执行后端、前端、Playwright browser smoke 和文档校验。当前 Playwright smoke 已包含 `web/e2e/batch-accounting-flow.spec.ts`，覆盖提交批量账务关系、直接重新加载并在已提交 bucket 展示关联 OA，以及撤回后直接重读并回到未提交 bucket；该 spec 还覆盖首屏 GET 暂时 503 后手动刷新恢复、窄桌面银行 rail 可读性、relation context 不污染页面 API shape、捕获浏览器隐藏错误，并在恢复/成功写操作后检查页面没有可见错误残留。批量账务的窄范围回归命令应在本地模块变更时优先运行；跨模块改动再升级为 `scripts/verify.sh backend` / `scripts/verify.sh frontend` / `scripts/verify.sh e2e`。

## 未测风险

- 真实 PostgreSQL 历史数据中批量账务 legacy relation / 半迁移 / 重复 case id 的全量回放仍需 staging 或生产前 dry-run。
- 真实 RabbitMQ/Redis/systemd workbench relation 后台任务、App Status 运行态收敛和长时间队列重试仍需环境 smoke。
...
- 关联台、银行明细、成本统计等下游页面对同一 relation fact 的最终显示仍由对应模块回归继续保护；Search 由 direct `/api/search` payload 回归保护。
