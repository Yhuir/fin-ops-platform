...
| 真实 PostgreSQL | 当前全部 0001–0113 migrations 上执行 bulk scope proof、年度 count/list、row lookup、processing fail-closed、列表单 I/O 与 submit OA-ID-scoped attachment SQL；5,000 行非命中 OA 和 5,000 条非 batch relation 样本上的 `EXPLAIN` 必须分别命中 0112 OA 类型索引与 0113 年度 relation count 索引 | `tests/test_batch_accounting_postgres_integration.py`（需要 visibly disposable `FIN_OPS_TEST_DATABASE_URL`） |
