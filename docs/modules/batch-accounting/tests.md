...
| Service / repository | `BatchAccountingService` 调用 Workbench payload、relation command service、relation facade；未提交/已提交 relation row lookup 只用 batch 专用 I/O，年份 count/list 使用固定查询数 bulk proof；银行候选只按结构化 counterparty 读取，OA 类型使用 migration 0112 覆盖的稳定组合表达式，OA 附件只按候选 IDs 读取；submit/withdraw 边界不变 | `tests/test_batch_accounting_api.py`、`tests/test_workbench_relation_read_facade.py`、`tests/test_workbench_sql_runtime.py`、`tests/test_batch_accounting_postgres_integration.py`、`tests/test_workbench_v2_api.py` |
| Relation read model | `workbench_relation` fresh/missing/stale、row id 去重、linked/unlinked projection、refresh enqueue、source version；12 scope proof 固定 1 条 SQL，count/list 总语句数固定为 2，候选 lookup 固定为 2–3 | `tests/test_workbench_relation_read_facade.py`、`tests/test_workbench_relation_sql_projection.py`、`tests/test_batch_accounting_api.py` |
| 真实 PostgreSQL | 当前全部 0001–0112 migrations 上执行 bulk scope proof、年度 count/list、row lookup、processing fail-closed 与 OA-ID-scoped attachment SQL；5,000 行非命中 OA 样本上的 `EXPLAIN` 必须命中 batch-only OA 类型索引 | `tests/test_batch_accounting_postgres_integration.py`（需要 visibly disposable `FIN_OPS_TEST_DATABASE_URL`） |
