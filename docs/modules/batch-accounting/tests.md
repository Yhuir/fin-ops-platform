...
| 真实 PostgreSQL | 当前全部 0001–0112 migrations 上执行 bulk scope proof、年度 count/list、row lookup、processing fail-closed、列表单 I/O 与 submit OA-ID-scoped attachment SQL；5,000 行非命中 OA 样本上的 `EXPLAIN` 必须命中 batch-only OA 类型索引 | `tests/test_batch_accounting_postgres_integration.py`（需要 visibly disposable `FIN_OPS_TEST_DATABASE_URL`） |
