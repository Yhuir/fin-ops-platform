...
| Service / repository | `BatchAccountingService` 调用 Workbench payload、relation command service、relation facade；未提交/已提交 relation row lookup 只用 batch 专用 I/O，年份 count/list 使用固定查询数 bulk proof；OA 附件只按候选 IDs 读取；submit/withdraw 边界不变 | `tests/test_batch_accounting_api.py`、`tests/test_workbench_relation_read_facade.py`、`tests/test_workbench_sql_runtime.py`、`tests/test_workbench_v2_api.py` |
