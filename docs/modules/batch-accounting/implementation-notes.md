...
- 真实原因：批量账务候选、年度 count/list 复用了通用逐 scope freshness proof，每个 scope 分别查询 scope metadata 和 dirty status；已提交 list 还在 groups 查询后重复证明。未提交 loader 同时无条件读取全部 OA 附件候选。首次发布把 query count 降到 p95 `10` 后，生产 40 样本仍证明 unsubmitted p95 `612.217ms`；剩余 DB 热点来自银行候选对结构化 counterparty 与两个历史 JSON 字段的 `OR`，阻断既有复合索引。
- 设计：新增 batch 专用 `get_batch_accounting_by_row_ids` facade/port/repository I/O；一个有序 bulk SQL 同时读取所有 scope metadata 与 current-effective dirty status。年度 count/list 固定为 2 条语句，候选 detail 固定为 2–3 条。OA 附件 SQL 只接受当前 OA IDs，submit/list 复用同一私有 helper 并显式区分 `all` scope 策略。第二轮只删除银行 JSON fallback，让现有 `workbench_rows_bank_counterparty_scope_idx` 生效，不新增 migration、索引、缓存或 read model。
...
- 旧链删除：BatchAccountingService 不再调用通用 `get_by_row_ids`；年度 count/list 不再逐月或重复 proof；未提交 loader 删除无 OA-ID 条件的附件全量扫描；银行候选删除 `payload.counterparty_name` / `payload.counterparty_name_raw` fallback。静态 guard 阻止这些路径回归。
