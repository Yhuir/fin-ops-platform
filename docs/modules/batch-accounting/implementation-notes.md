...
- 旧链删除：BatchAccountingService 不再调用通用 `get_by_row_ids`；独立年度 submitted-count facade/port/repository/manifest 合同已删除，年度 count/list 不再逐月或重复 proof；未提交 relation rows 后不再追加独立 proof/groups I/O；未提交 loader 删除无 OA-ID 条件的附件全量扫描和银行/OA/附件三个顺序 round-trip；银行候选删除 `payload.counterparty_name` / `payload.counterparty_name_raw` fallback；OA 类型删除两个 JSON 字段各自 `LIKE` 后用 `OR` 合并的未索引条件。静态 guard 阻止这些路径回归。
