...
- 关键决策：批量账务写后 lifecycle 不再排除整个 `workbench_read_model` domain，而是通过 executor override 把默认同步 rebuild executor 替换成轻量 enqueue executor；该 executor 只通过 `ReadModelRefreshGateway.enqueue_many("workbench", affected_scope_keys, reason="batch_accounting_relation_changed", metadata=...)` 投递 Workbench read model refresh。生产 SQL active generation 收敛依赖 runtime worker；旧 local persist 仅保留 best-effort/compat。
- 测试覆盖：新增 `test_batch_accounting_lifecycle_uses_lightweight_workbench_refresh_executor` 和 `test_batch_accounting_lightweight_workbench_executor_enqueues_refresh`，断言 wrapper 向通用 lifecycle 传入 `workbench_read_model` executor override，并把 normalized month scope 投递到 `workbench` refresh gateway。
- 验证命令：见本轮最终说明。
