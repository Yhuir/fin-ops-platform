...
- 设计：新增 batch 专用 `get_batch_accounting_by_row_ids` facade/port/repository I/O；一个有序 bulk SQL 同时读取所有 scope metadata 与 current-effective dirty status。年度 count/list 固定为 2 条语句，候选 detail 固定为 2–3 条。银行、OA、当前 OA 附件在一个 active-generation repository SQL I/O 中返回；附件由 `oa_candidate_ids` CTE 界定并与 submit 窄 loader 共享同一个私有匹配谓词。银行候选只读结构化 `counterparty_name`。OA 类型筛选改为 `coalesce(apply_type) || ' ' || coalesce(expense_type)` 的单一稳定表达式，migration 0112 只为 `source_kind='oa' and scope_key<>'all'` 建立部分 trigram 索引。不新增结构化列、缓存、第二 read model、queue 或 worker。
...
- 生产基线：shell p95 `104.966ms`；unsubmitted p95 `638.061ms`（未达 `500ms`）；submitted p95 `390.986ms`；Page Audit p95 `375.765ms`；dashboard DB query count p50/p95/p99 `52/66/71`。前三次 release 的 unsubmitted p95 依次为 `612.217ms`、`580.757ms`、`514.231ms`；候选单 I/O release 尚待部署验证。
