...
## 2026-06-26 - 移除前端 GET freshness 合同

- 目标：让批量账务页面不再消费或展示 read model freshness/status/scope 字段，继续通过直接重读 `GET /api/batch-accounting` 获取业务 rows。
- 影响范围：`web/src/pages/BatchAccountingPage.tsx`、`web/src/features/batchAccounting/*`、BatchAccounting Vitest、BatchAccounting E2E mock/spec、模块 README/state/tests/e2e 文档。
- 关键决策：本轮不伪造后端 direct SQL 迁移；后端 `BatchAccountingService` 仍可能通过 legacy relation facade 获取关系分布。前端合同已收口为业务 rows、summary、relations、pagination；legacy mutation read-model 错误转成中性关系状态重试文案。
- 测试覆盖：前端 API mapper 不暴露 freshness 字段；页面不显示 read model stale 详情；E2E fixture 不再为 BatchAccounting 返回 read model status/scope。
- 后续事项：后端 direct query batch 需要替换 `workbench_relation` facade/read model 读取路径，并同步删除后端 freshness API tests。
...
- 关键决策：本地 IO/route/service/legacy/read-model freshness/direct reload/test/docs 证据已足够支撑本地实现闭环，但缺少真实 PostgreSQL、worker drain、App Status 收敛、生产历史 relation/case-id collision 和真实大年份数据证据。因此不标 full closed，记录 `production-evidence-deferred`，且不依赖本地 `PGSQL_URL` 或 staging 数据库。
...
- 目标：把 `POST /api/batch-accounting/submit` 和 `POST /api/batch-accounting/{relation_id}/withdraw` 的 DTO/service/error mapping 与写后 scope/lifecycle orchestration 从 `server.py` inline handler 抽到 `BatchAccountingApiRoutes`，让 `server.py` 只保留 mutation session、JSON body 和 response mapping。
...
- 关键决策：`BatchAccountingService` 仍是 submit/withdraw 业务状态转换和 canonical command service 写入边界；`BatchAccountingApiRoutes` 只通过显式注入的 callback 调用 pair relation persist、derived lifecycle event、submit rollback snapshot restore，不依赖 `Application`、不直接写 relation internals，也不调度 Workbench page read-model persist。
...
- 测试覆盖：静态 guard 覆盖 `routes_batch_accounting.py` 注册与 delegation；API 回归覆盖 SQL read model loader、GET 不执行 legacy repair、显式分页。2026-06-27 direct API 合同已用 relation context 不进入页面 payload 的测试替换 stale/missing status 测试。
- 验证命令：`PYTHONPATH=backend/src python3 -m unittest tests.test_platform_runtime_boundary_guards.PlatformRuntimeBoundaryGuardTests.test_server_route_owner_inventory_stays_registered tests.test_platform_runtime_boundary_guards.PlatformRuntimeBoundaryGuardTests.test_batch_accounting_route_handlers_do_not_bypass_service_boundaries -v`；`PYTHONPATH=backend/src python3 -m unittest tests.test_batch_accounting_api.BatchAccountingApiTests.test_unsubmitted_list_uses_sql_read_model_loader_when_available tests.test_batch_accounting_api.BatchAccountingApiTests.test_unsubmitted_list_does_not_run_legacy_relation_repair tests.test_batch_accounting_api.BatchAccountingApiTests.test_unsubmitted_list_explicit_pagination_protects_first_screen_slo tests.test_batch_accounting_api.BatchAccountingApiTests.test_unsubmitted_list_omits_relation_freshness_fields -v`。
- 未测风险：本 slice 不连接生产 PostgreSQL、不 drain worker、不验证真实大年份性能；因为没有改变 SQL/queue/worker/read model 发布语义，生产验证不是完成条件。
- 后续事项：继续推进 `batch-accounting:submit-withdraw-route-side-effect-port`，把 mutation HTTP DTO/error mapping 与写后 lifecycle/read model scope contract 从 `server.py` 收敛到明确边界。
...
## 2026-06-19 - relation read model stale Browser 防 false-empty（历史，已被 direct API 合同替代）
...
- 当前状态：2026-06-27 direct API 合同已经移除 `GET /api/batch-accounting`、submit、withdraw payload 中的 `read_model_status` / `read_model_stale_reasons` / `read_model_scope_keys` / `refresh_enqueued`。本节只保留历史背景，不再作为当前实现口径。
- 当前覆盖：后端 API 测试只断言 relation context 不污染页面 payload，并继续保护 submit/withdraw 走 canonical command service；Browser 用例不再要求页面展示 stale warning/reason/scope。
...
- 批量账务不拥有独立 read model；当前列表读取仍可能内部使用 `workbench_relation` relation context，GET/submit/withdraw API 不透出 freshness/status/scope，mutation 写安全不依赖普通 relation distribution freshness。
- `GET /api/batch-accounting` 必须保持只读，不能为了修复历史关系在 GET 路径写入。
- 前端不得展示 relation read model freshness warning/reason/scope；GET 失败显示普通页面错误，写操作是否可提交由后端 canonical write safety、权限/session、DB 和 owner/version/idempotency 判定，普通 relation distribution 追赶中不应作为长期全局禁用理由。
...
- 提交/撤回成功后的前端 `workbenchRelationUpdated` 只是刷新提示，不替代后端 canonical write、dirty scope、worker 和 readiness。页面释放全屏操作 overlay 前必须直接重新加载批量账务 payload，且不再调用 operation barrier。
...
## 2026-06-26 - 移除写后 operation barrier 等待

- 目标：让批量账务页面遵循 direct API 读路径，submit/withdraw 写成功后直接重读页面 payload，不再等待 `workbench_relation` operation barrier。
- 影响范围：`BatchAccountingPage`、组件测试、Browser E2E 和本模块当前合同文档；后端 submit/withdraw response shape 暂保持兼容。
- 关键决策：保留全屏 operation overlay 表示“写入和重读进行中”，但 overlay 不再声明 read model 已经经 barrier 收敛；跨页面最终一致性继续由 dirty scope、worker 和各页面直接读取/诊断承担。
- 测试覆盖：`BatchAccountingPage.test.tsx` 和 `batch-accounting-flow.spec.ts` 覆盖 submit/withdraw 后直接 reload、成功 feedback、bucket 恢复和零 `/api/operation-barrier/status` 请求。
- 未测风险：真实基础设施 worker drain、下游页面最终展示和生产历史 relation 回放仍需 staging/runtime smoke。
...
- 当前状态：2026-06-27 direct API 合同已取代本段历史口径；GET/read facade 可在内部请求 relation context，但不得向批量账务 API payload 透出 `workbench_relation` freshness 诊断或 refresh enqueue 字段。submit/withdraw 写安全由 `WorkbenchRelationCommandService`、canonical relation、idempotency、owner 状态、权限/session、DB 可写性决定。
- 文档影响：本次只做 P2/P3 closure ledger 和测试入口表述收敛，不改变 API 或长期架构口径。
- 当前状态：2026-06-27 direct API 合同已取代本段历史测试名；当前覆盖见 `tests.md` 中 relation context 不污染 API shape、submit/withdraw canonical command service 和 operation overlay 回归。
...
- 后续事项：不要重新把 relation freshness 字段放回批量账务 API contract；若未来确实需要 mutation precondition，应使用独立业务错误码并先更新长期 API contract。
...
- 测试覆盖：更新 `web/src/test/BatchAccountingPage.test.tsx`，并由 `GlobalOperationOverlayContext.test.tsx` 覆盖共享 overlay 行为。
- 验证命令：见本轮最终执行记录。
