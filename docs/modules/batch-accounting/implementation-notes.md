...
- 设计：新增 batch 专用 `get_batch_accounting_by_row_ids` facade/port/repository I/O；一个有序 bulk SQL 同时读取所有 scope metadata 与 current-effective dirty status。生产复采后进一步把候选 relation 的 scope proof+referenced groups、年度 scope proof+submitted count 各合为一个 batch-only repository I/O。银行、OA、当前 OA 附件在一个 active-generation repository SQL I/O 中返回；附件由 `oa_candidate_ids` CTE 界定并与 submit 窄 loader 共享同一个私有匹配谓词。银行候选只读结构化 `counterparty_name`。OA 类型筛选改为 `coalesce(apply_type) || ' ' || coalesce(expense_type)` 的单一稳定表达式，migration 0112 只为 `source_kind='oa' and scope_key<>'all'` 建立部分 trigram 索引。不新增结构化列、缓存、第二 read model、queue 或 worker。
- 隔离：通用 `get_by_row_ids`、其他页面 facade、worker、read model 表结构、queue、command service、API DTO 和前端不变；0112 只增加批量账务候选查询可用的读性能索引，不改变事实、行数据或其他页面查询。
- 旧链删除：BatchAccountingService 不再调用通用 `get_by_row_ids`；年度 count/list 不再逐月或重复 proof；未提交 loader 删除无 OA-ID 条件的附件全量扫描和银行/OA/附件三个顺序 round-trip；银行候选删除 `payload.counterparty_name` / `payload.counterparty_name_raw` fallback；OA 类型删除两个 JSON 字段各自 `LIKE` 后用 `OR` 合并的未索引条件。静态 guard 阻止这些路径回归。
- 测试：`tests/test_workbench_relation_read_facade.py` 覆盖固定 statement count、fresh/missing/refreshing 等价、专用 port/facade；`tests/test_workbench_sql_runtime.py` 覆盖 OA-ID-scoped attachment 和组合 OA 类型 SQL；`tests/test_batch_accounting_api.py` 覆盖 service/API 输出回归；migration 和 runtime boundary guards 固化索引合同与旧链删除。`tests/test_batch_accounting_postgres_integration.py` 在本机 visibly disposable PostgreSQL 应用 0001–0112 后通过 2 项，真实执行 bulk proof、count/list/row lookup、processing fail-closed、附件收窄 SQL，并在 5,000 条非命中 OA 行上用 `EXPLAIN` 证明命中 `workbench_rows_batch_accounting_oa_type_trgm_idx`；临时数据库已删除并确认不存在。
- 生产基线：shell p95 `104.966ms`；unsubmitted p95 `638.061ms`（未达 `500ms`）；submitted p95 `390.986ms`；Page Audit p95 `375.765ms`；dashboard DB query count p50/p95/p99 `52/66/71`。前四次 release 的 unsubmitted p95 依次为 `612.217ms`、`580.757ms`、`514.231ms`、`513.385ms`；第四次已把 query-count p95 降到 `8`，但生产 DB p95 `254.613ms` 证明剩余串行 I/O 仍需删除。
- 发布后验收：unsubmitted/submitted 20-sample p95 `<=500ms`（目标 `<=300ms`），query count p95 `<=6`，Page Audit pass/fresh/drained/ready/0 issue；生产写 smoke 必须先通过全局 `app-health-operations` preflight，失败不得绕过。
