...
部署模块的 Spec-first E2E 目标是证明发布、CI、Nginx、systemd、worker、runtime readiness、rollback 和 production smoke 能支撑页面级 E2E 目标。它没有业务页面，但直接决定所有 Browser E2E 和 runtime gate 的可信度。
...
| `DEPLOY-E2E-005` | 发布后 `/health/ready`、App Health、required workers、background jobs 和 dependencies 无 current blocker。 | health ready + runtime_sync_closure_gate。 |
