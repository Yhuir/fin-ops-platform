...
- `stability` 会校验审计窗口内已有的真实 write-operation 样本；空闲窗口没有样本时记录 `empty_sample_window` 并通过，不为部署制造生产业务写入。已有样本缺失 expectation 或超出 SLO 仍 fail closed；显式 `full` closure 继续要求非空 event/expectation 证据。
