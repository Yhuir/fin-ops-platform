...
| Workbench unavailable OA relation repair | `finops-deploy-control workbench-unavailable-oa-relation-repair` | 只接受单一安全 `--case-id`；execute 必须携带同一 canonical relation/OA 快照 dry-run 返回的 SHA-256 fingerprint，并只调用正式 relation command/persist adapter |
