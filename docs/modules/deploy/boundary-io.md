...
| Controlled write smoke restore point | operator | `write-operation-restore-point <release> <run-id>` 只接受受限 run-id，在固定 root-owned 目录生成完整 PostgreSQL custom-format dump；DSN 仅经环境传递，dump 必须通过 `pg_restore --list` 并输出 SHA-256 manifest 后才可作为 apply 前恢复点 |
