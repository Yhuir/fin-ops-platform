...
| ACL env contract | `/etc/fin-ops/*.env` | prerequisite 检查 root-owned/non-writable env、DSN、fixed OA selector 与 role-sync config；strict contract 要求三项旧 admission key 和 legacy admin key 全部缺席。dev/test auth 分支已从 runtime 删除，不再依赖 env 守卫。发布只读断言，不在每次激活中重写 env |
...
| Schema rollback compatibility | exact candidate + 当前唯一 active release + production `public.schema_migrations` + root-owned compatibility evidence | `RELEASE.json` 绑定 migration count/head/fingerprint；plan 只读生产 schema。无 pending migration 不增加测试前置；有 pending migration 时，evidence 必须绑定 exact candidate/previous commits、PostgreSQL major、plan fingerprint、每个 pending 中间 schema head，以及 invoice/bank existing upsert、import enrichment、settings reset、correction+audit 同事务。缺失或漂移必须在 stop/migration 前失败；迁移后失败也不得无证据启动 previous binary |
...
| Import audit repair | `finops-deploy-control import-audit-repair` | 只调用固定 Python module；execute 必须携带同一数据快照 dry-run 返回的 SHA-256 fingerprint；生命周期修复必须同时显式提供唯一 `--batch-id` / `--file-id`；银行恢复必须显式绑定全部 source session/file、目标/保护 cohort 数、精确重复删除数、预期新增数和 operator |
...
- 对 `runtime`/`acl` 发布，`runtime_sync_closure_gate` 使用两个自动 profile：`preflight` 检查当前 stable runtime readiness、worker/queue/RabbitMQ 收敛、隔离事务写入能力和只读 canonical audit；候选 T+0/T+60/T+300 均使用 `stability`，只读验证真实页面 freshness、性能和 runtime 收敛，不 enqueue synthetic read-model refresh。`full` 是给已固定旧 helper 的只读兼容 alias；显式 enqueue-to-fresh 仍只走独立 `read_model_slo_smoke` 运维工具。无错误、fresh、p99 合格但 p95 超标的三样本窗口最多重采样一次，连续两窗超标仍 fail closed。页面 shell 使用公开 origin，API 使用内部服务 origin；旧 SSE route/tool 已退出 gate。
...
- `runtime`/`acl` 的 pre checkpoint 在切换前完成；候选激活后 T+0/T+60s/T+300s 均运行只读 `stability`。`frontend` 只做 pre/T+0 快速检查并以 exact dist/readiness/005/public asset 作为最终证据。所有 profile 都必须先验证最终 evidence 才返回成功。
