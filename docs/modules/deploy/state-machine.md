...
| 2026-08-28 | 将 `0161` 标记为 operational forward-only migration | `0161` 入队的正式关系规则收敛任务必须由按 `relation_mode` 选择正式关系的候选 worker 消费；禁止旧 binary 消费并遗漏 decision-origin 正式关系 | `tests.test_deploy_oa_script.DeployOAScriptTest.test_release_gate_blocks_unproven_schema_rollback_before_services_stop` |
| 2026-09-02 | 将 `0165` 标记为 forward-only migration | `0165` 删除旧 ACL JSON 字段并切换 OA 角色，禁止迁移后启动仍读取旧 tier 的 binary；失败时保持 maintenance 并前向修复 | `tests.test_deploy_oa_script.DeployOAScriptTest.test_release_gate_blocks_unproven_schema_rollback_before_services_stop` |
