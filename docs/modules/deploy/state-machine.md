...
| 2026-08-31 | 将 `0165` 标记为 forward-only migration | 删除退役成本时间/标签设置后，旧 release 会重新补入该键并破坏 Settings canonical Audit；禁止自动回滚恢复旧语义 | `tests.test_deploy_oa_script.DeployOAScriptTest.test_release_gate_blocks_unproven_schema_rollback_before_services_stop` |
