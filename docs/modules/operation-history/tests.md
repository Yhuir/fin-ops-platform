...
- Service/Repository：durable append、嵌套领域事件继承 HTTP request id、同 request 生命周期聚合、操作人快照/选项、固化证据优先且不额外查询关系历史、旧关联记录按类型去标识化兼容、敏感字段和内部 ID 移除、覆盖点后查询、append-only trigger；关系收据只在新指纹首次保存时记录 `receipt_generated`，每次成功返回 PDF 记录 `receipt_print_requested`，幂等重放不重复文件且永不伪造 `receipt_printed`。
...
`OperationHistoryPage.test.tsx` 新增进入时起止为空、同页查询/刷新/详情关闭保留日期、卸载重进无日期参数回归；既有管理员权限和 latest-request 旧响应隔离回归继续执行。查询的日期 draft/filters 继续是页面本地状态。
