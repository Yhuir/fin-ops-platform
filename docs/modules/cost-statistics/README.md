...
2026-07-25 起成本页用唯一 `effectiveCostPageState` 合并当前 explorer lifecycle 与 explorer freshness envelope。普通 relation、turnover、bank-flow 写入和 Workbench publish 都不直投 Cost；route 进入/重进、当前页查询变化、浏览器手动刷新或明确重试只重跑 explorer normal GET，focus/visibility/BFCache/跨页事件不触发业务 I/O。`project|bank|expense_type` GET 依次收敛 Workbench、Bank Detail 与 Cost exact scope；`time|bank_tag` 只收敛 Bank Detail exact scope并直接读取其结构化 rows，不等待或入队 Workbench/Cost。refreshing 使用 150ms、最多 200 次的页面自身有界、可取消重试。标签规则保存只改变 query 过滤设置并递增当前页 query nonce，不启动 operation barrier。只有明确 `fresh` 才开放成本业务操作；loading/refreshing/stale/unavailable/error 使用页面内内联状态轨与原生 `inert`，标题、Audit、App Shell 和导航始终在锁定边界外。
...
2026-07-16 起成本页面 Audit 的唯一 owner 是 `cost_statistics_page_audit.py`。统一 page key、通用只读 CLI 与 System Audit 都直接分派给该 owner，并继续复用 caller-owned repeatable-read read-only snapshot；共享 `page_business_audit.py` 不再包含成本合同、SQL、issue mapping 或 dependency dispatch。该 owner 历史上逐步把 proof 收敛为有界集合 SQL。2026-07-24 后，OA allocation exact-set仍由正式 Workbench OA+bank members重算；全银行 exact-set/金额/标签则由 canonical bank facts与独立 Bank Detail collector证明，不再审计已删除的 Cost bank-flow复制表。Audit保持只读、无入队/修复副作用，生产连续 pass仍是最终门禁。
