...
- 待分配详情可提供 `suggested_source_allocations`，仅作用户确认前的草稿。读取银行原始记录中的明确 OA 引用，并且逐 OA 的金额分布唯一时预填；仅金额相等、子集合合计或界面同行不构成依据。模糊组保持未填写，既有会话草稿和保存决定不被覆盖；预填本身不写库、不完成任务、不改变统计。
