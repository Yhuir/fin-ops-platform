...
2026-07-23 起成本页用唯一 `effectiveCostPageState` 合并当前 explorer lifecycle 与 explorer freshness envelope。普通 relation、turnover、bank-flow 写入和 Workbench publish 都不直投 Cost；首次访问、focus、hidden→visible 或轻量关系提示只重跑 explorer normal GET。`project|bank|expense_type` GET 依次收敛 Workbench、Bank Detail 与 Cost exact scope；`time|bank_tag` 只收敛 Bank Detail exact scope并直接读取其结构化 rows，不等待或入队 Workbench/Cost。refreshing 使用 150ms、最长 3s 的页面自身有界重试。标签规则保存只改变 query 过滤设置并递增当前页 query nonce，不启动 operation barrier。只有明确 `fresh` 才开放成本业务操作；loading/refreshing/stale/unavailable/error 使用页面内内联状态轨与原生 `inert`，标题、Audit、App Shell 和导航始终在锁定边界外。
