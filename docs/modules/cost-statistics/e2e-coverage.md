...
| --- | --- | --- | --- |
| `COST-E2E-001` | `covered` | `web/e2e/cost-statistics-flow.spec.ts`、`web/src/test/CostStatisticsPage.test.tsx`、`web/src/test/CostStatisticsApi.test.ts`、`tests/test_cost_statistics_api.py` | Browser 覆盖 fresh 首屏按时间、按项目、按银行和按 OA 费用类型 baseline；按银行/按 OA 费用类型用真实 Chromium 进入对应视图、选择银行账户/费用类型、展示后端 explorer 成本行并打开流水详情；按项目/按银行/按 OA 费用类型/按标签视图切换和时间范围切换已记录 operation latency。成本统计标签规则抽屉已覆盖打开、保存、当前 explorer normal GET、零 operation barrier，并分别记录 opener/save latency。更多银行/费用类型组合由 Vitest/API 覆盖，真实生产大数据性能仍归 staging 风险。 |
...
| `COST-E2E-003` | `covered` | `web/e2e/cost-statistics-flow.spec.ts`、`web/e2e/cost-statistics-relation-fanout.spec.ts`、`web/src/test/CostStatisticsPage.test.tsx` | Browser 已覆盖项目/费用类型/流水详情下钻，以及 relation 变化后访问成本页的 normal GET/refreshing/fresh 详情；项目/费用类型下钻、打开与关闭详情已记录 operation latency。文件名保留历史 `relation-fanout`，测试合同已是 access convergence。 |
...
| `COST-E2E-010` | `covered` | `web/e2e/cost-statistics-relation-fanout.spec.ts`、`web/e2e/imports-bank-transactions-flow.spec.ts`、`web/e2e/imports-invoices-flow.spec.ts`、`web/e2e/imports-etc-invoices-flow.spec.ts`、`web/e2e/bank-flow-rule-batches-flow.spec.ts`、`web/e2e/turnover-ledger-flow.spec.ts`、`web/e2e/settings-data-reset-flow.spec.ts`、相关 backend lifecycle/read model tests | 已覆盖 relation/import/bank-flow/turnover/settings 事实变化后独立进入成本统计，normal GET 从 Workbench dependency 到 Cost 自身 fresh，并断言对应成本行、项目、费用类型和银行字段；组件/后端另断言普通 relation 写和 Workbench publish 零 Cost enqueue。search 当前无独立前端 route；真实 worker drain、真实导入样本和生产 access-to-fresh 仍为 staging/生产风险。 |
