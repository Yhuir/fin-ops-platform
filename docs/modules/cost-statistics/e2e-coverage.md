...
| `COST-E2E-004` | `covered` | `web/e2e/cost-statistics-flow.spec.ts`、`tests/test_cost_statistics_service.py`、`tests/test_cost_statistics_api.py` | Browser 已覆盖 preview query、成功 download event 和 row-limit 错误反馈；导出中心打开、仅预览、导出成功和 row-limit 错误已记录 operation latency。 |
