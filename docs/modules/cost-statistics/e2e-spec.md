...
| `COST-E2E-008` | 关系净支出 OA 分摊 | `N=支出合计-明确付错退款`；恰好一个有效 OA 单元且一条支出流水时直接以 N 自动归因，不要求 O=N；其它关系未完成有效人工分配前从归因视图排除，time/bank_tag 原始事实不变 |
...
| `COST-E2E-016` | 人工分配闭环 | Drawer 打开后才加载全局队列；pending/stale 输入为空，allocated 可编辑；完整 OA 单元 × 来源矩阵使用非负两位小数，每条来源精确闭合且单元净成本非负；保存以 source fingerprint 和 expected version 防止陈旧写入，并与 audit 同事务提交 |
| `COST-E2E-017` | 多流水多单元人工归集 | policy 直接消费用户确认的支出正向/退款负向矩阵，不再合成比例矩阵；项目、银行、费用类型三视图人口和金额一致 |
| `COST-E2E-018` | 视图口径分组 | HeroUI 切换区以“配对归集”承载项目/银行/费用类型，以“流水分析”承载标签/时间；分组不增加页面、缓存或跨模块 I/O，窄屏可折行且无横向溢出 |
