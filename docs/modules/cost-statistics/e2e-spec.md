...
| `COST-E2E-008` | 关系净支出 OA 分摊 | `N=支出合计-明确付错退款`；`O=N` 时复用既有比例并按分闭合；`O!=N` 时禁止自动缩放，未完成有效人工分配前从归因视图排除，time/bank_tag 原始事实不变 |
...
| `COST-E2E-016` | 人工分配闭环 | Popover 打开后才加载；首次 pending/stale 输入为空；所有稳定 OA 单元必须填写非负两位小数且合计严格等于 N；保存以 source fingerprint 和 expected version 防止陈旧写入，并与 audit 同事务提交 |
| `COST-E2E-017` | 多流水多单元人工归集 | 有效人工金额作为列边界、各真实支出净额作为行边界，确定性按分矩阵的行/列/总额同时闭合，项目、银行、费用类型三视图人口一致 |
