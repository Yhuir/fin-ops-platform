...
成本统计页面面向项目和期间展示费用、发票、流水和核销关系。页面负责筛选、切换视图、下钻和导出反馈；金额归因、项目归属、candidate/linked relation 语义必须来自后端 cost attribution 边界。页面不能把关联台 open/proposed candidate 当作 confirmed 成本事实，也不能重新引入旧同步状态作为页面状态。
...
| Spec ID | 场景 | 优先级 | 验收标准 |
| --- | --- | --- | --- |
| `COST-E2E-001` | direct time/project/bank/expense baseline | P0 | 页面加载 direct explorer，按时间、项目、银行和费用类型展示一致金额；首屏不从前端伪造金额或项目范围。 |
...
| `COST-E2E-006` | direct explorer/export with local detail errors | P0 | explorer/export-preview/export 忽略旧同步字段并按 direct payload 渲染；transaction detail 的局部后端错误只影响详情弹窗，不影响页面级导出。 |
...
| `COST-E2E-010` | downstream/import/settings fan-out | P1 | 导入确认、项目范围设置、turnover/no-OA/ETC 等成本相关写入后，成本统计 direct API 展示一致结果；真实后台任务、outbox 和 cache warmup 只作为运行诊断/性能优化保护。 |

## 不属于本地 deterministic E2E 的风险

- 真实生产 PostgreSQL scope cleanup `--apply` 和历史脏数据修复。
- 真实 RabbitMQ/Redis/systemd 后台任务、direct API 和 cache warmup 收敛。
