...
| Workbench 输入边界 | `read_model.workbench_generations` active generation、`read_model.workbench_groups` | 成本统计必须先定位 active generation，再按 `generation_id + scope_key` 消费 groups；不能按裸 `scope_key` 扫描历史 generation。 |
