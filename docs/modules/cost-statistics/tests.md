...
- Phase 28 active-recovery热路径：`tests/test_cost_statistics_sql_runtime.py`证明`time|bank_tag`在durable Workbench/Cost recovery已经`pending|processing`时只返回独立`statistics=refreshing`，保留fresh Bank Detail rows，跳过完整`cost_statistics_read_models + active_months + child_proofs` SQL且不重复enqueue/计算Workbench canonical proof；无active recovery、规则版本漂移和exact Bank Detail scope继续走原完整证明。`tests/test_cost_statistics_postgres_integration.py`在全迁移一次性PostgreSQL真实执行轻量aggregate SQL并验证exact Workbench scope。
...
`bash scripts/verify.sh all` 会运行 backend unittest discover、frontend Vitest、deterministic Playwright smoke 和 build，覆盖完整成本统计、App Status、read model gateway、前端测试集、成本统计 browser 主流程和 Workbench→Cost dependency-ordered 收敛。单轮模块验证只跑最小闭环。
