...
## 全部时间默认与导出（2026-09-21）

- `CostStatisticsPage.test.tsx` 覆盖旧年月 session、五视图首次全部、同页刷新保留月份、重新进入清空日期、全部导出无伪造起止日、明确自定义日期未填时阻止请求。
- `cost-statistics-flow.spec.ts` 在浏览器验证五视图全部、主动选月、刷新、导出继承和硬刷新恢复全部；生产 `production-date-defaults.spec.ts` 覆盖实际路由导航及请求范围。
- 现有 Cost API/Policy/Repository/导出回归保护数据与权限；不新增后端接口或读模型。

## 外部往来款主标签置底（2026-09-21）

- Policy 覆盖付款/收款分离、收款原本在前、已经置底、单项置底、无目标标签、空集合、重复读取一致及输入/金额/计数不变。
- API 覆盖主标签顺序、搜索后置底、付款/收款分别下钻及与时间视图汇总一致；Repository 沿用单快照、批量查询回归，不增加查询。
- 页面组件验证保留服务端顺序、两项独立选择与子标签请求；既有 `cost-statistics-flow.spec.ts` 保护五视图及下钻流程。生产只读验证年月/搜索、真实页面末尾位置、两项下钻及五视图性能。
