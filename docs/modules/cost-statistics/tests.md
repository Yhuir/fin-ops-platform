...
| Frontend | `web/src/test/CostStatisticsApi.test.ts`、`CostStatisticsPage.test.tsx` | 首次加载、局部 surface/children/rows 加载、单次交互只发一个 explorer 请求、范围切换清除旧筛选、标签零金额与列宽、失败重试、筛选、导出、抽屉保存 |
