...
| Frontend | `web/src/test/CostStatisticsApi.test.ts`、`CostStatisticsPage.test.tsx` | 首次加载、局部 surface/children/rows 加载、当前视图搜索与过期请求保护、单次交互只发一个 explorer 请求、范围切换清除旧筛选、标签笔数/金额/顺序、时间真实字段、自动分页与局部失败重试、无手动加载按钮、导出、抽屉保存 |
