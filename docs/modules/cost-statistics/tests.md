...
| 前端页面交互 | P1 | `web/src/test/CostStatisticsPage.test.tsx`、`web/e2e/cost-statistics-flow.spec.ts`、`web/e2e/cost-statistics-relation-fanout.spec.ts` | covered | time/project/bank/expense view、drilldown、range picker、empty/error/refreshing、export center、后端导出失败消息展示、同一流水拆成多条成本行时项目费用类型下钻不丢行/不触发表格重复 key；真实 Chromium 覆盖按项目下钻、流水详情、导出中心错误反馈，以及 Workbench 成本关系 candidate 不计入、confirmed 后计入成本。 |
...
| 5. Frontend component and interaction tests | 适用 | `web/src/test/CostStatisticsPage.test.tsx`、`web/src/test/CostStatisticsApi.test.ts`、`web/e2e/cost-statistics-flow.spec.ts`、`web/e2e/cost-statistics-relation-fanout.spec.ts` | 覆盖页面状态、范围选择、drilldown、export center、后端失败消息展示、API mapper 和 cache；e2e 保护真实 Chromium tab、project scope、三段下钻、modal、preview、导出错误反馈和 relation fan-out 后的项目/金额/详情展示。 |
