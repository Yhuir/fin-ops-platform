...
读取失败可重读；保存 400/403/409 保留草稿，冲突须重读版本。网络/服务端结果不明确进入 `unconfirmed`，禁用再次提交，通过 GET 核实版本及完整选择。确认已生效后刷新统计；不一致则保留草稿供核对。范围变化中止旧请求并重置分页/下钻，不改变期间和搜索。无来源成本仅在任务中，不进入正式合计；范围外任务从列表数量移除但仍可按关系 ID 读取原决定。
