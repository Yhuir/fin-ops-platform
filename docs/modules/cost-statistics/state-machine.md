...
`pending + 未保存 + 明确来源/唯一金额 → 详情返回建议 → 可编辑会话草稿 → 用户保存 → 既有 pending/allocated 判断`。

读取建议不发生状态转换或写入，未保存建议不进入统计。无证据、分布歧义、过期、曾保存、非固定目标或存在退款/非成本时不预填；可独立证明的 OA 子组可预填，其他子组仍为空。折叠/展开和重新渲染不覆盖用户输入，已清空的草稿不被重新填回。
