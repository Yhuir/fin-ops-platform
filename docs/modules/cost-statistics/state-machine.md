...
`closed → loading → editing → saving → closed + canonical reread`。
读取失败可重读；保存 400/403/409 保留草稿，冲突须重读版本。网络/服务端结果不明确进入 `unconfirmed`，禁用再次提交，通过 GET 核实版本及完整选择。确认已生效后刷新统计；不一致则保留草稿供核对。范围变化中止旧请求并重置分页/下钻，不改变期间和搜索。无来源成本仅在任务中，不进入正式合计；全范围外任务从列表数量移除，按关系 ID 读取只得到当前范围投影；历史决定保留在存储中，恢复范围后重新读取。混合组只展示范围内来源。范围变更不自动确认未保存来源，已保存有效来源不因新增待分配来源而消失。退款归属不明进入 scope_refund_required，范围版本漂移的保存返回 409。
...
每次读取：完整正式事实 → 独立来源计算 → 成本范围投影 → allocated 或 pending。allocated 且 version=0 显示自动分配，无人工保存时间；已完成入口允许查询、重开并通过原 PUT 创建版本1人工决定。自动计算没有写入副作用。

部分组件自动确定时，已知来源进入成本，其余金额与原因保留 pending；固定 OA 目标不降为当前已分金额。详情草稿初始化合并确定行及剩余建议，脏草稿和旧响应保护不变。人工保存后统一使用保存决定；已有人工决定失效继续重核，关系撤回在下次读取消除成本。自动完成仍受已有资料完整性检查，缺资料不能靠再保存解决。

## 成本分配变更（2026-09-21）

自动分配完成仅进入统计，可从成本明细调整；人工列表完成态仅包含已保存人工决定。外部往来未人工确认保持 pending；金额锁定按 OA 单元保存，解锁不改变来源事实。
