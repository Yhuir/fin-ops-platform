...
- `bank_tag` 主标签顺序由 Policy 唯一生成：原排序后，将存在的外部往来款付款、收款按此顺序移动至列表末尾，单项也置底、缺项不补造；其余主标签相对顺序、子标签排序、字段和金额不变。前端按返回顺序展示，不维护第二套排序；无新增数据库或跨页面 I/O。
...
- 已删除详情独占来源证据、外层多对多直接拒绝自动、自动完成提前返回缺身份的旧路径。2026-09-21 起自动任务退出人工列表，由成本明细直接编辑。金额组合建议仍是人工业务能力，不作为自动结果或兜底。
...
- 银行有效分类 projection 输出 `turnover_role`，Cost repository 仅传递结构化语义；原银行、关联台、往来账及 OA 金额无写入变化。
- 详情单元新增 `lock_oa_amount: boolean`、`outside_cost_amount: money`；PUT 必填 `oa_amount_locks: [{unit_id, locked}]`，精确覆盖当前 OA 单元，人工补充项不含此字段。移除整个链路的 `amounts_fixed`，不保留旧请求 fallback。
- Repository 使用 migration 0176 的 `oa_amount_locks` JSONB 保存；历史金额等于原额则锁定，已减额则解锁，原始金额、来源、版本和指纹均不改。保存沿用同一事务/审计/CAS；状态从同一 Policy 重新生成。
...
银行用途与分类读取 `app.bank_transaction_units`；真实流水数量/余额来源仍是原始银行事实，统计流水数量去重父身份。按配置语义 `turnover_role=external_turnover` 排除本金，不能因同一 case 含本金而剔除利息子项。拆分后的用途金额继续执行现有唯一来源自动分配规则，不读取历史 `bank_split_requires_cost_confirmation` 标记；来源歧义或金额未闭合进入待分配，有效人工分配优先。指纹不包含整笔拆分版本，排序或无关子项修改不使既有分配失效。成本详情以父身份和原始金额展示，`bank_transaction_unit_id` 保留成本用途追踪。

验证：`tests/test_bank_split_consumers_postgres.py` 使用隔离 PostgreSQL 数据库覆盖原始金额不变、子项标签筛选、成本自动/待分配、待票金额和详情父身份、往来补充信息迁移。往来手工成员重建和含利息 case 的本金闭环由对应 service/query 单元测试覆盖。

- 一次性旧外部往来成本迁移仅通过成本 owner 撤销旧分配并保留审计，不写关联确认标记或无关关系版本。撤销后按当前用途事实运行同一自动分配规则；无法唯一确定的部分保留待分配。CLI 输出 affected/mixed/revoked case 清单，dry-run 不写，撤销与审计同事务回滚。
...
## 2026-09-24 同组本金 OA 的唯一来源证明

对混合本金与费用、全部为已拆分支出且包含多条 OA 的完整关系，成本 Policy 在剔除本金前复用 automatic_relation_sources：只有全组金额守恒、所有 OA 单元与银行来源完全覆盖、显式引用和历史来源均不冲突，才排除其全部单元仅对应本金的 OA。重复金额或部分证明不推断剩余 OA。余下唯一利息按已有自动规则直接进入成本，不要求保存；有效人工决定继续优先。不新增算法、持久化或读取 I/O。

独立历史本金组也必须复用同一完整来源证明，不能仅凭组内银行标签排除 OA。金额不平衡、显式引用冲突、来源容量未完整覆盖的历史组保留 OA；共享 OA 保护继续生效。历史分组不由展示对齐自动改写，错误正式归属通过既有 cancel-link 与 confirm-link 命令纠正并保留原审计。
