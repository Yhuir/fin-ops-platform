...
| 关系变更 | canonical relation source version + Cost query owner | relation transaction 和 Workbench generation publish 都不投递 Cost。`project|bank|expense_type` 首次访问发现 Workbench stale 时，通过既有 gateway ensure exact Workbench month，并只 stage 当前请求的唯一 `active|all:YYYY-MM|all` Cost scope；不 stage sibling scope。Cost worker复用 manifest dependency fail closed/defer，先精确补投 child，依赖 fresh 后才允许发布，避免页面轮询串行制造 Workbench→child→parent 三段空档。`time|bank_tag` 只消费 Bank Detail 语义 proof与成本 settings，因此跳过 Workbench/Cost row pre-gate，绝不入队 Cost。Workbench dirty 不得阻塞这两个全流水视图。标题 global statistics 可保留完整 parent lineage并独立返回 `statistics=null/refreshing`，不能反向阻塞已经 fresh 的 rows。 |
...
| Dirty scope | runtime queue | `project|bank|expense_type` GET 发现 Workbench stale 时 ensure exact Workbench，并通过同一 gateway stage当前请求的唯一 Cost scope；这是访问时 exact fan-in，不是写后 fan-out。Cost event可在 dependency non-fresh时短暂 defer，但 worker只按 manifest补投 exact dependency/child并由 durable active dedupe合并，依赖 fresh前不得读取或发布旧 payload。`time|bank_tag` 只可 ensure non-fresh Bank Detail scope。普通 import 只推进 canonical source version，不直投 Cost。worker 用 event `source_version` 做条件发布，完成 month 后才 fan-out 必需 parent；竞态失败保持 `refreshing`。relation direct delta、Workbench publish fan-out、sibling staging、repository 隐藏 fan-out 和第二队列路径禁止恢复。 |
| Access-to-fresh proof | 生产 smoke / Cost explorer | 从 mutation 成功到首次 Cost explorer GET 开始计时。OA allocation view 必须证明 Workbench→Bank Detail→Cost exact scopes依次收敛；全流水 view 必须证明仅 Bank Detail exact scopes收敛且零 Cost event。最终 explorer 必须 `200 + fresh + source_versions changed + business assertion changed`；任一 event done 都不能冒充页面可见。 |
...
- Miss/stale owner：`CostStatisticsQueryService` 对 `project|bank|expense_type` 先比较 canonical Workbench expected versions 与 active generation；不匹配时 ensure exact Workbench，并只 stage当前请求的 exact Cost month/parent。Cost worker沿 manifest依赖精确 defer/补投，页面有界自动轮询只观察同一 scope，不负责串行发现下一阶段；月份使用单 scope proof，parent 使用 set-based scope 映射。`time|bank_tag` 直接进入 Bank Detail profile，只为 non-fresh Bank Detail exact month入队，不入队 Workbench 或 Cost。repository gate可同时返回独立 global statistics 状态；statistics drift 只置空 statistics并精确 ensure其自身依赖，不能阻塞当前视图 rows。
...
- 成本 projection 内的 bank-detail source version、transaction tags、month rows 必须由一次 `snapshot_for_month(...)` 快照读取共同提供；旧 `source_versions_for_scope_keys(...) + get_by_transaction_ids(...) + list_by_month(...)` 三段读取链路已删除。快照在 repository 内使用 `REPEATABLE READ READ ONLY`，同时覆盖目标月 rows 与正式关系引用的跨月流水 ID；projection 只做纯内存拆分和归一化。访问入口的 Workbench dependency与own-scope staging owner是 `CostStatisticsQueryService`；它只能投递当前请求 Cost scope，不展开 child/sibling。已运行 Cost job 遇到 non-fresh 依赖时，由 `RuntimeWorker._enqueue_dependency_refreshes(...)` 按 manifest精确补投。
