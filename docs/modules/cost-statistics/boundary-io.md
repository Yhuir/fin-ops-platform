...
- 旧代码删除状态：legacy `CostStatisticsService` 与 `CostStatisticsReadModelService` 的 module/class/test/import 均已删除，route、Application、query、runtime、projection 和 API fixture 不保留对应 field、snapshot、persist callback 或 shim。仍有效的归集规则只由 `CostStatisticsSqlProjectionBuilder` 负责，并直接向正式 repository port 提交单 scope write model；导出 limit/error 由 `CostStatisticsQueryService` 负责。project/detail/export/export-preview 只调用 query owner；query miss/stale 只返回 refreshing 并入队 durable refresh。05-12 删除了 Application 的四个 expected/source/dependency wrapper、runtime source provider、query tag-selection provider 和 Redis delete；05-13 删除进程内成本 read model owner；05-14 又删除 repository/state-store/protocol/manifest 的全量 load 与无条件 save 合同、启动 snapshot key 和 broad save branch，runtime invalidation 只以 durable enqueue 成功作为失效证据，正式写入只剩 source-version conditional publish；05-19 又删除 worker unchanged 判定对 full-view loader 的依赖，改为唯一 parent metadata point query；05-20 删除 `cost_tax_sql_projection.py` 混合 owner，成本投影只属于 `cost_statistics_sql_projection.py`。历史 `cost_statistics_cache_warmup` job 入口仅作为兼容桥接，关闭或转入 `cost_statistics.read_model.refresh`，不写 payload/read model/Redis；它必须在统一部署窗口证明 active job 为零后再独立删除。旧 month/project HTTP合同仍使用 full view，必须取得生产 access-log/owner 证据后再删除。前端 5 分钟 explorer Map、首屏 `active:all` 预取和零调用 clients 也已删除。
...
- 发布边界：`CostStatisticsReadModelRepositoryPort.publish_cost_statistics_read_models(...)` 是成本 worker 的唯一正式发布 I/O。repository 复用现有 active-scope partial unique index，在一个事务内锁定唯一 `pending` / `processing` dirty row，避免扫描排序历史 terminal rows；版本精确相等才写 snapshot/rows、`published_source_version` 和 parent obsolete deletes。拒绝发布必须零 read-model 写入，projection 也不得写或删 Redis。05-14 已删除旧无版本 save 及其 repository/state-store/protocol/manifest 表面；禁止以 shim、fallback 或 broad state payload 恢复。
- Unchanged 判定边界：`CostStatisticsReadModelRepositoryPort.get_cost_statistics_scope_metadata(...)` 只按规范 scope point lookup parent `scope_key/entry_count/source_versions`。projection 只在完整 `source_versions` 精确相等时 skip；missing/mismatch 必须重建。该 I/O 不承担页面 freshness，不得读取 payload、两张结构化 rows、dirty queue、Workbench、Bank Detail 或 App Settings，也不得回退 `get_cost_statistics_view(...)`。
...
| Repository / SQL | `cost_statistics_read_model_repository.py`、`cost_statistics_sql_projection.py`、`postgres_repositories/read_models.py`、`postgres/migrations/0105_cost_statistics_freshness_gate.sql`、`postgres/migrations/0107_cost_statistics_structured_bank_flow_rows.sql` |
