...
| 关系变更 | canonical relation source version + Cost query owner | relation transaction 和 Workbench generation publish 都不投递 Cost。`project|bank|expense_type` 请求先读取现有 Cost gate；gate 已 non-fresh 时跳过 canonical 全量证明并只 ensure其 exact upstream/child。只有 gate 可 fresh 时才比较 canonical Workbench；Workbench stale时只 ensure exact Workbench month，不提前创建 Cost event。后续访问确认 Workbench fresh后才 enqueue exact Cost child，child完成后收敛parent。Cost worker复用 manifest dependency fail closed处理真实竞态，依赖 fresh 后才允许发布；禁止已知 stale dependency 下的高频 defer竞争。`time|bank_tag` 只消费 Bank Detail 语义 proof与成本 settings，因此跳过 Workbench/Cost row pre-gate，绝不入队 Cost。Workbench dirty 不得阻塞这两个全流水视图。标题 global statistics 可保留完整 parent lineage并独立返回 `statistics=null/refreshing`，不能反向阻塞已经 fresh 的 rows。 |
...
| Dirty scope | runtime queue | `project|bank|expense_type` GET 发现 Workbench stale 时只 ensure exact Workbench；下一次访问在 Workbench fresh 后才 enqueue exact Cost child，完成 month 后沿既有路径 fan-out 必需 parent。这是访问时 dependency-ordered exact fan-in，不是写后 fan-out；不得预投 known-blocked Cost、不得扩散到 sibling project scope。Cost event只在真实 TOCTOU 竞态下短暂 defer，并由 durable active dedupe合并，依赖 fresh前不得读取或发布旧 payload。`time|bank_tag` 只可 ensure non-fresh Bank Detail scope。普通 import 只推进 canonical source version，不直投 Cost。worker 用 event `source_version` 做条件发布；竞态失败保持 `refreshing`。relation direct delta、Workbench publish fan-out、repository 隐藏 fan-out 和第二队列路径禁止恢复。 |
...
- Miss/stale owner：`CostStatisticsQueryService` 对 `project|bank|expense_type` 先读取 repository gate。gate 已 non-fresh 时直接使用其 exact dependency/child scopes并跳过 canonical Workbench proof；只有 gate 可 fresh 时才比较 canonical Workbench expected versions 与 active generation。不匹配时只 ensure exact Workbench；后续有界页面轮询在 Workbench fresh 后才 enqueue exact Cost child，child完成后收敛parent，禁止 sibling project scope和预投 known-blocked Cost。Cost worker沿 manifest依赖精确处理真实 TOCTOU 竞态。`time|bank_tag` 直接进入 Bank Detail profile，只为 non-fresh Bank Detail exact month入队，不入队 Workbench 或 Cost。repository gate可同时返回独立 global statistics 状态；statistics drift 只置空 statistics并精确 ensure其自身依赖，不能阻塞当前视图 rows。
...
- 成本 projection 内的 bank-detail source version、transaction tags、month rows 必须由一次 `snapshot_for_month(...)` 快照读取共同提供；旧 `source_versions_for_scope_keys(...) + get_by_transaction_ids(...) + list_by_month(...)` 三段读取链路已删除。快照在 repository 内使用 `REPEATABLE READ READ ONLY`，同时覆盖目标月 rows 与正式关系引用的跨月流水 ID；projection 只做纯内存拆分和归一化。访问入口的 dependency ordering owner是 `CostStatisticsQueryService`：Workbench stale时只投递 exact Workbench；后续访问才投递当前 Cost child，parent由既有 child完成路径收敛，禁止 sibling project scope和提前 Cost staging。已运行 Cost job遇到真实 non-fresh竞态时，由 `RuntimeWorker._enqueue_dependency_refreshes(...)` 按 manifest精确补投。
