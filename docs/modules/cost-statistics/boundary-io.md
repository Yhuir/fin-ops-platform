...
| 关系变更 | canonical relation source version + Cost query owner | relation transaction 和 Workbench generation publish 都不投递 Cost。Cost GET 先比较当前 canonical Workbench expected versions 与 active generation versions；Workbench stale 时只 enqueue 请求所需的精确 Workbench 月份，返回 `refresh_dependency=workbench`，不同时 enqueue Cost。Workbench fresh 后的下一次同 scope GET 再比较 Cost proof，仅当 Cost stale 时 enqueue 当前 Cost scope。该两阶段依赖收敛复用现有 query/refresh gateway、queue、worker 和 repository，不新增 worker、表、HTTP、协调器或 fallback。 |
...
- Miss/stale owner：`CostStatisticsQueryService` 先比较 canonical Workbench expected versions 与 Cost repository 返回的 active Workbench versions。不匹配时只 enqueue 当前 Workbench 月份并返回 `refresh_dependency=workbench`；匹配后再经 `CostStatisticsReadModelRepositoryPort.get_cost_statistics_freshness_gate(...)` 执行 Cost dependency-bound PostgreSQL gate。Cost gate missing/non-fresh 时才 enqueue Cost 并返回空 `refreshing` envelope，禁止访问 Redis/full rows。只有两级 gate 均 fresh 且同一 snapshot 的业务 source/schema 匹配后，才委托 `ReadModelQueryGateway` 处理 Redis/payload，禁止同步 rebuild。
