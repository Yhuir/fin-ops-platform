...
| Workbench 月度输入 | `read_model.workbench_generations` active generation + `read_model.workbench_groups` | 先定位 active generation，再按 `generation_id + scope_key` 读取 `group_type=relation` 且含 OA+bank 的 `paired` / `unpaired` groups；candidate、unpaired singleton 和无 OA relation 不进入 OA 成本。Workbench 已完成 OA 准入合同由上游 generation 保证，Cost 不重新猜 workflow status；发票成员不参与资格判断。禁止按裸 `scope_key` 扫描历史 generation。 |
