...
| Dirty scope | runtime queue | `project|bank|expense_type` GET 发现 Workbench stale 时 ensure exact Workbench；月份请求只 stage当前 Cost scope，parent请求通过同一 gateway一次 stage同一页面所需的 exact children与parent。这是访问时 exact fan-in，不是写后 fan-out；不得扩散到 sibling project scope。Cost event可在 dependency non-fresh时短暂 defer，并由 durable active dedupe合并，依赖 fresh前不得读取或发布旧 payload。`time|bank_tag` 只可 ensure non-fresh Bank Detail scope。普通 import 只推进 canonical source version，不直投 Cost。worker 用 event `source_version` 做条件发布，完成 month 后才 fan-out 必需 parent；竞态失败保持 `refreshing`。relation direct delta、Workbench publish fan-out、repository 隐藏 fan-out 和第二队列路径禁止恢复。 |
...
- Miss/stale owner：`CostStatisticsQueryService` 对 `project|bank|expense_type` 先比较 canonical Workbench expected versions 与 active generation；不匹配时 ensure exact Workbench。月份请求只 stage当前 Cost month；parent使用同一 set-based scope映射一次 stage该页面的 exact Cost children与parent，禁止 sibling project scope。Cost worker沿 manifest依赖精确 defer/补投，页面有界自动轮询只观察同一请求，不负责串行发现下一阶段。`time|bank_tag` 直接进入 Bank Detail profile，只为 non-fresh Bank Detail exact month入队，不入队 Workbench 或 Cost。repository gate可同时返回独立 global statistics 状态；statistics drift 只置空 statistics并精确 ensure其自身依赖，不能阻塞当前视图 rows。
...
- 成本 projection 内的 bank-detail source version、transaction tags、month rows 必须由一次 `snapshot_for_month(...)` 快照读取共同提供；旧 `source_versions_for_scope_keys(...) + get_by_transaction_ids(...) + list_by_month(...)` 三段读取链路已删除。快照在 repository 内使用 `REPEATABLE READ READ ONLY`，同时覆盖目标月 rows 与正式关系引用的跨月流水 ID；projection 只做纯内存拆分和归一化。访问入口的 Workbench dependency与own-scope staging owner是 `CostStatisticsQueryService`；月份请求只能投递当前 Cost scope，parent请求只能投递同一页面集合内由 Workbench scope映射出的 exact children与parent，禁止 sibling project scope。已运行 Cost job 遇到 non-fresh 依赖时，由 `RuntimeWorker._enqueue_dependency_refreshes(...)` 按 manifest精确补投。
