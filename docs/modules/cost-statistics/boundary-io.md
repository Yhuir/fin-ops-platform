...
| Refresh scope | `cost_statistics` manifest | active/all month + parent aggregate。显式 `force_refresh=true` 的 parent 必须枚举全部月份 shard，并把该 metadata 传给每个 shard；月份 worker 必须绕过 relation delta 与 unchanged shortcut 做完整重建，不能用旧 `fresh` readiness 复用旧算法数据。 |
...
| 关系变更 | canonical relation source version + Cost query owner | relation transaction 和 Workbench generation publish 都不投递 Cost。具体月份 GET 比较该月 canonical Workbench expected versions 与 active generation；`scope=all` GET 通过 Workbench builder 的 set-based 多月份证明一次比较全部 canonical month proofs 与 active generations。Workbench stale 时只 enqueue 真正漂移的月份，返回 `refresh_dependency=workbench`，不同时 enqueue Cost。Workbench 全部 fresh 后，Cost parent gate 再把每个 concrete Cost shard 嵌入的 Workbench/Bank Detail versions、dependency dirty state 和 parent `source_shards` 与当前值比较；只 enqueue 漂移的 Cost child，全部 child fresh 后才 enqueue/publish parent。该两阶段依赖收敛复用现有 query/refresh gateway、queue、worker 和 repository，不新增 worker、表、HTTP、协调器或 fallback。 |
...
- Relation-origin 写入入口为零：relation repository、UoW、turnover writer、自动匹配/lifecycle registry 与 Workbench publish 都不拥有成本 I/O。成本只在页面访问中执行 Workbench dependency gate 与 Cost gate。`cost_statistics_relation_delta` / `workbench_shard_published` 的普通写产生者、`workbench_relation_changed` / `turnover_relation_changed` 成本 reason、HTTP consumer 与隐藏 scope expansion 已从生产主链删除；现存 delta worker/repository 能力暂不是普通写可达路径，由 27-06 旧代码分类门禁决定最终删除或显式运维保留，不得作为 fallback。
- Query owner：`CostStatisticsQueryService`；项目明细、流水详情、export-preview、export 都归属该 owner。
- Miss/stale owner：`CostStatisticsQueryService` 先比较 canonical Workbench expected versions 与 Cost repository 返回的 active Workbench versions。月份请求使用单 scope proof；parent 请求使用 `WorkbenchSqlProjectionBuilder.source_versions_for_scopes(...)` 与 `active_workbench_source_versions_by_scope(...)` 的 set-based 映射。不匹配时只 enqueue 漂移的 Workbench 月份并返回 `refresh_dependency=workbench`；匹配后再经 `CostStatisticsReadModelRepositoryPort.get_cost_statistics_freshness_gate(...)` 执行 Cost dependency-bound PostgreSQL gate。Cost parent gate 返回精确 `child_refresh_scope_keys`，query 只 enqueue 这些 child；只有 child 均 fresh但 parent lineage 落后时才 enqueue parent。禁止访问 Redis/full rows或把 parent 自身旧 `done` 当作 child freshness。只有两级 gate 均 fresh且 parent `source_shards` 等于当前 concrete shards 后，才委托 `ReadModelQueryGateway` 处理 Redis/payload，禁止同步 rebuild。
...
- Upstream read model 输入：全月月份 shard 消费 Workbench active generation；当前访问必须先证明该 active generation 已追上 canonical expected versions。现存 delta worker 只按其显式运维/历史事件 row IDs 点查 active generation，不是普通 relation 写入路径。父 scope 只读取月份 metadata，并以两次 SQL aggregate 生成小型 metadata，不加载月份业务 rows、Workbench `all`、历史 generation 或 child JSON arrays。
- Relation identity：普通访问收敛使用 Workbench active generation 中的正式 `case:<case_id>` identity。现存 delta worker 若由显式运维/历史事件调用，其 `case_id` 也必须映射同一 identity；禁止裸 `case_id`、双 identity 或放宽 Audit。
