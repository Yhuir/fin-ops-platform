...
- 成本统计标签规则是本模块唯一写入口；route 保持 thin owner，只委托 `AppSettingsService` 持久化 canonical selection/version/audit，不返回 `freshness_targets` / `operation_barrier_targets`。页面保存成功后清除当前 explorer/export 派生状态、递增当前 query nonce 并关闭抽屉；规则在 query/export 层过滤，因此不写 dirty scope、不等待 worker，也不触发全量 read model rebuild。relation 等真实投影 source mutation 仍只等待当前 exact cost scope。
