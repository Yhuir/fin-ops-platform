...
- Miss/stale owner：`CostStatisticsQueryService` 对 `project|bank|expense_type` 先比较 canonical Workbench expected versions 与 active generation；不匹配时只 ensure exact Workbench并返回 refreshing。页面下一次有界自动轮询在依赖匹配后再比较 Bank Detail 与 Cost child/parent，并只 enqueue当前 exact Cost child；月份使用单 scope proof，parent 使用 set-based scope 映射。`time|bank_tag` 直接进入 Bank Detail profile，只为 non-fresh Bank Detail exact month入队，不入队 Workbench 或 Cost。repository gate可同时返回独立 global statistics 状态；statistics drift 只置空 statistics并精确 ensure其自身依赖，不能阻塞当前视图 rows。
...
- 成本 projection 内的 bank-detail source version、transaction tags、month rows 必须由一次 `snapshot_for_month(...)` 快照读取共同提供；旧 `source_versions_for_scope_keys(...) + get_by_transaction_ids(...) + list_by_month(...)` 三段读取链路已删除。快照在 repository 内使用 `REPEATABLE READ READ ONLY`，同时覆盖目标月 rows 与正式关系引用的跨月流水 ID；projection 只做纯内存拆分和归一化。访问入口的 Workbench 依赖 enqueue owner 是 `CostStatisticsQueryService`；它在 Workbench stale 时不得同时投递 Cost。只有已运行 Cost job 遇到其它 non-fresh 依赖时，才由 `RuntimeWorker._enqueue_dependency_refreshes(...)` 精确补投。
