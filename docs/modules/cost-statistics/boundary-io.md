...
| 关系变更 | canonical relation source version + Cost query owner | relation transaction 和 Workbench generation publish 都不投递 Cost。`project`、`bank`、`oa_expense` 与 `time` 继续按其真实输入执行既有 Workbench/Bank Detail/Cost 两阶段证明。`bank_tag` 页面明细只消费已物化 `cost_statistics_bank_flow_rows`、Bank Detail 语义 proof 与成本 settings，不消费 Workbench/OA allocation；因此该 view 跳过 Workbench pre-gate，并使用独立 `bank_flow_refresh_status`、Bank Detail exact scopes 与 Cost bank-flow child scopes。Workbench dirty 不得阻塞 `bank_tag` rows，也不得因此 enqueue Workbench；标题全局统计仍保留完整 parent lineage，独立返回 `statistics=null/refreshing`，不能冒充与 rows 同一 freshness。所有 view 的 relation transaction 与 Workbench publish 均保持零 Cost fan-out。 |
...
| Source versions | read model/query gateway | 月份 scope 的完整业务 `source_versions` 包含当前 Workbench active generation 的完整 `workbench_source_versions`、`bank_detail_source_versions`、`bank_auto_tag_rules_version` 和 `bank_account_mappings_fingerprint`。projection 与 query 共用纯 helper；query 的当前依赖来自同一 gate SQL 的 settings/Workbench/Bank Detail snapshot，禁止 Application/runtime 逐个回调 owner。Bank Detail 的 `source_version` 是 worker 执行计数，嵌套 `workbench_relation_source_versions` 是 Bank Detail 自身 lineage；Cost 不消费这两个 provenance 字段。完整 Cost fresh gate/cache/Audit 只排除这两个已知键，仍精确比较 `bank_detail_source_signature`、row count、schema、标签规则及任何未知新增字段。`bank_tag` row profile 额外通过 `cost_statistics_bank_flow_source_versions(...)` 从同一完整 vector 只移除它不消费的 Workbench/OA allocation provenance；Bank Detail business signature、row count、schema、标签规则、银行账户映射与未知 Bank Detail 语义键必须保留并精确比较。runtime dirty `source_version` 只作为发布竞态与 cache namespace token，不得混入业务 `source_versions`。 |
...
- Miss/stale owner：`CostStatisticsQueryService` 对完整 lineage view 先比较 canonical Workbench expected versions 与 active Workbench versions；月份使用单 scope proof，parent 使用 set-based scope 映射。`bank_tag` row profile 明确跳过这一步，直接读取同一 cost-owned SQL gate 的独立 bank-flow proof。`get_cost_statistics_freshness_gate(...)` 一次返回完整 Cost status、bank-flow row status、标题 statistics status，以及精确 Workbench、Bank Detail、Cost child scope；query 只为当前 view 实际消费的 mismatch 入队。旧 broad active-dependency query 已删除：系统中不相关 scope 的 active queue event 不得阻塞 Cost。`bank_tag` rows 只有 Bank Detail/bank-flow Cost proof fresh 才允许 Redis/payload I/O；全局 statistics 可以独立 non-fresh，且不得反向阻塞 rows。
...
- 读取边界：`get_cost_statistics_freshness_gate(...)` 用单条 SQL 读取成本 metadata/current dirty、成本所需 App Settings 片段、当前 concrete month 的 Workbench/Bank Detail proof，并同时计算完整 Cost、bank-flow rows 与标题 statistics 三组独立状态/精确 refresh scopes。完整 lineage view 继续校验 active Workbench month、Cost child、Bank Detail 与 latest dirty；`bank_tag` row profile 只校验 Bank Detail 与 bank-flow Cost child，忽略无关 Workbench dirty。标题 statistics 始终保留 parent 全 lineage proof，但只影响 `statistics` 字段。`0122_cost_statistics_access_convergence_hot_paths.sql` 只为该证明补充四个窄索引，不改变 freshness 语义。页面 explorer 在当前 view gate 之后才允许 ETag、cost-local versioned Redis 和 `get_cost_statistics_page(...)`；cache miss 以一个 set-based SQL 返回 summary/facets/row count/`page_size+1` rows。cursor/ETag/cache key 绑定 published version、当前 view 业务 source versions、tag token 与规范 query。bulk/detail/export 继续使用完整 lineage gate，不机械继承 `bank_tag` row-only profile。
...
- 性能边界：首屏 API/read model 只能走 PostgreSQL gate + cost-local fresh Redis/page SQL；API miss 不同步扫描 Workbench/live service。`bank_tag` rows 不等待 Workbench full lineage，标题 statistics 单独收敛；生产 test-owned fixture 必须证明写快速返回、写后零 fan-out、首次访问只触发必要 scope 且 rows 在 3 秒内 fresh。前端不得恢复 TTL payload cache、full DTO、浏览器全量聚合或首屏 `active:all`。项目/费用类型导出筛选仅在用户动作后并行请求两个 bounded all-scope facets；time/bank-tag 不发该 I/O。当前 view-specific cursor 已本地闭环；在生产 SLO 证据前不得宣称通过，也不得在无 EXPLAIN/证据时增加索引或修改共享 pool。
