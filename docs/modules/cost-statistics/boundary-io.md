...
- 性能边界：首屏 API/read model 只能走 SQL read model + Redis fresh cache；API miss 不同步扫描 Workbench/live service。当前生产 HTTP SLO 目标继续按 page shell、explorer、summary p95 <= 1000ms 验收，worker direct refresh 保持父 scope 有界聚合。
