...
- v11 分片完成后，Audit 仅剩多 OA `expense_content` 拼接顺序差异：Python projection 使用 Unicode 确定性排序，PostgreSQL expected-set 曾继承数据库 locale 排序。Audit 改为 `C` collation，与 projection 的字节/码点顺序一致，避免集合相同但顺序不同的假阳性；业务金额和归属不变。
...
- 最终发布：精确 SHA `aa53af9a0c639d844e537c5d7bc0bbcc600b3918` 已部署为 `main-aa53af9a-20260718061400`；readiness 报告 release runtime 一致且 `status=ready`。受控 lifecycle all refresh 的旧 processing event 在既有 300 秒 lease 后被自动回收并正常 drain，没有手工改生产表、清队列或跳过 freshness gate。当前 `invoice_lifecycle` 与 `cost_statistics` 均为 `available`、`stale_count=0`、`unavailable_count=0`，outbox `pending/publishing/failed/publish_failed` 全为 0；最近 15 分钟 Cost refresh 为 `p50=94.875ms / p95=298.642ms / p99=409.644ms`。
- 最终读取性能：生产公网对 Cost explorer `active:all` 连续 30 次认证读取全部 `200/fresh`，`p50=141.108ms / p95=181.487ms / p99=max=234.867ms`，通过已确认的页面 `p95<=300ms / p99<=500ms` 门槛；业务基线为 `153` 行、总额 `4845657.940000`、银行流水 `128` 笔。
- 最终可逆写证明：真实 OA `oa-pay-2324`、银行流水 `txn_imported_0454`、发票 `inv_imported_0696` 的 withdraw 使 Cost 变为 `152` 行、总额 `4840657.940000`，业务 ACK 后 `376.194ms` 可见；confirm 恢复为 `153` 行、总额 `4845657.940000`，业务 ACK 后 `671.134ms` 可见。Cost 下游写后可见均通过 `<=3s` 门槛。同步 Workbench 写事务本身分别为 `5.674s` / `6.243s`，因此从请求开始到 Cost 可见分别为 `6.097s` / `6.914s`；该时间属于共享 Workbench 提交链，不冒充 Cost worker 延迟，也不在本模块叠加旁路。
- 最终正确性与恢复：基线、withdraw 后和 confirm 恢复后三轮，关联台、OA 待付款、成本统计 Page Audit 均为 `pass/fresh/drained`、`issues=0`、只读一致性 snapshot；恢复后 Workbench 正式 group identity 为 `case:decision:2026-06:bank_invoice_exact_amount:txn_imported_0454:inv_imported_0696`，OA 页面显示银行与发票 `relationCount=1`，Cost 恢复原业务基线。真实夹具最终保持 active/paired，不留下测试业务变更。
- 验证闭环：相关 backend/read-model/architecture 回归 `339 passed + 246 subtests`；专注 lifecycle/OA source 合同 `94 passed, 2 skipped`，snapshot 修正后补充集 `16 passed`；真实临时 PostgreSQL 应用全部 migrations 后 `3 passed`，临时库已删除；`lint`、docs、`git diff --check` 和标准 production build 通过。旧 lifecycle 页面 rows consumer 已从 worker 调用链删除，旧 pending-invoice canonical 重算、input/output live fallback 与错误双 identity 均无 production caller；页面 rows I/O 只因 OA 页面仍合法使用而保留，不作为 lifecycle fallback。至此本条生产闭环完成。
