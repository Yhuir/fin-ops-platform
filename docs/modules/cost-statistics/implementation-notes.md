...
- 有效人工单元金额作为列边界，各真实支出流水按原额分得的净支出作为行边界，policy 使用确定性整数分矩阵同时闭合行、列和总额。关系成员、OA 单元、账户、标签或金额变化都会改变 fingerprint，使旧记录 stale 并要求重新填写；不提供自动比例、首项默认、旧值沿用或任何 fallback。
