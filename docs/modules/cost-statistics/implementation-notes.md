...
- 354项后端unittest通过；125项pytest通过（包含源码函数式回归、真实PostgreSQL和migration；与unittest有重叠，不能相加视为独立测试）。全量97个前端文件、1,275项单元/组件测试通过，生产构建通过。跨页面39项浏览器回归首轮38通过，发现移动端组件默认负边距导致3px溢出；修正仅限Cost抽屉，来源4项复跑通过。随后成本页面13项浏览器复跑通过；新来源测试已加入原有e2e:smoke入口及成功页面错误检查，相关26项仓库检查通过。全仓后端首轮发现的迁移清单与测试登记问题已修正，现金兼容测试在其要求的专用隔离库5项通过；正式发布前继续复跑后端全量。
...
- 扩容测量：本机真实 PostgreSQL 6,230 个关联组、12,460 条银行流水及12,460个OA单元，service 单并发 p50/p95 为1,971/2,126ms；4并发为5,429/5,974ms；8并发为10,899/13,023ms。此项不是生产HTTP测量，说明当前直接聚合在较大数据量高并发下存在容量限制；不以缓存或旧数据伪装提速。实际生产耗时需在发布后单独测量。
