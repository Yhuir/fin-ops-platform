...
## 2026-08-20：分面长文本固定行高展开

- 项目、费用类型、银行账户、主标签和子标签继续使用同一个 `CostExplorerList`；行高固定为 66px 并预留稳定操作槽，文字默认省略，只有浏览器实测发生溢出时才显示 HeroUI `展开/折叠`。
- 完整文字放在轻量 Popover 中并允许选择、换行和滚动；同一列表只允许打开一个，选择条目、按 Escape、点外部或尺寸变化后不再溢出时关闭，避免内容覆盖相邻行或推高列表。
- 列表只接收纯文字 `getPrimaryText`，删除旧 `renderPrimary` 渲染入口；ResizeObserver 按列表复用并用 animation frame 合并测量，不增加逐行 observer、API、依赖、数据库或跨页面 I/O。
