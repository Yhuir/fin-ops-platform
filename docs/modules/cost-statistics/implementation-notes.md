...
- 五个视图页头统一只展示银行流水、支出流水和收入流水。三个项目成本 view 直接复用同一 canonical snapshot 已加载的银行行计算方向数量，不新增 API、SQL、缓存、read model 或 worker。
...
> 本节是当前视图、金额与账户归属的唯一有效决策。下文仅作为演进历史；不得据此恢复原始`按银行`、旧 time-tag 规则或兼容 payload。
