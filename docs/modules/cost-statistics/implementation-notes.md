...
## 2026-08-31：三个项目成本视图与银行账户归属

> 本节是当前视图与账户归属的唯一有效决策，取代下文所有“五视图”“成本归因/银行流水分组”“原始按银行/按标签/按时间”和 `time-tag-rules` 描述。下文仅作为演进历史，不能恢复旧运行时路径。

- 页面只保留一个`项目成本`分组，内部为`按项目`、`按费用类型`、`按银行账户`。没有新增“OA 配对”等术语；用户已经认可的`按银行账户`名称保持不变。
...
- 继续复用现有 canonical repository、policy、query service、分页和导出边界；不新增层、表、worker、cache 或依赖。本次无 schema/data migration、无数据库备份。
