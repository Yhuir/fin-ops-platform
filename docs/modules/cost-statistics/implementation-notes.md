...
- 应用提交 `0f9dc2bee468cb27e5d02fcd02fee04c6c2aa000` 已推送 remote main，并通过正式入口部署为 `main-0f9dc2be-20260909040727`。迁移0169成功（27ms），pre/T+0/T+30均PASS，没有回滚；API和4个登记worker正常，pending/failed outbox与dead-letter均0，页面canonical审计通过。
...
- 生产全部期间总成本仍为12,226,597.09元；拆来源后563条成本展示行。三个成本视角、8/9月分别一致，原银行两视角收支一致。跨视角抽查15次明细金额/来源/账户/标签/日期与银行证据一致；列表分页无重复，聚合导出包含成本来源明细14列。缺项目参数的探测请求按合同返回400，补入真实所选项目后预览和xlsx成功；未改接口绕过输入校验。空分配PUT为400，前后version和分配内容未变，未代替用户填写真实生产分配。
...
- 后续小范围修正删除Cost分支里已不可达的旧按月候选查询和重复来源ID处理；完全自动完成、无持久化决定的只读项不再生成未被消费的fingerprint。所有待分配项、已保存决定、实际写入仍保留原fingerprint/版本/事务/锁与审计。后端89项PG16回归通过；继续推送、部署这一修正并复测。
