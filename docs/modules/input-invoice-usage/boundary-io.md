...
| 页面 Audit icon | AppHealth operations audit API | admin-only；标题右侧 icon 调用只读 audit；只有结构化 integrity/freshness 通过且页面明确 `read_model_status=fresh` 才显示成功，unknown 不得伪装 fresh，问题数显示为 sample |
