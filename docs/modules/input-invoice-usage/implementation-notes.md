...
## 2026-08-25 - 支付状态 chip 颜色语义

- 表格按已有 canonical `paymentStatus.code` 选择颜色，不按展示文案猜测：已付款为绿色、待付款为橙色、待处理为中性色；现金往来和冲销类状态为信息色。
- 删除所有支付状态统一 hard-code 为 warning 的旧展示逻辑；API、规则计算、数据库、read model、worker 和请求数量均不变。
- Vitest 覆盖三个主要状态的独立 class；发布后用生产浏览器检查实际颜色和页面链路。
