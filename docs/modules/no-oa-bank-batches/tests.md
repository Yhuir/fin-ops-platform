...
> Legacy: 新通用方向为 `docs/modules/bank-flow-rule-batches/tests.md`。本文保留迁移完成前 no-OA 旧模块测试入口；不要把这里的 `selected_tag_codes` 兼容测试作为新流水规则批量处理的验收标准。
...
## 2026-06-29 - no-OA 标签规则 grid 与关联台 paired 要求
