...
| `NO-OA-E2E-002` | `covered` | `web/e2e/bank-flow-rule-batches-flow.spec.ts`、`web/src/test/NoOaBankBatchPage.test.tsx`、`web/src/test/NoOaBankBatchApi.test.ts`、`tests/test_no_oa_bank_batch_tag_selection_api.py`、`tests/test_app_settings_service.py` | Browser 覆盖标签 drawer 保存和 `no_oa_bank_batch:all` operation barrier；组件/API/后端覆盖紧凑 grid、只读银行标签列、`rules` 保存 body、兼容 `selected_tag_codes` 派生、版本冲突、inactive selected cleanup、新增银行标签默认要求 OA/发票，以及勾选发票时不生成免 OA 未提交候选。 |
...
| `NO-OA-E2E-004` | `covered` | `web/e2e/bank-flow-rule-batches-flow.spec.ts`、`web/e2e/cost-statistics-flow.spec.ts`、`tests/test_derived_data_lifecycle_service.py` | Browser 覆盖 no-OA submit 后进入成本统计，等待 `/api/cost-statistics/explorer` fresh，展示 `免OA手续费成本项目`、`手续费`、`8.80`、`网银手续费` 和银行证据，并检查下游成功展示后无可见错误残留。真实 worker drain 仍归 `NO-OA-E2E-010`。 |
| `NO-OA-E2E-005` | `covered` | `web/e2e/bank-flow-rule-batches-flow.spec.ts`、`web/src/test/NoOaBankBatchPage.test.tsx`、`tests/test_no_oa_bank_batch_api.py` | Browser 覆盖已提交 bucket、撤回 dialog、撤回原因、`expected_version` 请求体、撤回反馈、历史 bucket 只读、零提交/撤回按钮和成功后无可见错误残留。 |
| `NO-OA-E2E-006` | `covered` | `web/e2e/permissions-role-matrix.spec.ts`、`web/src/test/NoOaBankBatchPage.test.tsx`、后端 auth/API tests | Browser role matrix 覆盖 `read_export_only` 可查看但无提交/撤回按钮，tag drawer OA/发票复选框与保存禁用，且 role matrix mutation call list 为空。 |
| `NO-OA-E2E-007` | `covered` | `tests/test_no_oa_bank_batch_workbench_integration.py`、`tests/test_no_oa_bank_batch_service.py`、`tests/test_workbench_pair_relation_service.py`、`tests/test_workbench_candidate_grouping.py`、`web/src/test/NoOaBankBatchPage.test.tsx` | 后端 integration 覆盖 Workbench internal transfer 走 no-OA batch、no-OA/Workbench 双入口复用同一 fact、混合 internal transfer 拒绝、withdraw 后 Workbench 回 open；候选分组覆盖 no-OA relation 勾选发票但缺发票时降回 open、补齐后进入 paired；组件覆盖 internal transfer draft batch endpoint，并确认 internal transfer conflict 不暴露在主列表。 |
