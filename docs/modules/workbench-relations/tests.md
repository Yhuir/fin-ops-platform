...
- `tests/test_workbench_auth_context_idempotency.py::WorkbenchAuthContextIdempotencyTests::test_withdraw_link_targets_preview_row_months_when_relation_scope_is_all`：withdraw 对历史 `month_scope=all` 的 relation 使用 preview rows 推导 affected month shards，返回 affected month shards。
- `web/src/test/WorkbenchSelection.test.tsx::workbench action never waits on global relation scope when action response lacks precise targets`：前端遇到旧/异常 action response 只带 `workbench_relation:all` 时不调用 全局 relation scope 等待，不再误报operation barrier超时。
...
- `tests/test_workbench_relation_command_service.py`：command service confirm/cancel/withdraw 基座、withdraw preview lock、row-id batch cancel、metadata update、canonical write safety、idempotency、mode registry 和 active row conflict。
...
- `tests/test_workbench_relation_read_facade.py`：`workbench_relation` distribution、linked/candidate/unlinked rows、正式发票和 OA 附件发票 identity 去重。
...
- `tests/test_batch_accounting_api.py`、`tests/test_platform_runtime_boundary_guards.py`、`web/src/test/BatchAccountingPage.test.tsx`：批量账务 relation distribution 同步诊断、submit/withdraw command service 委托、submit/withdraw/repair 缺 command fail-fast、direct pair fallback 禁止和 canonical write safety。
- `tests/test_no_oa_bank_batch_*`：no-OA submit/withdraw、internal transfer confirm-link、command service 写入委托、relation distribution 同步诊断、Workbench paired/open 收敛。
- `tests/test_turnover_*`：turnover manual closure/withdraw command service 委托、relation distribution 同步诊断、Application wiring guard 和 workbench pair relation 集成。
- `tests/test_pending_invoice_service.py`：待找发票 attach/create 幂等、relation detail 读 distribution、manual/attach relation 写入委托 command service、relation distribution 同步诊断。
...
- `web/e2e/workbench-relation-fanout.spec.ts`：真实 Chromium 中从银行明细候选关系标签进入关联台，执行 confirm preview/submit/direct refetch，再回银行明细验证 `有oa` / `有发票` 标签；共享 `confirmWorkbenchRelation` flow 和银行明细下游成功节点都使用 `expectNoUnexpectedSuccessUiErrors`，防止写入成功后页面仍显示“操作失败”、同步失败或同步超时。
- `web/e2e/bank-details-export-download.spec.ts`：真实 Chromium 中先执行 Workbench confirm，再回银行明细导出全部银行；断言请求携带当前筛选、浏览器产生 download event，文件内容包含 linked relation 字段，并检查成功后没有导出失败/同步失败残留。
- `web/e2e/pending-invoices-export-download.spec.ts`：真实 Chromium 中先执行 Workbench confirm，再回待找发票搜索目标对方户名并导出；断言 export-preview/export 携带当前筛选和排序、不带分页，浏览器产生 download event，文件内容包含 OA、进项发票和 linked relation 字段，并检查成功后没有导出失败/同步失败残留。
...
- `web/e2e/workbench-relations-nonfresh-diagnostics.spec.ts`：真实 Chromium 中验证 relation-backed 待找发票关系数据不可用时显示诊断；`refreshing` 保留已有行和选择发票入口，`stale` 空 rows 仍显示同步诊断并禁用导出。
...
- `web/e2e/input-invoice-relation-fanout.spec.ts`：真实 Chromium 中验证进项发票使用页面消费 relation distribution；candidate OA/流水证据只展示且支付状态保持 `待处理`，Workbench confirm 后重新进入页面显示 linked 证据和 `已支付`，OA reverse drawer 中 candidate/linked 均不可勾选；进项、OA pending、税金和成本下游成功节点都会检查无操作失败/同步失败残留。
- `web/e2e/workbench-relations-tax-offset-fanout.spec.ts`：真实 Chromium 中先进入税金抵扣页确认 relation 影响前无目标进项计划行，再从 Workbench confirm，回到税金抵扣页验证重新请求 `/api/tax-offset`、显示 relation 影响后的 direct 进项计划行且无同步错误，并检查无成功后的错误残留。
...
- `web/e2e/workbench-withdraw-flow.spec.ts`：真实 Chromium 中先建立 paired group，再从关联台自身执行 withdraw preview/submit；断言 submit 带回 `operation_type`、`preview_id`、`expected_versions`，弹窗内 busy 锁定，direct refetch 后恢复 open group，并检查成功后无操作失败/同步失败残留。
- `web/e2e/workbench-candidate-split-flow.spec.ts`：真实 Chromium 中从未配对自动候选点击任意 row，preview 判定 `split_candidate`，submit 后通过 operation projection 或 direct Workbench refetch 隐藏候选，不做 operation barrier轮询；该用例保护 automatic decision 不被误当作 active relation withdraw，不写 relation lifecycle，并检查成功后无错误残留。
- `web/e2e/workbench-exception-flow.spec.ts`：真实 Chromium 中覆盖异常处理 apply/cancel 和 ignore/unignore 的 direct refetch；每个成功节点都会检查无操作失败/同步失败残留。
...
- `web/e2e/batch-accounting-flow.spec.ts`：真实 Chromium 中从批量账务未提交 bucket 选择银行流水和 OA，submit 后 direct reload，再进入已提交 bucket 验证 relation 与 OA 明细；随后 withdraw direct reload 并恢复未提交状态。
- `web/e2e/turnover-ledger-flow.spec.ts`：真实 Chromium 中从外部往来款 grouped table 选择同组两条 flow rows，confirm manual closure 后 direct reload grouped ledger，再从 toolbar withdraw 并验证未闭环恢复。
...
- `PostgresWorkbenchRelationRepository` load/save/history/scope 行为等价。
...
- `repair_workbench_pair_relation_integrity` 的核心 repair plan 必须覆盖 active relation 旧 row id 清理、OA 附件发票明细项 `oa-exp-*:item:*` 回挂父 OA、以及 repair 后 `amount_check` 重算；当前行集合必须来自显式 direct row snapshot 或 canonical facts，不能再读取 active generation。
...
- pending invoice attach/create 已覆盖 application service command delegation、canonical write safety 和 API 旧 shape 回归；读侧 legacy distribution response shape 仍由 facade 测试保护。
- no-OA submit/withdraw 已覆盖 success、rollback、version conflict、command delegation、active row occupation、single-source case reuse，并通过 guard 保护 page read-model worker 不回流。
- turnover manual closure/withdraw 已覆盖 command service 委托、缺 command fail-fast、API wiring guard 和 Workbench 集成。
- batch accounting submit/withdraw 必须委托 command service，并覆盖缺 command service 时不得 direct pair fallback。
- batch accounting relation distribution unavailable 只能作为读侧诊断；普通 distribution 追赶不应全局禁用具备 canonical write safety 的 mutation。
- ETC repair/delete 可见入口；已提交业务批次删除必须在本地 reset 前通过 canonical relation command 取消包含 summary 的 relation，relation distribution 暂不可用不得阻断本地 reset。
- input invoice OA reverse evidence detected 写入：success、command service unavailable、relation distribution stale/conflict 409、no half-write。

每个写 API 至少覆盖 success、missing fields、illegal state、permission/actor mapping、version conflict、idempotent repeat、canonical write safety failure；不再为 mutation 增加 relation distribution readiness/enqueue response 合同。
...
- relation 写入后 downstream dirty/outbox 必须按 row domain 路由：银行明细只刷银行流水月份，发票生命周期/进项使用/销项收款/税抵扣只刷发票或 OA 相关月份，`workbench_relation` 保留跨域 broad scope；Search read-model refresh 已删除；`cost_statistics` 只由未知 row type、bank+OA、no-OA batch 或 turnover 成本关系触发，bank+invoice 不应刷新成本统计。旧数据缺少事实表月份时必须报告 unknown / direct fact gap，不能回退读取 `read_model.workbench_rows`。
- relation 写入后 downstream dirty/outbox 必须使用 `high` priority，避免用户写操作后的真实同步被普通后台刷新排队拖慢；无法从 relation/bank/invoice/OA 事实拿到月份时必须保持 direct failure/unknown 语义，不能查 legacy Workbench rows。
- Search SQL index/projection 已删除；不得恢复 `search.read_model.refresh` 作为写后同步门禁。
- `/api/search` 直接返回 `SearchService.search(...)` payload；pending invoice legacy projection 不定义 Search 页面 freshness 或 jump target 合同。
...
- downstream `bank_detail`、`pending_invoice`、`input_invoice_usage`、`output_invoice_collection`、`oa_pending_payment`、`no_oa_bank_batch`、`turnover_ledger`、`cost_statistics`、`tax_offset` scope 覆盖；Search read-model scope 不再适用。
...
- read facade missing/stale/source mismatch 返回暂不可用，不伪装空关系；但 scope 已可用时，`get_by_row_ids` 中个别请求 row 缺失应由调用方按无 relation/unlinked 处理，不能阻断整页读取。
...
- relation distribution unavailable 时页面展示同步诊断，不能把暂不可用空关系当真实空；普通 distribution 追赶不应全局禁用具备 canonical write safety 的无关操作。
...
- 在关联台 confirm/withdraw 后，bank detail、pending invoice、invoice usage/OA pending、tax offset 或 batch accounting 通过后端 direct API 看到同一 relation；当前 Browser e2e 已覆盖 bank detail relation tag fan-out、银行明细 linked relation 字段真实下载、进项发票使用 candidate/linked relation fan-out、税金抵扣页 relation 后重新读取 direct tax offset payload、关联台自身 withdraw preview lock/submit/direct reload/open recovery、automatic candidate split 防误 withdraw 和候选隐藏、pending invoice row status fan-out、batch accounting submit/withdraw 后 direct reload 并进入对应 bucket，以及 turnover manual closure confirm/withdraw 后 direct reload 并恢复 grouped payload。成功写流必须额外断言页面没有残留“操作失败”、同步失败或同步超时 文案，避免“关系已建立但用户看到错误弹窗”的假成功。
...
- turnover closure submit/withdraw 影响 workbench_relation、cost；Search 通过 direct `/api/search` payload 反映；必须覆盖 canonical write safety 下不产生半写入。当前 Browser e2e 已覆盖小样本 confirm/withdraw 的 direct reload 与页面恢复，复杂 OA-bank merge、cost 最终显示和 Search direct payload 仍由后端 integration 和后续目标 smoke 保护。
...
```bash
PYTHONPATH=backend/src python3 -m pytest tests/test_workbench_pair_relation_service.py tests/test_workbench_relation_read_facade.py tests/test_workbench_relation_read_facade.py -q
...
`bash scripts/verify.sh all` 会运行全量后端 unittest、前端 Vitest、前端 build、deterministic Playwright smoke 和 docs check。当前 Playwright smoke 覆盖 app shell / AppHealth / session permission gate，并覆盖关联台 confirm 后银行明细 relation tags、银行明细 linked relation 字段真实下载、candidate relation 在银行明细/待找发票/OA 待付款中只展示证据且不驱动 linked-only 状态、relation-backed 待找发票 unavailable 诊断、销项收款红蓝票 relation 写入后 rows refresh 和人工依据展示、进项发票使用 candidate/linked relation fan-out、税金抵扣 relation 后重新读取 direct tax offset payload、OA pending linked fan-out、关联台自身 withdraw preview lock/submit/direct reload/open recovery、自动候选 split 防误 withdraw 和候选隐藏、exception apply/cancel/ignore/unignore recovery、transient network retry、409 stale preview、confirm/split/withdraw duplicate-submit guard、待找发票行状态跨页面同步、批量账务 submit/withdraw -> `workbench_relation` direct reload -> bucket recovery，以及外部往来 manual closure confirm/withdraw -> direct reload -> grouped recovery；共享成功写流 guard 会在 confirm 主链路后检查页面无操作失败/同步失败残留，银行明细、待找发票、进项使用、成本统计、OA pending、税金抵扣下游 fan-out 以及 Workbench withdraw/split/exception/network-recovery 成功节点也会在目标业务结果出现后复查无错误残留。真实 download event 覆盖银行明细、待找发票、销项收款和进项使用的 relation 字段导出；后端 unittest 覆盖 Search refresh path 不回流、遗留 search projection group jump target 和 `/api/search` direct payload。更复杂真实基础设施同步仍主要由后端 integration、Vitest、运行时 SLO 工具和 staging smoke 保护。

## 未测风险

- 本地 deterministic Browser/API relation Spec IDs 已覆盖：confirm -> bank details relation tags、银行明细 relation 字段真实下载、银行明细账户/日期/关键字/分类筛选导出和 `read_export_only` 下载权限、candidate relation 不驱动 linked-only 状态、relation-backed pending invoice unavailable 诊断、销项收款红蓝票 relation 写入后 rows refresh 和人工依据展示、进项发票使用 candidate/linked relation fan-out、税金抵扣 relation 后重新读取 direct tax offset payload、OA pending linked fan-out、关联台自身 withdraw preview/submit -> open recovery、自动候选 split 防误 withdraw 和候选隐藏、网络失败重试、409 stale preview、confirm/split/withdraw 重复提交防护、confirm -> pending invoice row status、batch accounting submit/withdraw -> bucket recovery、turnover manual closure confirm/withdraw -> grouped recovery、银行明细/待找发票/销项收款/进项使用 relation 字段下载均已覆盖；search API/runtime fan-out 已覆盖但没有独立 Browser route。剩余风险为未来 Browser search UI、新 relation 撤销入口、真实网络抖动、真实 XLSX 完整解析、生产 display audit 和真实后台任务收敛。
- 真实 PostgreSQL/RabbitMQ/Redis/systemd 后台任务收敛、生产历史 relation 半迁移、大数据 active generation 回放和真实导出/滚动性能仍需 staging 或生产只读 smoke。
