...
| 输入 | 来源 | 合同 |
| --- | --- | --- |
| manual command | Workbench/业务 owner API | normalize 后至少 2 个不同 requested canonical row ids、actor、tenant、idempotency、expected versions、既有 `note`；每个 exact requested id 都必须重读为 `oa|bank|invoice`，成员可同栏或跨栏，金额/方向和材料完整性不作为人工创建门槛。`amount_check.requires_note=true` 时 `note` 必填并写 relation/history；浏览器为一次 preview/confirm 或 withdraw 意图生成稳定 key，并在 ambiguous 网络重试中复用，缺失时 API fail closed。重复 identity、canonical 缺失/未知类型、active overlap/version drift 和非法 summary 仍 fail closed。银行分类不是通用 command router：mixed 或全 `internal_transfer` 银行成员仍生成 `manual_confirmed` command，不得转交 no-OA batch owner。唯一窄分支是 OA + 完整 canonical external-turnover 零差额闭环，它复用 Turnover validator 并以 `turnover_manual_closure`、对应 evidence/history 写入同一 command/UoW；缺结构化证明不降级猜测。 |
| relation anomaly projection | Reconciliation Workbench query | `amount_check` 保持关系组级事实；`manual_confirmed` 关系的非空 `note` 只投影到 `workbench_anomaly.confirmation.note`，供统一圆形异常 Popover 展示。不得把 `amount_check` 或 `note` 复制到 OA/流水/发票 row DTO，也不得建立第二套行级异常状态。 |
...
| withdraw command | owner API | paired/unpaired active case identity、完整 member set、preview id、expected versions、reason；preview 与 submit 都要求请求的 canonical member set 和该 case 当前完整 active member set 精确相等，子集、超集、跨 case 混选统一返回 `workbench_relation_exact_selection_required`。submit 必须在 relation UoW 当前事务内一次完成 typed canonical selection 校验和所选完整行水合，并从这些同事务行构建 OA source alias map；不得在事务外重新扫描 OA、静默退化为 ID 自映射或用 first-wins 解决 alias 歧义。只允许关系级撤回；preview 与 submit 通过同一个 pair-service 计划按 canonical history 恢复上一可证明的稳定拓扑。历史 predecessor 可以沿用当前 active case identity，但只限最近 confirm history 的精确 `before_relations`，并在同一事务先取消当前 topology 后恢复；无关 case 复用、重复 case、owner 冲突仍 fail closed。不得从 row metadata/display group 或调用方 fallback 猜测 predecessor。 |
...
| persisted completion requirement | reconciliation-workbench | 普通 relation 必须同时含银行流水才可能进入 `paired`；OA+发票的 active immutable ownership 在缺银行时保持同 case但进入 `unpaired`，并返回 `missing_row_types=["bank"]`。含银行流水的普通 relation 创建时写 `requires_oa`、`requires_invoice`、tag codes 和规则版本；关联台据此判定其余缺项，缺失 fail closed。材料满足后，关系内全部 OA 仍须 `workflow_status=completed`；任一进行中 OA 返回 `blocking_reasons=["oa_in_progress"]` 并保持原 case 在 `unpaired`。只有显式 `relation_mode=batch_accounting` 保留已登记材料豁免，该豁免不能跳过 OA 审批完成条件；ETC batch identity 只证明汇总行的 canonical owner，不绕过 requirements 或实际成员类型。持久化分类事实变化继续原子更新既有关系；规则语义变化由 durable incremental job 更新命中关系并追加 history。下游 linked ownership 仍只由 active status 决定。 |
...
- confirm/withdraw preview 的 direct canonical selection 不是 relation fact source。preview route 可以用它构造 selection groups、金额与 OA alias；submit 必须丢弃该 DTO，并保留服务端 actor/tenant、preview identity、expected relation versions、canonical repository 与 UoW 原子写合同。withdraw submit 的 canonical repository 在同一事务内返回已校验的完整 typed rows，alias map 只从该结果严格构建；source alias 同时指向多个 canonical OA 时返回 `workbench_write_conflict / canonical_selection_ambiguous`，整笔零写。
...
- OA source snapshot 发现某 OA 已不在 completed 或 admitted 集合时，通过 `remove_rows_from_active_relations` 从 relation 中移除该成员；剩余至少两个成员且无 immutable attachment parent binding 时保留同 case并追加 history，否则取消 relation。该清理不得直接 SQL、不得创建新 case；由 source snapshot 自己登记 exact flow 删除事件，因此 relation delta 保存必须抑制已失效 OA row-id 的普通支付状态重算，避免旧 handler 因找不到 canonical OA 反复失败。
- 任意 `N:M:K` member set 都合法，只要上游业务规则已证明安全并且成员非空、唯一、typed。
- 每次 typed relation save 若成员含 OA 与 bank（包括 active、withdrawn、restore topology），repository 必须在同一 PostgreSQL 事务登记 `oa.payment_status.reconcile`。handler 只以最新 active canonical outflow 为支付证据；relation 金额异常不阻断事件，relation writer 不直接访问 OA MySQL。
...
- requirement recalculation 必须先由 repository 集合查询按 active `status` 与正式 `relation_mode` 白名单选取关系，再从 relation member 对应 canonical 银行流水 `txn_month` 得到一个或多个精确月份；`candidate:` / `temp:` 等仍处于候选 mode 的记录不会进入集合，但 `decision:` case id 若已转为正式 mode 必须进入。随后完整预验证本 job 命中的全部 active 正式 relation；任一正式关系缺 case、canonical 银行月份、持久化 tag proof 或当前 tag rule 时整批零写并失败。关系 payload 的历史 `month_scope` 不得替代 canonical 月份证明。写入保留 case id、成员和金额事实，只替换 requirement metadata（包括目标规则版本），追加 `bank_relation_requirement_recalculated` history。重放同一 job 必须零新增写；只刷新实际写入关系的精确 `workbench_relation` 月份并标记相同 matching dirty scopes，禁止 page Workbench event 与 `all` fan-out。替代失败 rollout 时，旧任务必须在新任务成功后才 supersede；新任务失败时两条失败证据都保留为 attention。
...
- 来源保护统一覆盖 OA 附件与 ETC。撤回银行保留来源组，纯来源组普通撤回拒绝；人工拒绝原银行的 fingerprint 沿用现有保护。进行中 OA 可参与关联和同列对齐，但仍阻止整组进入已配对；未知/无效状态与资料缺失按原合同处理，成本规则不变。
...
- 匹配 fact 只从当前有效来源读取 OA owner；历史导入 payload 中残留的 OA 引用不能参与竞争。
- 先形成 OA/ETC 来源关系，再计算银行扩展，银行歧义不阻断确定的 OA+发票关系。后来确定的 OA 来源与原组冲突时，在原有 UoW 内校验 fact version/当前 case，移出发票、清理旧 binding metadata、加入正确组并保存历史；失败整体回滚，其他银行/OA成员保留。
- 不增加 worker、缓存、read model 或 GET 副作用。沿用现有 dirty-scope 并批量处理。

## 2026-09-21 补充凭证金额与关系隔离
