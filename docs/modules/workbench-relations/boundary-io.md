...
| requirement recalculation event | settings-maintenance worker | 规则保存事务产生的 job id、owner、目标版本和真实变化 tag codes；只读取 tag proof 命中的 active 正式 relation，并以完整 tag set 和当前规则重算。`candidate:` / `decision:` / `temp:` 历史候选标识不属于正式关系写事实，必须由集合查询排除并交给 matching/read model 按当前规则重建。上线收敛重试可携带一个失败旧 job id，只有替代任务完成关系写入和精确 refresh enqueue 后才把旧任务标记 superseded。 |
...
- requirement recalculation 必须先由 repository 集合查询排除 `candidate:` / `decision:` / `temp:` 非正式标识，再完整预验证本 job 命中的全部 active 正式 relation；任一正式关系缺 case、精确月份、持久化 tag proof 或当前 tag rule 时整批零写并失败。写入保留 case id、成员和金额事实，只替换 requirement metadata（包括目标规则版本），追加 `bank_relation_requirement_recalculated` history。重放同一 job 必须零新增写；只刷新实际写入关系的精确 `workbench` / `workbench_relation` 月份并标记相同 matching dirty scopes，禁止 `all` fan-out。替代失败 rollout 时，旧任务必须在新任务成功后才 supersede；新任务失败时两条失败证据都保留为 attention。
