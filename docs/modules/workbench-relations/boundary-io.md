...
| OA 附件发票不可变 binding | `WorkbenchPairRelationService`、`WorkbenchRelationCommandService`、`oa_attachment_invoice_linking` | `oa-att-inv-<oa-row-id>*` 可证明来自父 OA 的发票必须和该 OA 处于同一个 active relation。该 relation 是 source binding，不是用户配对结果；不能被普通撤回拆成 OA standalone + 发票 standalone。 |
...
- OA 附件发票 binding 合同：pair service 必须在 withdraw preview/submit 的状态转换中维护父 OA+自带附件发票 active relation；command service 必须把纯 OA+自带附件发票撤回变成不可提交 preview 和业务错误，防止 API 绕过前端禁用按钮。该规则不允许通过 `existing_case`、row payload `case_id` 或未标记 history 恢复任意旧关系。
