...
- 禁止只把 OA+附件发票作为 `oa_attachment_source_relation` display-only candidate 展示；可证明 source binding 必须写入 relation command 边界。
...
- `WorkbenchActionApiRoutes` 已拥有 exception preview/apply、confirm-link preview/submit、mark-exception、cancel-link、withdraw-link submit、cash special、update-bank-exception、OA-bank exception、personal advance repayment、cancel-exception、ignore-row 和 unignore-row 的现代 action delegation。
- `LegacyWorkbenchActionRoutes` 仍是旧 `/workbench/actions/*` compat-only owner，不引入现代 write facade、relation command 或 read model refresh 边界。
- 仍有一个现代 action preview 残留在 `Application`：`_handle_api_workbench_withdraw_link_preview(...)` 直接调用 `WorkbenchWriteFacade.preview_withdraw_link(...)`。
- 下一条边界选择为 `server-py:workbench-withdraw-link-preview-route-owner-extraction`。
...
## 2026-06-24 - Workbench unignore-row route owner extraction

目标：把现代 `/api/workbench/actions/unignore-row` 的 facade delegation 从 `server.py` 抽到 `WorkbenchActionApiRoutes`。

变更：

- `WorkbenchActionApiRoutes` 新增 `unignore_row(...)`。
- `WorkbenchActionApiRoutes.unignore_row(...)` 现在拥有 `WorkbenchWriteFacade.unignore_row(...)` 调用。
- `Application._handle_api_workbench_unignore_row(...)` 仍负责 JSON body parse 和 Workbench 写入 freshness guard。
- `Application._handle_workbench_unignore_row_payload(...)` 仍负责测试/内部 helper 的 response mapping，并通过 route owner 调用 facade。
- 新增静态 guard，确保 unignore-row 不会重新直接调用 `WorkbenchWriteFacade.unignore_row(...)`，同时确认 JSON/freshness/response mapping 仍保留。
- legacy quarantine guard 不再保留任何现代 Workbench action direct facade helper 例外。

未改变：

- exception preview/apply、confirm-link preview/submit、mark-exception、cancel-link、withdraw-link、cash special、update-bank-exception、OA-bank exception、personal advance repayment、cancel-exception 和 ignore-row 在本 slice 未触碰。
- response shape、status code、auth、freshness guard、idempotency、unignore row 规则、exception case 写入、operation barrier、read model refresh 和前端行为未改变。

下一条边界：`server-py:modern-workbench-action-route-owner-post-extraction-audit`。
...
## 2026-06-24 - Workbench ignore-row route owner extraction

目标：把现代 `/api/workbench/actions/ignore-row` 的 facade delegation 从 `server.py` 抽到 `WorkbenchActionApiRoutes`。

变更：

- `WorkbenchActionApiRoutes` 新增 `ignore_row(...)`。
- `WorkbenchActionApiRoutes.ignore_row(...)` 现在拥有 `WorkbenchWriteFacade.ignore_row(...)` 调用。
- `Application._handle_api_workbench_ignore_row(...)` 仍负责 JSON body parse 和 Workbench 写入 freshness guard。
- `Application._handle_workbench_ignore_row_payload(...)` 仍负责测试/内部 helper 的 response mapping，并通过 route owner 调用 facade。
- 新增静态 guard，确保 ignore-row 不会重新直接调用 `WorkbenchWriteFacade.ignore_row(...)`，同时确认 JSON/freshness/response mapping 仍保留。

未改变：

- unignore-row、exception preview/apply、confirm-link preview/submit、mark-exception、cancel-link、withdraw-link、cash special、update-bank-exception、OA-bank exception、personal advance repayment 和 cancel-exception 都未迁移。
- response shape、status code、auth、freshness guard、idempotency、ignore row 规则、exception case 写入、operation barrier、read model refresh 和前端行为未改变。

下一条边界：`server-py:workbench-unignore-row-route-owner-extraction`。
...
- `WorkbenchActionApiRoutes` 新增 `update_bank_exception(...)`。
- `WorkbenchActionApiRoutes.update_bank_exception(...)` 现在拥有 `WorkbenchWriteFacade.update_bank_exception(...)` 调用。
- `Application._handle_api_workbench_update_bank_exception(...)` 仍负责 JSON body parse、Workbench 写入 freshness guard 和 `_workbench_write_response(...)`。
- `Application._handle_live_workbench_update_bank_exception(...)` 仍负责测试/内部 live helper 的 response mapping，并通过 route owner 调用 facade。
- 新增静态 guard，确保 update-bank-exception 不会重新直接调用 `WorkbenchWriteFacade.update_bank_exception(...)`，同时确认 JSON/freshness/response mapping 仍保留。

未改变：

- OA-bank exception、personal advance、cancel exception、ignore/unignore、exception preview/apply、confirm-link preview/submit、mark-exception、cancel-link、withdraw-link 和 cash special 都未迁移。
- response shape、status code、auth、freshness guard、idempotency、exception case 写入、operation barrier、read model refresh 和前端行为未改变。

下一条边界：`server-py:workbench-oa-bank-exception-route-owner-extraction`。
...
- `WorkbenchActionApiRoutes` 新增 `cancel_link(...)`。
- `WorkbenchActionApiRoutes.cancel_link(...)` 现在拥有 `WorkbenchWriteFacade.cancel_link(...)` 调用和 `request_id` / `actor_id` / `tenant_id` 传递。
- `Application._handle_api_workbench_cancel_link(...)` 仍负责 JSON body parse、Workbench 写入 freshness guard、auth context 和 request timing。
- `Application._handle_live_workbench_cancel_link(...)` 仍负责 `_workbench_write_response(...)`。
- 新增静态 guard，确保 cancel-link 不会重新直接调用 `WorkbenchWriteFacade.cancel_link(...)`，同时确认 freshness/auth/request-id/response mapping 仍保留。

未改变：

- withdraw、cash special、bank exception、OA-bank exception、personal advance、cancel exception、ignore/unignore、exception preview/apply、confirm-link preview/submit、mark-exception 都未迁移。
- response shape、status code、auth、freshness guard、request timing、idempotency、relation 写入、operation barrier、read model refresh 和前端行为未改变。

下一条边界：`server-py:workbench-withdraw-link-route-owner-extraction`。
...
## 2026-06-24 - Workbench mark-exception route owner extraction

目标：把现代 `/api/workbench/actions/mark-exception` 的 facade delegation 从 `server.py` 抽到 `WorkbenchActionApiRoutes`。

变更：

- `WorkbenchActionApiRoutes` 新增 `mark_exception(...)`。
- `WorkbenchActionApiRoutes.mark_exception(...)` 现在拥有 `WorkbenchWriteFacade.mark_exception(...)` 调用。
- `Application._handle_api_workbench_mark_exception(...)` 仍负责 JSON body parse 和 Workbench 写入 freshness guard。
- `Application._handle_live_workbench_mark_exception(...)` 仍负责 `_workbench_write_response(...)`。
- 新增静态 guard，确保 mark-exception 不会重新直接调用 `WorkbenchWriteFacade.mark_exception(...)`，同时确认 freshness guard 和 response mapping 仍保留。

未改变：

- cancel/withdraw、cash special、bank exception、OA-bank exception、personal advance、cancel exception、ignore/unignore、exception preview/apply、confirm-link preview/submit 都未迁移。
- response shape、status code、auth、freshness guard、request timing、idempotency、relation 写入、operation barrier、read model refresh 和前端行为未改变。

下一条边界：`server-py:workbench-cancel-link-route-owner-extraction`。
...
## 2026-06-24 - server OA attachment repair relation read port extraction

目标：把 OA 附件上下文 repair 中的 active relation 读取迁移到显式 read port，保留 replace-existing command-service repair 写路径。

变更：

- 新增 `WorkbenchOaAttachmentRepairRelationReadPort`。
- `Application._workbench_oa_attachment_repair_relation_read_port(...)` 使用 `WorkbenchRelationCommandService(require_fresh_relations=False)` 构造 port。
- `_repair_active_relations_with_oa_attachment_context(...)` 通过 port 读取 active relations。
- 专项提现过滤、缺失附件 invoice 检测、before relation payload、`confirm_relation(..., replace_existing=True)`、changed case ids、changed scope keys、derived lifecycle event 和 persistence scheduling 不变。
- 新增静态 guard，防止该方法回退到 broad pair service direct read。

未闭环：

- confirm-link context expansion 和 auto-pair conflict precondition reads 仍需单独迁移。
- transaction-persist、rollback、case-id allocation、whole-state persistence snapshot surfaces 保持单独后续 slice。
- `workbench_relation` 模块仍未完整闭环，Go/Fiber/Go Worker 仍阻塞。
...
- 首次生产只读预览发现原始 OA 明细中的 `expense_item_id` 与规范化后的 `id` 形状差异。自动服务仍先锁定原始来源，但规则只消费既有 canonical hydration 输出；删除原始明细覆盖 canonical 明细的步骤。跨月/并发/回滚 PostgreSQL 样例改用真实来源字段形状，验证通过。
