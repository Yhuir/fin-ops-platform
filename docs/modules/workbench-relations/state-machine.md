...
没有 active case 时 fail fast；未配对 singleton 不能撤回。撤回以整条 active case 为边界；preview 和 submit 的 canonical member set 都必须与当前完整 active member set 精确相等，子集、超集或混入其它 case 成员都不得由后端自动扩张。submit 的 typed member 校验、完整 canonical 行水合和 OA source alias 解析必须发生在同一个 relation UoW 事务中；preview DTO 与事务外 live-row 扫描都不能成为提交 alias 事实源。alias 多义时返回 `canonical_selection_ambiguous` 并零写。系统读取最近一次 confirm history 的 `before_relations`，preview 与 submit 共用 pair service 的恢复计划和 case/owner 校验，并在同一事务的 current/restored case 与 canonical member locks 内重载和重验：目标 topology 未漂移、canonical typed member 仍存在、每个成员只有唯一 active owner。predecessor 可以沿用当前 active case identity，因为当前 topology 会在同一原子操作中先取消再恢复；这一例外只适用于最近 confirm history 中精确的 `before_relations`，无关 active/cancelled case 的复用、重复 predecessor case 或成员 owner 冲突仍然零写。没有可证明 predecessor 时成员才成为 unlinked singleton；任一恢复冲突整笔 fail closed。不得根据 row `case_id`、display metadata、非稳定 display ownership、提交阶段特有逻辑或隐藏 fallback 猜测恢复；同一 member set 不得伪装为恢复。OA 附件 source binding 的不可拆规则继续由 pair service/command service 显式处理。
