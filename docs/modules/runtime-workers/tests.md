...
| Worker idle poll | `RuntimeWorkerConfig`、`app/worker.py`、deploy env examples | PostgreSQL durable queue worker 默认 idle poll 必须保持 `0.25s`；required read model worker env 不得重新固定历史 `2s/5s` 慢轮询 |
