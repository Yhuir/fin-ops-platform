...
| 2026-06-14 | dependency-not-fresh 补投依赖时，如果依赖 dirty scope 已经 pending/processing，会 bump 新 source_version 并把等待者继续推迟。 | `tests/test_runtime_worker.py::RuntimeWorkerTests::test_run_once_does_not_bump_dependency_refresh_when_scope_already_active`、`tests/test_runtime_queue.py::RuntimeQueueRepositoryTests::test_read_model_refresh_is_active_checks_pending_or_processing_dirty_scope` | covered |
