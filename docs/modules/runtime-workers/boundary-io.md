...
| Active refresh state | PostgreSQL outbox + dirty scope | gateway 的 ensure/wakeup coalescing 必须查询 exact `job.outbox_events` `pending/processing`，只有活跃事件才证明 worker 会继续推进。dirty scope 的 `pending/processing/failed` 仍阻断 freshness，但若没有活跃 outbox event 就属于可恢复 orphan，下一次访问必须允许重新 enqueue；否则成本 parent 等返回 `refreshing` 的 handler 会留下永久 pending。canonical mutation、显式 repair/reapply 与 force 不走此 ensure 合并边界 |
