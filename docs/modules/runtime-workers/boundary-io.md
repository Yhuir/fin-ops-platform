...
| Active refresh state | PostgreSQL dirty scope | gateway 的 ensure/wakeup coalescing 必须查询 exact `job.read_model_dirty_scopes` `pending/processing`，因为 outbox event 可在 projection 完成前进入 terminal。worker 完成 dirty scope 前，页面轮询不得再次 bump 同 scope source version；canonical mutation、显式 repair/reapply 与 force 不走此 ensure 合并边界 |
