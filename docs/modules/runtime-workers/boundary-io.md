...
| Workbench dependent publish | `WorkbenchReadModelRefreshService` | Workbench 月分片 projection commit 只原子发布自身 generation并完成自身 dirty scope，不 enqueue `cost_statistics` 或任何其他页面。多个月份的 payload 计算和 generation staging/COPY可并行；重型数据写完后，短锁段只切换本次月份 active generation，不再在发布事务内同步扫描全部 active 月份或写 all-scope statistics。`month=all` 的 summary/counts 由 query repository 在 repeatable-read 页面快照中使用现有 canonical set-based SQL 计算，并在返回前复核 generation-set version。Cost消费者访问发现 Workbench stale时只 ensure exact Workbench；后续访问在 Workbench fresh 后才由现有 Cost gate enqueue exact child，child完成后收敛parent。Cost worker在任何 payload I/O 前仍以 manifest dependency fail closed处理真实竞态，依赖 fresh 后才发布。禁止恢复 known-stale dependency 下的 Cost预投递、`workbench_shard_published` fan-out、sibling project/page scope或新协调器。 |
