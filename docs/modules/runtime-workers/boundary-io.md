...
| Workbench matching schedule | canonical import/OA/relation writers -> `job.workbench_matching_dirty_scopes` | 写入方只标记精确月份，`workbench-matching` worker 负责 claim/complete/fail。禁止同时创建 `job.background_jobs.type='workbench_matching'` 伪进度；该进度没有执行 owner，也不是页面 read model。导入结果仅返回 `queued_matching_months`。 |
...
| Workbench matching source versions | matching worker / matching orchestrator | 只包含会改变确定性正式关系结果的规则、OA、附件解析、银行标签和异常版本；worker 启动及空轮询只能通过 `load_app_settings` 窄读取得银行规则，禁止为构造 settings service 加载全量 imports snapshot。direct page query 的 DTO/SQL schema 不得进入 matching stale-scan 输入，也不得无关重算全部历史 matching scope。 |
