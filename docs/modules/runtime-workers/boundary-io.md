...
| Workbench dependent publish | `WorkbenchReadModelRefreshService` | Workbench 月分片 projection commit 只原子发布自身 generation并完成自身 dirty scope，不 enqueue `cost_statistics` 或任何其他页面。Cost消费者在访问时比较 Workbench expected/active；stale时只 ensure exact Workbench并返回 refreshing，页面有界自动轮询在依赖 fresh 后才 enqueue对应 Cost child。Cost worker仍在任何 payload I/O 前以 manifest声明的 Workbench dependency fail closed。禁止恢复 `workbench_shard_published` fan-out、提前投递必然 defer的Cost任务或引入新协调器。 |
