...
| Bank requirement recalculation | `settings.bank_relation_requirements.recalculate.requested` | 包含 job id、owner、目标规则版本和 semantic changed tag codes。settings-maintenance worker 从当前 settings 和 active relation tag proof 集合式定位，先全量预验证再通过正式 relation command 逐 case 幂等更新；只 enqueue 实际写入关系的精确 `workbench` / `workbench_relation` 月份并标记同月 matching dirty，禁止 `all` 和页面请求内执行。 |
