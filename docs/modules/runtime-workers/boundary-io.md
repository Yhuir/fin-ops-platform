...
- 当前闭环：worker 入口使用 registration contract；worker instance、event type、env example、manifest/check command 和 App Health readiness 均由 `runtime_worker_registry.py` 派生。`workbench` primary claim 月份 shard 与 `all` fan-out command，`workbench-secondary` 只竞争 claim 月份 shard；二者共用同一 handler、durable queue 与 active-generation 发布合同。`cost-statistics` / `cost-statistics-secondary` 同样只增加同 event exact scope 的 bounded drain 并发，不新增投影路径。普通写后可见性走月份 shard + query-composed all，不存在全局 aggregate publish。OA 待付款由 `oa-pending-payment` 专属实例 claim `oa_pending_payment.read_model.refresh`，共享 `invoice-usage-collection` 只保留进项使用/销项收款。外部往来款页面已改为 direct canonical read，因此没有 `turnover-ledger` registration、event、handler、env、readiness 或 App Status worker owner。部署文档不再维护手写 worker 矩阵或 `sudo systemctl enable --now fin-ops-worker@...` 清单。
- 生产证据状态：既有 worker 治理已上线；外部往来 direct-read 候选仍需本次部署后验证 Turnover registration 被退役、历史 Turnover event 不再新增、页面读性能及 confirm/withdraw 跨页一致性。
...
| Claim scope filter | worker registry / worker env | 只用于确有生产证据的同 event type吞吐隔离：`workbench` primary claim 月份与 `all`，`workbench-secondary` 仅 `exclude_claim_scope_keys=('all',)`，使两个跨月 exact shard 可并行而显式维护 command 保持单 owner。`cost-statistics-secondary` 与 primary 竞争同一规范 scope，不复制业务规则。scope contract 仍由 read model scope policy 负责，不能把业务 scope 规则塞进 queue 层 |
...
| Workbench dependent publish | `WorkbenchReadModelRefreshService` | Workbench月分片projection commit只原子发布自身generation并完成自身dirty scope，不enqueue `cost_statistics`或任何其他页面。多个月份payload计算和generation staging/COPY可并行；重型数据写完后，短锁段只切换本次月份active generation，不再在发布事务内同步扫描全部active月份或写all-scope statistics。`month=all`的summary/counts由query repository在repeatable-read页面快照中使用现有canonical set-based SQL计算，并在返回前复核generation-set version。Cost消费者访问发现Workbench stale时，同次只ensure exact Workbench并stage当前project/page所需的exact Cost child，不stage parent或sibling；query把同次canonical proof附给两个event，Workbench/Cost worker分别验证token/scope后复用，避免重跑相同proof SQL。Cost worker在任何payload I/O前仍以manifest dependency fail closed/defer，依赖fresh后才发布，成功child再收敛parent。生产实测Workbench exact rebuild约1.33秒，因此只有`cost_statistics`等待`workbench_read_model_not_fresh`这一条依赖使用至少1秒defer，避免0.25秒热重试；其他worker/dependency继续使用既有配置。Workbench own target token可由现有PostgreSQL advisory-lock覆盖active/最新成功同target；Cost child的Workbench依赖token只合并active waiter，禁止恢复`workbench_shard_published` fan-out、写后Cost producer或新协调器。 |
...
| Cost statistics versioned publish | `CostStatisticsReadModelRefreshService` / repository | handler 必须从 event 取得非负整数 `source_version` 并显式传入 month/parent builder。repository 复用现有 partial unique index，在一个事务内锁定该 scope 唯一 `pending` / `processing` dirty row，版本精确相等才写 read model；随后 handler 以同一版本条件完成。发布被拒绝或完成竞态失败都保持 `refreshing`，不得污染 Redis 或投递 parent；月份仅在发布和完成都成功后 fan-out parent。普通 parent event 只做 rollup，禁止用 `app_status_readiness` 重新枚举历史 shard；显式 `force_refresh` parent 才 full fan-out。 |
| Cost statistics structured publish | `CostStatisticsSqlProjectionBuilder` / repository | manifest声明 `workbench`、`bank_detail` 两个 read dependency。month worker先按 Cost consumer semantic proof 比较 canonical Workbench expected/active：只排除 active-generation 执行游标 `source_version`，其余业务字段不一致仍抛标准 non-fresh并由 runtime worker精确去重/defer，禁止读取或发布旧业务 generation；随后才读取 fresh Bank Detail并条件发布 OA allocation `cost_statistics_rows`。`time|bank_tag` 直接读取 fresh Bank Detail rows，不经过 Cost worker；migration `0123` 删除旧 Cost bank-flow复制表。child是否需要刷新由 Cost query owner的 profile-specific gate判定。projection不写 Redis，禁止恢复第二 bank-flow writer、旧无版本 cache writer或 JSON dual-write。 |
| Shared relation proof consumers | consumer query owner / canonical relation repository | shared relation confirm/withdraw/cancel 只推进 canonical relation version，不由 UoW/repository 投递 `oa_pending_payment` 或其它 consumer。各消费者在页面访问时检测自身 source mismatch 并精确 enqueue；worker 的 exact-scope relation proof 必须包含 eligible active relation count + stable typed membership digest，局部 delta 必须先批量解析 UUID/legacy aliases，并经 canonical repository/freshness gate 发布，不得用 `max(updated_at)`、queue-drained 或缓存旧 proof 伪造 fresh |
| Relation-dependent invoice publish | `InvoiceUsageCollectionSqlProjectionBuilder` / runtime worker | input/output invoice projection 发布前仍必须通过 `workbench_relation` freshness gate；写入 scope 的 embedded relation proof 按该月份、该 invoice type 的 canonical invoice IDs 计算，只覆盖实际触达这些发票的 active relation。bank-bank 或另一 invoice type 的关系变化不得改变本 consumer proof。相关关系变化时 worker 短延迟 defer，禁止把旧 relation distribution 写成 fresh。 |
...
| Invoice lifecycle dependency refresh | `InvoiceLifecycleReadFacade` / `RuntimeWorker` / `ReadModelRefreshGateway` | lifecycle manifest 只声明既有 `pending_invoice`、`input_invoice_usage`、`output_invoice_collection`、`oa_pending_payment`、`workbench_relation` 依赖并由无环测试锁定；不新增 coordinator。访问 facade 先跳过已 fresh dependency，并一次性投递其余 exact month scope；`pending_invoice` 只投同月 `expense:all:YYYY-MM` 与 `income:all:YYYY-MM`。worker 的 fail-closed 补投同样拒绝 manifest 图外 scope，避免逐项延迟或异常字符串扩散；handler 已基于 canonical facts 抛出 `*_read_model_not_fresh` 时，旧 readiness 不能再把补投短路为 `already_fresh`。active 去重、normalize、validate、原子 durable dedupe 和持久化继续由正式 gateway / PostgreSQL durable queue 负责。显式 lifecycle `all` 才可按既有 contract 扩展。 |
