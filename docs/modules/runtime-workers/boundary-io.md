...
| Active refresh state | PostgreSQL outbox + dirty scope | gateway 的 ensure/wakeup coalescing 必须通过 repository 的 exact-scope 原子入口执行：同一事务 advisory-lock `tenant/type/key`，检查 `job.outbox_events pending/processing` 后才决定是否入队。只有活跃事件才证明 worker 会继续推进；orphan dirty 必须允许重新 enqueue。禁止 check-then-enqueue 两事务竞态为同一 scope 创建下一代 source version。canonical mutation、显式 repair/reapply 与 force 不走此 ensure 合并边界 |
