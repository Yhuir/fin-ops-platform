...
| Active refresh state | PostgreSQL outbox + dirty scope | gateway 的 ensure/wakeup coalescing 必须通过 repository 的 exact-scope 原子入口执行：同一事务 advisory-lock `tenant/type/key`，检查 `job.outbox_events pending/processing` 后比较规范化 metadata。覆盖关系为 `force > full scope > partial delta`；partial 不得吞掉 full，非 force 不得吞掉 force，partial 之间只有语义覆盖才 no-op。新增语义在该锁内合并 pending event，或为 processing event 建立 pending follow-up。只有活跃事件才证明 worker 会继续推进；orphan dirty 必须允许重新 enqueue。禁止两事务竞态或同 scope 丢失新语义。canonical mutation、显式 repair/reapply 与 force 不走此 ensure 合并边界 |
