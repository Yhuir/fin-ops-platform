...
| Worker instance env | deploy/systemd | 生产 systemd 必须传 `--registration <instance>` 与 `--worker-instance <instance>`；instance name、event types、claim scope filters 与 handler flags 由 registry 派生。`release-gate-activate` 在切换前后必须比较 registry required instances 与 systemd 已加载/已安装 concrete units，`unknown_worker_count` 和 `required_worker_not_ready` 均为 0 才允许 PASS；已启用、运行或失败但不在当前 registry 中的 `fin-ops-worker@*.service` 必须先被 stop/disable，禁止 WIP/历史实例继续消费队列或 crash-loop。该收敛不删除 env 文件，保留受控回滚能力。PostgreSQL durable queue worker 的默认 idle poll 为 `0.05s`，`workbench` 使用 `0.01s`；历史 poll 参数只允许由 deploy helper 精确迁移。BankFlow draft worker/env 已退休，不得被 helper 迁移、安装或启动；no-OA worker 保持独立登记。OA worker 拆分时，helper 还必须幂等删除既有 `invoice-usage-collection` env 中精确命中的 OA handler/event 参数，禁止旧 env 重新扩张 registry claim 边界 |
...
| Publish terminal recovery | PostgreSQL outbox / RabbitMQ dispatcher | consumer 已把 durable event 完成后，超过 publish lock timeout 的 `status=done/publish_status=publishing` 由 dispatcher 收敛为 `published`，不得重复发布；release gate 必须同时证明 publishing backlog 为 0 |
...
- `0086_runtime_queue_claim_hot_path.sql` 继续保护保留 worker 的队列 claim；生产发布后需要验证四个保留 read model 及其他登记任务的 backlog、耗时和失败率，并确认 BankFlow draft event/worker 为零。
