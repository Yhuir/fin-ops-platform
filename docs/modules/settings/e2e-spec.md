...
本文件定义 `/settings` 页面在真实浏览器中的业务验收合同。测试必须保护平台配置、权限、数据重置、OA 凭据、项目范围 fan-out、background worker 边界和下游页面一致性，而不是保护当前 React 组件实现细节。

## 模块目标

设置页面维护平台级配置事实。普通 settings 写入 `ApplicationStateStore`，OA 申请人凭据使用独立 secret repository，数据重置通过受保护后台 job 执行。任何设置变更都不能让旧 cache 被当作 fresh，也不能绕过权限、审计、lifecycle 或 worker 边界。
...
| `SETTINGS-E2E-002` | 项目范围保存到成本统计 direct fan-out | P0 | 用户把项目标记完成并保存后，请求必须包含 `completed_project_ids`；进入成本统计时 active scope 通过 direct payload 排除已完成项目，all scope 通过 direct payload 保留该项目和金额。 |
...
| `SETTINGS-E2E-004` | 数据重置安全边界 | P0 | data reset 必须 admin/password gate、protected targets、active job reentry、并发 409、失败不清数据、不泄露密码；重置后旧 cache 不得伪装 fresh。 |
...
| `SETTINGS-E2E-009` | App Status/background worker 可见性 | P1 | settings 变更和 data reset 引起的 job、outbox、worker heartbeat 或下游页面状态必须能被 App Status/App Health 或下游页面诊断；页面不能只靠本地事件宣称完成。 |
| `SETTINGS-E2E-010` | 真实基础设施 data reset 和后台任务收敛 | P1 | 真实 PostgreSQL/RabbitMQ/Redis/systemd/OA/对象存储环境下，data reset、项目范围变更、规则保存和 OA 凭据使用后，相关 worker、cache 和页面 direct API 最终收敛；该项必须在 staging/runtime smoke 验证。 |

## 不属于本地 deterministic E2E 的风险

- 真实生产数据重置的备份、PITR/恢复、worker pause/resume、Redis/cache 清理和 reset 后全页面 direct API smoke。
- 真实 OA 登录、RSA/token、OA 草稿、OA iframe/session 和目标申请人权限。
- 真实 PostgreSQL pgcrypto key、历史 settings payload、半迁移数据和大规模 state store。
- 真实 RabbitMQ/Redis/systemd 后台任务收敛、长队列重试、Nginx/对象存储和生产网络抖动。
