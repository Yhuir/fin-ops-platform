...
| `SETTINGS-E2E-002` | `covered` | `web/e2e/settings-data-reset-flow.spec.ts`、`web/e2e/cost-statistics-flow.spec.ts`、`web/src/test/SettingsPage.test.tsx`、`tests/test_app_settings_service.py`、`tests/test_derived_data_lifecycle_service.py` | Browser 覆盖项目标记完成、保存 settings、POST body `completed_project_ids`、进入成本统计后 active scope fresh 排除已完成项目、all scope fresh 保留项目和金额，并检查成功后无可见错误残留；后端覆盖 project scope lifecycle/dirty fan-out。 |
