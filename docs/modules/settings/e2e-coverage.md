...
| `SETTINGS-E2E-002` | `covered` | `web/e2e/settings-data-reset-flow.spec.ts`、`web/e2e/cost-statistics-flow.spec.ts`、`web/src/test/SettingsPage.test.tsx`、`tests/test_app_settings_service.py`、成本统计 API/policy tests | Browser 覆盖项目标记完成、保存 settings、POST body `completed_project_ids`，随后进入成本统计确认历史项目仍可见、请求不含 `project_scope`、项目范围切换 UI 不出现且无可见错误残留。后端覆盖项目状态与成本准入解耦。 |
...
`web/e2e/settings-data-reset-flow.spec.ts` 已接入 Playwright `operation-latency-*.json` 附件。本轮记录的设置页操作覆盖：设置页打开、数据重置 section 切换、清除银行流水影响确认、继续到 OA 密码复核、OA 密码填写、确认清理 POST 202 到首个 25% 进度反馈、data reset job polling 到完成、reset 后进入银行明细和待找发票验证 fresh read model、项目状态 section 打开、项目标记完成、保存设置，以及进入成本统计确认历史成本不受项目完成状态过滤。

## 下一轮补测建议

1. staging 运行真实 data reset smoke：备份、pause/drain、reset、worker 恢复、Redis/cache 清理、全页面 fresh。
2. staging 运行 settings project status 和 pending invoice rules 的真实 write-operation SLO audit，确认各自 canonical 保存与下游读取边界。
