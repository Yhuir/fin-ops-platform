...
| `SETTINGS-E2E-002` | `covered` | `web/e2e/settings-data-reset-flow.spec.ts`、`web/e2e/cost-statistics-flow.spec.ts`、`web/src/test/SettingsPage.test.tsx`、`tests/test_app_settings_service.py`、`tests/test_derived_data_lifecycle_service.py` | Browser 覆盖项目标记完成、保存 settings、POST body `completed_project_ids`、进入成本统计后 active direct payload 排除已完成项目、all direct payload 保留项目和金额，并检查成功后无可见错误残留；后端覆盖 project scope lifecycle/outbox fan-out。 |
...
| `SETTINGS-E2E-009` | `covered` | `tests/test_derived_data_lifecycle_service.py`、`tests/test_app_status_overview_service.py`、`tests/test_runtime_worker_registry.py`、`web/src/test/AppStatusIndicator.test.tsx`、`web/e2e/settings-data-reset-flow.spec.ts` | 后端覆盖 lifecycle、App Status overview、worker registry；Browser 通过项目范围到成本统计 direct payload 证明关键下游读取不使用旧数据；更多页面最终 UI 由各页面 Spec-first 覆盖。 |
| `SETTINGS-E2E-010` | `external-risk` | `bash scripts/verify.sh infra-smoke` staging gate、runtime worker/background-task tests、data reset service tests、write-operation SLO audit profiles | 本地 contract 覆盖 job、outbox、worker registry、App Status 和 Browser 用户路径；真实 PostgreSQL/RabbitMQ/Redis/systemd/OA/对象存储、生产备份恢复和全页面 direct API 收敛必须在 staging/runtime smoke 验证。 |

## 下一轮补测建议

1. staging 运行真实 data reset smoke：备份、pause/drain、reset、worker 恢复、Redis/cache 清理、全页面 direct API smoke。
2. staging 运行 settings project scope 和 pending invoice rules 的真实 write-operation SLO audit，确认 outbox/worker 到 direct API 的端到端时延。
