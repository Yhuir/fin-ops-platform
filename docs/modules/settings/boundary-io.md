...
| ACL GET/PUT | 仅 `YNSYLP005` 的后端 admin session | GET 返回完整 snapshot 和 OA 名称/状态；PUT 只接受正整数 `expected_version` 与完整 `accounts[{username,page_keys}]`，page key 必须来自可分配集合（含 cash，后端 18 项），账号缺席表示 denied；UI 从 pageRegistry 派生一个现金 checkbox，非 005 不自动授予现金 |
...
| OA manual import / attachment refresh result | 设置页 | manual import 仍只允许 `completed` 且 `can_import=true` 的 OA，POST 返回 202/canonical import_job；worker 精确读取已选来源，在同一事务写定向 OA projection、选中 marker、审计和 job 成功，终态返回精确 affected scopes。进行中或不存在的条目保留明确失败原因；全部失败提交 job.failed 和 failed 审计，可显式重试；部分成功保留逐条结果及 partial_success 审计/attention，不能显示全部成功；附件刷新是独立能力，POST 返回 202/event id，GET 返回受控 durable status，只有 `done` 才含逐 row 计数、promotion summary 和 affected scopes。页面随后必须用 exact row ID、原 form type、`statuses=completed,in_progress` 和 `page_size=2` 回读，并同时要求 `total=1` 与 row ID 唯一命中；0/多条均 fail closed。`in_progress` 日常报销只显示“附件已解析，待 OA 完成后进入统一发票池”，不得启用正式导入。`freshness_targets` 与 `operation_barrier_targets` 为空；后续业务页面 normal GET 读取 canonical facts。 |
