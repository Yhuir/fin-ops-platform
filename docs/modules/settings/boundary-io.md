...
| 银行账户映射手工录入字段 | 银行流水导入页 | `AppSettingsService.get_bank_account_mappings_payload()` 在持久化的 `id/last4/bank_name/short_name` 之外，按银行名称派生只读 `manual_entry_reference_field={key,label}`；该派生字段不回写设置存储，流水录入服务仍以服务端派生结果决定允许的官方参考号字段。 |
