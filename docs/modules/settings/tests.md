...
| 数据重置 | `tests/test_settings_data_reset_service.py`、`web/src/test/SettingsPage.test.tsx`、`web/src/test/WorkbenchSelection.test.tsx`、`web/e2e/settings-data-reset-flow.spec.ts` | 覆盖 admin/password gate、job progress、protected targets、reset OA rebuild、失败不清数据不泄密；Browser e2e 覆盖真实 Chromium 下影响确认、OA 密码复核、job polling 和 settings reload。 |
...
- Browser e2e: `/settings` -> 数据重置 -> 影响确认 -> OA 密码复核 -> `settings_data_reset` job polling -> settings reload -> 全局成功反馈。
...
Nightly full suite 应覆盖本模块的后端 settings/data-reset/credential tests、前端 SettingsPage/WorkbenchSelection/AppStatus tests、deterministic Playwright `settings-data-reset-flow` smoke、docs verify。模块级快速验证使用上方命令。
