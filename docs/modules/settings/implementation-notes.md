...
## 2026-09-15 - 顶部 Tabs 与统一设置工作区

- 8 个子页共用居中工作区、水平 HeroUI Tabs 和白色分隔行；OA 配置标签/控件对齐，凭据为双列表单，访问账户保留独立双栏，重置保持逐项操作。
- 移除旧 SettingsTreeNav、移动端 Select、三档宽度和对应 CSS；不新增依赖、接口、存储、缓存或兼容路径。
- 普通保存明确标为“保存全部设置”，保留跨 Tab 草稿并提示未保存状态；ACL/凭据/reset 的提交与权限合同不变。
- 验证：组件测试覆盖跨 Tab 草稿与一次保存；Playwright 覆盖 4 种宽度的 8 个页面、实际 Tab 位置、权限矩阵、访问账户失败/恢复、重置完整流程和项目完成后的历史成本。截图检查发现并修复原生 Tab 默认全宽造成的横向遮挡。
- 性能验证分别记录本地交互耗时与生产验证结果，不将合成 API 的浏览器耗时当作生产性能。

## 2026-09-03 - 设置子页面紧凑化与旧 UI 链路删除

- 目标：统一设置子页面为简约、紧凑的 HeroUI 排版，移除静态解释文案和数据重置大卡片。
- 影响范围：仅 `SettingsPage` 前端组件、设置页 scoped CSS、前端交互/回归测试和模块文档；API、数据库、read model、worker、权限与数据重置后端合同不变。
- 关键决策：桌面端固定窄导航；项目与待找发票筛选改为标签页 + 单表/列表；银行映射、凭据、OA 设置采用受限内容宽度；数据重置保留预览、恢复点、密码复核、原因和 job progress，但不展示静态内部实现说明。
- 旧链路删除：移除三代并存的旧设置页 CSS，包括旧大卡片、旧项目双列、旧标签菜单、旧导航描述/计数、旧原生控件和旧数据重置进度按钮样式；没有保留兼容分支或 fallback UI。
- 测试覆盖：更新 Settings component/interaction、项目状态跨页、数据重置、表格表面和全量前端回归测试；浏览器验收覆盖桌面/窄屏及主要子页面。
- 性能：生产构建 CSS 从 942.05 kB / gzip 108.18 kB 降至 919.53 kB / gzip 105.43 kB；Settings chunk 从约 62.27 kB / gzip 17.88 kB 降至 62.00 kB / gzip 17.80 kB。
- 文档影响：页面呈现与测试合同更新；模块边界和 I/O 未变化，因此不修改 `boundary-io.md`。
...
- 发布前使用 root-owned `settings-normalize <exact release> --dry-run/--execute` 从 canonical payload 和 `raw_payload.normalized_payload` 删除旧键；dry-run 必须只报告该键，避免持久化旧状态污染 Settings Audit，且不创建备份。
- 不影响银行明细模块自己的标签与自动标签规则。
