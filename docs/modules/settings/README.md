...
- 业务规则：银行交易标签、银行自动标签、待找发票标签组、免 OA 和往来款标签选择。
...
| 银行标签/自动标签保存 | `bank_auto_tag_rules_changed` / bank tag settings audit | 银行明细、免 OA、关联台候选、往来款、成本、搜索 |
...
| 数据重置 | `settings_reset_completed`、`startup_stale_scan`、read model/dirty scope/cache cleanup | 所有列表页、导入、关联台、App Status/App Health |
