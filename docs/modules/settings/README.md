...
- 数据重置：银行流水域、发票域、OA 源重置与重新同步；必须保护禁止删除目标、保留必要事实、记录 job progress，并避免旧 cache 被误判为可用 direct payload。

设置模块本身多数事实写入 `ApplicationStateStore`，但它的变更会扇出到 lifecycle、worker、App Status 和多个页面 direct API。任何改动都必须先做影响面评估。
...
| 项目范围变化 | `project_scope_changed` 或等价 lifecycle/outbox | 成本统计、搜索、关联台项目展示 |
| 访问控制变化 | state store + OA role sync | 页面可见性、写入权限、导出权限、数据重置权限 |
| OA 导入过滤/留存/promotion 设置变化 | state store，后续 OA reset/resync 或 sync 使用；OA 附件发票 promotion 默认 `link_existing_only`，`disabled` 完全跳过，只有 `create_missing` 才允许创建缺失正式发票 | OA 待付款、进项/销项、税金、成本、关联台、统一发票池 |
| OA 申请人凭据维护 | 独立 credential repository；不进入普通 settings payload | 进项发票使用 OA 反提草稿、真实 OA 登录/token provider |
| 数据重置 | `settings_reset_completed`、cache/lifecycle cleanup | 所有列表页、导入、关联台、App Status/App Health |
| 启动补扫 | `startup_stale_scan` 默认关闭；启用时只创建 workbench matching rescan 诊断/任务 | 关联台 matching 候选补扫；不直接刷新用户可见页面 payload |
...
- API contract、DTO shape、错误字段、权限校验、状态值或 direct payload 可用性字段变化。
- 业务状态、UI 状态、cache/lifecycle 状态、worker 状态或状态流转变化。
- 跨页面刷新、domain event、derived lifecycle、outbox 或缓存边界变化。
