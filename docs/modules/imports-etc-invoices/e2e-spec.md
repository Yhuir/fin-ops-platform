...
本文件定义 `/imports/etc-invoices` 的 Browser E2E 合同。测试应基于 ETC 对账任务、zip preview、confirm job、derived lifecycle、worker 和 direct read 边界设计，不以当前代码行为作为需求事实源。
...
| `IMPORT-ETC-E2E-007` | confirm 失败 | confirm API/worker 入队失败时，页面必须显示错误，不展示“已开始后台导入”，不把下游 direct payload 伪装成已收敛。 |
...
| `IMPORT-ETC-E2E-010` | derived lifecycle fan-out | `etc_import_confirmed` 必须触发 Workbench、invoice lifecycle、tax offset、cost statistics 和 historical ETC repair 的真实下游影响；Search 通过 `/api/search` direct payload 反映业务结果，不恢复 Search dirty scope/read model refresh。 |
| `IMPORT-ETC-E2E-011` | 真实基础设施后台任务收敛 | PostgreSQL/RabbitMQ/Redis/systemd import worker、derived lifecycle worker、对象存储、真实 OA 草稿和下游 direct API 结果必须在 staging 或生产只读 smoke 验证。 |

## 非目标

- 不在 deterministic Browser E2E 中连接真实 OA、对象存储、票根网 zip 或生产数据库。
- 不把 mocked confirm job queued 等同于真实后台任务完成。
