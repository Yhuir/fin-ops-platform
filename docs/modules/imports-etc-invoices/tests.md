...
| 59 个 ZIP 在请求内串行执行 verified writes，仍超过代理 60 秒；并发中途失败遗漏清理后续成功对象 | `tests/test_etc_import_session_store.py::EtcImportSessionStoreTests::test_durable_save_runs_verified_archive_writes_with_bounded_concurrency`、`tests/test_etc_import_session_store.py::EtcImportSessionStoreTests::test_durable_save_cleans_all_successful_parallel_writes_after_one_failure` |
