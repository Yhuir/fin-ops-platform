...
| Browser e2e | `web/e2e/imports-etc-invoices-flow.spec.ts`、`web/e2e/permissions-role-matrix.spec.ts` | 真实 Chromium 加载 ready task、选择 task、上传 zip、预览 audit/导入项、`preview_stale`、`stale_reconciliation_task_preview` 清空预览、confirm 失败、确认后展示 background job feedback、ETC 票据/税金抵扣/成本统计下游 direct downstream payload，并断言成功节点没有导入失败/后台导入失败/同步失败可见残留且不走通用 `/imports/files/*`；read-only 用户不能上传/预览/确认导入 |
...
| Derived lifecycle | `RuntimeWorkerDerivedLifecycle.refresh_after_etc_invoice_link(...)`、`DerivedDataLifecycleService` | `etc_import_confirmed` 必须产生 Workbench、invoice lifecycle、tax offset、cost statistics、historical ETC repair 和 search 相关真实下游影响；页面用 direct API 重读验证 |
...
- Browser e2e 必须覆盖 ready task selector、zip preview、audit/review copy、confirm job feedback、preview stale、stale task preview、confirm failure、ETC 票据/税金抵扣/成本统计下游 direct downstream payload，以及 ETC 导入不误走通用 files import API。
...
- confirm API/worker 入队失败必须错误可见，不能展示“已开始后台导入”，不能把下游 direct payload 伪装成已收敛。
...
- confirm 完成后必须触发 `etc_import_confirmed`，让税金抵扣、成本统计、关联台 summary 和 search 通过 direct API/outbox/真实后台任务收敛。
...
| 4. Read model/cache/background job tests | 适用 | `tests/test_import_job_queue.py`、`tests/test_derived_data_lifecycle_service.py`、`tests/test_runtime_worker_registry.py`、`tests/test_app_status_overview_service.py`、`tests/test_tax_offset_api.py`、`tests/test_platform_runtime_boundary_guards.py` | 覆盖 import worker、`etc_import_confirmed` lifecycle、tax/cost/workbench 下游影响、App Status job/dependency，并防止恢复 page read-model refresh 作为 ETC 导入闭环证明。 |
| 5. Frontend component and interaction tests | 适用 | `web/src/test/ImportCenterPage.test.tsx`、`web/src/test/EtcApi.test.ts`、`web/src/test/EtcTicketManagementPage.test.tsx`、`web/src/test/AppStatusIndicator.test.tsx`、`web/e2e/imports-etc-invoices-flow.spec.ts`、`web/e2e/permissions-role-matrix.spec.ts` | 覆盖 ETC standalone route、preview/confirm/stale/unmount、API mapper、business batch UI、global job status，以及真实浏览器 ready task/zip/confirm job、preview stale、stale task preview、confirm failure、ETC 票据/税金/成本下游 direct downstream payload 交互、成功后无导入失败/后台导入失败/同步失败可见残留和 read-only 导入门禁。 |
| 6. End-to-end business-flow integration tests | 适用 | `tests/test_etc_backend.py`、`tests/test_workbench_v2_api.py`、`web/e2e/imports-etc-invoices-flow.spec.ts` | 覆盖 task-aware zip import -> business batch -> ETC metadata/附件 -> Workbench summary/open row -> submitted/delete recovery；Browser e2e 覆盖导入页 preview/confirm job、失败时不误报 job success，并在 deterministic mock 下覆盖 ETC 票据、税金抵扣、成本统计 direct downstream payload 展示和成功后无错误残留；真实 worker 完成仍需 staging。 |
| 7. Existing feature regression tests | 适用 | 上述全部、`tests/test_platform_runtime_boundary_guards.py`、`docs/modules/etc-tickets/tests.md`、`docs/modules/tax-offset/tests.md`、`docs/modules/cost-statistics/tests.md`、`web/e2e/imports-etc-invoices-flow.spec.ts` | 每次改 ETC import、business batch、已存在 canonical invoice 关联、summary row 或 lifecycle 时，都必须回归下游页面旧行为，并保护 runtime worker 不恢复旧的 ETC canonical invoice 创建路径。 |
...
| Browser 中 confirm 或下游 direct payload 成功后仍残留导入失败/同步失败提示 | `web/e2e/imports-etc-invoices-flow.spec.ts` 的 success visible-error guard，断言导入页、ETC 票据、税金抵扣和成本统计成功节点没有导入失败、后台导入失败或 同步失败等可见错误残留 |
...
- Browser e2e smoke：ready task 加载 -> 选择 ETC 对账任务 -> 上传两份 zip -> preview audit/新增/重复/附件补齐/异常项 -> confirm -> `etc_invoice_import` background job feedback -> ETC 票据/税金抵扣/成本统计 direct downstream payload 展示导入证据 -> 无导入失败/后台导入失败/同步失败可见残留。
...
- ETC import confirm -> ETC metadata/已存在 canonical invoice 关联 -> `etc_import_confirmed` -> 关联台 summary、税金抵扣、成本统计通过 direct API/outbox/真实后台任务收敛。
- Staging runtime smoke：真实 ETC zip confirm 后验证 `etc_invoice_import` job completed、derived lifecycle/outbox 处理完成、Workbench/tax/cost direct APIs 展示导入证据，并用 direct `/api/search` smoke 验证搜索结果；不再以 `*.read_model.refresh` profile 或 page freshness SLO 作为闭环证明。
...
真实 staging/发布前 runtime smoke：

- 真实 ETC zip confirm 后确认 `etc_invoice_import` job completed。
- 确认 derived lifecycle/outbox 已处理，Workbench/tax/cost direct APIs 展示导入证据。
- 用 direct `/api/search` smoke 验证搜索结果；该检查不恢复 `*.read_model.refresh` profile 或 page freshness SLO。

## Nightly CI 覆盖

`scripts/verify.sh all` 会运行后端 `unittest discover`、前端 Vitest、build、deterministic Playwright smoke 和 docs check，因此 nightly CI 会覆盖本模块现有自动化测试、`web/e2e/imports-etc-invoices-flow.spec.ts` 的 confirm、downstream direct payload 和成功后无错误残留，以及 `web/e2e/permissions-role-matrix.spec.ts` 的共享导入 read-only 门禁；ETC 导入 spec 也覆盖 confirm 后 ETC 票据、税金抵扣、成本统计 downstream direct payload。真实大 zip、真实对象存储、真实 OA 草稿和真实后台任务收敛仍需 staging 或发布前 smoke。
...
- 真实对象存储、Postgres/RabbitMQ/Redis/systemd import worker 收敛、worker crash/retry、RabbitMQ wakeup 未由本地单测完全证明；仍需要 staging 中真实 ETC confirm 样本产生 completed job、processed outbox 和下游 direct payload 证据。
- 真实 OA 草稿创建、人工提交确认、Nginx `/api/` 和 `/fin-ops-api/` 代理路径仍需发布后 smoke。
- Browser e2e 当前覆盖 deterministic mock 下的 ready task zip preview/confirm job feedback、ETC 票据批次、税金抵扣和成本统计下游 direct downstream payload；大数据 ETC business batch 列表、真实 worker 完成后的 Workbench/search/historical repair 展示、长任务源文件、真实浏览器导出/删除/网络恢复仍是 `documented-risk`。
