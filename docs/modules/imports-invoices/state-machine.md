...
failed + close 保留执行结果并登记 disposition；needs_review + discard 复用预览终结后取消任务。两者均 acknowledged，不参与当前待处理统计，错误和审计保留。disposition 是处理事实而非新的执行状态；普通确认已知不生成 disposition。已处置任务不能被旧确认、重试、重新预览或取消请求修改。
