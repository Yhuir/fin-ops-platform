...
| 共享工作流 | `web/src/components/imports/ImportWorkflowPage.tsx` | 每文件票据方向、preview stale、重复审计、session restore、route unmount cleanup、job feedback |
...
| 5. Frontend component and interaction tests | 适用 | `web/src/test/ImportCenterPage.test.tsx`、`web/src/test/ImportsApi.test.ts`、`web/src/test/AppStatusIndicator.test.tsx` | 覆盖每文件方向、预览审计、错误提示、session restore、route unmount、API mapper、全局 status popover。 |
| 6. End-to-end business-flow integration tests | 适用 | `tests/test_workbench_v2_api.py`、`tests/test_tax_offset_api.py`、`tests/test_invoice_lifecycle_page_integration.py` | 覆盖 import confirm -> stale protection -> workbench/tax/invoice lifecycle 下游刷新。 |
| 7. Existing feature regression tests | 适用 | 上述全部，以及下游模块测试矩阵 | 每次改 shared import、invoice fact、lifecycle、read model 或 App Status 时，都必须回归发票导入和下游页面旧行为。 |
...
`scripts/verify.sh test` 会运行后端 `unittest discover`、前端 test 和 build，因此 nightly CI 会覆盖本模块现有自动化测试。真实大文件、真实 Postgres/RabbitMQ/Redis/systemd worker drain 仍需 staging 或发布前 smoke。
