...
## 2026-09-20 - OA 来源接管与导入幂等

- 目标与 I/O 见本模块及 OA 集成、关联台关系模块 boundary-io 的同日来源优先合同。
- 删除对象去重/merge 中 OA 票被首次 Excel 覆盖的路径，以及显示层人工归属优先、维护工具给 OA 票补人工归属的旧路径。
...
- 根因：`_merge_invoice_from_normalized` 对已有正式来源保留名称，后续重复输入仅贡献 provenance；页面审计却把每次重复输入当作名称的权威值。关联台人工补录重用既有发票时，名称识别差异会错误阻断全站发布。
- 修复：在现有字段审计内区分已有正式 owner 的重复名称差异，保留 warning 和双方值；原 owner、税号、身份、金额、正式 payload 和来源关系检查继续有效。移除所有 linked terminal 行名称必须相等的无差别判断，不新增并行审计路径。
- 范围：仅只读审计分类和回归测试，不更改名称事实、历史输入、数据表、业务写入或 UI，不需要数据库备份或数据修复。
...
- Browser 回归现验证发票导入不会伪造项目成本。下文早期记录中“发票导入直接产生成本统计影响行”的表述已被本节取代。
...
- 修复：共享 reader 在遍历每个 worksheet 前调用维度重算，继续复用既有 120 MiB/文件、32 sheets、20 万 rows、256 columns、200 万 cells、512 MiB 解压体积与压缩比门禁；超限明确拒绝，不承诺无界文件。
- 回归：同一畸形 XLSX fixture 同时经过银行和发票解析路径，必须恢复全部行；既有模板识别、文件 hash 防重、发票 canonical identity 和批量去重行为不变。
...
## 2026-09-23 主体识别与历史核对

- 目标：修复 OA 正式发票全文猜主体、名称截断，并在导入抽屉显示非阻断名称差异。复用现有 parser、导入 DTO 和精确原件修复工具；不新建表/API 路由/worker。
- 历史核对：生产只读分页读取 1,317 张正式发票，其中 454 张具有 OA 附件来源；检查 44 个 input Excel 登记，31 个能通过现有归档完整性及标准“发票基础信息”读取。394 张可匹配该标准原件，35 张主体字段有差异；60 张未匹配，不能猜值修复。其余 13 个文件包括 9 个非标准/多事实 sheet、4 个原件登记缺失，不作为本次自动修复依据。
- 精确修复预览：35 张全部可由既有工具证明同身份、日期和总额；只纠正主体字段，金融字段按数值核验不得变化。先部署正确解析器，再执行修复和定向重解析；保留身份、来源及已确认关系。
- 限制：未匹配税务原件不等于发票错误；对这些记录保留清单，不宣称全库历史值均已证明。

## 2026-09-23 部分财务证据修复

复用现有 financial_review_reason 与强身份关联；替换整票丢弃、PDF过早停止、同票先到先得和人工非空预填串值行为。无新基础设施。详细取舍、测试与发布验收见[闭环记录](../../dev/oa-attachment-partial-financial-evidence-plan.md)。
