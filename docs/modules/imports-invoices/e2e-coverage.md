...
| `IMPORT-INVOICE-E2E-004` | `covered` | `web/e2e/imports-invoices-flow.spec.ts`、`web/src/test/ImportsApi.test.ts`、`tests/test_import_file_service.py`、`tests/test_workbench_v2_api.py` | Browser 覆盖 `preview_stale` 错误可见、无 success、无 Workbench refresh；API/mapper 覆盖固定“重新预览”文案。 |
| `IMPORT-INVOICE-E2E-005` | `covered` | `web/e2e/imports-invoices-flow.spec.ts` | Browser 覆盖 confirm 失败错误可见、无 success、无 Workbench refresh。 |
| `IMPORT-INVOICE-E2E-006` | `covered` | `web/e2e/imports-invoices-flow.spec.ts`、`tests/test_derived_data_lifecycle_service.py`、`tests/test_invoice_lifecycle_page_integration.py` | Browser 覆盖 confirm 后刷新 Workbench、清空草稿，并检查导入成功后没有导入失败、后台导入失败或 read model 失败等可见错误残留；后端覆盖发票导入 lifecycle fan-out。真实 worker drain 仍归入 `IMPORT-INVOICE-E2E-009`。 |
...
`web/e2e/imports-invoices-flow.spec.ts` 已接入 Playwright `operation-latency-*.json` 附件。本轮记录的操作覆盖：打开发票导入页、选择发票文件、选择每文件进/销项方向、开始预览、确认导入并等待 Workbench summary 刷新、损坏文件未导入项 tab、损坏文件混合导入确认、慢预览首个禁用反馈、preview stale 确认错误、confirm server error，以及导入后进入销项收款、进项使用、税金抵扣、待找发票、OA 待付款和成本统计 fresh 页面并继续切换项目/费用类型验证导入成本证据。
