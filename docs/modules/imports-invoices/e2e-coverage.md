...
| `IMPORT-INVOICE-E2E-006` | `covered` | `web/e2e/imports-invoices-flow.spec.ts`、`web/src/test/ImportCenterPage.test.tsx`、`tests/test_derived_data_lifecycle_service.py`、`tests/test_invoice_lifecycle_page_integration.py` | Browser 覆盖 confirm 后只等待显式 operation barrier targets、零 Workbench 页面请求并清空草稿；组件测试另覆盖 targets 为空时零 barrier/零 Workbench 请求。后端覆盖发票导入 lifecycle fan-out；真实 worker drain 仍归入 `IMPORT-INVOICE-E2E-009`。 |
...
`web/e2e/imports-invoices-flow.spec.ts` 已接入 Playwright `operation-latency-*.json` 附件。本轮记录的操作覆盖：打开发票导入页、选择发票文件、选择每文件进/销项方向、开始预览、确认导入并等待显式 operation barrier targets、损坏文件未导入项 tab、损坏文件混合导入确认、慢预览首个禁用反馈、preview stale 确认错误、confirm server error，以及导入后进入销项收款、进项使用、税金抵扣、待找发票、OA 待付款和成本统计 fresh 页面并继续切换项目/费用类型验证导入成本证据。
