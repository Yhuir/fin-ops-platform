...
| 4a. System Audit direct-canonical 子页 proof | 适用 | `tests/test_audit_etc_tickets_read_model_tool.py`、`tests/test_page_audit_registry.py`、`tests/test_app_health_api.py`、`web/src/test/AppHealthOperationsPage.test.tsx` | 覆盖零 page read model、System Audit dispatch、三 bucket、creating durable attempt 且不做 OA 外部超时判断、pending draft/submission、submitted/not-submitted occupancy、typed edges、canonical invoice bridge、import queue和只读 snapshot；ETC 页面无 Audit UI。 |
