...
| 4a. Page Audit direct-canonical proof | 适用 | `tests/test_audit_etc_tickets_read_model_tool.py`、`tests/test_page_audit_registry.py`、`tests/test_app_health_api.py`、`web/src/test/PageAuditIcon.test.tsx` | 覆盖零 page read model 的诚实注册、统一 API dispatch、结构化列/registered payload、batch/task/file/invoice/import/submission typed edge、card/ticket/supplement 引用与唯一归属、canonical invoice bridge、active import job 即使 task 已完成仍阻断 queue、低于 max attempts 的 failed job 仍按终态失败处理、精确关联 imported/closed task 的历史 failed/dead-lettered job 计为 covered、未覆盖终态失败阻断 integrity、只读 snapshot 和 UI direct-canonical success gate。 |
