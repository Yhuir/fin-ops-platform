...
> 修改 `ETC票据管理` 相关业务状态、UI 状态、派生数据状态或 worker 状态前必须读取本文件。当前没有独立状态机时，在对应小节写明“不适用原因”，不要删除文件。
...
- direct unavailable：ETC 页面本身不触发 OA 自动检测；关联台后台投影/worker 诊断由关联台或 App Status 展示。
- permission disabled/hidden：权限不足时隐藏或禁用创建、导入、草稿、人工确认入口；删除入口不做流程状态阻塞，后端只保留版本并发校验和本地清理一致性校验。

## Direct Payload / Worker 状态

- ETC 业务批次列表直接读取业务批次事实源；关联台是否出现 `etc_invoice_summary` 由 Workbench direct query/relation facts 决定。
- `submitted` 人工确认会隐藏散落 ETC 发票，并让 Workbench open 区投影一条合并行；投影失败时不应把批次回滚成未提交。
- `etc_invoice_summary` 的展示金额可以保留千分位格式；direct Workbench payload 必须同时携带结构化金额，用于行金额、分组搜索文本和金额过滤。
- direct refetch / downstream diagnostics 触发来源：ETC 导入确认、OA 草稿创建、人工提交确认、人工未提交确认、业务批次本地删除/重置、关联台普通配对关系确认或撤回。
...
- 失败恢复：优先重跑真实后台任务或相关 direct API；业务批次、ETC 发票占用和审计事实不得从前端临时修补。导入确认的同一 session 只有 queued/running 或近期 succeeded job 可复用；failed、acknowledged、cancelled 等旧 job 必须允许重新确认并创建新 job。
...
| 2026-06-12 | 已提交 ETC 业务批次删除/reset 的 summary relation 取消、历史 repair 和 existing link 生产写入迁入 `WorkbenchRelationCommandService`；删除/reset 不再由 Workbench relation read model freshness 阻断 | ETC 业务批次删除、绑定 reconciliation task 删除、历史 repair/migration/link 工具、Workbench relation 事实源 | `tests/test_etc_backend.py`；`tests/test_workbench_relation_command_service.py`；`tests/test_historical_etc_business_batch_migration_service.py`；`tests/test_platform_runtime_boundary_guards.py` |
...
| 2026-06-09 | `etc_invoice_summary` 增加结构化金额并支持 workbench numeric/search 展示，同时修复历史已提交批次数据 | 关联台金额搜索、ETC 历史批次闭环、Workbench direct API | `tests.test_workbench_v2_api`；生产数据 SQL 验证 |
| 2026-06-09 | Workbench direct 读取将已提交 ETC 业务批次作为 `etc_invoice_summary` 一等来源，保留业务批次上报金额和数量 | ETC 人工已提交批次、关联台 open 区 summary、direct Workbench rows | `tests.test_workbench_v2_api`；`tests.test_etc_backend`；`web/src/test/EtcTicketManagementPage.test.tsx` |
