...
有效通行支出以有效链接呈现已匹配/未找到，不再产出高度可信或待接受的中间流程。还款、退款、明确排除显示不参与。自动配对不自动选中报销；既有选择、确认、导入、OA状态继续独立。

refresh 在可编辑 task 上检查版本，结果不变则版本/审计不变。重新解析是单独来源操作；人工固定当前配对、解除（拒绝该边）、恢复自动分配和移除/恢复票根均经同一 matcher 收敛。已确认和关闭 task 不可用 refresh 改配；先走既有 reopen。导入状态CAS保留既有确认版本，避免预览失效。
...
- 票根文本上传（2026-09-08）：选择/拖拽允许 TXT/TEXT 和无扩展名，保留 MIME 为空的真实文件；混选不支持后缀时整批不提交。有 blocking issue 是“已上传但内容需处理”，不是传输失败或全部解析成功；坏文件零可核对明细，原件可删除后修正重传。可能已有写入的异常只精确重读一次，不重 POST；两次读取都失败时明确状态未知并保留当前视图。上传与补读按当前批次身份隔离。
