...
## 2026-09-14 - 信用卡与票根匹配改进（已实施）

- [详细实施计划与复审](../../dev/etc-credit-card-ticket-matching-plan.md)：根因是商户OCR缺字导致候选提前被排除；原文件重放24对，临时内存隔离实验可得到30对。另确认解析完整性、无界日期兜底、人工关系状态及全任务回写风险。
- 设计采用现有parser/matcher/task/repository/UI边界内的单一分配链，当前任务点读点写；明确旧代码删除、日期规则建议、性能目标及下游回归。直接执行，不使用GSD，不新增hash、表、缓存、worker或审批流程。
- 用户已授权实现、提交 main、部署和生产核对。已确认数据不自动重算；生产仅通过正式API处理本次未提交批次。

- 已实施统一分配、逐页解析、显式重解析、人工拒绝和当前任务 CAS；旧金额兜底与指数搜索已删除。原始文件本地复核为 30 对 / 1711.33 元，未找到两笔合计70.51元。新旧行为以本条和当前 boundary-io 为准，下方历史 fallback/needs_review 记录已被替代。

- 发布复审补充：页内文字层只有页眉、交易表为图像时，按该页无有效交易行且存在图像进入既有 OCR；新增反例保护，不把非空页眉当成交易提取完成。

## 2026-09-08 - 票根无扩展名文本上传（已实施，验证发布见链接）

- [详细实施计划](../../dev/etc-ticket-root-text-upload-repair-plan.md)：ETC 票根入口显式接受无扩展名纯文本，其他共享文件检查调用默认保持严格；不新增解析框架、API、数据库字段或 worker。
- 前轮内存模拟额外复现坏记录被静默丢弃，以及前条时间配后条金额；用户已同意把文本记录边界/错误反馈纳入同一修复，不仅修扩展名。本轮补齐多选文件不得静默过滤、上传结果未知时精确重读和独立任务并发测量规则，不扩大模块范围。
- 前端保留紧凑上传区，补齐文件选择/拖拽与多文件部分成功后的真实状态；复用来源、解析 issue、版本与删除链路。
- 用户后续已授权实施、提交 main、部署并生产验证。已实现单一入口许可、严格记录边界、文件上传单次 POST 与失败精准补读；范围与最终运行证据见上方详细记录第 10 节。没有迁移、备份、新表/依赖/worker 或额外门禁。
- 功能提交 `6f2ab25ea` 已部署且现有发布检查通过；真实生产无扩展名上传、去重、拒绝前零写入、坏来源删除重传及 4 并发均通过。GET p95 425.36 ms，4 并发上传 p95 1998.22 ms（每文件 10 条、12 样本）；五个合成批次已按 ID 清理，原有 8 个 submitted 批次未变。原生系统选择器上传最后一步仍未完成，不能用 mock 浏览器或 API 成功替代；完整证据和边界见详细记录第 11 节。
...
- 关键决策：页面 detail/task 并发，切换或异步列表自动迁移 selection 时同步清空旧 task；所有 task mutation 额外校验 task ID 等于当前 business batch 的 task ID；显式 row batch 优先于旧 draft result；manual status 只保留 active bucket effect 一个 list reload owner。后端以 `save_etc_oa_draft_attempt` 对目标 business batch 做 version lock/CAS，只合并 attempt 改动的行；相同 idempotency key/recovery outcome 只修复 linked task 元数据。缺 task fail closed，recovery 只接受唯一 JSON boolean 与互斥证据。local store 保存失败时回滚 task 和 audit counter 后再允许重试。
- Audit：creating stale 只读取 durable `oa_draft_prepared/oa_draft_outcome_unknown.created_at` 或 payload `updated_at`，不读取每次 upsert 都会刷新的 formal row `updated_at`；not-submitted retained membership 不算当前 owner，资源已被另一个 submitted batch 合法接管时由新 owner lifecycle 校验，不要求旧成员继续是 unsubmitted；pending batch 必须与 task 的 draft metadata 精确一致。
- 文档影响：同步更新 boundary I/O、状态机、测试矩阵和实施记录；产品三 bucket 口径与 API 成功 DTO 不变。
- 测试覆盖：前端 71 项通过；Audit 14 项通过；reconciliation service 95 项通过、1 项条件性 skip；新增异步自动选择 owner 失效、submitted invoice 历史复用整页 Audit pass，以及真实 local store 失败回滚/跨实例单审计测试。此前独立 service/store 不丢更新、同 key/recovery 部分失败修复、strict recovery、缺 task、durable stale clock 和精确 target tests 继续通过。
...
- 影响范围：`EtcBusinessBatchApplicationService` 新增 `revoke_oa_draft_payload(...)`；`EtcBusinessBatchApiRoutes` 新增 `revoke_oa_draft(...)`；`server.py` 删除 `_handle_api_etc_business_oa_draft_revoke(...)`。
- 关键决策：核心状态迁移、审计事件、幂等和持久化仍由 `EtcService.revoke_business_batch_oa_draft(...)` 承担；application service 负责 access scope 和 canonical invoice link/refresh sequencing；route owner 负责 payload 到 service 的映射和错误/成功 envelope。
- 文档影响：只更新本实施记录和 modular IO analysis/state；产品/API 长期事实不变。
- 测试覆盖：新增 `test_etc_business_batch_oa_draft_revoke_route_resets_batch_and_invoices`，更新 `test_etc_business_batch_routes_do_not_keep_removed_legacy_handlers` 静态 Guard，并回归 OA draft/revoke service/API tests。
- 验证命令：见 `.planning/refactors/modular-io-boundaries/analysis/server-py-etc-business-oa-draft-revoke-route-callback-collapse-2026-06-25.md`。
- 未测风险：未做生产 browser/admin/write 验证；business-batch route owner 仍需 local closure audit。
- 后续事项：执行 `server-py:etc-business-route-owner-local-closure-audit`。
...
- 关键决策：对账任务按正式 `oa_submitted_confirmed -> closed` 语义补齐，不在前端隐藏未提交任务；`etc_invoice_summary` 的展示金额与结构化 `amount_value` 都从实际 canonical ETC 发票明细求和，OA/批次申请金额只保留在其自身业务字段，不能冒充“ETC发票合计”。
...
- 生产收口：恢复操作允许对已恢复记录做幂等归一化，canonical task title、OA process status、submission invoice count/amount 以 36 张真实成员重新计算；执行后复用 historical ETC maintenance lifecycle 标记业务批次月份的 Workbench matching dirty scope，禁止只刷新页面 read model 而遗漏重新配对。
