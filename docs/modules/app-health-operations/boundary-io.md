...
| Health probe | app health endpoints | 返回 readiness/status；单次 App Health snapshot 只读取一次 runtime status 和一次 background-job snapshot，Workbench 仍执行完整 generation consistency 合同，`outbox_backlog` 只统计当前 attention 状态、不扫描历史 `done`。`/health/ready` 仍实时读取 durable facts、不加伪 freshness cache，但同一请求内 outbox current-effective 集合与 dirty-scope current-effective 集合分别只计算一次，再生成当前 blocker aggregate/scope diagnostics；不得扫描历史 `done` dirty scopes |
