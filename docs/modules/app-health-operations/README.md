...
- Readiness：只覆盖 manifest 登记的 `workbench_relation`、`search`、`no_oa_bank_batch`，事实源是 `read_model.app_status_readiness` 和 current-effective durable queue；已退役页面没有 readiness。
