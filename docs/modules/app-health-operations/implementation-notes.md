...
- 关键决策：单次 App Health request 显式复用一份 runtime snapshot 和一份 background-job snapshot；Workbench 继续执行完整 generation consistency 检查，不能用 lightweight status 漏报不一致，但其 `outbox_backlog` 只统计 pending/processing/failed/dead-lettered，不扫描历史 done。active generation 发布后不可变，昂贵 consistency 结果按精确 `scope_key + active_generation_id` 复用，generation 变化自动失效；refreshing/building 期间直接以当前 dirty/building 事实判定，不读取旧 consistency，也不引入 TTL freshness。ready probe 不增加缓存，而是在一个 SQL request 内各 materialize 一次 outbox current-effective 与 dirty-scope current-effective 集合，再从该集合生成当前 blocker count/age/publish/by-scope 结果；不再为了 readiness 扫描历史 `done` dirty scopes。生产复测进一步确认 historical refresh sample lateral query 不是 readiness blocker 却仍占用主要 SQL 时间，因此从 ready 热路径删除；current failed/dead-lettered 仍由 current-effective outbox blocker 覆盖，完整 refresh duration/failure 指标继续由 `/health`、`/metrics` 和 Operations dashboard 提供。
- 测试覆盖：App Health runtime/lightweight port 回归；ready summary current-effective SQL、历史指标隔离与 query-count contract 回归。
...
- 生产性能复验补充：read-model duration 聚合只读取最近 7 天，并按 event type 通过现有 `(event_type, updated_at)` partial index 各取最多 512 条；避免扫描时间窗内全部事件，保持现有 response shape 和指标语义。
...
## 2026-07-10 - 页面 Audit 证明能力闭环

- 目标：把 9 个页面 Audit 从 scope/source-version/有限 missing 检查提升为可证明“已登记 App 内部合同”的只读一致性审计，避免 freshness=fresh 但 relation 或页面字段已漂移时误报通过。
