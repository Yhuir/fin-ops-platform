...
| SSE first-event smoke | `tests/test_sse_smoke_probe.py` | 覆盖 `/api/app-health/stream` 的 event-stream 首事件 SLO、auth 缺失、HTML fallback、错误状态码和事件名校验；关联台页面 SSE endpoint 已删除并由默认 probe/测试锁定不再出现。 |
...
- SSE smoke -> `/api/app-health/stream` 必须返回 `text/event-stream`，首事件 p95 目标按 `<= 1000ms` 解释；关联台不再提供 `/api/workbench/events`。缺 token/cookie 返回 `auth_missing`，HTML fallback、错误事件名、首事件超时或零 probe 均失败；最终 `runtime_sync_closure_gate` 必须把 `sse_first_event_smoke` 纳入必经检查，并用 `sse_smoke_empty_samples` 拒绝空证据。
