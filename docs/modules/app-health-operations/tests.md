...
| HTTP routes | `server.py` `/api/app-health*`、`/api/operations/app-health-dashboard`、统一 `/api/operations/app-health/page-audit`、`/api/operations/app-health/input-invoice-usage-refresh`、`/api/operations/app-health/output-invoice-collection-refresh` | auth guard、SSE contract、dashboard admin-only、cache stale after refresh error、页面审计 admin-only/只读/fail-closed、进项/销项刷新 admin-only 且只通过 runtime queue 入队 |
