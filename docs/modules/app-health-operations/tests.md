...
| 层级 | 当前入口 | 回归风险 |
| --- | --- | --- |
| App Health page | `web/src/pages/AppHealthOperationsPage.tsx` | admin-only、只读 dashboard、进项使用 Audit icon 只读按钮、刷新失败保留旧 payload、unknown 显示 `--` 而不是 0 |
...
| HTTP routes | `server.py` `/api/app-health*`、`/api/operations/app-health-dashboard`、`/api/operations/app-health/input-invoice-usage-audit`、`/api/operations/app-health/output-invoice-collection-audit`、`/api/operations/app-health/input-invoice-usage-refresh` | auth guard、SSE contract、dashboard admin-only、cache stale after refresh error、进项/销项审计 admin-only 且只读、进项使用刷新 admin-only 且只通过 runtime queue 入队 |
...
| Operations dashboard data inventory | `tests/test_operations_dashboard_service.py` | 覆盖 dashboard 发票 inventory 从 canonical `app.invoices.source_links` 统计 `manual` / `oa_attachment`，从 `app.invoices.invoice_type` 统计 `input_invoice` / `output_invoice`，`oa_attachment.supplementary_count` 统计 OA 解析进入发票池但不在手工导入中的数量；OA inventory 上次同步时间优先使用 `app.oa_sync_runs(sync_type='oa_projection')` 成功 run，已完成 OA 按 `app.oa_applications` 完成态统计，进行中 OA 按 OA 待付款 read model all-scope 的 `viewCounts.in_progress` 等价唯一 OA ID 统计并防止回退到 `app.oa_applications.workflow_status`；同时覆盖导入历史只包含手工银行流水和发票批次，以及 import history 查询失败时只降级 history 不阻断总览 |
| 进项使用全量审计 API | `tests/test_app_health_api.py`、`tests/test_audit_input_invoice_usage_read_model_tool.py` | 覆盖 `/api/operations/app-health/input-invoice-usage-audit` admin-only、无 PostgreSQL 连接 fail-closed、admin 返回只读三边对账报告、发现 relation/read model 问题时输出 `issues_found` 而不写数据；工具单测覆盖缺失发票、重复 row、金额不一致、active relation 未投影、跨 scope relation 漏分发、candidate 误入等 invariant。 |
| 销项收款全量审计 API | `tests/test_app_health_api.py`、`tests/test_audit_output_invoice_collection_read_model_tool.py` | 覆盖 `/api/operations/app-health/output-invoice-collection-audit` admin-only 只读报告，复用真实 PostgreSQL runtime connection 对账 canonical 销项发票、`output_invoice_collection` rows/scopes 和 `workbench_relation` rows/groups/scopes；工具单测覆盖缺失发票、重复 row、金额不一致、active relation 未投影、跨 scope relation 漏分发、candidate 误入等 invariant。 |
| 进项使用受控刷新 API | `tests/test_app_health_api.py` | 覆盖 `/api/operations/app-health/input-invoice-usage-refresh` admin-only、runtime queue 缺失 fail-closed、scope policy 拒绝非法 scope、合法 scope 去重后通过 `ReadModelRefreshGateway` 入队 high-priority `input_invoice_usage.read_model.refresh`，不直接写 read model 或 relation。 |
...
- admin 调用 `/api/operations/app-health/input-invoice-usage-audit` -> 后端用自身 PostgreSQL 连接只读审计 `app.invoices`、`app.workbench_pair_relations`、`read_model.input_invoice_usage_*`、`read_model.workbench_relation_*`、`job.read_model_dirty_scopes` -> `overall_status=pass` 且 `blocking_issue_count=0` 才能作为真实库进项使用页面全量数据和配对关系正确的证据；非 admin 403、无 PostgreSQL 连接 503、审计发现问题返回 200 + `issues_found`，不写入修复。
- admin 调用 `/api/operations/app-health/output-invoice-collection-audit` -> 后端用自身 PostgreSQL 连接只读审计 `app.invoices`、`app.workbench_pair_relations`、`read_model.output_invoice_collection_*`、`read_model.workbench_relation_*`、`job.read_model_dirty_scopes` -> `overall_status=pass` 且 `blocking_issue_count=0` 才能作为真实库销项收款页面全量数据和配对关系正确的证据；非 admin 403、无 PostgreSQL 连接 503、审计发现问题返回 200 + `issues_found`，不写入修复。
