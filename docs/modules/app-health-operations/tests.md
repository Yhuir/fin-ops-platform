...
| HTTP routes | `server.py` `/api/app-health*`、`/api/operations/app-health-dashboard`、`/api/operations/app-health/input-invoice-usage-audit`、`/api/operations/app-health/output-invoice-collection-audit`、`/api/operations/app-health/page-audit`、`/api/operations/app-health/input-invoice-usage-refresh`、`/api/operations/app-health/output-invoice-collection-refresh` | auth guard、SSE contract、dashboard admin-only、cache stale after refresh error、进项/销项审计 admin-only 且只读、页面业务审计 admin-only 且只读、进项/销项刷新 admin-only 且只通过 runtime queue 入队 |
...
- admin 点击 7 个页面标题右侧 Audit icon 或调用 `/api/operations/app-health/page-audit?domain=<domain_key>` -> 后端用自身 PostgreSQL 连接只读审计页面 source facts、read model rows/scopes/source_versions、durable refresh state 和相关 Workbench relation 分发 -> `overall_status=pass` 且 `blocking_issue_count=0` 才能作为对应页面 App 内部全量数据和配对关系正确的证据；非 admin 403、未知 domain 400、无 PostgreSQL 连接 503、审计发现问题返回 200 + `issues_found`，不写入修复。该证据不替代外部银行/OA 系统来源完整性证明。
