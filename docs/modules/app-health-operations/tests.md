...
| Operations dashboard data inventory | `tests/test_operations_dashboard_service.py` | 覆盖 dashboard 发票 inventory 从 canonical `app.invoices.source_links` 统计 `manual` / `oa_attachment`，`oa_attachment.supplementary_count` 统计 OA 解析进入发票池但不在手工导入中的数量；同时覆盖流水、手工发票、OA 解析和 OA 单据同步导入历史，以及 import history 查询失败时只降级 history 不阻断总览 |
...
- dashboard 发票来源只展示 `手工导入` 和 `OA 解析`；`OA 解析` 来自 canonical invoice `source_links[].source_type='oa_attachment_invoice'`，括号内数量是 OA 解析来源且不含 `manual_invoice_import` source link 的发票数。`普通导入` 和 `ETC` 不再作为 App Health 发票来源展示；OA 附件 OCR cache 不作为 dashboard 发票 inventory 事实源。
- dashboard 主页面只展示最新 5 条导入历史，右侧抽屉展示全量历史；每条数量使用真实成功导入/同步数，不用预览候选数或附件总数。
