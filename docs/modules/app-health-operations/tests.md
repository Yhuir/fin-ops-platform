...
| HTTP routes | `server.py` `/api/app-health*`、`/api/operations/app-health-dashboard`、统一 `/api/operations/app-health/page-audit` | auth guard、SSE contract、dashboard admin-only、整体构建失败时 cache stale fallback、局部指标失败时仍刷新当前 inventory/import status、页面审计 admin-only/只读/fail-closed；已退休页面 refresh routes 必须保持 `404` 且零 queue write |
