...
- admin 点击 7 个页面标题右侧 Audit icon 或调用 page audit -> 同一 service/repository 边界检查 source/read model/source_versions/durable refresh/relation；dirty/outbox 查询按 tenant 隔离，返回 integrity/freshness/queue 三类状态。unknown 页面状态不得显示 Fresh，样本截断必须显式标记。
