...
| 日期 | 变更 | 状态机影响 | 测试/验证 |
| --- | --- | --- | --- |
| 2026-07-22 | Phase 27 删除 OA sync/import-state/普通写 Search fan-out | OA sync、import confirm 与普通关系/规则写只推进 canonical source version；`/api/search` 访问发现 mismatch 后经 gateway enqueue 当前精确 scope。以下 2026-06/07 条目保留为历史演进记录，不是当前生产路径。 | write-operation impact/SLO、Search API/worker、architecture guards |
