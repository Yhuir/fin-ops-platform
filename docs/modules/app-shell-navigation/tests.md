...
| 5. Frontend component and interaction tests | 适用，已补 | `PageRouteHost.test.tsx`、`AppSidebar.test.tsx`、`App.test.tsx`、`SessionGate.test.tsx`、`PageSessionStateContext.test.tsx`、`useFinanceTableSession.test.tsx` | 覆盖 route、sidebar、compact drawer、session gate、page session、table session 和 full shell smoke。 |
