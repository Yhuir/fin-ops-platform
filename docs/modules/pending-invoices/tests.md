...
| 前端交互 | P1 | `web/src/test/PendingInvoicesPage.test.tsx`、`web/e2e/pending-invoices-fanout.spec.ts`、`web/e2e/pending-invoices-filter-sort-flow.spec.ts`、`web/e2e/pending-invoices-attach-existing-flow.spec.ts`、`web/e2e/pending-invoices-income-status-flow.spec.ts`、`web/e2e/pending-invoices-export-download.spec.ts`、`web/e2e/pending-invoices-rules-save-flow.spec.ts` | covered | four-zone table、filters、状态多选筛选、首屏默认 `已支付待开票`、rules drawer、conflict、detail drawers、选中工具栏 attach existing、候选流水关联 chip、preview 冲突原因和禁用确认、收入批量标记、refreshing 时选择栏可用、导出下载错误消息可见；Browser e2e 覆盖真实导航、Workbench confirm 弹窗、返回待找发票后的行状态、状态过滤保留、对方户名列筛选、金额升/降序排序、rows 首屏暂时加载失败后的错误态空行文案/导出禁用/刷新恢复、规则 drawer 保存后 pending read model barrier/rows refresh、规则保存暂时失败时抽屉内错误/草稿保持/无全局阻塞/重试恢复、选择已有发票抽屉 candidates/preview/confirm/conflict、attach confirm 暂时失败后的错误提示/无半写/重试恢复、收入批量标记成功/拒绝/暂时失败重试，以及关联后导出预览/下载包含 OA、发票和 relation 字段、row-limit 失败反馈。 |
