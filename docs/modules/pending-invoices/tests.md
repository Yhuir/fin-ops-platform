...
| 多关系成员展示 | `workbench_relation` distribution、pending invoice rows payload | `bank_transactions`、`input_invoices`、`oa` 分区必须按 relation 成员聚合；多流水时银行栏展示真实对方户名列表且不显示交易时间，不用 `+N` 替代户名；多发票/多 OA 仍用 `+N` 表示该类型全部成员；不能同时展示 primary，也不能把多流水成员重复输出为 standalone 行。 |
...
| source-version 合同对齐 | P0 | `tests/test_search_pending_sql_runtime.py::SearchPendingSqlRuntimeTests::test_pending_invoice_writer_and_api_source_version_contracts_match`、`tests/test_search_pending_sql_runtime.py::SearchPendingSqlRuntimeTests::test_pending_invoice_repository_ignores_zero_row_historical_shards_for_aggregate_source_versions`、`tests/test_search_pending_sql_runtime.py::SearchPendingSqlRuntimeTests::test_pending_invoice_filtered_scope_keeps_zero_row_month_dependency_proofs` | covered | API expected-source 与 SQL projection writer 稳定包含 `invoice_lifecycle_policy_schema_version`、`bank_detail_source_versions`、`workbench_relation_source_versions`；aggregate scope freshness 不得被零行历史 shard 或旧父 scope metadata 的 source versions 污染，同时保留 filtered 合法零行月份的 dependency proof。 |
