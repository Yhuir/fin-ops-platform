...
| `PENDING-E2E-003` | `covered` | `web/e2e/workbench-relations-candidate-semantics.spec.ts`、后端 relation/read model tests | Browser/后端回归已证明未正式化 decision 或历史 candidate 兼容值不驱动 linked-only 状态，也不产生 mutation；待找发票只把 active linked relation 当作已关联证据。 |
