...
| `PENDING-E2E-003` | `covered` | `web/e2e/pending-invoices-fanout.spec.ts`、`tests/test_pending_invoice_service.py` | Browser/后端回归已证明待找发票只把 active linked relation 当作已关联证据，非正式分布输入被忽略且不产生 mutation。 |
| `PENDING-E2E-004` | `covered` | `web/e2e/workbench-relations-nonfresh-diagnostics.spec.ts`、`web/src/test/PendingInvoicesPage.test.tsx`、`tests/test_pending_invoice_canonical_query.py` | Browser/组件覆盖 canonical rows 单次请求、无 read-model 诊断/轮询和真实空集；后端覆盖只读一致性 snapshot 与合法空集。 |
...
| `PENDING-E2E-009` | `covered` | `web/e2e/pending-invoices-rules-save-flow.spec.ts`、`web/src/test/PendingInvoicesPage.test.tsx`、`web/src/test/PendingInvoicesRulesSaveTimeout.test.tsx`、后端 rules tests | Browser 覆盖规则保存和当前 rows 重新 GET；组件断言不调用 write-time freshness barrier。503 时草稿保留且不刷新 rows，重试成功后才读取 canonical rows。 |

## Operation latency baseline

Playwright latency 附件覆盖页面打开、加载失败重试、筛选排序、relation fan-out、导出、候选/preview/confirm、收入批量状态和规则保存。canonical 单次请求与真实空集由 `workbench-relations-nonfresh-diagnostics.spec.ts` 直接断言。

## 下一轮补测建议

1. 在 staging/真实 infra 中补 attach existing、income status 和 rules save 后 canonical GET 的 PostgreSQL 并发/锁等待/代理超时证据。
2. 继续验证 50k 以上最大 scope 的 rows/statistics/facets 与导出耗时。
