...
| `PENDING-E2E-009` | `covered` | `web/e2e/pending-invoices-rules-save-flow.spec.ts`、`web/src/test/PendingInvoicesPage.test.tsx`、`web/src/test/PendingInvoicesRulesSaveTimeout.test.tsx`、`web/src/test/GlobalOperationOverlayContext.test.tsx`、后端 rules tests、`tests/test_read_model_architecture_guards.py` | Browser 覆盖支出规则抽屉保存、`PUT /api/pending-invoices/rules` contract、当前 rows normal GET/刷新中提示，以及成功后没有保存失败/同步失败/read model 失败残留；第一次保存暂时 503 时抽屉内错误可见、草稿保持、全局操作弹窗不阻塞且不触发 rows 刷新，第二次重试成功后才刷新。后端/guard 证明 rule write 零页面 job，支出/收入版本隔离、conflict 和访问时 exact-scope 收敛。 |
