...
- rows、filter-options、export-preview、export 必须读取同一 canonical PostgreSQL 口径；成功零行是真实空态，失败必须显示 error，不能 fallback 到旧 read model。
- 支出/收入/全部方向、状态多选、关键字、列筛选、排序和分页必须只改变查询口径，不改变事实状态。
- OA/流水/发票 relation 不是本页私有事实；linked 已关联证据只来自 active canonical pair relation，并排除 `turnover_manual_closure`；未正式化 decision 不作为关系状态。
...
| `PENDING-E2E-004` | canonical loading/empty/error | 成功 rows 只请求一次且不显示 read-model 诊断或定时轮询；成功零行显示真实空态；请求错误显示错误态并禁用导出。 |
...
| `PENDING-E2E-006` | 导出失败或超限 | 后端 row-limit / canonical query / 下载错误必须显示结构化错误；不能显示下载成功。 |
...
| `PENDING-E2E-009` | 规则保存 | 支出/收入规则版本独立；保存成功后直接重跑 canonical rows GET，不等待页面 read-model barrier；保存暂时失败时显示抽屉内错误、保留草稿、允许重试、不触发 rows 刷新。 |

## Canonical read 合同

- Browser mock 不得为 pending-invoice rows/rules/export 返回 `read_model_status/source_versions` 或 202。
- 后端/API 测试必须覆盖只读 snapshot、canonical relation、非法参数、权限、空集、写后重新 GET 和无旧 gate/enqueue。
- 本地 Browser E2E 证明页面行为；真实 PostgreSQL 大数据、并发、锁等待和代理超时仍需 staging/运维 smoke。
