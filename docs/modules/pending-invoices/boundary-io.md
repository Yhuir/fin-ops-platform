...
| 页面只读 Audit | `PageBusinessAuditIcon` / AppHealth operations API | admin-only 调用 `page-audit?domain=pending_invoices`；以 active 银行流水及 collapsed relation 内全部银行成员为 independent canonical expected-set；折叠成员的方向/月必须分别回到 canonical 银行流水重算，不能继承主展示行方向，主行方向、成员金额/交易日/对方户名及 status 结构仍需一致；同时要求共享 relation 双向 edge equality 与只读一致性快照；页面只消费结构化 integrity/freshness/queue 与有上限 issue samples，不直接读取审计表或触发修复 |
