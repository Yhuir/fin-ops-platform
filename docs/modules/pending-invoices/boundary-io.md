...
| 关联/规则写入 | pending invoice services | 写后触发 pending_invoice/search/invoice_lifecycle 相关 scope；关联台撤回若没有 live bank direction，只能把该月 expense+income 两个 pending scope 一并声明为受影响，禁止用绝对金额符号推断方向 |
