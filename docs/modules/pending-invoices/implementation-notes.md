...
- 进程级采样显示并发 4 会产生超过请求数的高 CPU PostgreSQL 执行进程；rows 主查询在同一 request-scoped transaction 内把 `max_parallel_workers_per_gather` 设为 `0`，避免 HTTP 并发与查询并行相乘。候选、详情、写链和全局数据库配置不受影响。
