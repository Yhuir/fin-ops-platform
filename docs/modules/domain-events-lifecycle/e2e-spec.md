...
| --- | --- | --- |
| `DOMAIN-E2E-001` | 页面 A 执行业务写入后，受影响页面 B/C 能刷新或提示刷新，但不会把前端 event 当成最终事实。 | 页面重新读取 API/read model；event 只触发刷新。 |
...
| `DOMAIN-E2E-004` | import、relation、tax、settings、no-OA、turnover 等高 fan-out 动作必须把 affected read models 传到 runtime/read model 边界。 | lifecycle -> dirty/outbox -> worker -> page fresh 证据。 |
