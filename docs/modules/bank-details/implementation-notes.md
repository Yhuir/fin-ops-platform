...
## 2026-08-08 - 标签筛选平衡布局与非模态滚动

- 目标：消除银行明细标签筛选中由等高 Grid 行造成的大面积留白，并允许筛选框打开时在框外滚动底层流水。
- 影响范围：仅银行明细 HeroUI 标签筛选 Popover、专属 CSS 和前端测试；API、canonical facts、分类保存、权限、审计、read model/worker 和其他页面不变。
- 关键决策：保留 HeroUI Popover/ListBox 语义，使用原生非模态 Popover 和 CSS 平衡多列；标准桌面无内部纵向滚动，窄屏或低高度视口保留单列滚动兜底；不增加滚轮监听、DOM 高度测量或 JavaScript 瀑布流。
- 文档影响：仅记录本模块 UI 决策；产品、API、模块 boundary I/O 和运行时合同不变。
- 测试覆盖：更新 BankDetailsPage 组件合同和大表 Browser flow，覆盖多列完整可见、无桌面内滚动、框外滚动底层流水、窄屏兜底及既有分类菜单回归。
- 验证命令：见本轮最终说明。
- 未测风险：标准桌面以当前 active 标签容量验收；未来标签数量超出物理视口容量时由既有窄屏/低高度兜底保证可访问。
- 后续事项：无；只有标签容量突破桌面一屏合同且真实使用受影响时，才重新评估分组信息架构。
...
## 2026-08-10 - 内部转账候选等值连接

- 生产执行计划显示内部转账候选在同一个 materialized base 上执行嵌套扫描，数据库主查询约 `277ms`。现于 base 一次计算金额键，并先拆分支出/收入候选，再以金额键等值连接。
- 生产事实源只读 A/B 的数据库执行约降至 `148ms`；内部转账金额、账户差异、两日时间窗、公司户名和明确标记规则均不变。未新增索引、缓存、read model、worker、依赖或 API 字段。
