...
| read model refresh worker | P0 | `tests/test_bank_details_sql_runtime.py`、`tests/test_bank_detail_available_month_scope_provider.py` | covered | `all` fan-out 到月份 shard；月份 scope rebuild 后按 source version complete；available-month provider 固定从银行流水日期字段提取 `YYYY-MM` scope 并在无月份时回退 `all`。 |
