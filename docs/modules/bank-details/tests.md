...
- 新增/更新测试：`BankDetailSqlProjectionBuilderTests.test_relation_source_summary_tracks_cross_month_members_by_legacy_and_canonical_id`、`test_rebuild_passes_scope_legacy_and_canonical_ids_to_relation_source_summary`、`test_bank_detail_audit_uses_uuid_identity_and_stable_source_versions`，并将 bank-detail schema 合同提升到 v10。
...
- Audit 独立性：AppHealth expected-side 从 canonical bank legacy/UUID identities 与 active relation row membership 重新聚合 summary，不读取 projection 的 row tags/case 作为 expected 值，也不复用 builder helper。
...
- 覆盖点：银行明细关系标签投影只能读取 `WorkbenchRelationReadFacade.get_by_row_ids(...)` 的 relation distribution，不允许恢复 Application 私有整页 Workbench 读取作为 fallback。
- 验证命令：见本轮最终说明。
...
| 前端交互 | `BankDetailsPage`、`web/src/features/bankDetails/api.ts` | loading/empty/error/stale/refreshing、drawer、filter、pagination、export、权限、当前页写后 normal GET、跨页/浏览器生命周期零自动 refetch。 |
...
| 分类/规则写入持久化和 refresh fan-out | P0 | `tests/test_bank_auto_tag_rules_api.py`、`tests/test_bank_details_sql_runtime.py`、`tests/test_bank_detail_read_model_refresh_producer.py`、`tests/test_platform_runtime_boundary_guards.py` | covered | 版本冲突、明确 category store、side-effect port、bank-detail/turnover refresh、queue unavailable 失败和 owner 边界；refresh producer 测试固定 bank detail enqueue 只能走 gateway，Redis 只作为 optional wakeup。 |
...
| read model refresh worker | P0 | `tests/test_bank_details_sql_runtime.py`、`tests/test_bank_detail_available_month_scope_provider.py`、`tests/test_bank_detail_derived_lifecycle_executor.py` | covered | `all` fan-out 到月份 shard；月份 scope rebuild 后按 source version complete；available-month provider 固定从银行流水日期字段提取 `YYYY-MM` scope 并在无月份时回退 `all`；derived lifecycle executor 固定显式月份优先、`all` fan-out、默认 `all` 和 enqueue payload shape。 |
...
| 关系标签投影 | P0 | `tests/test_bank_details_service.py`、`tests/test_bank_details_sql_runtime.py`、`web/e2e/workbench-relation-fanout.spec.ts` | covered | relation distribution row、OA/invoice-only 边界、失败降级、不读 legacy candidate matches；Browser e2e 覆盖未正式化 decision / 历史 candidate 兼容值不显示 linked 标签，关联台 confirm 后页面显示 `有oa`/`有发票`。 |
...
| 2. Service-layer tests | 适用 | `tests/test_bank_details_service.py`、`tests/test_bank_details_export_service.py`、`tests/test_bank_auto_tag_rules_api.py`、`tests/test_bank_details_sql_runtime.py` | 覆盖 service 编排、relation provider、导出、明确 category store、side-effect port、审计和 refresh 失败。 |
...
| 5. Frontend component and interaction tests | 适用 | `web/src/test/BankDetailsPage.test.tsx`、`web/src/test/BankDetailsApi.test.ts`、`web/src/test/PageRouteHost.test.tsx`、`web/e2e/bank-details-initial-state.spec.ts`、`web/e2e/bank-details-stale-refreshing.spec.ts`、`web/e2e/bank-details-filtered-export-permissions.spec.ts`、`web/e2e/bank-details-large-scroll-flow.spec.ts`、`web/e2e/bank-details-category-flow.spec.ts`、`web/e2e/bank-details-auto-tag-rules-flow.spec.ts` | 覆盖页面加载、默认当前年 query、账户余额、默认列、fresh 空态、筛选、年份/月度/全部时间筛选、分页、长列表/宽字段/窄屏/菜单遮挡、drawer 保存/重应用、候选确认/撤销、人工补分类/清除、导出、当前页写后 normal GET、focus/visibility/BFCache/跨页事件零自动 refetch、stale/refreshing/schema_mismatch/missing/false-empty/network recovery/abort，以及权限回归。 |
| 6. End-to-end business-flow integration tests | 适用 | `tests/test_bank_auto_tag_rules_api.py`、`tests/test_bank_details_sql_runtime.py`、Workbench/no-OA/turnover/import 相关模块测试、`web/e2e/workbench-relation-fanout.spec.ts`、`web/e2e/imports-bank-transactions-flow.spec.ts`、`web/e2e/bank-details-initial-state.spec.ts`、`web/e2e/bank-details-export-download.spec.ts`、`web/e2e/bank-details-stale-refreshing.spec.ts`、`web/e2e/bank-details-filtered-export-permissions.spec.ts`、`web/e2e/bank-details-large-scroll-flow.spec.ts`、`web/e2e/bank-details-category-flow.spec.ts`、`web/e2e/bank-details-auto-tag-rules-flow.spec.ts` | 本模块现有集成以真实 application service、side-effect port、read model lifecycle 和 API 为主；Browser e2e 覆盖银行明细首屏账户余额/默认列/fresh 空态、Workbench confirm 后银行明细 relation tags fan-out、银行流水导入确认后银行明细显示导入行、confirm 后导出文件包含 linked relation 字段、当前账户 + 月度 + 关键字 + 分类筛选导出、分页状态不限制导出、长列表/窄屏关键操作、只读导出权限、denied/expired session gate、admin 分类写入、自动标签规则保存/reapply freshness、候选确认/撤销、外部往来三层人工补分类/清除，以及非 fresh read model 页面诊断、account retry、network recovery 和导出业务错误；真实导入到更多页面完整 smoke 仍归 staging/nightly 风险项。 |
...
| 长期 | 标签/分类写入成功但 refresh fan-out、审计或 Workbench invalidation 未执行。 | `tests/test_bank_auto_tag_rules_api.py`、`tests/test_bank_details_sql_runtime.py`、`tests/test_platform_runtime_boundary_guards.py` | covered |
...
| 2026-06-24 | 分类写后副作用回流到 `Application._after_bank_category_confirmation_mutation`，导致 refresh、turnover fan-out、Workbench invalidation 或 audit 绕过显式 side-effect port。 | `tests/test_platform_runtime_boundary_guards.py::PlatformRuntimeBoundaryGuardTests::test_bank_detail_server_read_cache_helpers_stay_on_application_service_boundary`、`tests/test_bank_details_sql_runtime.py::BankDetailSqlRepositoryTests::test_category_mutation_side_effect_port_suppresses_fallback_enqueue_audit_and_invalidate`、`tests/test_bank_auto_tag_rules_api.py::BankAutoTagRulesApiTests::test_bank_category_mutation_side_effect_port_enqueues_turnover_ledger_all_refresh` | covered |
...
6. `关联台确认/撤回 -> workbench relation distribution -> bank details relation tag projection -> 重新进入银行明细或手动刷新后 refetch`。当前 Browser e2e 覆盖 confirm 后回到银行明细显示 `有oa` / `有发票`；withdraw 仍待补。
...
PYTHONPATH=backend/src python3 -m unittest tests.test_bank_auto_tag_rules_api tests.test_bank_details_routes tests.test_platform_runtime_boundary_guards -v
