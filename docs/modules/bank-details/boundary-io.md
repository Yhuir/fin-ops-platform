...
| 标签/分类写操作 | route/service | PostgreSQL 通过 `BankTransactionCategoryMutationWriter` 在同一事务锁定 canonical 银行流水、定向更新分类/确认事实、写审计并批量写受影响 month scope dirty/outbox；local runtime 保留本地 snapshot port |
...
| 标签副作用 | relation/downstream read models | canonical 写入、persistent audit、`bank_detail` / `bank_flow_rule_batch` / `workbench` / `workbench_relation` / `invoice_lifecycle` / `cost_statistics` / `search` / `turnover_ledger` / `pending_invoice` dirty/outbox 在同一事务提交；Workbench matching 只写精确月份扩展窗口。`bank-flow-rule-batches` 只能读取 active 标签并维护自身 OA/发票规则 |
