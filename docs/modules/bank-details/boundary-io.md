...
| 标签/关系/source proof | `bank_detail` query/worker + canonical relation repository port | projection schema v12 在每个已发布 row payload 保留 canonical `bank_transaction_updated_at`，供下游按明确 transaction IDs 做逐行 OCC；month scope 同时保存稳定 `bank_transaction_category_source_signature`、canonical `workbench_relation_source_versions`、本月 canonical bank row count、自动分类所需 `month ± 2 days` context row count 与 canonical bank `max(updated_at)`。query repository 分别用一次 set-based SQL 重算银行事实、标签签名与关系 summary；银行行新增/删除/更新或相邻两日上下文变化、标签 mismatch、关系 mismatch 任一成立都把 exact `bank_detail` month 判为 non-fresh，旧 scope 缺少新增 proof 时同样 fail closed。关系 summary 按 `month_scope == month OR active relation row_ids overlap 当月银行流水 ids` 纳入跨月 eligible relation，并明确排除不会生成银行页面关系标签的 `turnover_manual_closure`。该 summary 已完整证明页面消费的关系标签来源，页面应用服务只允许入队失配的 `bank_detail` 月份，不再查询、等待或入队共享 `workbench_relation` distribution；全部 proof 均按请求 scopes 批量计算，不逐月 N+1。自动规则另比较当前 settings rule version 与 scope `bank_auto_tag_rules_version`。全部 proof 都在 Redis 读取前完成，禁止 stale cache 冒充 fresh |
...
- 2026-07-24 精确 scope：`bank_detail_scope_summary(...)` 在每个 scope signature 中返回 effective `freshness_status`；downstream tag consumer 只补投真正 source/schema drift 的月份，不再因整体 payload stale 回退 enqueue 请求中的全部月份。
- 2026-07-24 canonical bank proof：页面 query 在一次 set-based SQL 中比较本月 canonical row count、`month ± 2 days` 自动分类上下文 row count 与 `max(updated_at)`；修复 `app.bank_transactions` 已变化但旧 projection 仍被判 fresh 的假新鲜路径，不增加写后 fan-out。
- 2026-07-24 rolling baseline：migration `0124_bank_detail_canonical_source_proof.sql` 只为 projection row_count 已与 canonical 相等的历史 scope 补齐 context/update proof；row_count 不一致的 scope 保持缺失并由访问 gate 判 stale。迁移不写 dirty/outbox、不触发 worker，避免发布后首次无日期页面访问把 proof 升级扩大成全历史重建。
- 2026-07-24 账户余额 proof：schema v2 的 `bank_account_balance:all` projection 保存参与余额计算的 canonical 流水数量与 `max(updated_at)`；账户列表读取在既有 scope/dirty gate 内重算并比较这两个字段。新增、删除或更新银行流水后即使 writer 零 fan-out，下一次银行明细访问也必须把余额 projection 判 stale 并只 enqueue `bank_account_balance:all`，禁止继续仅凭 scope `status=fresh` 返回旧余额。
