...
| 标签 source proof | `bank_detail` query/worker | projection schema v11 在每个 month scope 保存稳定 `bank_transaction_category_source_signature`；query repository 以一次 set-based SQL 从 active category + confirmation canonical facts 重算当前签名，任一 mismatch 都返回 stale 并 enqueue exact month。自动规则另由 application service 比较当前 settings rule version 与 scope `bank_auto_tag_rules_version`。两条 proof 都在 Redis 读取前完成，禁止 stale cache 冒充 fresh |
