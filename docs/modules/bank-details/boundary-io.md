...
| 标签/关系/source proof | `bank_detail` query/worker + `WorkbenchRelationReadFacade` | projection schema v11 在每个 month scope 保存稳定 `bank_transaction_category_source_signature`、canonical `workbench_relation_source_versions`、本月 canonical bank row count、自动分类所需 `month ± 2 days` context row count 与 canonical bank `max(updated_at)`。query repository 分别用一次 set-based SQL 重算银行事实、标签签名与关系 summary；银行行新增/删除/更新或相邻两日上下文变化、标签 mismatch、关系 mismatch 任一成立都把 exact `bank_detail` month 判为 non-fresh，旧 scope 缺少新增 proof 时同样 fail closed。关系 summary 按 `month_scope == month OR active relation row_ids overlap 当月银行流水 ids` 纳入跨月关系。页面应用服务同时通过 `source_versions_for_scopes(...)` 一次批量校验共享 `workbench_relation` scopes，并经正式 gateway 只入队失配月份；全部 proof 均按请求 scopes 批量计算，不逐月 N+1，也不让银行 worker 等待 relation distribution。自动规则另比较当前 settings rule version 与 scope `bank_auto_tag_rules_version`。全部 proof 都在 Redis 读取前完成，禁止 stale cache 冒充 fresh |
