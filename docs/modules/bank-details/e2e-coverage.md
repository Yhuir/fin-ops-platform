...
| `BANK-E2E-006` | `covered` | `web/e2e/bank-details-auto-tag-rules-flow.spec.ts`、`web/src/test/BankDetailsPage.test.tsx`、`web/src/test/GlobalOperationOverlayContext.test.tsx` | Browser 已覆盖 drawer 保存 PUT 请求体、`expected_version`、当前可见月份 affected scope、重新应用不触发保存、保存/重应用后直接刷新银行流水且不请求旧操作屏障；Vitest 覆盖同一 direct reload 合同。 |
...
| `BANK-E2E-008` | `covered` | `web/e2e/bank-details-stale-refreshing.spec.ts`、`web/src/test/BankDetailsPage.test.tsx`、`web/src/test/BankDetailsApi.test.ts` | Browser 已覆盖 direct rows 不显示 旧投影诊断、direct empty rows 显示空态且导出仍按 API 返回、交易请求网络失败后用户触发重试恢复；Vitest/API 覆盖旧投影同步字段被前端 mapper 忽略。 |
...
- 新增 `web/e2e/bank-details-initial-state.spec.ts` 保护首屏业务合同：默认当前年范围必须请求 accounts/transactions，全部账户视图必须显示总余额、账户余额、默认列、候选 relation tags、自动分类和 direct 空态。
...
- 更新 `web/e2e/bank-details-stale-refreshing.spec.ts` 为 direct payload 和恢复合同：页面不显示旧投影同步诊断，direct 空 rows 显示空态且导出可继续走 API，交易请求网络失败后用户重试应恢复当前 rows。
...
- 模块 Browser Spec ID `BANK-E2E-001..010` 已覆盖；银行明细本地 deterministic 下载已覆盖 XLSX workbook 解析，真实生产代理 header、Excel/Numbers 打开结果、真实后台任务收敛和生产超大数据性能属于 staging/专项风险。
