...
| `BANK-E2E-006` | `covered` | `web/e2e/bank-details-auto-tag-rules-flow.spec.ts`、`web/src/test/BankDetailsPage.test.tsx`、`tests/test_bank_auto_tag_rules_api.py` | Browser 已覆盖 Drawer 保存 PUT 请求体、`expected_version`、保存/reapply 成功后当前 transactions normal GET、reapply 不提交规则，以及两个动作均零 operation barrier/页面 fan-out。相关操作均记录 latency。 |
| `BANK-E2E-007` | `covered` | `web/e2e/bank-details-category-flow.spec.ts`、`tests/test_bank_auto_tag_rules_api.py`、`tests/test_bank_transaction_category_service.py`、`web/src/test/BankDetailsPage.test.tsx` | Browser 已覆盖本页获权账号只能确认当前候选、确认后刷新为 `manual_confirmed`、撤销回候选状态，以及 unmatched 行外部往来三层人工补分类、请求结构化 turnover 字段、清除后回到待分类；保存和撤销/清除已记录 operation latency。 |
...
- `web/e2e/bank-details-stale-refreshing.spec.ts` 文件名保留在现有 inventory 中，但内容已切换为 direct-query resilience：证明无 freshness polling、空结果真实、网络失败可由用户重试恢复。
- `web/e2e/bank-details-filtered-export-permissions.spec.ts` 保护筛选导出和页面权限合同：当前账户、月度时间筛选、关键字、分类必须同时带入交易请求和导出请求；page size/翻页只影响列表请求，导出按当前筛选全量导出；本页获权账号可执行正常业务操作；forbidden/expired session 不渲染银行明细页且不调用 protected API。
...
1. 继续从全局 inventory 推进 `workbench-relations` 导出权限/筛选组合、OA pending linked access convergence 或更多撤销链路。
2. 推进 `imports-bank-transactions` 账户冲突、重复行、任务失败和更多下游页面访问收敛。
