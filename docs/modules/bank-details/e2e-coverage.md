...
| `BANK-E2E-006` | `covered` | `web/e2e/bank-details-auto-tag-rules-flow.spec.ts`、`web/src/test/BankDetailsPage.test.tsx`、`web/src/test/GlobalOperationOverlayContext.test.tsx`、`tests/test_read_model_architecture_guards.py` | Browser 已覆盖 Drawer 保存 PUT 请求体、`expected_version`、成功后当前 transactions normal GET、重新应用不触发保存，以及显式 reapply job blocked 时显示 warning 而非“操作失败”；后端/guard 证明普通规则保存零页面 fan-out，只有显式 reapply 可等待 exact month target。相关操作均记录 latency。 |
