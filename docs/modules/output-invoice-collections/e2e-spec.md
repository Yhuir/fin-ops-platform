...
| `OUT-COLL-E2E-002` | 手动收款状态/提醒 -> rows refresh | P0 | 保存状态和提醒后，页面重新请求 rows，行状态更新为后端返回结果，drawer 关闭且失败不静默吞掉。 |
| `OUT-COLL-E2E-003` | 正式收据 preview/create/history | P0 | 预览展示收据信息，创建必须带 idempotency key；创建后 rows refresh，history 展示真实 receipt fact。 |
