...
| title Audit icon / real data audit | `web/src/test/OutputInvoiceCollectionsPage.test.tsx`、`tests/test_app_health_api.py`、`tests/test_audit_output_invoice_collection_read_model_tool.py` | admin 用户标题右侧显示 Audit icon；点击只调用 `/api/operations/app-health/output-invoice-collection-audit` 只读 GET，并展示 Audit 成功、全部数据正确、全部配对关系正确和 Fresh。真实库仍必须以 audit payload `overall_status=pass` 且 `blocking_issue_count=0` 作为证明；工具对账 canonical 销项发票、`output_invoice_collection` rows/scopes、`workbench_relation` rows/groups/scopes、dirty scopes、source_versions、重复/孤儿/缺失 member、金额合计、跨 scope relation 分发和 candidate 误入。 |
...
| 红蓝票关系 | `tests/test_output_invoice_collection_lifecycle.py::test_red_relation_overlay_adds_manual_evidence`、`tests/test_output_invoice_collection_api.py::OutputInvoiceCollectionApiTests::test_lifecycle_write_routes_overlay_rows_and_create_real_receipt_history`、`web/src/test/OutputInvoiceCollectionsPage.test.tsx`、`web/e2e/output-invoice-red-relation-fanout.spec.ts` | Browser 已覆盖确认红蓝票关系 -> barrier -> rows refresh -> drawer 已有依据展示，以及确认后导航税金抵扣/成本统计并由下游 fresh read model 展示 relation 影响结果；同一 flow 覆盖撤销人工关系 -> barrier -> rows refresh -> 依据消失并恢复行状态。API/Vitest 锁定确认返回具体月份 `freshness_targets`，前端在 barrier resolve 前不得刷新 rows。确认和撤销两个成功点都会断言页面没有操作失败、同步失败或 read model 失败等错误残留。search 当前没有独立 Browser route，沿用 API/runtime 证据。 |
