...
| 场景 | 当前覆盖 | 缺口/风险 |
| --- | --- | --- |
| fresh rows/filter/sort/pagination/summary | `tests/test_output_invoice_collection_service.py`、`tests/test_output_invoice_collection_api.py`、`tests/test_invoice_usage_collection_sql_runtime.py::InvoiceUsageCollectionSqlRuntimeTests::test_output_repository_summary_counts_relation_invoice_members_not_rows`、`web/src/test/OutputInvoiceCollectionsPage.test.tsx`、`web/e2e/output-invoice-collections-flow.spec.ts` | `summary.invoiceCount` 按唯一销项发票 ID 统计并驱动标题右侧 `销项票 N`，不使用当前表格筛选后的 `pagination.total` 行数；linked 多销项发票 relation 归并为一条 row 时仍计入全部成员，keyword/filter/month 改变时标题总数保持全量口径。`test_page_size_limit_protects_first_screen_slo` 用 250 行 synthetic 数据验证后端 `page_size=200` 上限和 `page_size>200` 的 `invalid_paging`；前端首屏 rows 请求锁定 `page=1&page_size=20`，每页选项限制为 20/50/100；Browser 覆盖 keyword search、发票号码排序、收款状态/发票号码筛选、page-size 切换后的 rows 请求参数和表格结果同步；本轮新增 rows 首屏暂时 503 后错误态空行、普通空态消失、导出禁用和刷新恢复；money/date 组合由 Vitest/API 覆盖，大数据 SQL EXPLAIN 和浏览器性能仍为真实环境风险 |
| title Audit icon / real data audit | `web/src/test/OutputInvoiceCollectionsPage.test.tsx`、`web/src/test/PageAuditIcon.test.tsx`、`tests/test_app_health_api.py`、`tests/test_audit_output_invoice_collection_read_model_tool.py` | admin 用户标题右侧只读 Audit；只有 `audit_status.integrity=pass`、`freshness=fresh` 且页面 read model 明确为 fresh 才显示成功。只有全局 relation timestamp 前进时，Audit 必须检查该时间之后的 relation 是否与本方向、本 scope 的 canonical/PostgreSQL/OA source-link 发票 identity 相交；纯银行变化不报本页 source-version mismatch，任何其它版本差异仍阻塞。页面 fresh gate 合同未因此放宽。问题数量是有上限 sample，unknown/live-query 不得伪装 fresh。 |
| stale/missing/source version mismatch / repository unavailable | `tests/test_invoice_usage_collection_sql_runtime.py::test_output_api_stale_returns_refreshing_without_stale_rows`、`tests/test_invoice_usage_collection_sql_runtime.py::test_output_api_requires_sql_repository_in_production_without_live_scan`、`tests/test_invoice_usage_read_live_fallback_guards.py`、`web/src/test/OutputInvoiceCollectionsPage.test.tsx`、`web/e2e/output-invoice-collections-flow.spec.ts` | API 覆盖 stale/source mismatch/生产 SQL repository unavailable -> `202 refreshing` 且不返回 stale rows、不 live scan；read application fallback 关闭时 rows/filter-options/export-preview/relation detail 返回 refreshing，export 返回 structured 409，不调用 `OutputInvoiceCollectionQueryService` live query；Vitest/Browser 覆盖页面显示刷新诊断、不显示普通空态、不泄露 stale reason、不展示旧 rows 或写入口；Vitest 额外覆盖 rows 为 fresh 但 filter-options stale 时仍按非 fresh 处理并禁用导出，避免筛选读模型 stale 被伪装成 fresh empty。真实 worker drain、Redis/RabbitMQ/systemd 需 infra-smoke/staging。 |
| read model repository port / freshness boundary | `tests/test_invoice_usage_collection_sql_runtime.py::OutputInvoiceCollectionReadModelRepositoryPortTests::test_port_excludes_unrelated_read_model_methods`、`tests/test_output_invoice_collection_api.py::OutputInvoiceCollectionApiTests::test_relation_details_require_sql_repository_in_production_without_live_rebuild`、`test_relation_details_use_fresh_sql_read_model_row_without_live_rebuild`、`tests/test_invoice_usage_collection_sql_runtime.py::InvoiceUsageCollectionSqlRuntimeTests.test_projection_builder_persists_invoice_relation_source_versions`、`test_projection_builder_marks_empty_scopes_with_source_versions`、`test_projection_builder_prunes_invoice_usage_collection_scope_shards`、`tests/test_postgres_state_store.py::PostgresStateStoreTests::test_read_model_repositories_use_optional_read_connection`、`tests/test_read_model_architecture_guards.py::ReadModelArchitectureGuardTests::test_output_invoice_collection_app_level_projection_helpers_do_not_return`、`.planning/refactors/modular-io-boundaries/analysis/read-model-output-invoice-collection-local-implementation-closure-audit.md` | 覆盖 `OutputInvoiceCollectionReadModelRepositoryPort` 只暴露 manifest-listed output 方法，PostgreSQL state-store read wiring 使用 optional SQL read connection，projection builder output save/mark/prune 走窄 port，relation detail 生产缺 detail repository 时 `202`/refreshing/enqueue 且不 live rebuild，fresh SQL detail row 可直接构造 relation details，且旧 app-level output projection helper 不能回到 `Application`。本地实现支持已 accounted；真实 PostgreSQL/worker/App Status/high-row/browser evidence 仍 deferred。 |
| 统一关系 OA/流水/发票项多项展示 | `tests/test_output_invoice_collection_service.py::test_unified_relation_payload_exposes_multiple_oa_bank_and_output_invoices`、`tests/test_output_invoice_collection_service.py::test_multi_output_relation_emits_single_net_collection_row`、`tests/test_invoice_usage_collection_sql_runtime.py::test_output_repository_save_persists_oa_relation_columns`、`tests/test_invoice_usage_collection_sql_runtime.py::test_output_api_schema_stale_enqueues_refresh_when_unified_relation_fields_missing`、`web/src/test/OutputInvoiceCollectionsPage.test.tsx::shows invoice aggregate with +N entry point for multi output invoice relations` | 覆盖同一统一 relation 下多 OA、多收入流水、多销项发票 summaries，linked 多销项发票 relation 只投影一条净额收款行且负数/红字发票进入 summaries，`/relation-details?kind=oa|bank|invoice`，SQL 原生 OA columns，旧 payload 缺 `oa/invoiceRelations` 触发 refreshing，以及前端多销项发票栏显示发票主信息、价税合计和额外项 `+N`。真实 Browser 长表视觉和 relation group 跨行聚合仍需专项 smoke。 |
...
| 红蓝票关系 | `tests/test_output_invoice_collection_lifecycle.py::test_red_relation_overlay_adds_manual_evidence`、`tests/test_output_invoice_collection_api.py::OutputInvoiceCollectionApiTests::test_lifecycle_write_routes_overlay_rows_and_create_real_receipt_history`、`web/src/test/OutputInvoiceCollectionsPage.test.tsx`、`web/e2e/output-invoice-red-relation-fanout.spec.ts` | Browser 覆盖确认/撤销、barrier、本页 rows、relation 导出和成本统计 consumer；税金抵扣不属于 relation downstream。 |
| 正式收据 create/void/reissue/history/settings | `tests/test_output_invoice_collection_lifecycle.py::test_receipts_are_idempotent_and_history_is_real`、`tests/test_output_invoice_collection_lifecycle.py::test_receipt_numbers_are_unique_under_concurrent_creates_and_reset_periods`、`tests/test_postgres_migrations.py::test_output_invoice_receipt_numbering_schema_contract`、`tests/test_output_invoice_collection_api.py`、`web/src/test/OutputInvoiceCollectionsPage.test.tsx`、`web/e2e/output-invoice-collections-flow.spec.ts` | 本地覆盖并发创建、月度/年度/不重置序列、PostgreSQL 唯一约束 contract、receipt create/void/reissue 响应具体月份 `freshness_targets` 和真实 Chromium 创建/历史/作废/重开展示；Vitest 锁定 receipt 创建后等待返回的具体月份 operation barrier target 后才刷新 rows；Browser 创建、作废、重开和历史成功后检查无可见错误残留，并断言作废/重开 reason POST body、history reload、barrier 和 rows refresh；同时覆盖创建正式收据暂时 503 后 idempotency key 仍发送、预览 drawer/错误/重试入口保持、rows 不提前刷新、history 不伪读，第二次创建成功后才显示已出收据；也覆盖作废/重开暂时 503 后原因弹窗和输入值保持、history/rows 不提前刷新，第二次确认后才更新 history/rows；真实数据库压力与生产历史样本仍需 staging/生产验证 |
...
| 权限与 admin-only 设置 | `tests/test_output_invoice_collection_api.py::test_detail_routes_require_output_collection_read_session`、`web/src/test/OutputInvoiceCollectionsPage.test.tsx`、`web/e2e/output-invoice-collections-flow.spec.ts`、`web/e2e/permissions-role-matrix.spec.ts` | Browser 已覆盖 `read_export_only` 可读/可导出/可看只读规则和收据历史，但不显示状态/提醒、红蓝票、待出收据、收据编号设置、收据作废/重开，且全程零 mutation API；admin 覆盖收据编号设置打开/保存、零 rows reload、零 barrier 和 operation latency；全角色全页面矩阵仍由 permissions-and-audit 模块统一审计。 |
...
| 6. End-to-end business-flow integration tests | 适用，已有关键链路级回归 | `tests/test_invoice_lifecycle_page_integration.py`、`tests/test_derived_data_lifecycle_service.py`、`tests/test_invoice_usage_collection_sql_runtime.py`、`web/e2e/output-invoice-collections-flow.spec.ts`、`web/e2e/output-invoice-red-relation-fanout.spec.ts`、`web/e2e/workbench-relations-tax-offset-isolation.spec.ts`、`tests/test_search_pending_sql_runtime.py` | 覆盖本页 lifecycle/receipt/relation 与成本/search consumer，并显式保护 tax-offset 非消费者隔离。 |
...
- linked relation 下多张销项发票必须只输出一条净额收款行，负数/红字发票必须出现在 `invoiceRelations.summaries`，不能重复显示同一 364800 relation group 或漏掉负数发票：`test_multi_output_relation_emits_single_net_collection_row`。
- 标题右侧 `销项票 N` 必须使用未筛选全量 rows summary 的唯一销项发票数，不能使用 row/group 行数或当前筛选后的 total；linked 多销项发票 relation 归并为一条 row 时仍计入全部成员：`tests/test_invoice_usage_collection_sql_runtime.py::InvoiceUsageCollectionSqlRuntimeTests::test_output_repository_summary_counts_relation_invoice_members_not_rows`、`web/src/test/OutputInvoiceCollectionsPage.test.tsx`。
...
- 页面轮询在同月份 refresh event 正在 `pending/processing` 时不得按旧 blocking 状态再次 enqueue；scope state 必须在同一 SQL snapshot 返回 active event，页面保持 refreshing 但零重复 target：`tests/test_invoice_usage_collection_sql_runtime.py::InvoiceUsageCollectionSqlRuntimeTests.test_scope_state_keeps_active_refresh_out_of_enqueue_candidates`、`tests/test_output_invoice_collection_read_model_fresh_gate_service.py::OutputInvoiceCollectionReadModelFreshGateServiceTests.test_active_refresh_event_blocks_rows_without_duplicate_enqueue`、`test_active_refresh_event_does_not_requeue_for_other_month_statistics`。
- 生产 PostgreSQL runtime 下缺少 `output_invoice_collection` SQL read repository 时不能回退旧 `OutputInvoiceCollectionQueryService.list_rows` live scan；relation detail 缺 SQL repository/detail lookup 时也必须 `202`/refreshing/enqueue，不能 live rebuild：`tests/test_invoice_usage_collection_sql_runtime.py::InvoiceUsageCollectionSqlRuntimeTests::test_output_api_requires_sql_repository_in_production_without_live_scan`、`tests/test_output_invoice_collection_api.py::OutputInvoiceCollectionApiTests::test_relation_details_require_sql_repository_in_production_without_live_rebuild`。
- `OutputInvoiceCollectionReadModelRepositoryPort` 不能暴露 input usage、OA pending payment、pending invoice 或 Workbench relation source-version 方法；projection builder output save/mark/prune 和 relation detail row lookup 必须走该窄 port：`tests/test_invoice_usage_collection_sql_runtime.py::OutputInvoiceCollectionReadModelRepositoryPortTests::test_port_excludes_unrelated_read_model_methods`、`InvoiceUsageCollectionSqlRuntimeTests::test_projection_builder_persists_invoice_relation_source_versions`、`test_projection_builder_marks_empty_scopes_with_source_versions`、`test_projection_builder_prunes_invoice_usage_collection_scope_shards`。
- fresh SQL rows 返回前必须叠加 lifecycle overlay：`test_sql_fresh_rows_route_applies_lifecycle_overlay_before_response`。
- 纯银行 Turnover relation 确认/撤回只推进全局 `workbench_pair_relations_updated_at` 时，不得误报销项页 source-version mismatch；若同 scope 后续 relation 含 canonical/PostgreSQL/OA source-link 销项发票 identity，或其它 relation source-version key 变化，仍必须阻塞：`tests/test_audit_output_invoice_collection_read_model_tool.py::AuditOutputInvoiceCollectionReadModelToolTests::test_clean_audit_passes_without_writes`。
...
- 真实浏览器中销项红蓝票关系确认后，销项页自身和成本统计 relation consumer 必须读取 confirmed 结果；税金抵扣不消费 relation，不得因红蓝票关系新增进项计划行：`web/e2e/output-invoice-red-relation-fanout.spec.ts`、`web/e2e/workbench-relations-tax-offset-isolation.spec.ts`。
...
- 红蓝票关系确认/撤回 -> relation overlay -> collection status/receipt eligibility 更新 -> 导出 relation 字段 -> 成本统计/search consumer 更新；tax-offset 不读取 relation，也不接收该写入的 refresh。
...
PYTHONPATH=backend/src python3 -m unittest \
  tests.test_invoice_usage_read_live_fallback_guards \
...
- 红蓝票关系确认/撤销后的本页 rows refresh、人工依据展示/消失、行状态恢复、relation 字段导出与成本统计 consumer 已由 Browser 覆盖；tax-offset 非消费者隔离由独立 Browser/repository 反证保护。
