...
| `RECON-WB-E2E-009` | 统一异常抽屉与金额异常 ignore/restore | P1 | legacy 异常 apply/cancel/ignore/unignore 和 OA/发票精确金额差异都从统一右侧抽屉处理；默认折叠显示关系组三栏计数/总金额，展开显示完整成员；金额异常可忽略进入“已忽略的异常”并撤回忽略，主表 chip 与准确计数同步变化；操作后只通过当前关联台正常 GET 更新页面，closed exception relation 必须走 canonical command service，零 downstream job。 |
