...
`RECON-WB-E2E-017`：列表金额列无“选择子项”菜单；详情单项选择与整笔选择共用归属规则。跨关系占用项禁选，当前项选中后关闭详情仍能撤回完整关系。拆分编辑中不可选择；保存后清除该银行旧选择、回读新版本再允许选择；原流水金额保持不变。覆盖 `workbench-split-selection.spec.ts`。
