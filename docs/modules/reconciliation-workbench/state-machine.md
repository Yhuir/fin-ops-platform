...
历史文件/无金额 → 待填写凭证金额；有效文件+已填金额 → 按实际金额核对；一致解除对应异常，不一致显示差额。编辑文件和金额在一次保存后生效，失败保留原有效组。清空文件 → 金额不参与核对；正式发票覆盖 → 凭证保留但不计额；正式发票关联撤回 → 重读当前依据。所有状态变化保持进行中 OA 的 unpaired 条件，文件/金额/OA 子项金额变化不继承旧异常审阅结论。
