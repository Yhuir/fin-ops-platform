...
| operation pending | `GlobalOperationOverlayProvider` 包裹中的确认、撤回、异常、忽略等写操作 | 带后端 operation projection 的确认/撤回写 API 成功后只等待 `workbench_relation` 操作级 barrier fresh，再应用后端 projection 并释放 overlay；Workbench month/all active generation 和下游 read model 后台追赶。没有 operation projection 的旧动作仍需等待目标 read model/scope fresh 或重新读取 Workbench active generation 后释放。失败时展示错误并保持阻塞，用户确认后返回页面。 |
