...
异常抽屉的筛选状态不创造新的业务状态：`bucket(unpaired|paired) -> view(amount|document_only) -> amount code`。金额分类由服务端保证每个关系最多一个；金额与资料并存时只进入该金额分类，资料只作为附属 evidence。没有金额分类且存在至少一个附件异常时才进入 `document_only`。一个关系无论含多少异常 item，在当前 bucket 内都只属于一个视图/分类并只计数一次；切换 bucket/view/code 必须取消旧请求并从新查询首页开始。接受或撤回决定只改变目标关系的服务端分区，前端成功重读后保持当前 bucket/view/code 并刷新当前列表，不自动切换到目标 bucket。

异常决定的作用域取当前 canonical 组的成员月份：成员都在同一月时写入单月决定；同一关系跨多个自然月时写入全局决定并在各月视图复用。不得因为页面使用 `month=all` 或成员跨月而拒绝审阅，也不得拆成多个相互冲突的月度决定。

只有当前 direct canonical descriptor 计算出的 `workbench_anomaly` 进入状态机；用户可见金额 item 严格是七种互斥三栏分类，正式发票局部差异沿用现有整体分类与共享发票定位；凭证子项差额没有整组分类时使用“明细金额不一致”。三栏不完整、金额无效、方向未知/冲突或总额完全一致时不得生成七种整组金额分类；凭证子项可确定差额则独立归入“明细金额不一致”。历史人工分类字段、ignore/restore、WEX/row-ignore 不得改变分区、成员、抽屉或计数。

未配对工具栏不提供第二套人工异常入口；右上统一入口显示 `未配对异常 n | 已配对异常 m`。

## 2026-09-11 明细补充凭证

无有效凭证 → 上传成功并 canonical 回读 → 该明细资料已补齐 → 发票栏同行显示全部文件名。上传失败不改变完整性；删除最后一份/文件失效 → canonical 回读恢复真实缺失或未解析状态。资料补齐不等于新增正式发票，也不掩盖 OA—流水差额或已有正式发票差额。组是否进入已配对仍由完整 canonical 事实计算；无有效银行关系的单据不会因文件上传凭空建立关系。
...
历史文件/无金额 → 待填写凭证金额；有效文件+已填金额 → 按实际金额核对；一致解除对应异常，不一致仍按原规则归入金额异常；凭证列表只显示凭证金额，具体核对金额由既有异常入口提供。编辑文件和金额在一次保存后生效，失败保留原有效组。清空文件 → 金额不参与核对；同项正式发票与凭证 → 合计核对；正式发票身份或金额变化 → 凭证金额待重新确认；正式发票关联撤回 → 重读当前依据。所有状态变化保持进行中 OA 的 unpaired 条件，文件/金额/OA 子项金额变化不继承旧异常审阅结论。
