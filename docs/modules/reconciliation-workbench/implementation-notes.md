...
## 2026-08-31 - 多收入关系收据收敛为单张

- 生产验证发现：同一付款单位的多笔收入跨交易日期时，旧草稿会先按日期拆组，再尝试把发票唯一归组；这会让已经显示“编辑并打印收据”的关系在点击后返回歧义冲突，且偏离“一组多收入、多发票生成一张收据”的产品口径。
- 收口：一个 active relation 固定生成一张收据；全部收入流水金额合计为不可变核对金额，默认日期取最新收入交易日期。多条流水的规范化付款方必须唯一，否则资格投影不显示动作、canonical draft 同时明确 fail closed；不再按交易日期拆分，也不按发票购买方猜测归属。
- 旧链删除：删除后端按付款方/交易日期/币种分组及发票归组分支，删除前端不可达的 `receipt_invoice_group_ambiguous` 错误映射，改为唯一付款方冲突。API shape、数据库、read model、worker、队列、缓存和 PDF 模板均不变；本次不创建数据库备份，不删除或修改主数据库事实。
...
- 修复：candidate、summary hydration、formal matching 与 full/detail row-id OA projection 共用一个 set-based OA source alias SQL 边界，合并 payload aliases、owned item/attachment parent identity 与 active `app.oa_source_aliases`。full/detail 直接在既有 row-id 查询中携带 aliases，不增加 SQL statement；历史 cache bridge 只用于 `0153` 一次性强证据迁移，禁止进入页面热查询。不增加金额、项目、文件名或顺序 fallback，不新增 API、表、索引、read model、worker、cache 或依赖。
...
- 修复：正式关系已把 `etc-summary-{batch_id}` 持久化为 invoice 成员时，投影不再把该行降级为 display-only summary；关系外附加摘要仍保持原折叠展示语义。schema 升级为 v15，避免 v14 失败 generation/cache 被继续消费；没有新增表、worker、队列、缓存或兼容链。
...
- 产品合同：关系组 OA 与发票金额字段完整时按分精确合计，任何差异都显示 `金额不一致`，不推断真假、不设置容差；缺金额不误报。忽略后显示 `已忽略：金额不一致`，仍保留在主关系和“已忽略的异常”，可恢复。
...
## 2026-08-21 - Filter-options 异常候选与 OA 申请人投影收窄

- direct repository 仅收窄查询内部数据：filter-options 普通请求只计算会跨区的基础 paired 关系异常，unpaired 异常视图仍覆盖两区；OA 申请人只投影类型、流程和申请人文本。API DTO、异常规则和模块 I/O 不变，旧宽投影不保留。
...
## 2026-09-11 - 补充凭证资料判断与同行展示

- 历史“只供展示”的凭证规则由本次明确需求替代：分页前只批量读取 active 文件所证明的 OA/item 身份，用于资料完整性与正式发票比较范围；文件名、类型、时间等元数据仍只在当前页一次批量装配。没有恢复逐明细 SQL 或全量文件元数据聚合。
- 一项多凭证合并为全宽文件列表，移除伪发票列、重复缺票入口和前端局部替换 helper。上传/删除复用原服务，完成后一次 canonical 回读，权限、软删除、去重及审计保持。
- 本地 PostgreSQL 覆盖 active/删除/文件失效、混合正式发票、共享发票和 SQL/Python 分区一致；浏览器覆盖上传—回读—预览—删除—恢复，桌面及 390px 窄屏。窄屏同时修正关联台表头受通用 pane min-height 影响而挤掉内容区的问题。
- 未新增表、migration、后台任务、依赖、兜底分支或生产财务数据修复。
...
## 2026-09-12 电信多 OA/流水历史对应修复

- 原因：正式合并保留了成员与确认历史，但页面只重算金额对应。重复金额无法唯一定位后，流水落入残余带；金额算法又存在重复子集合先到先得、重复银行单额超配、精确匹配后再次分配的缺陷。
...
### 2026-09-12 纯流水关系折叠补齐

- 根因：展示函数和总金额搜索 SQL 同时要求关系包含 OA/发票，导致同标签、同方向/币种的正式纯银行关系被排除。
- 修改：纯银行关系复用既有 bank_folds；搜索移除 OA/发票候选限制。真实成员、关系写入、批次 owner 和前端组件保持原有边界。
- 验证：71 项相关后端测试、29 项真实 PostgreSQL 查询测试、140 项前端交互测试、16 项浏览器流程通过；lint、文档检查及生产构建通过。无数据库迁移、无新增依赖、无兜底路径。

- 生产性能复测发现纯流水候选过宽：搜索 1.00 时 14 个纯流水关系进入水合但均不匹配。纯流水仅允许整组折叠，因此候选 SQL 对纯流水使用金额精确相等，OA/发票对应区域继续保留原候选上界规则；不新增缓存或索引。增加微额流水查询测试，保护正确命中和无效候选零水合。

## 2026-09-15 日常报销子项完整展示

- 核查：当前 461 张 OA 中有 113 张日常报销、312 个真实费用子项。4 张多子项报销被 ETC collapsed_summary 隐藏，另有 2 张单子项隐藏；8 张旧报销的 33 个子项只有 expense_content，旧展示未读取该字段。
- 决策：父 OA 继续作为配对/选择单位，子项作为展示和成本依据；不将 1 张 OA 变成 4 张 OA，不从事由拆金额。发票折叠独立于 OA 明细。无子项的支付申请沿用父行。
- 边界：关联台分页 SQL / DTO 批量传递必要字段；OA 待付款、待发票、进项使用详情通过原查询增加公共费用字段，不增加逐行请求。成本分配不改。
- 历史数据：113 张原始 OA 的 312 项均已按源身份、行序、金额、内容完成只读核对；补齐支付方式、发票种类、票据张数及缺失的原始费用文本，事务记录每项 before/after 审计。禁止重建 ID、改金额、改关联。
- 验证：纯投影/源枚举、精简与完整 DTO、重复内容/单子项/折叠组、详情隐私字段、真实 PostgreSQL 原始日期、浏览器子项展示与父单选择；并回归 OA 待付款、进项使用、待发票、成本分配。


## 2026-09-23 - 组级未配对原因解释（展示方案已被后续收口替代）

- 真实根因：凭证已正确计入，但同组另一 OA 子项存在6.57元超额；旧组角落图标和“明细1”不足以解释长组的整体状态。
- 原组件内完成：可见组级原因、权威金额拆分、精确子项上下文、局部差额文案；共用主表/异常抽屉原 Popover，保留原审阅流程。删除两处分段分支重复图标 JSX、旧绝对定位和序号式明细说明。
- 额外实测修正：Escape 关闭后的焦点恢复不再重开详情；不增加全局事件/网络副作用。
- 无后端业务、API、数据库/迁移变化，无正式发票伪造，无自动接受。性能、发布与验收证据见 [执行计划](../../dev/workbench-group-blocker-explanation-plan.md)。

## 2026-09-23 - 恢复异常 icon

- 前次增加独立提示行及解释文字超出所需范围。本次恢复原图标布局，金额 Popover 仅显示权威两方/三方金额，删除描述索引和提示条链路。
- 原审阅记录移入已有异常抽屉，保留只读查看；资料异常原处理入口不变。凭证计额、关系和配对规则均不修改。
- 无依赖、后端/API、数据库或 worker 变更。详细验证及发布证据见 [修复记录](../../dev/workbench-anomaly-icon-restoration.md)。


## 2026-09-24 顶部任务条及金额列子项入口移除

- 已发布代码：`224264a6b`（包含 `9370eb643`），release `main-224264a6b-20260924-ui-cleanup`。正式脚本 frontend profile 的 pre/T0 均 PASS，API 与四个登记 worker 正常，无数据库迁移、备份或业务数据写入。
- 删除全局任务条组件、专属样式及 primaryJob/extraCount 派生，任务动作移入既有运行状态弹层；金额列删除选择菜单，单子项选择由关联台详情注入公共编辑器，其它页面不注入关联操作。
- 生产验证捕获详情接口 `split_version` 与列表 `bank_split_version` 的差异，在详情 API adapter 明确映射；修正浏览器 fixture，保护原金额、子项 ownership 和版本，不增加旧字段 fallback。
- 本地相关单元/组件/适配测试共 263 项通过，29 项浏览器业务/布局回归通过。包括 90 项 WorkbenchSelection、79 项 shell/拆分/选择状态、12 项详情/API/标签、82 项 WorkbenchApi；浏览器覆盖 app shell、响应式、拆分保存/选择/确认/撤回、OA 待付款、进项使用及待找发票。
- 已运行 `bash scripts/verify.sh lint`、`bash scripts/verify.sh docs`、`git diff --check`、`npm --prefix web run build`；Vitest 对应上述文件执行 `--run`，Playwright 对应 app-shell、app-shell-responsive、bank-transaction-splits、workbench-split-selection、oa-pending-payments-flow、input-invoice-usage-flow、pending-invoices-fanout 的 Chromium 用例。构建通过，现有 CSS/chunk size 警告仍在。
- 测试分类：业务选择/状态规则、前端 API 适配合同、组件交互、跨组件 E2E、旧页面回归适用；服务/持久化和 read model/cache/worker 实现未改，不新增这两类后端测试。真实生产写操作不做试验，确认/撤回与保存失败等在确定性测试覆盖；生产只读 GET 与本地选择验证。
- 真实样例只读通过：原金额 1001497.22；子项 1000000.00 与 1497.22；金额列没有旧选择入口，标签悬浮金额正确，详情可选择并取消利息；运行状态弹层可打开。成本搜索总额仍为1497.22，外部往来对应本金支出仍为1000000.00。生产写请求为零、浏览器异常为零；不将该对手方全部其它贷款余额声称为零。
- 性能：同一客户端/公网、每接口20次、并发2、各预热1次，总计100个计时请求，全部成功。下表为毫秒，目标 p95≤1000、p99≤2000。最后一次采样时间：2026-09-24T07:20:45.295910+00:00 至 2026-09-24T07:21:13.999832+00:00。

| 接口 | 发布前 p95 | 最终 p50 | 最终 p95 | 最终 p99 | 结果 |
| --- | ---: | ---: | ---: | ---: | --- |
| 关联台 | 1017.1 | 936.6 | 1017.4 | 1018.6 | p95 未达标 |
| 后台任务 | 179.7 | 176.3 | 496.0 | 619.2 | 通过 |
| 拆分详情 | 131.0 | 125.7 | 139.2 | 140.3 | 通过 |
| 银行明细 | 554.6 | 508.1 | 553.8 | 556.5 | 通过 |
| 成本统计 | 857.3 | 752.4 | 838.8 | 843.4 | 通过 |
...
## 2026-09-24 确认关联备注汇总行入口

- 备注正文从异常抽屉底部审阅区移至汇总行备注 icon 的 Popover，入口位于异常 icon 左侧，展开后仍可查看。摘要已有 `workbenchAnomaly.confirmation.note`，不新增请求或详情预取；没有备注不显示入口。
- 沿用 HeroUI Popover 与 lucide 图标。局部 grid 为操作区预留宽度，支持 hover、keyboard、touch、移入复制、长文本内部滚动。区分 hover 打开与触发器聚焦打开的关闭状态，防止 Escape 后焦点恢复再次打开或后续键盘进入无法打开。
...
## 2026-09-25 预览关系表信息分层

- 确认／撤回保留左右对照与每侧OA、流水、发票三列；同组连续表格、明确子关系对齐、共享列跨行，删除关系卡片间距、圆角与重复框线。后端分组和提交成员保持原样，不按同额或排列重新匹配。
- 收支与银行后四位复用chip；OA保留项目/申请人/金额，银行保留对方/父金额/成员标签，发票保留销方/价税合计。PreviewRecordDetails按需展示次要字段，复用HeroUI Portal，不发详情请求。删除原生title、税额说明、内联本次关联金额及无用途的求和和样式。
- 真实浏览器发现并修复悬浮后点击关闭及Escape后重新聚焦不能打开的问题；最终仅保留open状态、跨浮层阅读关闭计时器与卸载清理。
...
## 2026-09-25 预览黄蓝关系底色与居中

- 沿用Impeccable Ledger Calm：同组黄蓝贯通，操作前中性标题、操作后淡蓝标题与细蓝顶线。每条记录独立居中，保留共享列跨行和真实关系顺序。收支与原金额合为chip，银行与未拆分类均使用chip，拆分标签继续悬浮显示子项金额。
- 只修改共用预览、父容器的一项展示输入及局部CSS；删除白灰交替和旧对齐规则，不改后端、HTTP合同、公共组件、数据库和worker，不创建备份。配色仅依据已有组ID与次序，不建立业务映射。
- 本地6个Vitest文件295项通过，生产构建通过；保留依赖既有CSS与chunk体积警告。浏览器最终回归及生产部署结果补记于下方。
- 发布前同一生产样例10次withdraw preview均200，p50为110.9ms、P95为217.8ms；原关系1组、撤回后5组，原流水1001497.22元。生产仅预览，不写财务关系。

- 回归发现详情浮层的 Esc 关闭与焦点恢复相互触发；预览局部组件记录当前焦点访问的关闭状态，防止立即重开，并在离开或新一次点击/悬浮时解除。浏览器测试等待浮层实际获得焦点后发送 Esc，额外覆盖两次键盘访问，4个排版场景连续3轮共12项通过。
- 最终32项确定性浏览器回归全部通过（1.6分钟），覆盖确认/撤回、拆分版本与子项、权限、网络恢复、stale/error以及跨页面fanout。焦点修正后预览组件8项再跑通过，生产构建与docs检查通过。

### 正式发布与生产验收

- 修复提交 `abc365428` 已推送 origin/main；正式发布 `main-abc365428-20260925-preview-bands`，frontend profile pre/T0检查 PASS、部署退出0、未回滚。API及4个登记worker均active，运行目录指向该版本。
- 对同一真实关系 `case:CASE-AUTO-0257` 比较发布前后的完整 `before`、`after`、`amount_summary`，完全一致：操作前1组、操作后5组，原流水1001497.22元，按物理流水去重为1笔。
- 生产浏览器1600×1000验证黄蓝贯通、单元格居中、主金额及账户chip、不同关系分行、全部5个操作后关系可见、横向溢出0px；700×900验证前后切换和固定确认入口。详情悬浮/移入/点击/关闭通过，浏览器异常0、财务写请求0。截图保留在本地Codex交付目录。
- 同一客户端独立采样各10次真实preview请求，全部200；发布前p50/P95为110.9/217.8ms，发布后115.0/169.6ms。P95按nearest-rank计算；样本量有限，不据此宣称全App性能提升。生产浏览器单次响应357ms、预览就绪604ms、响应后渲染247ms（发布前251ms）、详情悬浮65ms。
- 本次不修改服务、API、read model、worker、金额或关系判定，没有数据库迁移和备份。生产以只读查询及preview核验，实际确认/撤回写流程由本地32项确定性E2E覆盖。
- 最终生产route-shell巡检通过：1个测试遍历16个页面，用例28.3秒，无会话阻断、浏览器错误或写请求。验证记录随后独立提交，生产运行时代码保持 `abc365428`。

## 2026-09-25 发票跨行对齐与深色表头

根因：合并预览丢失原关系中发票的覆盖范围；前端只能把整列发票等分填满关系组。新增只读成员范围字段，复用精确历史划分和既有三栏布局，删除对已有范围仍强制整列共享的路径。范围投影与成本 OA/银行分组分开，前后预览和关联台共享跨行规则。表头沿用关联台深蓝灰，保持居中和黄蓝关系底色。

不新增依赖、数据库表、迁移、缓存或 worker，不创建数据库备份。生产验证只读查询和 preview，实际财务关系不试写；确认/撤回写路径由确定性回归保护。发布前真实合并 preview 10 次均200，p50 411.4ms、P95 600.9ms（nearest-rank）。实施后验证结果在发布完成后补记。
