...
| exception bucket query | Workbench API / read repository | `/groups` 可选 `exception_bucket=active|processed`；repository 在 active generation group payload 上按 legacy exception state 与 `amount_anomaly.state` 过滤后再计算 total/row counts 并执行有界分页，不在前端拉取全量关系后过滤。 |
...
| amount mismatch anomaly | 前端统一异常抽屉 | 关系组与其发票行可携带 additive `amount_anomaly={code,label,display_label,state,fingerprint,oa_total_minor,invoice_total_minor,delta_minor}`；只有 OA/发票金额字段完整且按分合计不等时存在。active 显示 `金额不一致`，ignored 显示 `已忽略：金额不一致`。页面复用 `AppDrawer`、HeroUI `ToggleButtonGroup`/`DisclosureGroup` 与既有 `RelationGroupGrid`：默认只读三栏成员数和总金额，展开时读取已返回的组内成员，不发第二条详情链路；read-export 只读，full-access/admin 可按关系组忽略或撤回忽略。状态不改变 relation 分区、成员、canonical 金额或下游 linked 语义。 |
