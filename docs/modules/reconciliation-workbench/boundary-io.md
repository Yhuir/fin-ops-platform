...
| generation source versions | Workbench normal GET / projection builder | expected 与 published generation 使用同一个 scope vector：relation/exception/override/rule versions，加 requested month 与 active relation members 的 OA、银行流水、发票 canonical `updated_at` 和 active pending claim version；`all` query使用对应全量 vector。canonical 变化只让被访问 scope 判 stale 并走现有 gateway，禁止恢复写后 fan-out。 |
