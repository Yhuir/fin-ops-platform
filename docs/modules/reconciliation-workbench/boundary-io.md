...
| Bank Transaction Paired Policy metadata | `workbench_relation` / bank-flow-rule-batch submit、tag-rule sync、turnover/manual closure sync、legacy no-OA submit | 所有含银行流水的 group 是否进入 paired 区都必须读取银行流水 row 上的物化 policy metadata。`requires_oa=true` 必须存在 OA row；`requires_invoice=true` 必须存在发票 row；两者都为 false 时银行-only 或部分栏位 relation 可 paired；metadata 缺失或字段缺失默认等价于 `requires_oa=true, requires_invoice=true`。Workbench 不读取当前标签设置作为 fallback。bank-flow submit/withdraw/reset 不是 `WorkbenchWriteFacade` 入口，但其 API 返回的 operation barrier 必须包含 `workbench_relation` 与 `workbench` 的 `all` + 受影响 month scope，因为关联台首屏读取 `month=all` active generation。 |
...
| paired/open 分区 | 前端 workbench components | 任何含银行流水的 active/candidate group 都先按 Bank Transaction Paired Policy 判定 required row type；缺少 required OA 或发票 row 必须留在 open 区，满足后才进入 paired。普通两栏 `manual_confirmed` relation 若含银行且缺 policy 或缺 required row type，仍留在 open。带 immutable binding metadata 的 OA+自带附件发票两栏 active relation不含银行流水时只是不可拆 source binding，也必须留在 open 区。 |
