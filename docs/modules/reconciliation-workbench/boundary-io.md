...
| page relation dependency | `WorkbenchQueryFacade` -> `WorkbenchRelationReadFacade` | combined initial GET 在读取/写入 Redis page cache 前检查页面登记的共享 relation distribution proof。月份请求只检查该月；`all` 先批量枚举 canonical object、active relation 与已有 projection 的 concrete months，再以固定查询数比较全部 scope。non-fresh 时只 enqueue mismatch scopes 并把页面标为 refreshing/stale；后续 `/groups` 继续绑定 initial 返回的 Workbench generation version，不重复执行全页 dependency proof。禁止关系写路径 fan-out、逐月 N+1 或 relation `all` 伪 projection。 |
