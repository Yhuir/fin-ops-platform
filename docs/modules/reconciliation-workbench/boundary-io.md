...
追加发票入口的不足额判断使用已归属 canonical 发票的 `tableValues.grossAmount`（价税合计），不能使用代表未税金额的 `row.amount`；价税合计未知时不推断为未税或零。此判断只影响入口展示，不写发票金额或关系。
...
- 普通 groups 搜索复用已有 `groups_source_search_hits`，先选命中的完整关系，再计算会改变 paired/unpaired 分区的异常。命中关系的所有 OA、银行和发票成员仍完整参与金额、附件证据及指纹计算，不能按命中成员裁剪；精确计数和 cursor 分页仍在最终查询内完成。
