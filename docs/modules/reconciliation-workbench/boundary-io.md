...
- Release A 上线后已通过全量 Workbench rehydrate 使旧 `open`/candidate/decision generation 被新的 paired/unpaired generation 原子替换；不得原地修改旧 active generation。当前 v7 发布必须在 exact release 激活和 migration `0125` 成功后，使用正式 `finops-deploy-control workbench-rehydrate <release> --json` 做一次受控 rehydrate，再运行最终页面/Audit/SLO 矩阵；这是版本迁移维护操作，不属于普通页面 3 秒 SLO，也不得由普通页面隐式触发全量迁移。该纯派生投影迁移不创建额外数据库备份，依赖既有 release rollback 与 test-owned fixture 恢复。Release B 的旧状态 drop migration 只在 A 的零访问和数据安全证据通过后创建，并使用届时下一个可用版本；不得复用已被 OA 使用的 0104。
