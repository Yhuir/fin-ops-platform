...
| generation source versions | Workbench normal GET / projection builder | expected 与 published generation 使用同一个 scope vector：relation/exception/override/rule versions，加 requested month 与 active relation members 的 OA、银行流水、发票 canonical `updated_at` 和 active pending claim version；`turnover_manual_closure` 仍属于 Workbench 主 generation canonical proof，但不属于共享 `workbench_relation` distribution。`all` query使用对应全量 vector。同一次 normal GET 的 freshness status 已完成该 canonical proof 后，后续 payload/version/cache gate 必须复用同一结果，禁止对同一 scope再次执行完整 proof；若 freshness status I/O 不可用，既有 payload source-version fallback 仍 fail closed。canonical 变化只让被访问 scope 判 stale 并走现有 gateway，禁止恢复写后 fan-out。 |
| access refresh coalescing | Workbench normal GET / durable queue | normal GET 发现 stale 后只 ensure 当前 exact scope。当前 freshness view 已证明 exact scope 存在 `pending/processing` outbox event 时直接返回 `refreshing`，后续轮询不得重复执行全月份 canonical proof 或 schema scan；dirty 没有 active event 时必须标记 stale 并返回 exact scope 重新入队。真实 relation/canonical mutation 和显式 repair 不适用 ensure 合并。 |
| page source freshness | `WorkbenchQueryFacade` -> `WorkbenchQueryFreshnessService` -> `WorkbenchSqlProjectionBuilder` + active generation repository | 关联台投影直接读取 canonical facts 与 `app.workbench_pair_relations`，不消费 `workbench_relation` distribution，因此 combined initial 不得阻塞或刷新 `workbench_relation`。月份请求只比较当前 Workbench generation 与该月 canonical source vector；`all` 用一次月份枚举、一次 canonical bulk proof 和一次 active-generation bulk proof 返回真正 mismatch 的 `refresh_scope_keys`，只 enqueue 这些 Workbench 月份。普通 relation 写后访问不得退化成 `workbench:all` 全月份重建，也不得产生 relation projection I/O；只有 active generation 全部缺失或无法得到 exact recovery scope 时，既有 `all` fan-out command 才作为冷启动/显式恢复入口。non-fresh gate 立即返回轻量首屏状态，不读取旧 generation 的完整首屏；收敛后只读取一次 fresh 首屏。后续 `/groups` 继续绑定 initial 返回的 generation version。`server.py` 只组装 connection/repository/stale-reason ports，不拥有 scope 比较或 mismatch 聚合。 |
...
- relation UoW/turnover writer 不拥有成本计算、存储或下游刷新 I/O；repository、自动匹配命令和 lifecycle registry 不声明成本 I/O。Workbench generation publish 也不再发布 Cost fan-out。Cost 访问先读取现有 gate；gate 已 non-fresh 时跳过 canonical Workbench 全量证明并只 ensure gate 返回的 exact upstream/child scope。只有 gate 可 fresh 时才检查 Workbench canonical expected/active generation 版本；上游 stale 时只 ensure 当前 exact Workbench 月份，不预投 Cost。后续页面访问确认 Workbench fresh 后才 enqueue 当前 exact Cost child，child 完成后沿既有路径收敛 parent。Cost worker 以 manifest dependency fail closed/defer 处理真实竞态，依赖 fresh 后才发布；禁止 known-stale dependency retry storm、sibling project/page scope、写后 fan-out或第二协调链路。
