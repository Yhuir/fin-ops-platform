...
| OA expense-item display | 前端 | 父 OA 是唯一可选/可撤回成员；前端只用 `expense_items.id` 与发票行 `source_expense_item_id` 构造复合展示带。子项 UI 复用父 OA selection id，不能产生虚拟 canonical row、独立 action 或第二关系链路。 |
