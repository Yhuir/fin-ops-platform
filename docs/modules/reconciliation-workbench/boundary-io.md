...
- Partition：month scope active generation；`month=all` 查询组合 active month shards，不再要求 ordinary write 后生成 materialized all generation。composed all 的 groups/rows/row detail 必须只读取 active 月度 generation；可合并 group id（如 `case:`）按业务 case 合并，非可合并临时 group id 必须带 source scope 前缀，避免跨月覆盖。
