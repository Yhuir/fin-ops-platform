...
| 写操作 | workbench action/relation services | 写后污染受影响 workbench/workbench_relation/downstream scopes；confirm/cancel/withdraw 的 action I/O 只接受和输出 canonical Workbench row id。撤回操作必须把已选 OA source alias 作为 `row_id_aliases` 显式传入 relation command 边界，历史 relation fact 中的 OA source alias 不能传入 relation groups、amount check、response 或 operation barrier targets；canonical 后与当前 active relation 同 row-set 的 after/restored relation 必须过滤为无关系 after state。 |
