...
| `RECON-WB-E2E-009` | covered | `web/e2e/workbench-exception-flow.spec.ts`、`web/src/test/groupDisplayModel.test.ts`、`web/src/test/WorkbenchExceptionDrawer.test.tsx` | 统一异常抽屉覆盖默认折叠/展开、未配对异常处理/撤回忽略、ignore/unignore，以及 OA/发票 1 分差异的主表 chip、进入已忽略、撤回忽略完整链路；组件/显示模型补充覆盖组合发票同行、一个比较单元一个 chip 和 OA 附件缺失占位。 |
