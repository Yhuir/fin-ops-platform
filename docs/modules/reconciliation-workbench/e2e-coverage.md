...
| `RECON-WB-E2E-008` | `covered` | `web/e2e/workbench-permissions-flow.spec.ts`、`web/e2e/permissions-role-matrix.spec.ts`、`web/src/test/SessionGate.test.tsx` | Browser 已覆盖 `read_export_only` 可查看 open/paired/processed/ignored 状态，但确认、撤回、split candidate、异常 apply/cancel、ignore/unignore 写入口必须隐藏或 disabled，且不发出任何 Workbench mutation API 或 operation barrier。 |
