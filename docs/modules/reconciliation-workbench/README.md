...
Workbench SQL active generation 必须把 active relation 的 `special_metadata`、`amount_check`、`display_tags` 和 `source_versions` 带入三栏投影后再做 grouped/open 分区。`完全关联`、`自动匹配`、`三栏已配对` 等展示 tag 只能作为 UI 证据，不能替代 canonical active relation ownership；没有 `app.workbench_pair_relations.status='active'` 的 row-set 不能被提升为 confirmed paired owner。批量账务等外部 owner 写入的 OA+银行 relation 即使暂时只有两栏，也必须进入已配对区；relation metadata 指定的 ETC summary/invoice summary 必须随同一 case 发布，不能在 open 区留下 standalone 残留。已确认 active relation 的分区优先级高于“三栏是否齐全”的展示完整度：同一 `case_id` 下带非 `automatic_decision` `relation_mode` 且 relation code 为 `fully_linked` 的多栏 relation，即使暂时只有 OA+银行或 OA+发票两栏，也属于已配对区；只有旧 display tag、旧 `case_id` 或纯 `automatic_decision` 的候选仍留在 open 区等待人工确认或拆分。
