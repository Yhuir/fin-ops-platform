...
- 选择汇总的成员数量按 canonical typed identity 去重，金额沿用关系方向：临时付款选择的银行金额为支出减收入，临时收款选择反向计算；正式关系只消费组级 `amount_check` 权威三栏金额，不能因页面已经加载全部成员就改为本地绝对值求和。主方向未知、冲突或正式金额合同缺失时显示 `--`，不猜测、不发额外请求，也不阻断完整正式关系的既有选择动作。
