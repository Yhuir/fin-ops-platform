...
Workbench SQL active generation 必须把 active relation 的 `special_metadata`、`amount_check`、`display_tags` 和 `source_versions` 带入投影后再做 grouped/open 分区。`完全关联`、`自动匹配`、`三栏已配对` 等展示 tag 只能作为 UI 证据，不能替代 canonical active relation ownership；没有 `app.workbench_pair_relations.status='active'` 的 row-set 不能被提升为 confirmed owner。含银行流水的 relation 是否进入 paired 区，只读取 relation fact 中物化的 Bank Transaction Paired Policy metadata：`requires_oa=true` 时必须有 OA row，`requires_invoice=true` 时必须有发票 row，缺 metadata 时 fail closed。普通不含银行流水的两栏 `manual_confirmed` relation 仍作为 canonical `case:<case_id>` open group 等待补齐三栏或显式业务例外。OA 自带附件发票 source binding 只证明父 OA 归属和不可拆，不是 paired 例外；缺少银行流水或不满足 relation metadata requirement 时必须保留在 open 区。外部往来 `turnover_manual_closure` 写入 active relation 后证明外部往来收支闭环和 row ownership，分区同样只按 relation metadata 的 required row type 判定。relation metadata 指定的 ETC summary/invoice summary 必须随同一 case 发布，不能在 open 区留下 standalone 残留。自动 paired decision 不再作为业务关系直接展示；matching worker 必须先通过 `WorkbenchRelationCommandService.confirm_relation(...)` 创建或升级 active relation，并把 decision 标记为 `consumed`。只有 free、paired、paired display、金额核对 matched、无 active 冲突且同 row-set 未撤回的自动 decision 才能直接正式化；open/proposed decision 只作为内部建议，未正式化前按未配对处理。

自动 decision 正式化后允许保留原 `decision:*` case identity；`case:decision:*` 只能表示历史来源，不能替代当前关系状态。groups list、group detail 和 all-scope owner 仲裁必须优先读取 `group_type=manual_confirmed` / 正式 `relation_mode`，只隐藏仍无正式关系状态或明确为 `automatic_decision` / `automatic_match` 的旧可见组。查询语义变化只 bump groups page cache schema，不得为此改写 canonical relation 或强制重建 active generation。
...
关联台 selection 以 group 为操作上下文：已配对区和未配对区点击任意 OA、银行流水或发票 row，都会带入该 row 所在的完整 group。确认关联、异常处理和撤回入口都基于该 group context；撤回一次只能处理一个 active relation group。已配对区只有撤回关联语义；没有 active relation 的未配对 automatic decision/candidate 不是可撤回关系，preview 必须按 relation not found 拒绝，不能 suppress 候选事实源。
