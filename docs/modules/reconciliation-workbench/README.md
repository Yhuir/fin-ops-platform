...
撤回 preview 的“操作后”三栏必须按真实 after relation 分组；没有被恢复进 after relation 的 row 逐行独立展示，不能因为 row 上残留旧 `case_id` 又合成一行。提交成功后前端不再用本地 optimistic paired/open 重排伪造结果；写操作进入 `GlobalOperationOverlayProvider` 全屏等待后端返回的 operation freshness targets（通常为受影响月份的 `workbench_relation` 与 `workbench` shard）达到 fresh，再应用同一写 API 返回的后端 operation projection 替换受影响 group。`workbench:all` active generation 继续后台聚合追赶，不作为确认/撤回 overlay 释放的阻塞条件；它仍必须最终 fresh，不能伪装同步。OA sync dirty/refreshing、无权限和 App Health write safety blocked 仍必须阻断写入；普通 read model blocked/red 只影响读侧诊断和具体 API precondition，不应全局禁用无关写操作。
