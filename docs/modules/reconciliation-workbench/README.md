...
补充凭证现在按子付款项保存一个明确总金额，与多份文件原子编辑；同项正式发票与凭证合计、历史未知金额不推断。API、计额、异常、三栏和旧入口移除规则见 [凭证金额闭环](boundary-io.md#2026-09-21-子付款项凭证金额闭环)。
...
发票详情也不创建第二套状态：行状态来自当前 `invoice_bank_relation`，原始 `detail_fields.status=pending` 在 Workbench API 映射边界移除，不能显示或改名成“待处理”。同一边界只翻译已知的内部发票种类与来源值；共享详情组件和其它页面不承担此规则。
...
- 页面只使用紧凑三栏：已配对/未配对各自只渲染一套 `WorkbenchZone`，标题、区域搜索、选择汇总与区域动作保持同一工具栏；银行流水的“全部 + 年月”筛选固定在区域标题栏最右侧、栏显示菜单之前，已配对与未配对在本次访问分别保存筛选状态，重新进入页面时两区日期恢复全部，非日期偏好继续按 session 恢复。栏显示菜单只允许切换 OA、银行流水、进销项发票，至少保留一栏。经典布局、紧凑/经典切换、区域放大/恢复及对应状态和样式已删除，不得恢复并行布局路径。紧凑三栏隐藏横向滚动与还借款日期列，长文本在单元格内直接换行显示，不创建行单元格 hover Popover；银行流水和发票的合法行级动作收进 `···`，OA 仍无行级操作列。
- 选择汇总的成员数量按 canonical typed identity 去重，金额沿用关系方向：付款选择的银行金额为支出减收入，收款选择反向计算；正式关系通常消费组级 `amount_check` 权威三栏金额。唯一需要结合选择上下文计算的是 `amount_check.direction=unknown` 的纯银行正式关系：当组合选择已由 OA/发票确定主方向且正式银行成员完整加载时，按该方向计算关系内收支净额，不能沿用脱离上下文的绝对值总额。主方向未知、冲突、正式银行成员不完整或其他正式金额合同缺失时显示 `--`，不猜测、不发额外请求，也不阻断完整正式关系的既有选择动作。
...
- 费用子项展示中，缺少 canonical ownership、合计不等、显示组成不完整的父 OA 级一对多，以及重复来源、重复金额、零/非法金额、方向未知或冲突，都不建立推断同行；无显式来源的 2～6 条金额即使合计相等也不做组合推断。已分段栏中的这些记录进入独立残余展示带，完全没有精确配对的栏继续按 group-level 占满整组高度。金额兜底不写 relation、不改变 membership、selection 或 action identity。
...
普通多 OA 关系优先消费后端 `display_subgroups`：历史 OA/银行小组保留为同一共享行轨，支持多对一，不重复渲染金额；未能唯一确定的成员保留共享区块。发票保持组级共享，已有报销费用子项与凭证来源分段不变。完整规则见 [boundary-io](boundary-io.md#2026-09-12-历史-oa流水展示分段)。
