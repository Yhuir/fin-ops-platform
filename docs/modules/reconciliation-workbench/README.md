...
- 页面默认使用紧凑三栏：已配对/未配对各自只渲染一套 `WorkbenchZone`，标题、区域搜索、选择汇总与区域动作保持同一工具栏；栏显示、紧凑/经典布局和放大/恢复统一收进一个 HeroUI 菜单。经典三栏仅作为同组件的临时对比模式，不并行挂载、不复制请求、选择状态或 mutation 回调。紧凑模式隐藏横向滚动与还借款日期列，长文本按列合同省略并通过 HeroUI Popover 展示全文；银行流水和发票的合法行级动作收进 `···`，OA 仍无行级操作列。
