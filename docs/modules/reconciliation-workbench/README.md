...
OA 附件状态固定为“无OA附件 / OA发票附件未解析 / OA发票待归属”。只有“未解析”显示“录入发票”，打开单一右侧抽屉并在内部切换补充凭证上传与多张手工录入；补充凭证不进入发票池，手工发票整批进入统一发票池并原子关联当前 OA 子付款项。金额异常只在唯一来源证明具体单票时落该发票，否则落 OA 子项或 OA/流水组级位置。
...
- 异常抽屉宽度固定为 `min(1740px, 96vw)`。折叠态只显示 OA、流水、发票三栏金额/数量摘要，并在展开箭头前显示一个感叹号；hover/focus/click 后由 HeroUI Popover 展示异常 Chip。展开态直接复用只读 `RelationGroupGrid`，所以感叹号与主表定位完全一致；动作区只保留决定按钮。不得新增关系汇总栏、第四栏、重复 Chip、逐项复选框、人工金额下拉或原生 HTML 表单控件。
