...
| 2026-06-15 | canonical relation 已 active，但 all-scope active generation 仍从旧 month shard 发布同一银行流水/发票为 open temp group，导致预览页看起来可再次确认。 | `tests/test_workbench_sql_runtime.py::WorkbenchSqlRuntimeTests::test_repository_all_scope_suppresses_open_rows_claimed_by_canonical_active_relation`、`tests/test_workbench_sql_runtime.py::WorkbenchSqlRuntimeTests::test_repository_generation_consistency_reports_active_relation_rows_in_open_zone`、`tests/test_workbench_v2_api.py::WorkbenchV2ApiTests::test_confirm_link_preview_for_already_active_relation_returns_withdraw_preview`、`web/src/test/WorkbenchSelection.test.tsx::confirm preview for an already linked selection submits withdraw instead of confirm` | covered |
