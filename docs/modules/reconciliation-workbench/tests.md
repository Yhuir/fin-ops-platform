...
- 性能与边界：分类为现有 group 内固定规模纯计算，前端定位为单次 Map/Set 遍历；未新增 API round-trip、逐行 I/O、表、migration、read model、worker、cache 或依赖。PostgreSQL candidate 仍在分页前用三栏总额、成员和附件状态计算 review fingerprint，但不再为全量关系递归构建费用项—发票连通分量；附件归属只扫描含 OA 费用项的关系组，并只接受 canonical `source_expense_item_id = expense_item_id`，旧行号候选链不得恢复；仍有生产消费者的父 OA 历史身份桥继续只用于“待归属”证明。分页前只读取原费用项数组，补充凭证由当前可见页 hydration 唯一加载。PostgreSQL integration 同时保护 SQL/Python fingerprint 一致、旧递归/行号候选 CTE 和全量 supporting-document 投影缺席，以及 summary/detail 分区一致。
