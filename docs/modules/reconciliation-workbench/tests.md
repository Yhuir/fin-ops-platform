...
| 2026-06-14 | canonical relation 已写入且 `workbench_relation` 已 fresh，但 relation mutation 没有刷新 Workbench active generation，导致关联台继续读取旧 generation，把已确认银行流水和发票显示在不同行。 | `tests/test_workbench_relation_repository.py::test_relation_downstream_refresh_routes_scope_keys_by_row_domain`、`tests/test_write_operation_slo_audit.py::WriteOperationSloAuditTests::test_workbench_relation_confirm_profile_requires_canonical_relation_and_downstream_scopes` | covered |
