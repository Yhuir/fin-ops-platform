...
- Repository/SQL：`tests/test_workbench_page_query_repository.py` 保护 candidate 与 summary hydration 都读取 payload、owned item/attachment、indexed attachment cache identity bridge 和 active OA source alias；`tests/test_oa_projection_sql_runtime.py` 与 `tests/test_workbench_query_service.py` 保护 full/detail 的既有 row-id 查询在单条 statement 内携带同一 alias 集合；`tests/test_oa_attachment_invoice_linking.py` 以两个历史 parent 子付款项证明按 `row_index` 精确映射当前 canonical items。
