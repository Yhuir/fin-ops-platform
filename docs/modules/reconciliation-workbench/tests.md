...
- 未配对 canonical row 若携带旧 `candidate:` / `decision:` / `temp:` ownership，输出必须清理与该候选 ownership 绑定的 mode 装饰且仍保持 singleton；active override 必须优先于同 row 的 exception projection，`pending_input_invoice` 等合法 control fields 必须与 canonical override 精确一致。
