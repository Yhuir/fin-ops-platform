...
- Frontend component / interaction：`WorkbenchApi.test.ts`、`WorkbenchExceptionDrawer.test.tsx`、`WorkbenchSelection.test.tsx` 覆盖 URL/DTO 映射、标题同行的最外层 bucket、两类视图、四个父组下的七分类数量、空分类、切换时 latest-wins、默认分类 cursor 续读不回填 code、bucket 总数不被当前分类 `page.total` 覆盖，以及接受/撤回后保持当前 bucket/view/code 并只刷新当前列表。
- Browser E2E：`workbench-exception-flow.spec.ts` 保护首开金额分类、四组七分类层级、游标加载更多仍省略自动 code、切换仅资料视图、录票前关闭异常抽屉，以及 accept/keep/withdraw 不自动跳转、Popover 和响应式链路。
...
- 业务：`test_workbench_amount_check_service.py` 验证凭证实际金额、多文件计一次、未知/零、同项正式发票加凭证、发票依据变化要求重新确认及关联撤回、明细差额抵消、OA 子项金额变化使审阅指纹失效、正式发票缺金额不输出假合计。
...
- UI：`RelationGroupGrid.test.tsx` 覆盖三张票同段、票少加号录入精确目标、只读/异常允许录入/无权限边界、旧按钮消失。主页面和异常共用 grid；初次待归属抽屉的失败与刷新回归保持。
...
- `RelationPreviewTriPane.test.tsx`：精确 before/after 组、明确子组同行、共享发票跨行、歧义多对多不伪配对、同名同金额500个成员不截断、拆分父金额与部分成员标签、空组、税额说明移除、详情按需显示双方税号且卸载清理。
- `WorkbenchSelection.test.tsx`：沿用原确认/撤回请求、金额状态、版本/幂等、权限、提交错误与回读失败用例；更新紧凑表格定位及备注必填断言。
- `workbench-preview-layout.spec.ts`：真实浏览器验证桌面左右关系边界、footer 备注位置、700px 切换及备注保留、2 OA +15 流水大组、长公司名受控显示、双方税号在详情完整可读，税额说明不显示、单一滚动区、无额外 preview/submit 请求。截图关闭动画，以稳定布局验收。
...
### 2026-09-25 预览关系表精简回归

- 新增拆分撤回浏览器样例：操作前一组、操作后五组；同父流水按一笔计数、每个关系仅展示实际子项，主金额始终为1001497.22，标签浮层金额1497.22。五组完整显示且不发真实提交请求。
- 详情 hover→移入阅读→Escape→点击重开→键盘聚焦全链验证；桌面/窄屏、长名称、15笔流水和备注保留继续覆盖。组件测试验证详情不提前渲染、父数据不变和500个不同成员不丢失。
- 生产 route-shell smoke 额外识别“OA 会话已失效／登录状态已过期”，过期 token 不能再算作页面验证通过。生产需有效管理员登录态；不得跳过认证来宣称业务链路成功。

### 2026-09-25 黄蓝关系背景与居中验收

- 组件增加未拆分收支金额／银行／分类chip、重排与新增组的着色测试；继续保护精确成员、共享发票、歧义多对多、拆分父金额、空组和500成员。
- Playwright直接检查黄蓝computed background、前后标题底色差异、记录垂直居中和金额chip水平居中的几何位置；新增两笔8000元流水共享16000元发票样例，保护发票跨行只显示一次。原长文本、窄屏、5组拆分撤回、hover／键盘详情与提交链路回归保留。
