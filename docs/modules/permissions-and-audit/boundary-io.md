...
| Audit event | business service/route | unsafe HTTP route 先经 operation history 语义注册表归一为稳定 action code/用户动作/对象文案，再记录服务端 session 的 actor id/name/account 快照和结果；不信任 body actor |
