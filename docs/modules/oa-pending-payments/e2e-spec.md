...
| `OA-PENDING-E2E-007` | in-progress OA 人工关联支出流水并自动写回 | 抽屉默认展示全部支出流水，已配对/已关联进行中 OA 行禁选；提交创建 OA 待付款独立 pending relation 和 bank claim，不写 Workbench active relation，并在后端校验通过时自动写回 OA MySQL pay status；成功后等待 `oa_pending_payment` operation barrier fresh 再刷新 rows。 |
