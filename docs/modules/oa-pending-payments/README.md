...
- `backend/src/fin_ops_platform/services/已删除 invoice_usage_collection_sql_projection.py`
- `backend/src/fin_ops_platform/services/已删除 invoice_usage_collection_read_model_refresh.py`
- `backend/src/fin_ops_platform/services/invoice_usage_collection_source_versions.py`

## 当前边界

关注 OA 单据、支出银行流水、进项发票、Workbench relation、direct API rows/filter-options、invoice lifecycle 分发、OA MySQL 支付状态写回、详情 drawer 和异常反馈。OA 待付款以 OA 申请为主行，`paymentStatus` 由 `InvoiceLifecyclePolicy` / `OaPendingPaymentQueryService` 或等价 lifecycle read boundary 判定；页面不得自行定义付款状态。
...
- `in_progress` 视图的准入事实源才是 OA MySQL `t_payment_simple.flow_id`。direct read boundary 先读取该表的有效 `flow_id`，再用 `flow_id` 匹配 OA Mongo `form_data._id`；只有匹配成功且当前仍进行中的 OA 才进入进行中表格。
...
- `in_progress` 仍保留“关联支出流水”右侧抽屉，作为自动匹配失败后的人工兜底。抽屉从已选 OA 打开时必须携带 `oa_row_ids`，候选接口按这些 OA 的月份读取支出流水；没有 OA 上下文的旧调用才保留全部支出流水语义。抽屉可按全部、未配对、已配对、已关联进行中 OA 筛选；只有没有 Workbench active relation、也没有 active pending bank claim 的支出流水可选择。该关联成功后同样触发自动写回：支出流水合计等于 OA 金额且可解析 `flow_id` 时，把 `t_payment_simple.pay_status` 写成已支付，并让表格 chip 从“未写回”刷新为“已写回”。
...
- 当 OA sync 发现已有关联流水的进行中 OA 变成已完成时，`OaPendingPaymentRelationPromotionService` 会把 active pending relation promotion 为普通 Workbench active relation，并把原 pending relation 标记为 `promoted`、释放 bank claim；页面通过 direct rows/detail 重新读取，不再依赖 Workbench/OA 待付款 page read-model refresh。迁移前残留的 `special_metadata.origin=oa_pending_payment_in_progress` Workbench active relation 由 migration `0073_oa_pending_payment_bank_relations.sql` 撤回并迁移到独立 pending relation。
...
当前页面 rows/filter-options/detail 直接通过 OA 待付款 API 读取后端组装结果；rows 和 relation-details 响应不返回 `readModelStatus`、`read_model_status`、scope key、stale reasons 或 refresh flags。前端只按 direct rows/filter/detail payload 渲染，不显示页面级刷新诊断、不阻塞自动匹配、不轮询 operation barrier。detail drawer 只消费 `detailAvailable=false` 的本地不可用语义。

后端 `OaPendingPaymentReadModelService` 已删除；`oa_pending_payment` SQL read model、dirty scope 和 worker 只作为历史迁移对象或负向 guard，不作为页面 rows/filter/detail 的架构合同。

历史 `oa_pending_payment:all` fan-out scope 不再是页面读取合同。当前 all/month 查询必须由 direct query service 读取实际 rows、OA projection、bank/import facts 和 relation facts；前端页面不使用 legacy freshness 证明决定 rows/filter UI。
...
- 跨页面刷新、domain event、derived lifecycle、outbox/background job 或缓存边界变化。
