...
- `completed` 和 `in_progress` 只要已经存在有效支出流水 active relation 或 active pending relation 且支出合计等于 OA 金额，都可由显式“自动匹配/写回”命令写回 `t_payment_simple.pay_status=1`；页面首屏读路径不得自动发起写命令，也不再提供人工“确认已支付并写回”按钮。
