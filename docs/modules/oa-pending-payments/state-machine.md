...
| 支出流水抽屉候选 | `unmatched` / `matched` / `linked_in_progress` | bank import facts + Workbench active relation + `app.bank_transaction_relation_claims` | 抽屉默认展示全部支出流水；只有没有 Workbench active relation、也没有 active pending bank claim 的 `unmatched` 流水可被用户选择并关联到进行中 OA。 |
...
| bank link drawer | 用户在进行中 OA 视图勾选未写回 OA 后打开 | 调用 `GET /api/oa-pending-payments/bank-transaction-candidates` 默认展示全部支出流水；支持全部、未配对、已配对、已关联进行中 OA 筛选；只有未配对流水可勾选。提交后调用 `POST /api/oa-pending-payments/link-bank-transactions` 创建 OA 待付款独立 pending relation 和 bank claim；若支出合计等于 OA 金额且 `flow_id` 可解析，响应同时携带自动写回结果。 |
