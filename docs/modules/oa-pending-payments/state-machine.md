...
> 修改 `OA待付款核对` 相关业务状态、UI 状态、direct API 数据流、derived data/worker legacy 链路前必须读取本文件。OA 待付款状态必须由后端 policy/read boundary 给出，页面不得自行推断。
...
| 流程视图 | `in_progress` | `oa_application.workflow_status` | 进行中 OA 视图；下一次 OA sync 或 direct API 读取发现 OA 已完成后必须从该视图移除。 |
...
- 普通 `app.oa_applications` 投影承载已完成/历史未知 OA，并作为 `completed` 视图的统一事实源；OA 待付款 direct read boundary 同时接入普通 completed projection 和专用 payment-admitted projection。设置页手工搜索/导入状态筛选只影响手工导入行为，不能把已准入的进行中 OA 从本页面源头过滤掉。
- `t_payment_simple.id` 不得当作 OA ID；页面可以携带它作为支付状态记录诊断字段，但 OA 匹配和写回只使用 `flow_id`。
- `oa.sync` 完成后只更新 OA projection / promotion facts；不再入队 `oa_pending_payment.read_model.refresh` 页面 read-model scope。页面通过 direct rows/filter/detail API 读取当前 projection。
...
- `oa_pending_payment` 月份派生数据刷新成功读取当前 `t_payment_simple.flow_id` 准入集合后，必须取消同月 active pending relation 中完全不再准入的 OA 关系并释放对应 bank claim，让流水回到关联台候选/未配对口径；支付状态 repository 不可用或准入源读取失败时不得释放，避免把外部依赖故障误判为准入消失。
...
- 禁止前端 rows/filter-options 消费 direct payload freshness 字段来隐藏 rows、替换空态、阻塞自动匹配或轮询刷新。
- 禁止把 detail unavailable 当作 rows/filter 页面级状态；`detailAvailable=false` 只在 drawer 内展示“详情暂不可用”。
- 禁止旧 `oa_pending_payment.read_model.refresh` event 或 worker 回流；不得用 legacy rebuild/fan-out 覆盖 direct projection/canonical facts。
...
| refreshing | 用户显式刷新 rows/filter-options 时请求未完成 | 保持当前表格语义或展示表格内刷新文案；不读取 direct payload freshness 字段。 |
...
| detail unavailable | detail API 返回 `200` / `detailAvailable=false` | drawer 展示“详情暂不可用”和后端业务原因。 |
...
| auto reconcile | 页面具备写权限、rows/filter-options 直接请求已完成且页面无错误 | 调用 `POST /api/oa-pending-payments/auto-reconcile-bank-transactions`；自动匹配 in-progress OA 与未配对支出流水，并对 completed/in-progress 中已匹配流水的 OA 自动写回 `t_payment_simple.pay_status=1`。rows 加载失败或页面正在刷新时不得触发自动写命令；成功后直接刷新 rows，不再等待 operation barrier。 |
...
- OA 待付款页面主要靠重新请求 direct API/read boundary 收敛，不把同浏览器 domain event 或旧 read model status 当事实源。
- rules drawer 使用 pending invoice rules API；保存后的真实 fan-out 由后端 lifecycle、outbox/background job facts 或 direct API 重新读取证明。
- 页面卸载后不 replay 事件；返回页面重新加载 rows/filter-options。

## Runtime / Guard 状态

OA 待付款页面当前没有 page-level read model worker。rows/filter/detail 只由 direct query service、OA projection、payment-admitted in-progress OA、银行/发票事实、Workbench relation 和 OA pending relation 组装。

当前规则：

- OA sync / OA rebuild 只更新 OA projection、promotion facts 和真实后台任务状态；不投递 `oa_pending_payment.read_model.refresh`。
- 银行流水导入、发票导入、Workbench 关系确认/撤回、OA 待付款自动匹配/写回、进行中 OA 抽屉关联支出流水、pending relation promotion、batch accounting relation change、turnover relation change 后，页面通过 direct rows/filter/detail 重新读取最新事实。
- 待找发票规则保存和人工发票相关事件只通过真实 lifecycle/outbox/background job 或 direct refetch 影响页面；不得恢复 invoice usage collection page read-model worker。
- `startup_stale_scan` 默认关闭，且不直接刷新 OA 待付款 read model；只有后续 matching 结果真实变化并触发业务 lifecycle 时才间接影响。
- 历史 legacy projection missing/stale/source mismatch 只作为下线前诊断或负向 guard；页面 rows/filter-options/detail API 保持 direct loading/error/empty/detailAvailable 语义。
- App Status 读取真实 worker heartbeat、background jobs、outbox、dependencies 和 OA sync 状态；不得把 OA pending page read-model readiness 重新接回 domain 状态。

失败恢复：

1. 先看 `/api/app-health.app_status` 中 `oa_pending_payments` domain、OA sync、background jobs、outbox、dependencies 和 required worker heartbeat。
...
4. 如果 detail 返回 `detailAvailable=false`，先确认 direct detail 所需 OA/bank/invoice/relation facts 是否可用，不手工查 live facts 补页面。
5. 如果页面长期不收敛，检查 direct query service 输入事实、OA sync、pending relation、Workbench relation 和真实 background jobs；不要手工 rebuild page read model。
6. 如果生产仍出现 legacy read-model worker 相关日志，先确认服务器已发布到当前 release 并移除旧 systemd/env；不要用一次性手工 rebuild 判定生产已闭环。
...
| 2026-06-22 | 自动匹配增加 direct payload gate 与 API HTML fallback 防护 | rows/filter-options 仍 refreshing 或加载失败时不触发自动匹配/写回；前端 API 遇到根 `/api/*` 或 `/fin-ops/api/*` HTML fallback 时重试 canonical `/fin-ops-api/*`，避免路径错配把后台自动命令错误暴露给用户 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/src/test/apiClient.test.ts` |
