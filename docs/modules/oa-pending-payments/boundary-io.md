...
## Audit v21 relation source 与 object identity（2026-07-12）
...
- shared relation 的银行成员类型允许 canonical `bank` 与 `bank_transaction` 两种登记值；query/projection 边界必须归一到同一银行对象并把每个成员写入 `bankTransaction.summaries`。不得因类型别名跳过关系成员。
