...
| payment reconcile event | `job.outbox_events` / `oa-sync` worker | relation writer 同事务登记 `oa.payment_status.reconcile`；worker 按最新 active OA+outflow topology 幂等写外部状态与 PostgreSQL snapshot。完整 OA 权威快照确认已管理 flow 消失时，同事务删除 PG snapshot 并登记 `remove_missing_oa_statuses`；worker 在外部删除前复查 completed + admitted canonical flow 并批量删除 MySQL 状态。 |
...
- payment status：`app.oa_pending_payment_status_snapshots`，OA sync/payment reconcile owner；支付状态只由最新 active OA+outflow topology 收敛，不持久化写回归属门禁。支付状态从属于 canonical OA：只有完整 `all` 同步能声明曾管理 flow 已删除；month sync、精确附件刷新和 retention 裁剪都无权删除外部状态。
...
| authoritative OA deletion | OA source snapshot -> `oa.payment_status.reconcile` -> MySQL adapter | 完整 `all` 扫描确认曾管理 flow 不再属于 completed/admitted 后，PG 状态和关系在 snapshot 事务内清理，外部 MySQL 状态由 durable event 幂等批量删除；执行前复查 canonical OA，重现 flow 不删除。 |
