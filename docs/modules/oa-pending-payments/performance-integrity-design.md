...
- SQL计划为gate `0.090ms`、aggregate/facets `5.755ms`、bounded page `0.306ms`，无physical/temp I/O；304只执行gate。本地证据当时不支持新增 cache，但 2026-07-17 公网 1000 次 fresh `200` 的原基线 `p95=292.945ms` 与进程 rolling DB `p95=87.450ms` 已补充生产等量证据。版本化 cache release 部署后 1000/1000 fresh `200` 为 `p50=174.569ms`、`p95=281.536ms`、`p99=424.983ms`；cache-hit 去事务 release 的 50 次公网样本仍为 `p95=266.338ms`，rolling DB query count 从 6 降到 5。剩余 5 次操作中只有 1 次是 OA freshness gate，另外 4 次来自 global guard 与 module route 重复执行动态权限读取。最终最小收敛只删除 OA 路径上的前置重复 guard，继续由模块 read/write auth port 执行完整权限；不新增 session cache、index、partition、schema、worker或 API，也不修改其它页面鉴权路径。
