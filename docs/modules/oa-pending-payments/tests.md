...
| OA 自动匹配和自动写回 | P0 | `tests/test_oa_payment_status_service.py`、`tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts` | covered | 页面进入后调用 `auto-reconcile-bank-transactions`；后端复用关联台 OA-bank 精确金额/精确合计规则，只自动确认无冲突的 in-progress OA 与未配对支出流水；completed/in-progress 已有 active 支出流水 relation 且金额相等时自动写回 `t_payment_simple.pay_status=1`；前端写成功后等待 `oa_pending_payment` operation barrier fresh 再重新请求 rows；Browser 覆盖单次自动请求、成功提示、rows/read model 重新请求后 `已写回`、无人工写回按钮、409 失败可见且不半写。 |
