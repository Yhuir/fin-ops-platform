...
| OA 支付状态写回 | OA MySQL `t_payment_simple`、`OAPaymentStatusRepository` | `flow_id` 必须解析到 OA Mongo 文档 ID；`t_payment_simple.id` 不能当 OA ID；候选流水必须确认后才写回同一 `flow_id` 的 `pay_status=1`；非进行中 OA 不允许 confirm-paid。 |
...
| 进行中 OA 确认已支付写回 | P0 | `tests/test_oa_payment_status_service.py`、`tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | covered | 解析 `flow_id`、校验 OA 为进行中、校验 outflow 和金额相等、确认 Workbench relation、写回 `t_payment_simple.pay_status=1`、入队 workbench/oa_pending_payment 2 秒目标 refresh。 |
...
| 前端页面交互 | P1 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-flow.spec.ts` | covered | sidebar route、`已完成 OA/进行中 OA` 切换、两种视图统一 OA/支付状态/流水/发票四分组表格、确认已支付按钮、写回状态、首屏有界 `page_size=20` 请求、column filters/sort、drawer、rules drawer、empty、refreshing detail unavailable；Playwright 补充真实 Chromium 下首屏、搜索、筛选、排序和详情/规则抽屉闭环。 |
...
| 3. API contract tests | 适用 | `tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | 覆盖 rows/filter-options 的 `view_mode`、rows `summary.viewCounts` 前端使用、confirm-paid 成功响应 shape、validation/not found、权限、read model refreshing、detail unavailable 和 source version stale。 |
...
| 5. Frontend component and interaction tests | 适用 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/src/test/TableAlignmentStyles.test.ts`、`web/e2e/oa-pending-payments-flow.spec.ts` | 覆盖 loading/empty/error、首屏有界分页请求、`已完成 OA N条/进行中 OA N条` 切换、两种视图统一四分组表格、支付状态筛选、确认已支付按钮/POST body、写回状态、筛选/排序、drawer、规则 drawer、refreshing detail、OA/流水内部三栏和发票纵向展示 CSS/DOM 布局；Playwright 补充真实浏览器只读链路和无横向滚动断言。 |
| 6. End-to-end business-flow integration tests | 适用 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_pending_invoice_api.py`、`tests/test_derived_data_lifecycle_service.py`、`tests/test_invoice_usage_collection_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-flow.spec.ts` | 覆盖候选流水确认 -> Workbench relation confirm -> OA MySQL 写回 -> workbench/oa_pending_payment refresh enqueue；也覆盖规则保存/关系变化 -> lifecycle/dirty scope -> OA read model refresh -> 页面刷新语义；真实 worker drain 仍为 documented-risk。 |
