...
| 进行中 OA 确认已支付写回 | P0 | `tests/test_oa_payment_status_service.py`、`tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts` | covered | 解析 `flow_id`、校验 OA 为进行中、校验 outflow 和金额相等、复用已有 active relation 或确认 Workbench relation、写回 `t_payment_simple.pay_status=1`、入队 workbench/oa_pending_payment 2 秒目标 refresh；前端写成功后必须先等待 `oa_pending_payment` operation barrier fresh，再重新请求 rows，不得提前读旧投影；Browser 覆盖切换到进行中 OA、确认按钮 `确认中` 防重、POST body、rows/read model 重新请求后 `已写回`、成功后无操作失败/同步失败/read model 失败残留，以及 409 失败可见且不半写。 |
| 进行中 OA 右侧抽屉关联支出流水 | P0 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_workbench_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts` | covered | 抽屉默认请求全部支出流水，可筛选已配对/未配对/已关联进行中 OA；前端只允许勾选未配对流水；后端拒绝收入流水；提交创建 `origin=oa_pending_payment_in_progress` 的 Workbench active relation、刷新表格和关联台，不写 `t_payment_simple.pay_status`；Workbench 投影显示“已关联进行中OA”chip；前端写成功后必须先等待 `oa_pending_payment` operation barrier fresh，再重新请求 rows；Browser 覆盖 disabled candidate、relation_status 筛选、提交 body、rows/read model 重新请求、仍 `未写回` 且不调用 confirm-paid、成功后无操作失败/同步失败/read model 失败残留、409 失败可见且不半写。 |
...
| 5. Frontend component and interaction tests | 适用 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/src/test/TableAlignmentStyles.test.ts`、`web/e2e/oa-pending-payments-flow.spec.ts`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts`、`web/e2e/oa-pending-payments-nonfresh-flow.spec.ts`、`web/e2e/workbench-relations-oa-pending-fanout.spec.ts` | 覆盖 loading/empty/error、首屏有界分页请求、显式刷新入口、`已完成 OA N条/进行中 OA N条` 切换、两种视图统一四分组表格、支付状态筛选、确认已支付并写回按钮/POST body、进行中 OA checkbox、支出流水关联抽屉默认全部/已关联禁选/提交 body、写回状态、筛选/排序、drawer、规则 drawer、rows non-fresh 不当真实空态、rows 暂时失败不伪空态、refreshing detail、写后 operation barrier、OA/流水内部三栏和发票纵向展示 CSS/DOM 布局；Playwright 补充真实浏览器 rows 暂时 503 后错误态不伪空态/手动刷新恢复、只读链路、confirm-paid 成功/失败/防重/刷新链路、bank-link 抽屉禁选/筛选/提交/刷新/失败链路、成功写流无可见错误残留、rows/detail non-fresh 诊断链路、无横向滚动断言和 Workbench confirm 后 linked fan-out。 |
...
| 写后立即读旧 OA 待付款投影 | `web/src/test/OaPendingPaymentsPage.test.tsx::waits for the OA pending payment barrier before reloading after confirm-paid`、`web/src/test/OaPendingPaymentsPage.test.tsx::waits for the OA pending payment barrier before reloading after bank link`、`web/src/test/OaPendingPaymentsPage.test.tsx::waits for OA pending payment barrier before reloading after rule save` | confirm-paid、link-bank 和规则保存成功后都必须等待 `oa_pending_payment` operation barrier fresh，再重新读取 rows。 |
