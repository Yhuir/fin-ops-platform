...
- 新增/更新测试：`tests/test_oa_pending_payment_api.py::OaPendingPaymentApiTests::test_confirm_paid_route_is_removed_from_route_owner`、`tests/test_oa_pending_payment_command_service.py::OaPendingPaymentCommandServiceTests::test_manual_confirm_paid_command_is_not_exposed`、`tests/test_platform_runtime_boundary_guards.py::PlatformRuntimeBoundaryGuardTests.test_oa_pending_payment_routes_use_route_owner`、候选接口 filters 断言。
- 覆盖点：后端不再注册 `/api/oa-pending-payments/confirm-paid`，command service 不再暴露 `confirm_paid(...)`；测试 mock 不再提供旧 confirm-paid handler；`bank-transaction-candidates` 仍回显 `oaRowIds`，但不再输出 `monthScopes` 旧月份收敛字段。
- 验证命令：见本轮最终说明。
...
| 跨模块 fan-out | OA rebuild、发票导入、银行导入、关系确认/撤回、待找发票规则、invoice lifecycle | 必须最终刷新 `oa_pending_payment` read model；pending rules 必须由 `pending_invoice_rules_changed` 显式 lifecycle domain 入队，不得依赖 workbench invalidation 隐藏副作用。 |
...
| OA 已支付逐行写回 | P0 | `tests/test_oa_payment_status_service.py`、`tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_oa_pending_payment_service.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts` | covered | 页面进入后不得自动发起写命令；只有 `paymentStatus=paid` 且 `oaPaymentWriteback` 未写回的行显示“写回”；后端 `writeback-paid` 重新校验 completed Workbench active relation 或 in-progress active pending relation、outflow、金额合计和 `flow_id` 后写回 `t_payment_simple.pay_status=1`；已写回行 no-op 且不 enqueue refresh；无有效 active relation 时 409，不半写；前端写成功后等待 `oa_pending_payment` operation barrier fresh 再重新请求 rows；Browser 覆盖单次逐行写回请求、成功提示、rows/read model 重新请求后 `已写回` 且隐藏按钮、409 失败可见且不半写。 |
| 进行中 OA 右侧抽屉关联支出流水 | P0 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_oa_pending_payment_service.py`、`tests/test_workbench_sql_runtime.py`、`tests/test_workbench_relation_sql_projection.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts` | covered | 从已选 OA 打开抽屉时，前端候选请求必须携带 repeated `oa_row_ids`，后端返回全部支出流水，不按 OA 月份收敛；抽屉可按全部、未配对、已配对、已关联进行中 OA 筛选分类，并分页浏览超过首屏的候选流水；前端只允许勾选未配对流水；后端拒绝收入流水；提交创建 OA 待付款独立 pending relation 和 bank claim，不写 `app.workbench_pair_relations`；Workbench active generation 和 Workbench relation SQL projection 都会排除 active pending bank claim，避免进行中 OA 流水进入关联台未配对/候选；outflow、金额和 `flow_id` 校验通过时自动写回 `t_payment_simple.pay_status=1`；前端写成功后必须先等待 `oa_pending_payment` operation barrier fresh，再重新请求 rows；Browser 覆盖 disabled candidate、relation_status 筛选、提交 body、rows/read model 重新请求后 `已写回`、无人工写回按钮、成功后无操作失败/同步失败/read model 失败残留、409 失败可见且不半写。 |
...
| 进行中 OA 准入消失释放 pending claim | P0 | `tests/test_invoice_usage_collection_sql_runtime.py::InvoiceUsageCollectionSqlRuntimeTests::test_projection_builder_releases_pending_relation_when_oa_admission_disappears`、`::test_projection_builder_does_not_release_pending_relation_when_oa_admission_projection_is_refreshing` | covered | `oa_pending_payment` 月份 read model refresh 成功读取当前 `t_payment_simple.flow_id` 准入集合后，必须取消同月 active pending relation 中完全不再准入的 OA 关系并释放银行流水 claim；支付状态 repository 不可用或 OA admission projection 非 ready 时跳过释放，避免依赖故障误释放。 |
| OA 付款状态判定 | P0 | `tests/test_oa_pending_payment_service.py`、`tests/test_invoice_lifecycle_policy.py`、`tests/test_invoice_lifecycle_page_integration.py` | covered | OA 主行、decimal total、多流水合并、active linked relation 驱动 `paid`、无 linked relation 驱动 `unpaid`、金额差额不产生第三状态、lifecycle policy delegate。 |
...
| 缺失或非法付款证据 | P0 | `tests/test_oa_pending_payment_service.py` | covered | active linked 付款关系即显示 `paid`；缺失银行事实、非支出流水或金额差额不产生第三状态，但 `oaPaymentWriteback.syncStatus=not_required` 阻断写回按钮和写回命令。 |
| 服务端筛选/排序/分页 | P0 | `tests/test_oa_pending_payment_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_invoice_usage_collection_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | covered | keyword、month、bank account、bank direction、payment status、native SQL columns、非法参数；前端“全部 + 月份”合并控件默认不传 `month`，选择具体月份后传 `month=YYYY-MM`，点击“全部”清除月份；`test_page_size_limit_protects_first_screen_slo` 用 250 行 synthetic 数据验证后端 `page_size=200` 上限和 `page_size>200` 的 `invalid_paging`；前端首屏 rows 请求锁定 `page=1&page_size=20`，每页选项限制为 20/50/100。 |
| API contract | P0 | `tests/test_oa_pending_payment_api.py` | covered | rows、filter-options、OA/bank/invoice/detail、`kind=oa|bank|invoice` relation detail、bank candidates、writeback-paid、link-bank、旧 confirm-paid/auto-reconcile routes removed、错误 shape、权限 403。 |
...
| lifecycle fan-out | P0 | `tests/test_derived_data_lifecycle_service.py`、`tests/test_pending_invoice_api.py` | covered | OA rebuild / invoice lifecycle / pending rules 计划显式覆盖 `oa_pending_payment_read_model`；pending rules API 断言会入队 `oa_pending_payment`，并防止回退到 workbench 隐藏副作用。 |
...
| 前端页面交互 | P1 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-flow.spec.ts`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts`、`web/e2e/oa-pending-payments-nonfresh-flow.spec.ts`、`web/e2e/workbench-relations-oa-pending-fanout.spec.ts` | covered | sidebar route、`已完成 OA/进行中 OA` 切换、“全部 + 月份”合并选择器、两种视图统一 OA/支付状态/流水/发票四分组表格、已支付且 sync ready 的未写回行内写回、已写回行隐藏按钮、金额差额已支付行不展示写回按钮、进行中 OA 勾选与支出流水关联抽屉、写回状态、首屏有界 `page_size=20` 请求、显式刷新入口、column filters/sort、drawer、rules drawer、empty/error、rows non-fresh 诊断、rows 临时失败错误态、refreshing detail unavailable；Vitest 锁定首屏不自动写、writeback-paid/link-bank/规则保存成功后等待 `oa_pending_payment` operation barrier，barrier fresh 前不得刷新 rows；Playwright 补充真实 Chromium 下首屏、rows 暂时 503 后错误态不伪空态/手动刷新恢复、搜索、筛选、排序和详情/规则抽屉闭环，进行中 OA 逐行写回成功与失败闭环，进行中 OA bank-link 抽屉禁选/筛选/提交/自动写回/失败零半写闭环，rows/detail non-fresh 诊断闭环，并覆盖 Workbench confirm 后 OA 待付款行从 `未支付` 刷新为 `已支付`。 |
...
| 类别 | 是否适用 | 当前测试入口 | 说明 |
| --- | --- | --- | --- |
| 1. Business core unit tests | 适用 | `tests/test_oa_pending_payment_service.py`、`tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_payment_status_service.py`、`tests/test_invoice_lifecycle_page_integration.py` | 覆盖付款状态、金额边界、多关系合并、缺失事实、支出/收入方向、逐行写回状态流转、`flow_id` 准入/候选解析和 lifecycle policy delegate。 |
...
| 3. API contract tests | 适用 | `tests/test_oa_pending_payment_api.py`、`web/src/test/apiClient.test.ts`、`web/src/test/OaPendingPaymentsPage.test.tsx` | 覆盖 rows/filter-options 的 `view_mode`、rows `summary.viewCounts` 前端使用、writeback-paid 和 link-bank-transactions 成功响应 shape、bank-transaction-candidates、旧 confirm-paid/auto-reconcile routes removed、root `/api/*` 与 `/fin-ops/api/*` HTML fallback 到 canonical `/fin-ops-api/*`、validation/not found、权限、read model refreshing、detail unavailable 和 source version stale。 |
...
| 5. Frontend component and interaction tests | 适用 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/src/test/TableAlignmentStyles.test.ts`、`web/e2e/oa-pending-payments-flow.spec.ts`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts`、`web/e2e/oa-pending-payments-nonfresh-flow.spec.ts`、`web/e2e/workbench-relations-oa-pending-fanout.spec.ts` | 覆盖 loading/empty/error、首屏有界分页请求、显式刷新入口、`已完成 OA N条/进行中 OA N条` 切换、全部月份/具体月份合并选择器、两种视图统一四分组表格、支付状态筛选、首屏不自动写、已支付未写回行内写回、已写回行隐藏按钮、无旧 toolbar 自动匹配按钮、进行中 OA checkbox、支出流水关联抽屉携带已选 `oa_row_ids`、已关联禁选、提交 body、写回状态、筛选/排序、drawer、规则 drawer、rows non-fresh 不当真实空态、rows 暂时失败不伪空态、refreshing detail、写后 operation barrier、OA 内部五栏、流水内部三栏和发票纵向展示 CSS/DOM 布局；Playwright 补充真实浏览器 rows 暂时 503 后错误态不伪空态/手动刷新恢复、只读链路、writeback-paid 成功/失败/刷新链路、bank-link 抽屉禁选/筛选/提交/自动写回/失败链路、成功写流无可见错误残留、rows/detail non-fresh 诊断链路、无横向滚动断言和 Workbench confirm 后 linked fan-out。 |
| 6. End-to-end business-flow integration tests | 适用 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_pending_invoice_api.py`、`tests/test_derived_data_lifecycle_service.py`、`tests/test_invoice_usage_collection_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-flow.spec.ts`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts`、`web/e2e/workbench-relations-oa-pending-fanout.spec.ts` | 覆盖既有 active relation -> 逐行 writeback-paid -> OA MySQL 写回 -> oa_pending_payment refresh enqueue；覆盖进行中 OA 抽屉关联 -> 自动写回 OA MySQL -> OA pending/Workbench refresh enqueue；也覆盖规则保存/关系变化 -> lifecycle/dirty scope -> OA read model refresh -> 页面刷新语义；Browser e2e 覆盖 rows 暂时加载失败 -> 错误态/非普通空态 -> 手动刷新 -> fresh rows/pagination 恢复，进行中 OA writeback-paid -> rows/read model 重新读取 -> 写回状态更新且无错误残留、失败零半写，进行中 OA link-bank -> rows/read model 重新读取 -> relation evidence 和写回状态更新且无错误残留，以及 Workbench confirm -> OA pending rows fresh 后页面刷新；真实 worker drain 仍为 documented-risk。 |
...
| 历史问题 | 回归入口 | 保护点 |
| --- | --- | --- |
| 准入源消失后 pending claim 未释放 | `tests/test_invoice_usage_collection_sql_runtime.py::InvoiceUsageCollectionSqlRuntimeTests::test_projection_builder_releases_pending_relation_when_oa_admission_disappears`、`::test_projection_builder_does_not_release_pending_relation_when_oa_admission_projection_is_refreshing` | active pending relation 的 OA 不再出现在当前 `t_payment_simple.flow_id` 准入集合时，旧 bank claim 必须释放，否则流水既不在 OA 待付款 read model 行，也不回到关联台；OA admission projection 非 ready 时必须跳过释放。 |
...
| 进行中 OA 候选流水被直接写回 | `tests/test_oa_pending_payment_command_service.py::OaPendingPaymentCommandServiceTests::test_writeback_paid_rejects_row_without_paid_relation`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts` | 候选/未确认 relation 不能写回；后端必须校验 active relation、outflow、金额和 `flow_id`；Browser 覆盖首屏不自动写、逐行写回单次 mutation、成功后刷新为 `已写回`，失败时保留 `未写回`。 |
| 旧写入口回流 | `tests/test_oa_pending_payment_command_service.py::OaPendingPaymentCommandServiceTests::test_manual_confirm_paid_command_is_not_exposed`、`tests/test_oa_pending_payment_api.py::OaPendingPaymentApiTests::test_confirm_paid_route_is_removed_from_route_owner`、`tests/test_platform_runtime_boundary_guards.py::PlatformRuntimeBoundaryGuardTests.test_oa_pending_payment_routes_use_route_owner` | 后端不得恢复手工 `/api/oa-pending-payments/confirm-paid` route、旧 `/api/oa-pending-payments/auto-reconcile-bank-transactions` route、`OaPendingPaymentCommandService.confirm_paid(...)` 或 `auto_reconcile_bank_transactions(...)`；写回只能通过 `writeback-paid` 或 link-bank 成功后的自动写回路径。 |
...
| OA/流水/发票右侧详情抽屉排版退化 | `web/src/test/OaPendingPaymentsPage.test.tsx::opens OA, bank, relation drawers and reuses pending invoice rules endpoint` | OA 待付款详情抽屉继续复用 `oa_pending_payment` detail API；展示层使用 `InputInvoiceUsageDetailDrawer layout="table"` 渲染统一 key/value 表格，不改变 detail payload、read model 或 API I/O。 |
| 进行中 OA 抽屉候选接口错误按 OA 月份过滤导致空候选 | `tests/test_oa_pending_payment_command_service.py::OaPendingPaymentCommandServiceTests::test_bank_transaction_candidates_with_selected_oa_returns_all_months`、`tests/test_oa_pending_payment_command_service.py::OaPendingPaymentCommandServiceTests::test_bank_transaction_candidates_with_selected_oa_missing_month_still_returns_all`、`tests/test_oa_pending_payment_command_service.py::OaPendingPaymentCommandServiceTests::test_bank_transaction_candidates_filters_by_relation_status_tabs`、`tests/test_oa_pending_payment_api.py::OaPendingPaymentApiTests::test_bank_transaction_candidates_use_in_progress_projection_from_query_service`、`web/src/test/OaPendingPaymentsPage.test.tsx` | 抽屉候选请求携带已选 `oa_row_ids`；后端返回全部支出流水，不按 OA 月份限制；即使 OA 缺月份也不能返回空候选；filters 不再输出 `monthScopes`；全部、未配对、已配对、已关联进行中 OA 分类由后端 relation status 输出；API route 保留 repeated query params。 |
| 进行中 OA 抽屉候选超过首屏后无入口继续浏览 | `web/src/test/OaPendingPaymentsPage.test.tsx::paginates bank candidate drawer and keeps filter context` | 抽屉显示当前页/总页数；下一页请求带 `page=2`，并保留已选 `oa_row_ids` 和 relation status；切换分类后回到 `page=1`。 |
| 已写回的 active 支出流水 relation 反复触发写回和 read model refresh | `tests/test_oa_pending_payment_command_service.py::OaPendingPaymentCommandServiceTests::test_writeback_paid_is_noop_when_oa_is_already_written` | 已有 valid active relation 且同一 `flow_id` 已经 `pay_status=1` 时，逐行 `writeback-paid` 必须 no-op：不再次 mark-paid、不返回写回记录、不 enqueue read model refresh。 |
| 写后立即读旧 OA 待付款投影 | `web/src/test/OaPendingPaymentsPage.test.tsx::waits for the OA pending payment barrier before reloading after paid writeback`、`web/src/test/OaPendingPaymentsPage.test.tsx::waits for the OA pending payment barrier before reloading after bank link`、`web/src/test/OaPendingPaymentsPage.test.tsx::waits for OA pending payment barrier before reloading after rule save` | writeback-paid、link-bank 和规则保存成功后都必须等待 `oa_pending_payment` operation barrier fresh，再重新读取 rows；当 mutation 响应返回具体月份 scope 与 `all` 时，前端优先等待具体 `oa_pending_payment:<YYYY-MM>`，不把 fan-out-only `all` 当作优先可见性证明；规则保存无具体 OA refresh response 时 fallback 到当前可见 scope。 |
| 旧 `Application` rebuild 链路绕过 worker projection builder | `tests/test_oa_pending_payment_api.py::OaPendingPaymentApiTests::test_legacy_application_rebuild_helpers_are_removed`、`tests/test_invoice_usage_collection_sql_runtime.py` | OA pending payment read model rebuild 只能走 `InvoiceUsageCollectionReadModelRefreshService` -> `InvoiceUsageCollectionSqlProjectionBuilder` -> `OaPendingPaymentReadModelRepositoryPort`；`Application` 上不得恢复 `list_oa_pending_payment_scope_shards`、`mark_oa_pending_payment_scope_empty`、`rebuild_oa_pending_payment_read_model_scope` 或 live rows helper。 |
| rows read model 仍 refreshing 时触发写回 | `web/src/test/OaPendingPaymentsPage.test.tsx::does not show paid writeback while OA pending payment read model is still refreshing` | 页面必须先让 rows/filter-options 完成 fresh gate；refreshing/stale/unavailable 或加载失败时不得展示或触发 `writeback-paid`，避免后台同步中叠加写命令和刷新队列。 |
| 写回暂时失败后本页不再重试 | `web/src/test/OaPendingPaymentsPage.test.tsx::allows paid writeback retry after a failed attempt` | writeback-paid 请求失败时不得把行或 scope 永久标记为已完成；用户可再次点击行内写回按钮重试。 |
...
| completed OA 已有 active relation 但未写回 | `tests/test_oa_pending_payment_command_service.py::OaPendingPaymentCommandServiceTests::test_writeback_paid_writes_completed_oa_when_existing_relation_is_paid` | 逐行 `writeback-paid` 处理 completed 已关联流水时，必须从 active relation 解析 OA 和银行流水；缺失关系或金额/方向不合法不得静默写回 `t_payment_simple.pay_status=1`。 |
...
| 已支付未写回 OA -> 逐行写回 -> OA 写回 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_oa_pending_payment_service.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts` | Browser mock 已覆盖首屏不自动写、点击行内“写回”后写回、rows/read model refresh、read model non-fresh 时不展示写回按钮和失败零半写；staging 仍需配置真实 OA MySQL env，并确认 `t_payment_simple.pay_status` 与页面 `oaPaymentWriteback` 同步。 |
| 进行中 OA 勾选 -> 支出流水抽屉关联 -> 表格刷新并写回 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_oa_pending_payment_service.py`、`tests/test_workbench_relation_sql_projection.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts` | Browser mock 已覆盖已配对/已关联禁选、提交后 rows/read model refresh、自动写回和失败零半写；Vitest/API/command 测试补充锁定抽屉候选请求携带已选 OA row id，并返回全量支出流水候选池；staging 仍需用真实支出流水验证 OA 待付款独立 pending relation、关联台不显示该进行中 OA、OA 待付款刷新，以及 `t_payment_simple.pay_status` 被 link 后自动写成已支付。 |
...
- pending rules 对 OA 待付款的 fan-out 已由 `pending_invoice_rules_changed` 显式列出 `oa_pending_payment_read_model`，并由 `tests/test_derived_data_lifecycle_service.py` 与 `tests/test_pending_invoice_api.py` 共同保护；若新增规则影响其它 read model，必须继续扩展 lifecycle plan，而不是恢复 workbench 隐藏副作用。
