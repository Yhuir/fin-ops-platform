...
| 进行中 OA 确认已支付写回 | P0 | `tests/test_oa_payment_status_service.py`、`tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | covered | 解析 `flow_id`、校验 OA 为进行中、校验 outflow 和金额相等、复用已有 active relation 或确认 Workbench relation、写回 `t_payment_simple.pay_status=1`、入队 workbench/oa_pending_payment 2 秒目标 refresh。 |
| 进行中 OA 右侧抽屉关联支出流水 | P0 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_workbench_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | covered | 抽屉默认请求全部支出流水，可筛选已配对/未配对/已关联进行中 OA；前端只允许勾选未配对流水；后端拒绝收入流水；提交创建 `origin=oa_pending_payment_in_progress` 的 Workbench active relation、刷新表格和关联台，不写 `t_payment_simple.pay_status`；Workbench 投影显示“已关联进行中OA”chip。 |
...
| 前端页面交互 | P1 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-flow.spec.ts` | covered | sidebar route、`已完成 OA/进行中 OA` 切换、两种视图统一 OA/支付状态/流水/发票四分组表格、确认已支付并写回按钮、进行中 OA 勾选与支出流水关联抽屉、写回状态、首屏有界 `page_size=20` 请求、column filters/sort、drawer、rules drawer、empty、refreshing detail unavailable；Playwright 补充真实 Chromium 下首屏、搜索、筛选、排序和详情/规则抽屉闭环。 |
...
| 5. Frontend component and interaction tests | 适用 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/src/test/TableAlignmentStyles.test.ts`、`web/e2e/oa-pending-payments-flow.spec.ts` | 覆盖 loading/empty/error、首屏有界分页请求、`已完成 OA N条/进行中 OA N条` 切换、两种视图统一四分组表格、支付状态筛选、确认已支付并写回按钮/POST body、进行中 OA checkbox、支出流水关联抽屉默认全部/已关联禁选/提交 body、写回状态、筛选/排序、drawer、规则 drawer、refreshing detail、OA/流水内部三栏和发票纵向展示 CSS/DOM 布局；Playwright 补充真实浏览器只读链路和无横向滚动断言。 |
| 6. End-to-end business-flow integration tests | 适用 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_pending_invoice_api.py`、`tests/test_derived_data_lifecycle_service.py`、`tests/test_invoice_usage_collection_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-flow.spec.ts` | 覆盖候选流水确认 -> Workbench relation confirm -> OA MySQL 写回 -> workbench/oa_pending_payment refresh enqueue；覆盖进行中 OA 抽屉关联 -> Workbench relation confirm -> OA pending/Workbench refresh enqueue但不写 MySQL；也覆盖规则保存/关系变化 -> lifecycle/dirty scope -> OA read model refresh -> 页面刷新语义；真实 worker drain 仍为 documented-risk。 |
| 7. Existing feature regression tests | 适用 | 上述全部测试 + `tests/test_app_status_readiness_backfill.py`、`web/e2e/oa-pending-payments-flow.spec.ts` | 每次变更都要保护旧 API shape、旧 completed 视图筛选/排序/分页、旧 detail payload、权限、App Status、页面布局和浏览器层详情抽屉/规则抽屉入口。 |
...
| 进行中 OA 候选流水被自动写回 | `tests/test_oa_pending_payment_command_service.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | 候选只显示 chip 和“确认已支付”；POST 前必须用户点击，后端必须校验 workflow/outflow/金额/flow_id。 |
| 进行中 OA 抽屉关联被误当作已写回 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | link-bank-transactions 只创建 Workbench relation 并刷新表格；不调用 OA MySQL 写回；已配对/已关联进行中 OA 流水在抽屉禁选。 |
...
| 进行中 OA 候选流水 -> 确认已支付 -> OA 写回 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | staging 需配置真实 OA MySQL env，并确认 `t_payment_simple.pay_status` 与页面 `oaPaymentWriteback` 同步。 |
| 进行中 OA 勾选 -> 支出流水抽屉关联 -> 表格/关联台刷新 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_workbench_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | staging 需用真实支出流水验证 Workbench active relation、关联台“已关联进行中OA”chip、OA 待付款刷新，以及 `t_payment_simple.pay_status` 不被 link 动作修改。 |
...
- `scripts/verify.sh e2e` / `npm run e2e:smoke` 覆盖 Playwright browser smoke，其中 `web/e2e/oa-pending-payments-flow.spec.ts` 保护 rows/filter/sort/detail/rules drawer，`web/e2e/workbench-relations-candidate-semantics.spec.ts` 保护 candidate OA/银行/发票 chip 不把付款状态直接推成 paid。
...
- deterministic Playwright 已覆盖浏览器首屏、筛选/排序、OA/流水/发票详情、规则抽屉和 candidate/linked 负面语义；本地 synthetic page-size guard 不替代真实 PostgreSQL 大数据 EXPLAIN、浏览器滚动或真实网络中断恢复。
