...
| 影响面 | 当前事实源 | 需要关注的旧功能 |
| --- | --- | --- |
| 主行身份 | OA MySQL `t_payment_simple.flow_id` + payment-admitted OA projection / OA Mongo | 列表以已进入支付状态管理准入表且能匹配 OA Mongo `_id` 的 OA 申请为主行；缺少银行或发票时不能丢掉已准入 OA 行，未入表的重复/异常 OA 不能进入页面。普通 `app.oa_applications` projection 只服务已完成/历史未知 OA。 |
| 流程视图 | OA projection `workflow_status` + rows `summary.viewCounts` | `completed` 只含已准入且已完成/历史未知 OA，`in_progress` 只含已准入且进行中 OA；OA sync/read model refresh 后已完成 OA 必须从进行中视图移除；切换按钮数量必须按同一筛选口径统计。 |
...
| OA 支付状态写回 | OA MySQL `t_payment_simple`、`OAPaymentStatusRepository` | `flow_id` 必须解析到 OA Mongo 文档 ID；`t_payment_simple.id` 不能当 OA ID；候选流水、自动决策或 active relation 必须在用户点击“确认已支付并写回”后才写回同一 `flow_id` 的 `pay_status=1`；非进行中 OA 不允许 confirm-paid。 |
| 支出流水证据 | `ImportNormalizationService`、Workbench relation read facade | 只允许支出流水作为付款证据；收入流水或缺失流水事实必须进入异常/待复核，不得算已付。进行中 OA 抽屉关联只创建 Workbench relation，不写 OA MySQL。 |
...
| 场景 | 优先级 | 当前覆盖 | 状态 | 说明 |
| --- | --- | --- | --- | --- |
| `t_payment_simple.flow_id` 准入源和 tab 数量 | P0 | `tests/test_oa_payment_status_service.py`、`tests/test_oa_pending_payment_service.py`、`tests/test_invoice_usage_collection_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | covered | MySQL repository 返回最新 flow_id 支付状态；`PaymentAdmittedOAProjectionAdapter` 根据 flow_id 精确读取 OA Mongo row_id 候选；query service 先按 payment status 表准入再按 workflow status 分流；SQL read model 返回 `summary.viewCounts`；前端切换按钮展示 `已完成 OA N条 / 进行中 OA N条`。 |
...
| 进行中 OA 确认已支付写回 | P0 | `tests/test_oa_payment_status_service.py`、`tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts` | covered | 解析 `flow_id`、校验 OA 为进行中、校验 outflow 和金额相等、复用已有 active relation 或确认 Workbench relation、写回 `t_payment_simple.pay_status=1`、入队 workbench/oa_pending_payment 2 秒目标 refresh；Browser 覆盖切换到进行中 OA、确认按钮 `确认中` 防重、POST body、rows/read model 重新请求后 `已写回`、成功后无操作失败/同步失败/read model 失败残留，以及 409 失败可见且不半写。 |
| 进行中 OA 右侧抽屉关联支出流水 | P0 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_workbench_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts` | covered | 抽屉默认请求全部支出流水，可筛选已配对/未配对/已关联进行中 OA；前端只允许勾选未配对流水；后端拒绝收入流水；提交创建 `origin=oa_pending_payment_in_progress` 的 Workbench active relation、刷新表格和关联台，不写 `t_payment_simple.pay_status`；Workbench 投影显示“已关联进行中OA”chip；Browser 覆盖 disabled candidate、relation_status 筛选、提交 body、rows/read model 重新请求、仍 `未写回` 且不调用 confirm-paid、成功后无操作失败/同步失败/read model 失败残留、409 失败可见且不半写。 |
...
| 前端页面交互 | P1 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-flow.spec.ts`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts`、`web/e2e/oa-pending-payments-nonfresh-flow.spec.ts`、`web/e2e/workbench-relations-oa-pending-fanout.spec.ts` | covered | sidebar route、`已完成 OA/进行中 OA` 切换、两种视图统一 OA/支付状态/流水/发票四分组表格、确认已支付并写回按钮、进行中 OA 勾选与支出流水关联抽屉、写回状态、首屏有界 `page_size=20` 请求、显式刷新入口、column filters/sort、drawer、rules drawer、empty/error、rows non-fresh 诊断、rows 临时失败错误态、refreshing detail unavailable；Playwright 补充真实 Chromium 下首屏、rows 暂时 503 后错误态不伪空态/手动刷新恢复、搜索、筛选、排序和详情/规则抽屉闭环，进行中 OA confirm-paid 成功/失败/防重/刷新闭环，进行中 OA bank-link 抽屉禁选/筛选/提交/刷新/失败零半写闭环，rows/detail non-fresh 诊断闭环，并覆盖 Workbench confirm 后 OA 待付款行从 `支付少了` 刷新为 `已支付`。 |
| 表格样式/布局回归 | P1 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/src/test/TableAlignmentStyles.test.ts`、`web/e2e/oa-pending-payments-flow.spec.ts` | covered | compact table、OA 内部申请人/项目/金额三栏、流水内部对方户名/金额/摘要三栏、发票纵向展示、支付状态窄列、银行金额/方向 chip 非重叠、空流水/空发票 dash、项目下申请时间、真实 Chromium 下无横向滚动。 |
...
| 类别 | 是否适用 | 当前测试入口 | 说明 |
| --- | --- | --- | --- |
| 1. Business core unit tests | 适用 | `tests/test_oa_pending_payment_service.py`、`tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_payment_status_service.py`、`tests/test_invoice_lifecycle_page_integration.py` | 覆盖付款状态、金额边界、多关系合并、缺失事实、支出/收入方向、进行中 OA confirm-paid 状态流转、`flow_id` 准入/候选解析和 lifecycle policy delegate。 |
| 2. Service-layer tests | 适用 | `tests/test_oa_pending_payment_service.py`、`tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_invoice_usage_collection_sql_runtime.py` | 覆盖 query service、command service、OA payment status repository 最新 flow_id 列表、read model service、queue enqueue、detail lookup、projection builder、refresh service 和大页请求上限。 |
| 3. API contract tests | 适用 | `tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx` | 覆盖 rows/filter-options 的 `view_mode`、rows `summary.viewCounts` 前端使用、confirm-paid 成功响应 shape、bank-transaction-candidates、link-bank-transactions、validation/not found、权限、read model refreshing、detail unavailable 和 source version stale。 |
| 4. Read model/cache/background job tests | 适用 | `tests/test_oa_projection_sync_service.py`、`tests/test_invoice_usage_collection_sql_runtime.py`、`tests/test_oa_projection_sql_runtime.py`、`tests/test_postgres_migrations.py`、`tests/test_runtime_worker_registry.py`、`tests/test_app_status_overview_service.py` | 覆盖普通 completed-only OA projection、OA sync 到 OA 待付款 read model fan-out、payment-admitted OA projection、SQL read model view counts、source versions、all/month scope、worker event stale guard、registry 和 App Status。 |
| 5. Frontend component and interaction tests | 适用 | `web/src/test/OaPendingPaymentsPage.test.tsx`、`web/src/test/TableAlignmentStyles.test.ts`、`web/e2e/oa-pending-payments-flow.spec.ts`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts`、`web/e2e/oa-pending-payments-nonfresh-flow.spec.ts`、`web/e2e/workbench-relations-oa-pending-fanout.spec.ts` | 覆盖 loading/empty/error、首屏有界分页请求、显式刷新入口、`已完成 OA N条/进行中 OA N条` 切换、两种视图统一四分组表格、支付状态筛选、确认已支付并写回按钮/POST body、进行中 OA checkbox、支出流水关联抽屉默认全部/已关联禁选/提交 body、写回状态、筛选/排序、drawer、规则 drawer、rows non-fresh 不当真实空态、rows 暂时失败不伪空态、refreshing detail、OA/流水内部三栏和发票纵向展示 CSS/DOM 布局；Playwright 补充真实浏览器 rows 暂时 503 后错误态不伪空态/手动刷新恢复、只读链路、confirm-paid 成功/失败/防重/刷新链路、bank-link 抽屉禁选/筛选/提交/刷新/失败链路、成功写流无可见错误残留、rows/detail non-fresh 诊断链路、无横向滚动断言和 Workbench confirm 后 linked fan-out。 |
| 6. End-to-end business-flow integration tests | 适用 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_pending_invoice_api.py`、`tests/test_derived_data_lifecycle_service.py`、`tests/test_invoice_usage_collection_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-flow.spec.ts`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts`、`web/e2e/workbench-relations-oa-pending-fanout.spec.ts` | 覆盖候选流水确认 -> Workbench relation confirm -> OA MySQL 写回 -> workbench/oa_pending_payment refresh enqueue；覆盖进行中 OA 抽屉关联 -> Workbench relation confirm -> OA pending/Workbench refresh enqueue但不写 MySQL；也覆盖规则保存/关系变化 -> lifecycle/dirty scope -> OA read model refresh -> 页面刷新语义；Browser e2e 覆盖 rows 暂时加载失败 -> 错误态/非普通空态 -> 手动刷新 -> fresh rows/pagination 恢复，进行中 OA confirm-paid -> rows/read model 重新读取 -> 写回状态更新且无错误残留、失败零半写，进行中 OA link-bank -> rows/read model 重新读取 -> relation evidence 更新但 `未写回` 且无错误残留，以及 Workbench confirm -> OA pending rows fresh 后页面刷新；真实 worker drain 仍为 documented-risk。 |
...
| 进行中 OA 候选流水被自动写回 | `tests/test_oa_pending_payment_command_service.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts` | 候选/自动证据只显示可确认动作；POST 前必须用户点击，后端必须校验 workflow/outflow/金额/flow_id；Browser 覆盖确认中禁用、单次 mutation、成功后刷新为 `已写回`，失败时保留 `未写回`。 |
| 进行中 OA 抽屉关联被误当作已写回 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts` | link-bank-transactions 只创建 Workbench relation 并刷新表格；不调用 OA MySQL 写回；已配对/已关联进行中 OA 流水在抽屉禁选；Browser 断言 confirm-paid 零调用、刷新后仍 `未写回`。 |
...
| 网络波动重复提交 OA 进入进行中视图 | `tests/test_oa_pending_payment_service.py::OaPendingPaymentQueryServiceTests::test_payment_status_table_admits_oa_rows_before_view_mode_filtering` | 只有 `t_payment_simple.flow_id` 中存在的 OA Mongo `_id` 进入页面；未入表的重复/异常 OA 不展示。 |
| 普通 OA projection 混入进行中 OA | `tests/test_oa_projection_sync_service.py`、`tests/test_oa_projection_sql_runtime.py`、`tests/test_workbench_relation_sql_projection.py` | `oa.sync` 只写 completed/legacy 到 `app.oa_applications`，普通 list/read SQL 带 completed filter；待付款页面另走 payment-admitted projection。 |
...
| 进行中 OA 候选流水 -> 确认已支付 -> OA 写回 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-confirm-paid-flow.spec.ts` | Browser mock 已覆盖确认写回、rows/read model refresh 和失败零半写；staging 仍需配置真实 OA MySQL env，并确认 `t_payment_simple.pay_status` 与页面 `oaPaymentWriteback` 同步。 |
| 进行中 OA 勾选 -> 支出流水抽屉关联 -> 表格/关联台刷新 | `tests/test_oa_pending_payment_command_service.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_workbench_sql_runtime.py`、`web/src/test/OaPendingPaymentsPage.test.tsx`、`web/e2e/oa-pending-payments-bank-link-flow.spec.ts` | Browser mock 已覆盖抽屉默认全部、已配对/已关联禁选、提交后 rows/read model refresh、失败零半写和 `confirm-paid` 零调用；staging 仍需用真实支出流水验证 Workbench active relation、关联台“已关联进行中OA”chip、OA 待付款刷新，以及 `t_payment_simple.pay_status` 不被 link 动作修改。 |
