...
| 根 `/api/*` 被 SPA HTML fallback 吞掉 | `web/src/test/apiClient.test.ts::falls back to canonical fin-ops API prefix when root API returns the SPA shell under fin-ops`、`web/src/test/apiClient.test.ts::falls back to canonical fin-ops API prefix when a root API request returns HTML outside the fin-ops page path` | API client 在确认同源根 `/api/*` 返回 HTML 时，重试 canonical `/fin-ops-api/*`，不再依赖当前页面路径必须是 `/fin-ops/`；正常 JSON/API 错误不重试。 |
