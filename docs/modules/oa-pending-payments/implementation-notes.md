...
- 收口：只有完整 `all` OA 权威扫描能比较此前已管理 flow 与当前 completed/admitted 并集。确认消失后，同一 PostgreSQL snapshot 事务移除 PG status、清理 relation 并登记 `oa.payment_status.reconcile(operation=remove_missing_oa_statuses)`；relation cleanup 抑制无效 row-id 普通 reconcile，不保留并行旧路径。
...
- 边界：month sync、精确附件刷新和本地 retention prune 都不能证明 OA 源删除，禁止触发外部删除；未曾进入 canonical scope 的 external status 不被本系统宣称 ownership。页面 GET 不访问 MySQL/Mongo，热路径查询数量不变。
