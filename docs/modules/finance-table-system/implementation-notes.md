...
- 影响范围：全局 CSS 字体/数字/密度 token、成本统计时间标签、设计合同和静态回归门禁；不改表格业务 I/O、页面查询、API、read model、worker 或关联台内部结构。
- 关键决策：复用系统字体与现有 HeroUI/FinanceTable，不新增字体包和第二套组件；字号只保留不小于 12px 的业务层级，字重只保留 400/500/600/700，数字统一 tabular nums，表格行高明确为 36/44/60px。
- 文档影响：更新 `DESIGN.md` 和本实施记录；模块边界与 I/O 未变化，因此 `boundary-io.md` 不变。
- 测试覆盖：`DesignTokens.test.ts` 阻止小于 12px 的业务字号、非标准字重和未加载字体假设回归；`TableLayoutTokens.test.ts` 保护 36/44/60px 密度合同。
- 未测风险：系统字体会按操作系统选择本机字形；这避免网络字体下载和 CLS，但不同操作系统的中文字形仍可能有细微差异，需在生产常用浏览器做视觉 smoke。
