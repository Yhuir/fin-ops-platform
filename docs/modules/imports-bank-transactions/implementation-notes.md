...
- 历史 `import_files.raw_payload` 存在不保存文件级 `success_count/duplicate_count`、或保留初始化 `0/0` 但已有完整 `row_results` 的正式 schema。修复从 row results 的 `created/status_updated/duplicate_skipped` decision 重建 before/after 计数，并要求它们与同一 batch 的正式迁移前/后计数精确相等；两项文件计数原本同时存在时写入 after 事实，原本都不存在时不新增，缺失其中一项则拒绝。batch 的正式计数迁移始终执行，不把初始化占位值当业务事实。
