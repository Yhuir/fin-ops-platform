...
| 复核明细分页 | `GET /imports/files/sessions/{session_id}/review-rows?file_id={file_id}&offset&limit` | `limit` 最大 100；返回当前 session 的稳定切片和 `total/has_more`。session 摘要不携带无界 `row_results`、`normalized_rows` 或 `duplicate_groups`，页面不得从摘要恢复全量复核列表。 |
...
## 2026-09-23 拆分与导入撤销并发

导入撤销读取本批次独占父流水及当前子项身份后，按正式关联 owner 获取成员/case advisory 锁，再锁父流水；顺序与流水拆分保存一致。锁后重新读取子项及正式关系，若期间发生变化则明确冲突并整体回滚，不继续删除。原始银行事实与子项只能在既有撤销业务事务内清理，关联撤销、审计及刷新保持原 owner。真实 PostgreSQL 并发测试验证等待关系锁时不提前占有父行锁、变更子身份后撤销返回冲突且父事实保留。
