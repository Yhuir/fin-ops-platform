...
| 生产 cohort 恢复 | 精确 source session/file、目标/保护/重复删除数门禁、保护集合不相交、唯一 fingerprint/reference duplicate、指纹漂移时唯一 statement-position 二级证据、dry-run 匹配证据、关系/核销 fail-closed、文件 hash、审计计数 CAS、可重复受控重放 | `tests/test_bank_import_dedup_repair_service.py`、`tests/test_import_file_service.py`、`tests/test_import_audit_repair_ops.py` |
