...
| 5. Frontend component and interaction tests | 适用 | `web/src/test/ManualBankTransactionBatchEditor.test.tsx`、`web/src/test/ImportCenterPage.test.tsx`、`web/src/test/AppStatusIndicator.test.tsx`、`web/e2e/imports-bank-transactions-flow.spec.ts`、`web/e2e/permissions-role-matrix.spec.ts` | 覆盖手工抽屉入口、多笔增删、动态字段、校验、预览/确认、文件上传/字段映射 retry、错误、session restore、job feedback、App Status popover。 |
...
| 2026-08-29 手工流水被误当上传文件 | 手工流水 synthetic file 必须登记 `manual-bank-entry-audit.v1` 并明确不存在 file object/hash；真实上传文件仍使用 `import-page-audit.v1` 严格合同。迁移只修正 template/payload 同时证明为手工录入且没有文件 provenance 的旧行。 | fixed by `tests/test_postgres_repositories_core.py`、`tests/test_audit_bank_transaction_import_page.py`、`tests/test_postgres_migrations.py` |
| 2026-08-31 手工流水预览单行误显示“不可录入” | 手工预览响应必须在有界的 1–50 笔范围内返回逐笔 `decision/decision_reason`；`created` 与 `file_ids` 必须一致。普通 session 摘要继续裁剪无界 `row_results/normalized_rows`，前端不得把缺失 decision 静默解释成业务上的“不可录入”。 | fixed by `tests/test_import_file_api.py`、`web/src/test/ImportsApi.test.ts`、`web/src/test/ManualBankTransactionBatchEditor.test.tsx` |
...
| 2026-08-12 受控重放门禁缺少字段级诊断 | admin-only Page Audit 必须输出 duplicate row 的 decision/reason 登记状态、statement position 双方完整性和字段级 mismatch 名称，不暴露账户、金额、余额等业务原值；非 duplicate 行不附加该诊断。 | fixed by `tests/test_audit_bank_transaction_import_page.py` |
| 2026-08-12 去重恢复 owner-reclassification 被 Page Audit 误判为未登记重放原因 | repair 写入的唯一 reclassification reason 与三类 runtime replay reason 分开登记；Page Audit 可按严格 statement position 证明历史 owner 转移，字段漂移仍阻断，普通预览/确认/stale gate 不接受 repair reason。 | fixed by `tests/test_audit_bank_transaction_import_page.py`、`tests/test_import_preview_audit.py` |
...
- 下游页面最终展示依赖银行明细、成本统计等模块自己的 canonical query，以及关联台 active-generation 回归；本模块 Browser e2e 已覆盖银行明细导入行、成本统计银行事实展示和 OA 成本隔离，不覆盖真实生产大数据延迟或关联台完整分组。
