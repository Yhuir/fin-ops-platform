...
| 派生生命周期 | `_execute_derived_data_lifecycle_event("settings_reset_completed")` | 重置后旧 cache 不能伪装成当前页面事实 |
...
| Settings/Workbench UI 的 impact confirmation、OA password、progress reentry、权限隐藏、多页面 direct reload contract | `web/src/test/SettingsPage.test.tsx`、`web/src/test/WorkbenchSelection.test.tsx`、`web/e2e/settings-data-reset-flow.spec.ts` | 已覆盖；Browser e2e 覆盖真实 Chromium 下设置页影响确认、OA 密码复核、job polling、完成后 reload 与全局反馈，并在 reset 完成后进入银行明细验证 direct empty、进入待找发票验证 direct rows。 |
...
| 4. Cache/background job tests | 适用 | `tests/test_settings_data_reset_service.py`、`tests/test_app_health_api.py`、`tests/test_background_job_service.py`、`tests/test_runtime_state_policy.py` | 覆盖 lifecycle fan-out、cost statistics clear、job attention/active、runtime state policy | P1 | 真 Redis cache、真实 Postgres outbox/后台任务收敛需要 staging/nightly smoke |
| 5. Frontend component and interaction tests | 适用 | `web/src/test/SettingsPage.test.tsx`、`web/src/test/WorkbenchSelection.test.tsx`、`web/e2e/settings-data-reset-flow.spec.ts` | 覆盖确认、密码弹窗、cancel、错误、progress reentry、权限隐藏；Browser e2e 覆盖真实页面 job polling/reload 和 reset 后跨页重新读取银行明细/待找发票 direct API | P1 | 真实浏览器视觉、长任务 progress、网络断开恢复需 smoke |
| 6. End-to-end business-flow integration tests | 适用 | `tests/test_settings_data_reset_service.py` 间接集成；`web/e2e/settings-data-reset-flow.spec.ts` 覆盖 UI -> job -> reload -> downstream reads | 覆盖 reset -> lifecycle/rebuild -> API payload 的核心路径，以及设置页发起 job 到 reload，再进入银行明细 direct empty 和待找发票 direct rows 的浏览器路径 | P1 | 真实导入数据 -> reset -> 后台任务收敛 -> 多页面 direct API 需 staging smoke |
| 7. Existing feature regression tests | 适用 | 各业务模块测试 + 本模块 reset 测试 | 覆盖银行/发票/OA/ETC/成本/App Health 关键旧行为 | 无 P0 | 每次改 reset action 必须列出旧页面/API/export/权限影响面 |
...
| 2026-06-20 | data reset 完成后浏览器只停在 Settings 成功反馈，未证明受影响页面会重新读取 direct payload 或避免旧银行流水残留 | `web/e2e/settings-data-reset-flow.spec.ts` | reset 后继续打开银行明细，断言 direct rows payload 不含 `read_model_status` 且旧流水为空；再打开待找发票，断言 `pending_invoice` rows 业务行可见 |
| 2026-06-28 | data reset service 继续依赖 in-memory `WorkbenchReadModelService` 统计/清空旧 page cache，导致 Settings reset 成为 Workbench page read-model service 的 active 注入点。 | `tests/test_settings_data_reset_service.py`、`tests/test_platform_runtime_boundary_guards.py::PlatformRuntimeBoundaryGuardTests::test_settings_data_reset_pair_snapshot_uses_explicit_port`、`test_workbench_read_model_service_is_removed` | `SettingsDataResetService` 不再处理 `workbench_read_models` page snapshot，不接收 `workbench_read_model_service`；server source-version hash 使用 `snapshot_version(...)`。 |
...
- 生产前 staging：备份 PostgreSQL / 对象存储 / runtime config -> 执行一种 reset -> 后台任务收敛 -> App Health 绿灯 -> 多页面确认 direct API。
- 银行 reset：导入银行+发票 -> 建立匹配 -> reset bank -> 银行事实清空，发票事实和受保护目标保留，关联台 direct API 重新读取。
- 发票 reset：导入银行+发票+税金认证 -> reset invoices -> 税金和发票相关页面重新读取，银行事实保留。
...
- Browser e2e：Settings 数据重置 -> 影响确认 -> OA 密码复核 -> job create/polling -> settings reload -> 全局成功反馈 -> 银行明细 direct empty -> 待找发票 direct rows。
...
- 大生产库 reset 后后台任务、outbox 清理、App Health 收敛、页面 direct API 结果需要 staging 数据量验证。
