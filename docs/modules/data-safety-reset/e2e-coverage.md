...
1. 准备 staging 数据集，执行一种 reset action 后验证 App Health、outbox/worker heartbeat、关键页面 direct API 收敛。
2. 将 staging runbook 的真实后台任务、PITR/对象存储恢复和多页面 direct API 结果回填；本地 Browser mock 只能证明 UI 消费 direct API contract，不能证明真实数据恢复闭环。
