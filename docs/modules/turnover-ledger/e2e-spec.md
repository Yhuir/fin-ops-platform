...
| `TURNOVER-E2E-003` | 手动零差额闭环 confirm | P0 | 用户选择同一 group 的真实 flow rows，至少一收一支且差额为 0；提交前必须等待 `turnover_ledger:all` fresh 并重拉台账、用最新 row versions 生成 `expected_versions`；成功后等待 `turnover_ledger` 和受影响 `workbench_relation` barrier，不得显示“操作失败”或浏览器隐藏错误。 |
