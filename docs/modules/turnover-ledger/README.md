...
- 手动闭环：用户在页面选择同一往来组多条真实银行流水，至少一收一支且收支合计差额为 `0.00`。Turnover domain 只做无副作用业务校验；写事务仅通过 `WorkbenchRelationCommandService` 写 canonical Workbench active pair relation，不再重复写 Turnover relation/event。确认成功后，外部往来台账在对应流水展示“收支闭环”，关联台保留同一个 `turnover_manual_closure` active case/evidence。新写入的 Workbench relation 必须携带 `special_metadata.requires_oa=true`、`requires_invoice=false`、`paired_requirement_source` 和版本；Workbench 只按 relation metadata 判断 required row type 是否满足。bank-only 闭环因缺 OA 仍留 open 待处理区；OA + 银行且 metadata 声明无需发票时可进入 paired。metadata 缺失的旧关系 fail closed，仍按三栏完整才 paired，必须由规则保存同步链路补齐 metadata 后才能按新规则分区。若所选银行流水已处在仅含 `oa` + `bank` 的 active relation 中，闭环确认必须把这些既有关联一起合并进同一个 `turnover_manual_closure` active case；若既有 relation 包含发票或其他 row type，外部往来页不得替换，必须转关联台处理完整关系。
