...
- 台账读取：页面 GET 直接通过 `TurnoverLedgerQueryService` 委托 `TurnoverLedgerService` 组装 flat/grouped payload，不读取 `turnover_ledger` SQL read model、`ReadModelQueryGateway` 或 refresh metadata。`TurnoverLedgerSqlProjectionBuilder`、read-model repository port 和 `read_model.turnover_ledger_rows` 已删除，不得恢复为页面或运维兼容面。
...
- 下游影响：外部往来关系变更影响 `turnover_ledger`、`workbench`、`workbench_relation`、成本统计、Search direct payload 和前端跨页刷新提示。
- 操作闭环：前端 tag-selection、extra 保存、manual closure confirm/withdraw 必须接入 `GlobalOperationOverlayProvider`。manual closure 提交前必须重新加载 grouped payload，并按原始 bank row ids 在同一 group 内重绑定最新 flow rows，用最新 `categoryVersion` 生成 `expected_versions`；grouped GET 不返回旧同步诊断字段，也不靠页面同步状态禁用闭环。写 API 成功后页面直接重读 grouped ledger；Workbench、成本统计和 Search 通过各自 direct API / operation projection / 真实后台 outbox 事实收敛，不作为外部往来写操作的 overlay 释放条件。后端 stale precondition/canonical write safety 失败仍必须阻断。
- App Status：`turnover_ledger` domain 只保留页面域展示，不再绑定 turnover read model、refresh worker 或 `turnover_ledger.read_model.refresh` job type；写后由页面 direct grouped GET 和仍存在的下游 worker/domain 自行收敛。

不属于本模块事实源：

- 银行明细分类规则的长期业务口径归 `bank-details` 和产品规格维护。
- Workbench 已配对区事实由 Workbench pair relation/read boundary 维护，不能由 Turnover query 层临时拼接或直接读取 pair service snapshot；Turnover 页面只能消费 query service 返回的 Workbench relation 状态字段。外部往来页只展示正向 chip：`linked_oa=true` 显示“已关联 OA”，`linked_invoice=true` 显示“已关联 发票”，`cash_closure_linked=true` 显示醒目的“收支闭环”。不得再显示“已关联业务单据”“未闭环”“部分已闭环”“候选关联”等旧 chip。
