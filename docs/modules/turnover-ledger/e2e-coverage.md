...
| `TURNOVER-E2E-008` | covered | `test_turnover_ledger_extra_service.py`、API/UoW tests、`TurnoverLedgerApi.test.ts`、`TurnoverLedgerPage.test.tsx`、`turnover-ledger-flow.spec.ts` 的单次 detail GET、动态 relation、日期格式、A→B 乱序/只保存 B/OCC 场景 | 使用 test-owned relation 做一次保存/恢复；不得修改真实业务 relation |
