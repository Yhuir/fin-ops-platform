...
| 确认/撤回写操作 | write facade/UoW | 已知 affected months 的写路径触发 turnover/workbench/workbench_relation/cost/search affected month scopes；UoW 必须先汇总全部目标，经 scope policy normalize 后调用 runtime queue 的 transaction-bound batch enqueue，在同一条 SQL 内原子写 dirty scopes/outbox，禁止按 read model/scope 逐条往返。现代 closure confirm 调用 `preview_zero_difference_closure` 只验证所选银行事实，不写 Turnover relation/audit；随后 prepared relation command 执行一次 freshness、一次所选 bank member lock、一次 scoped canonical relation snapshot，并只写 Workbench active case。准备上下文必须绑定同一 service/transaction、月份和允许成员集合，任何不匹配都 fail closed。禁止恢复重复 Turnover relation 持久化、独立 freshness precheck、二次 active-relations DB load 或全表/全快照重建。跨月准备必须同时保留全部精确月份作为 `scope_keys_hint`，并把 `turnover_closure_affected_months` 写入 relation metadata。cash-closure 撤回在 mutation 前通过 relation read port 读取该 metadata，并在事务内复核 row types 只能为 `oa/bank` 且至少两条 bank；旧关系无 metadata 时只用 canonical relation 中的 bank members 经既有月份 resolver 补足。无法得到精确月份必须 fail closed，禁止再返回永远不会产生 current readiness 的 `turnover_ledger:all` command scope 作为 operation barrier target |
