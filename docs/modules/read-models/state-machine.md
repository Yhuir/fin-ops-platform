...
| 2026-06-26 | dependency source_versions skip closure | invoice lifecycle / no-OA worker 的 unchanged skip 必须覆盖真实依赖 source_versions；no-OA worker 一致时只 complete dirty scope，不 rebuild、不保存 snapshot；bank detail tag read port 必须支持下游 SQL fresh gate | `python -m pytest tests/test_bank_details_sql_runtime.py tests/test_invoice_lifecycle_sql_projection.py tests/test_no_oa_bank_batch_read_model_refresh.py -q` |
