...
- 关键决策：SLO miss 继续是 failure，不放宽目标；已提交写在恢复前通过响应返回的精确 `outbox_event_ids` 复用现有 write-operation SLO 边界等待 fan-out 完成，再读取 isolation baseline 和执行正式撤回。receipt 后的 consumer gate 只重试 `202/not fresh/503`，内容断言与 fresh latency failure 不重试。禁止用固定 sleep、忽略 202、直接 SQL或取消 SLO failure 修复。
...
- 关键决策：Workbench active repair/current-effective 状态优先于旧 consistency failure。`read_model_status=refreshing/rebuilding` 时，同一 immutable active generation 已缓存的失败 proof 继续作为 `consistency_status=failed` 诊断；未缓存 proof 返回 `consistency_status=refreshing`，不伪装 fresh，也不在 repair 热路径重跑重型 consistency SQL。两者都不写 unavailable dependency 或全局 blocked。`ReadModelQueryGateway` 使用 refresh gateway 的 actual enqueue events 判断 `refresh_enqueued`，active scope coalescing 返回 false。
