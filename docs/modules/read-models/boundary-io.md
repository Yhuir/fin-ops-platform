...
| Parent/shard freshness | Repository/API fresh gate | 父 scope 不能在子 scope dirty/missing 或 child source lineage 漂移时返回 fresh。`cost_statistics` 先读取现有 Cost gate；gate 已 non-fresh 时直接使用其 `workbench_refresh_scope_keys`、`bank_detail_refresh_scope_keys` 或 `child_refresh_scope_keys`，跳过 canonical 全量证明。只有 gate 可 fresh 时才执行 Workbench set-based canonical→active month proof。Workbench stale时只 ensure exact Workbench，不提前 stage Cost；后续访问在 upstream fresh 后才 enqueue exact Cost child，child完成后收敛parent。禁止 sibling project/page scope和 known-stale dependency retry storm。Cost worker仍沿 manifest dependency fail closed处理真实竞态，dependency fresh 前不得发布。禁止把系统中任意 active Workbench/Bank Detail event 当作 Cost 全局阻塞条件。普通 Cost parent worker 只 rollup，只有显式 `force_refresh` 可 full fan-out；parent-only `done` 不能伪装 fresh。 |
