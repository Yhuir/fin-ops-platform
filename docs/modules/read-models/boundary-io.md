...
- 当前非阻塞风险：Search 曾有一次生产 grouped-run 高延迟样本，targeted rerun 通过；Workbench groups admin smoke 有一次 probe shape `400`，不是 stale-as-fresh 证据。2026-07-02 release `pscip-l4-workbench-insert-5f530d1b5` 的 5s critical read model gate 为 16/16 pass，scope contract clean；但 Workbench 1s targeted smoke 仍 fail，最近样本 `enqueue_to_fresh_ms=1499.147`、`handler_duration_ms=1280.981`，critical 5s run 中 Workbench 为 `1160.102ms`。72h 真实写操作 audit 仍显示 Workbench relation confirm/withdraw cross-page 历史长尾；按新 release 激活时间过滤后 confirm/withdraw/no-OA withdraw 均缺当前 release 真实写样本。因此性能专项仍 open，不能声明高性能全域闭环。
