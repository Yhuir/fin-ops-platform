...
| Active refresh coalescing | `ReadModelRefreshGateway` / `RuntimeQueueRepository` | `api_*` 与其它 ensure/wakeup reason（包括成本统计读取触发的 `cost_statistics_workbench_dependency_stale`）用 `job.outbox_events` 的 exact tenant/type/key `pending/processing` 事件判断 active。dirty scope 仍是 freshness 事实，但 dirty pending 且没有 active event 时必须重新 enqueue，以恢复 handler 已返回 `refreshing` 却未完成 dirty 的 orphan scope；active event 存在时复用当前 source version，禁止页面轮询重复 bump。真实 mutation/repair/reapply 与显式 force 仍保留独立版本语义 |
...
| Parent/shard freshness | Repository/API fresh gate | 父 scope 不能在子 scope dirty/missing 或 child source lineage 漂移时返回 fresh；`pending_invoice` 父 scope 必须聚合子月份 dirty status。`cost_statistics` parent 访问先只读 exact active outbox；已有 Workbench/Bank Detail/当前 project Cost refresh 时直接返回 refreshing，避免每次页面 poll 重跑全月份证明。队列无 active event 后才用 Workbench set-based canonical→active month proof 定位 exact upstream drift，再在一个 Cost gate 中比较全部 concrete child 的 Workbench/Bank Detail versions、latest dependency dirty 与 parent `source_shards`；只 enqueue 漂移 child。普通 Cost parent worker 只 rollup，禁止读取易失 readiness 扩张到历史 shards；只有显式 `force_refresh` 可 full fan-out。parent-only `done` 不能伪装 fresh，也不能恢复写后 fan-out。 |
...
| Relation source fast path for downstream workers | `WorkbenchRelationReadModelRepositoryPort` / canonical repository | 实际消费 `workbench_relation` distribution 的下游 worker 只能通过 workbench-relations repository port 读取 active relation source rows/source summary。OA 等直接消费 canonical relation 的投影仍按各自读 gate 收敛；Workbench UoW 不再为它们直接投递月份。任一路径都禁止把 relation source 伪装成页面 fresh payload。 |
| Source-version proof | Scope rows / API fresh gate | `source_versions_unchanged` 只能在 own schema version 与依赖版本都匹配时跳过重建。主 `workbench` generation proof 必须覆盖 requested month 与其 active relation members 的 OA、银行流水、发票 canonical `updated_at`，以及该月 active pending claim、relation、exception、override 版本；`all` query proof覆盖对应全量 canonical source，不能只依赖静态 OA sync schema version。同一次 Workbench GET 已由 freshness status 完成 canonical proof 时，payload gate 必须复用该 proof，不得重复执行同一 scope 的完整 canonical 查询；freshness status 不可用时保留 fail-closed payload proof。一个请求校验多个 `workbench_relation` 月 scope 时，允许用一次 canonical bulk SQL 返回逐 scope 版本映射以消除查询 N+1，但每个 scope 的 source/member 边界、mismatch reason 和精确 enqueue 语义必须与单月 proof 完全等价；禁止用年度汇总版本替代逐月证明 |
