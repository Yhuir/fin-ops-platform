...
- 当前非阻塞风险：Search 曾有一次生产 grouped-run 高延迟样本，targeted rerun 通过；Workbench groups admin smoke 有一次 probe shape `400`，不是 stale-as-fresh 证据。2026-07-02 release `pscip-l4-workbench-insert-5f530d1b5` 的 scope contract clean；但最新 5s critical grouped run 为 14/16 pass，`turnover_ledger:all` 和 `bank_flow_rule_batch:2026-02` 在 grouped run 超过 5s，随后 targeted retry 分别 `993.910ms`、`455.961ms` pass，说明仍有 grouped-run 尾延迟。Workbench 1s targeted smoke 仍 fail，最新样本 `enqueue_to_fresh_ms=1485.007`、`handler_duration_ms=1181.262`。72h 真实写操作 audit 仍显示 Workbench relation confirm/withdraw cross-page 历史长尾；按新 release 激活时间过滤后 confirm/withdraw/no-OA withdraw 均缺当前 release 真实写样本。因此性能专项仍 open，不能声明高性能全域闭环。
