...
- 状态：运行时 read model 已收敛为关联台 active-generation `workbench` 与共享 relation distribution `workbench_relation`。
- 当前边界可信度：high。
...
- 两个 read model 的 manifest、scope、refresh enqueue、worker、freshness/status 与维护 backfill。
...
| --- | --- | --- | --- | --- |
| `workbench` | month shard；`all` 仅冷启动/显式恢复 fan-out | `workbench` | `WorkbenchQueryFacade` | `PostgresReadModelRepository` active-generation methods |
...
共享 relation 模型的 `all` 只是 maintenance fan-out 命令。关联台页面的 `all` 查询由 active month
...
| Workbench amount anomaly | Workbench grouping / exception decision repository | active generation payload 可携带 additive amount anomaly；decision 按 exact month/scenario 读取，bucket 查询只过滤既有 group payload。不得为该异常新增 manifest、scope、worker、queue、Redis owner 或第二 read model。 |
...
| Readiness/status | App Status / Operations | 只包含当前 manifest 两项；retired event/readiness 只可作为历史清理对象，不能进入当前状态 |
