...
| Active refresh coalescing | `ReadModelRefreshGateway` / `RuntimeQueueRepository` | `api_*` 与其它 ensure/wakeup reason 只能用 `job.read_model_dirty_scopes` 的 exact tenant/type/key `pending/processing` 行判断 active；outbox terminal 不代表 projection 已完成。active 时必须复用当前 source version，禁止页面轮询重复 bump、重置 dirty scope 或使 in-flight publish superseded。真实 mutation/repair/reapply 与显式 force 仍保留独立版本语义 |
