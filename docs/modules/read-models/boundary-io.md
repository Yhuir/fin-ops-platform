...
- Operation barrier 只能读取调用方明确提交的 target scopes，并复用 App Status 的 readiness/dirty/outbox/worker 状态折叠语义；不得在每次页面轮询中调用 `app_status_runtime_snapshot()` 扫描全部模块，也不得以 Redis、内存 TTL 或忽略 outbox 的轻量响应冒充 fresh。
