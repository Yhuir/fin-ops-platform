...
| Transactional refresh targets | 显式 import/reapply/repair writer | 只有定义了业务事务内刷新合同的显式 import/reapply/repair 可直接写 dirty scope/outbox，并必须使用等价 scope contract。Workbench confirm/withdraw/cancel/split/exception/ignore/cash 以及 bank-flow/no-OA/batch-accounting/turnover 普通写入零 target，不得恢复 `refresh_metadata.downstream_scope_types`、UoW target planner 或 repository 隐式 scope 扫描。 |
