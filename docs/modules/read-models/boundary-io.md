...
| Active refresh coalescing | `ReadModelRefreshGateway` / `RuntimeQueueRepository` | `api_*` 与其它 ensure/wakeup reason 必须调用 `enqueue_read_model_refresh_if_inactive(...)`。repository 在同一事务内对 exact `tenant/type/key` 取得 PostgreSQL advisory lock，再检查 `job.outbox_events` 的 `pending/processing` 并按需入队，禁止恢复“先查 active、再另开事务 enqueue”的竞态。dirty scope 仍是 freshness 事实，但 dirty pending 且没有 active event 时必须重新 enqueue；active event 存在时不改变 dirty/source version。真实 mutation/repair/reapply 与显式 force 仍保留独立版本语义 |
