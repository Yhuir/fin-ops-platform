...
| Parent/shard freshness | Repository/API fresh gate | 父 scope 不能在子 scope dirty/missing 或 child source lineage 漂移时返回 fresh。`cost_statistics` 先用 Workbench set-based canonical→active month proof 定位 exact upstream drift，再由一个 Cost gate 分别返回 `workbench_refresh_scope_keys`、`bank_detail_refresh_scope_keys` 与 `child_refresh_scope_keys`；query 只 enqueue 真正缺失的 upstream。月份请求可同时 stage当前 Cost scope；parent请求可一次 stage同一页面/同 project scope的exact Cost children与parent，禁止 sibling project/page scope。Cost worker沿 manifest dependency fail closed/defer，dependency fresh 前不得发布。禁止把系统中任意 active Workbench/Bank Detail event 当作 Cost 全局阻塞条件，旧 broad active-dependency query 已删除。普通 Cost parent worker 只 rollup，只有显式 `force_refresh` 可 full fan-out；parent-only `done` 不能伪装 fresh。 |
