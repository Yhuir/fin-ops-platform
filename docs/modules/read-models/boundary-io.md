...
| Invoice lifecycle dependency reads | `InvoiceLifecycleReadModelRepositoryPort` / upstream read models | own schema version 为 `2`。月份 refresh 只读取 exact fresh `pending_invoice`、`input_invoice_usage`、`output_invoice_collection` 与 `oa_pending_payment` 投影；任一依赖 missing/non-fresh 时必须在自身发布前 fail closed，由既有 dependency refresh 机制收敛。禁止恢复 `SearchPendingSqlProjectionBuilder`、input/output query service 或 canonical live rebuild fallback。 |
