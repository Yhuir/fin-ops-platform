...
| Query freshness request | API/read facade | 必须返回 fresh/stale/refreshing 或等价状态。`cost_statistics` 是 cost-local opt-in gate：query owner 必须先通过 repository port 执行 metadata-only PostgreSQL gate，只有 fresh 才进入共享 `ReadModelQueryGateway` 的 Redis/full-payload 路径；不得修改共享 gateway 默认顺序影响其他页面。`oa_pending_payment` 同样先执行自己的单 statement freshness/version gate 与 ETag 判断再 opt in：cache key 绑定 tenant/query/contract/version token；hit 不进入 read transaction，miss/Redis fallback 才在 OA repeatable-read snapshot 中重跑 gate并确认 version 未变后读取 rows。竞态返回 `202`，不写其它页面 cache 或失效目标 |
...
| Invoice lifecycle upstream rows | `InvoiceLifecycleReadModelRepositoryPort` / `pending_invoice` | 只读 exact `expense:all:YYYY-MM` 与 `income:all:YYYY-MM` fresh shards；dirty/missing 在 rows I/O 前 fail closed，fresh 才按 `scope_month + direction` 读取 payload。禁止回流 canonical pending-invoice builder、HTTP 或 live fallback |
...
| Fresh payload | 页面 API/Redis | Redis 只能缓存 fresh gate 后 payload；版本化 key 必须让当前 gate 能机械拒绝旧版本，不能要求 writer 广播无边界 cache invalidation |
...
| Workbench relation fan-out | runtime queue / workers | confirm/withdraw/cancel UoW 显式输出 `workbench`、`workbench_relation` 和 metadata 声明的 downstream scopes；含发票关系必须同时输出 `input_invoice_usage` 与 `output_invoice_collection`，因为当前两者的 scope proof 共用 relation source version；受影响 OA 成员月份必须在同一事务直接输出 `oa_pending_payment` target。OA projector直接读 canonical relation，不消费或等待 `workbench_relation` read model；其它真实消费 relation read model的下游仍必须先做 upstream fresh gate。写响应的 `freshness_targets` / `operation_barrier_targets` 必须至少包含本次受影响的 `workbench` 与 `workbench_relation` month scopes，避免前端等完 relation 后立即读取仍在 refreshing 的 groups read model。精确 month 事件同时携带 `relation_deltas + row_ids` 时，worker 必须走 relation-only delta：从当前 scope proof 只推进 canonical pair-relation version，active relation、pending claim 和 source objects 均只按受影响 row ids读取，再通过局部 save port替换 overlap groups/rows；禁止重算整月对象集合、整月 relation 集合或其它未变 source versions。只有 row ids但没有显式 relation delta 的事件继续走通用局部投影；缺少 row ids、`all` fan-out、首次 scope/mismatched schema或 row/case metadata超出合并上限时执行 full month rebuild。显式 `force_refresh=true`必须从 `all` parent传播到每个 month child，并绕过全部局部/unchanged优化执行 full rebuild。`PostgresWorkbenchRelationRepository(..., enqueue_refreshes=False)`是该主链路的持久化-only adapter，不能写 hidden outbox；旧 snapshot repository的兼容 fan-out只能投递具体月份，不得把普通 relation写入扩成即时 `workbench:all` |
...
| Relation source fast path for downstream workers | `WorkbenchRelationReadModelRepositoryPort` / canonical repository | 实际消费 `workbench_relation` distribution的下游 worker如 `bank-detail`、`search-pending`只能通过 workbench-relations repository port读取 active relation source rows/source summary。OA 待付款是隔离的明确例外：Workbench UoW直接投递 OA月份，OA projector通过 `PostgresWorkbenchRelationRepository`批量读取 canonical relation，不等待 distribution read model。两条路径都禁止把 relation source伪装成页面 fresh payload |
| Source-version proof | Scope rows / API fresh gate | `source_versions_unchanged` 只能在 own schema version 与依赖版本都匹配时跳过重建 |
| Invoice lifecycle dependency reads | `InvoiceLifecycleReadModelRepositoryPort` / upstream read models | own schema version 为 `2`。月份 refresh 只读取 fresh `pending_invoice`、`input_invoice_usage`、`output_invoice_collection` 与 `oa_pending_payment` 投影；任一依赖 non-fresh 时必须在自身发布前 fail closed。pending-invoice exact 月方向 scope 缺失时，只有 fresh Bank Detail 月 scope 且同方向结构化行数为零，repository 才输出携带 Bank Detail source proof 的空集；存在银行行或缺 proof 时仍 fail closed。OA 必须走 lifecycle 专用 exact-month source I/O，同时读取 scope fresh gate、source versions 与 rows；不能复用由页面 service 另行承担 freshness 的 rows-only I/O。禁止恢复 `SearchPendingSqlProjectionBuilder`、input/output query service、canonical live rebuild 或 stale-as-empty fallback。 |
...
- Operation barrier 只能读取调用方明确提交的 target scopes，并复用 App Status 的 readiness/dirty/outbox/worker 状态折叠语义；outbox 当前事实必须按目标 scope 的最新事件折叠，不能重扫历史事件，也不能忽略 failed/dead-letter/publish-failed。不得在每次页面轮询中调用 `app_status_runtime_snapshot()` 扫描全部模块，也不得以 Redis、内存 TTL 或忽略 outbox 的轻量响应冒充 fresh。
