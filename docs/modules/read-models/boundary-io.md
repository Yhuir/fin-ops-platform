...
| Parent/shard freshness | Repository/API fresh gate | 父 scope不能在子scope dirty/missing或child source lineage漂移时返回fresh。`cost_statistics`先读取现有Cost gate；gate已non-fresh时直接使用其`workbench_refresh_scope_keys`、`bank_detail_refresh_scope_keys`或`child_refresh_scope_keys`，跳过canonical全量证明。只有gate可fresh时才执行Workbench set-based canonical→active month proof。Workbench stale时，同次只stage当前project/page所需的exact Workbench与Cost child；不stage parent或sibling。Cost worker沿manifest dependency fail closed/defer，dependency fresh后发布child并由成功child收敛parent。Workbench与Cost parent的稳定target token在共享advisory lock内合并active/最新成功同target，避免poll和完成窗口重复任务；dirty orphan/failed不得被历史成功短路。禁止把系统中任意active Workbench/Bank Detail event当作Cost全局阻塞条件。普通Cost parent worker只rollup，只有显式`force_refresh`可full fan-out；parent-only`done`不能伪装fresh。 |
