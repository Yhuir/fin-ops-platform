...
| Active refresh coalescing | `ReadModelRefreshGateway` / `RuntimeQueueRepository` | `api_*` 与其它 ensure/wakeup reason 必须调用 `enqueue_read_model_refresh_if_inactive(...)`。repository 在同一事务内对 exact `tenant/type/key` 取得 PostgreSQL advisory lock，再检查 `job.outbox_events` 的 `pending/processing`。覆盖关系固定为 `force > full scope > partial delta`：full 覆盖 partial，partial 不覆盖 full，非 force 不覆盖 force；partial 之间只在规范化 metadata 已覆盖 incoming `row_ids`、`case_ids` 与 `relation_deltas` 时 no-op。新增语义在同一锁内合并 pending event，或为 processing event 建立 pending follow-up。禁止两事务竞态，也禁止同 scope 丢失新 delta。orphan dirty 必须重新 enqueue；真实 mutation/repair/reapply 与显式 force 保留独立版本语义 |
...
| Parent/shard freshness | Repository/API fresh gate | 父 scope 不能在子 scope dirty/missing 或 child source lineage 漂移时返回 fresh。`cost_statistics` 先用 Workbench set-based canonical→active month proof 定位 exact upstream drift，再由一个 Cost gate 分别返回 `workbench_refresh_scope_keys`、`bank_detail_refresh_scope_keys` 与 `child_refresh_scope_keys`；query 只 enqueue 真正缺失的 upstream 或 Cost child。禁止把系统中任意 active Workbench/Bank Detail event 当作 Cost 全局阻塞条件，旧 broad active-dependency query 已删除。普通 Cost parent worker 只 rollup，只有显式 `force_refresh` 可 full fan-out；parent-only `done` 不能伪装 fresh。 |
