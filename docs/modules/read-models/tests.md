...
## 2026-07-26 Turnover direct canonical read cutover

- 外部往来已退出本模块：页面 API 在一个 PostgreSQL repeatable-read 只读快照内直接查询 canonical bank rows、categories、settings、manual extras/relations 和 Workbench pair relations。
- 下文 2026-07-26 以前的 Turnover read-model/worker 条目仅是历史演进记录；对应 runtime repository、projection、producer、worker registration、manifest 和测试入口均已删除，不是当前合同。
- 当前防回归入口：`tests/test_turnover_ledger_query_service.py`、`tests/test_turnover_ledger_postgres_integration.py`、`tests/test_platform_runtime_boundary_guards.py`、`tests/test_read_model_manifest.py` 和 `tests/test_runtime_worker_registry.py`。
...
- 可能影响的旧页面：所有仍依赖 read model 的页面，尤其关联台、银行明细、待找发票、进项/销项/OA 待付款、税金抵扣、成本统计、免 OA、批量账务和 App Health；外部往来不再消费本模块 I/O。
...
| Cost / tax summary boundary | 适用 | `tests/test_cost_statistics_sql_runtime.py`、`tests/test_cost_statistics_runtime_service.py`、`tests/test_cost_statistics_derived_lifecycle_executor.py`、`tests/test_tax_offset_sql_runtime.py`、`tests/test_tax_offset_worker_rebuild_executor.py`、`tests/test_tax_offset_derived_lifecycle_executor.py`、`tests/test_tax_offset_cache_warmup_executor.py` | 成本统计和税金抵扣继续保持各自 read model contract、repository port、query owner 和 worker；外部往来已迁出 read-model runtime，由页面 API 在单次只读快照内直接查询 canonical facts。 | P1 |
...
| 5. Frontend component and interaction tests | 间接适用 | `web/src/test/*Page.test.tsx`、`web/src/test/PageRouteHost.test.tsx`、页面级 Playwright specs；显式 job另含 `OperationBarrierApi.test.ts` | 页面必须正确消费 fresh/refreshing/stale/empty/error；普通写成功后零 barrier并只重跑当前页 normal GET；focus/visibility/BFCache/跨页旧事件零业务 I/O；显式 import/reapply/job 才可等待 exact barrier | 真实浏览器逐页面访问与手动刷新需生产/Playwright smoke | P1 | 页面可能把 refreshing 空 rows当真实空结果或恢复旧跨页刷新入口 |
...
- Service-layer tests：外部往来 direct query 由 `tests/test_turnover_ledger_query_service.py` 和 PostgreSQL integration test 保护；read-model 模块不再暴露 Turnover repository port。
- Read model/cache/background job tests：manifest、worker registry 和 runtime boundary guards 证明 Turnover event、dirty scope、worker 与 projection 不能回归。
...
| --- | --- | --- | --- | --- | --- |
| 2026-07-23 | Workbench active repair 为压低 App Health 延迟而跳过重型 consistency audit 后，`consistency_status` 在没有缓存 proof 时错误返回 `fresh`；历史测试仍要求每次 repair 请求重扫 generation，既造成全量门禁失败，也会恢复生产慢查询。 | active-repair current-effective 优先级与 immutable-generation cache 只控制是否执行 audit，没有单独表达“repairing、尚未检查”；返回 DTO 把空 failures 直接映射为 fresh。 | `tests/test_workbench_sql_runtime.py::WorkbenchSqlRuntimeTests::test_repository_reports_inconsistent_workbench_generation_as_refreshing_during_active_repair`、`test_repository_reuses_consistency_result_for_immutable_active_generation` | `PYTHONPATH=backend/src python3 -m unittest tests.test_workbench_sql_runtime.WorkbenchSqlRuntimeTests.test_repository_reports_inconsistent_workbench_generation_as_refreshing_during_active_repair tests.test_workbench_sql_runtime.WorkbenchSqlRuntimeTests.test_repository_reuses_consistency_result_for_immutable_active_generation -v` | active repair 无缓存 proof 时返回 consistency refreshing；已有同 generation 失败 cache 时零重扫保留 failed 诊断；两者 read model 均为 refreshing 且不恢复重型 SQL |
...
PYTHONPATH=backend/src python3 -m unittest tests.test_turnover_ledger_query_service tests.test_turnover_ledger_postgres_integration -v
...
- 历史新增/更新测试曾覆盖 `read_model.turnover_ledger_rows` 的 multi-values bulk path；该 runtime table 已在 2026-07-26 direct canonical read cutover 中退出运行链路。
- 当前回归测试改由 `tests/test_turnover_ledger_query_service.py`、`tests/test_turnover_ledger_postgres_integration.py` 与 boundary guards 证明旧 projection 不再参与页面结果。
