...
> 当前模块测试唯一共享 `workbench_relation` read model，以及关联台 page generation 与其它已退役页面
...
1. Manifest、scope policy、App Status registry 和带 `read_model_key` 的 worker registration
   精确等于 `workbench_relation`。
2. 包括关联台 page generation 在内的已退役页面没有 projector、producer、refresh service、worker handler、RabbitMQ
...
4. 唯一 projection 通过 gateway、durable queue、freshness proof 和登记 worker闭环；
   关联台页面直接读取 canonical facts，relation 模型的 `all` 只作 fan-out command。
...
| 2. Service / repository | 适用 | `tests/test_read_model_query_gateway.py`、`tests/test_read_model_refresh_gateway.py`、关联台与 relation distribution 的 service/repository tests |
...
| 7. 既有功能回归 | 核心适用 | 全量 backend/frontend/build/E2E，加静态负向合同保护其它页面、唯一保留 read model、import/OA/matching jobs |
...
- retired `*.read_model.refresh` event、scope type、worker instance、env 示例和 RabbitMQ
  dispatch 不存在。
- 关联台 active generation/status/cache/worker runtime 必须不存在，公开 canonical payload 会递归剥离历史 generation/read-model 字段；旧 refresh endpoint 固定 404。
...
- 所有 direct 目标页面（含关联台）均由 canonical snapshot 返回。
- 唯一保留的 `workbench_relation` queue/readiness/worker 正常。
