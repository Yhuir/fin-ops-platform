...
> 当前模块测试关联台 active-generation `workbench`、共享 `workbench_relation` read model，以及其它已退役页面
...
1. Manifest、scope policy、App Status registry 和带 `read_model_key` 的 worker registration
   精确等于 `workbench`、`workbench_relation`。
...
4. 两个 projection 均通过 gateway、durable queue、freshness proof 和登记 worker闭环；
   关联台页面只读取 fresh active generation，relation 模型的 `all` 只作 fan-out command。
...
| 7. 既有功能回归 | 核心适用 | 全量 backend/frontend/build/E2E，加静态负向合同保护其它页面、两个保留 read model、import/OA/matching jobs |
...
- 除关联台外的目标页面及成本统计、外部往来均由 canonical snapshot 返回；关联台由 fresh active
  generation 返回。
- 两个保留 read model queue/readiness/worker 正常。
