...
| `READMODEL-E2E-002` | `partial` | Workbench Browser flows 覆盖 stale/refreshing/missing/failed 时不把旧 generation 伪装为 fresh；共享 gateway 单测覆盖 `workbench_relation` missing/stale 入队。bank details、tax offset、cost、invoice、OA pending、no-OA、batch accounting、turnover 等 direct-canonical 页面由各模块 loading/error/empty tests 证明失败不会伪装为空数据，不再声明 read-model stale 状态。 | Workbench 与 relation distribution 的真实生产组合由每次 release gate 继续验证。 |
| `READMODEL-E2E-003` | `covered` | Workbench Browser flows 覆盖 relation write 后恰好一次 direct canonical GET；其它 direct-canonical 页面 Browser flows 覆盖写后普通 GET 且不调用 legacy barrier。`tests/test_operation_freshness_barrier.py` 只保护仍登记 target 的 barrier contract。 | 真实业务写操作的生产 latency 归 `READMODEL-E2E-006`。 |
...
| `READMODEL-E2E-005` | `covered` | registry/manifest 当前只登记共享 `workbench_relation`；架构守卫证明 Workbench page、Search/no-OA projection/event/worker 均不再登记，`bash scripts/verify.sh infra-smoke` 提供真实基础设施 opt-in gate。 | 每个新 release 仍需重新执行唯一保留 read model 的 drain/readiness、五个 required worker 和 retired page worker/event 零残留检查；真实业务写操作 latency 归 `READMODEL-E2E-006`。 |
