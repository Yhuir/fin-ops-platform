...
| `READMODEL-E2E-003` | `covered` | Workbench Browser flows 覆盖 relation write 后 generation/fresh reload；direct-canonical 页面 Browser flows 覆盖 write 后普通 GET 回读且不调用 legacy barrier。`tests/test_operation_freshness_barrier.py` 只保护仍登记 target 的 barrier contract。 | 真实业务写操作的生产 latency 归 `READMODEL-E2E-006`。 |
...
| `READMODEL-E2E-005` | `covered` | Phase 39 将 registry/manifest 收敛为 `workbench`、`workbench_relation` 两项；架构守卫证明 Search/no-OA projection/event/worker 不再登记，`bash scripts/verify.sh infra-smoke` 提供真实基础设施 opt-in gate。 | 每个新 release 仍需重新执行两个保留 read model 的 drain/readiness 与 retired worker/event 零残留检查；真实业务写操作 latency 归 `READMODEL-E2E-006`。 |
