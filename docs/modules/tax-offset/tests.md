...
| 4. Read model/cache/background job tests | 适用 | `tests/test_tax_offset_sql_runtime.py`、`tests/test_tax_offset_worker_rebuild_executor.py`、`tests/test_tax_offset_derived_lifecycle_executor.py`、`tests/test_read_model_refresh_gateway.py`、`tests/test_runtime_worker_read_model_refresh_scopes.py`、`tests/test_derived_data_lifecycle_service.py`、`tests/test_app_status_overview_service.py`、`tests/test_platform_runtime_boundary_guards.py` | 覆盖 SQL projection、compat worker rebuild executor、derived lifecycle read model/cache executor、Redis fresh month/summary cache envelope、refresh gateway、worker all fan-out、dirty scope、lifecycle fan-out 和 App Status。cache warmup job executor/service 仍是下一边界，当前 `Application` owned warmup payload build/upsert/persist 行为必须补测试和 static guard。 |
