...
| OA 投影与 sync worker | canonical 投影原子 upsert、结构化附件、cache-before-OA/OA-before-cache 双顺序 indexed identity bridge、legacy row id 迁移、完整权威快照单一删除 owner、relation/PG payment 清理、external exact-flow 删除事件、删除前 completed+admitted 复查、month/retention 不误删、formal relation 事务内重新验证 canonical 成员、零页面 fan-out、retention cutoff、API enqueue 不 inline sync、缺 queue fail-closed、AppHealth 状态只读 durable facts、生产 timer 周期性 enqueue `oa.sync` | `tests/test_oa_projection_sql_runtime.py`、`tests/test_oa_pending_payment_source_snapshot_repository.py`、`tests/test_oa_payment_status_reconcile_service.py`、`tests/test_oa_payment_status_reconcile_repository.py`、`tests/test_oa_payment_status_service.py`、`tests/test_oa_pending_payment_postgres_integration.py`、`tests/test_workbench_relation_command_service.py`、`tests/test_workbench_relation_repository.py`、`tests/test_worker_oa_sync.py`、`tests/test_runtime_worker_registry.py`、`tests/test_runtime_queue_ops.py`、`tests/test_deploy_runtime_examples.py`、`tests/test_app_status_overview_service.py`、`tests/test_app_health_api.py` |
...
| OA 手动搜索/导入 | fast search、正式导入 completed-only；精确附件刷新 POST 202/GET durable status、worker-only Mongo/OCR/promotion、targeted owner commit、无 stale deletion；completed 与 `in_progress + expense_claim` 都走同一 promotion/matching，进行中 owner 缺失/漂移、其它进行中表单、缺失/OCR/promotion 均 fail closed；前端 exact row 回读覆盖 `completed+in_progress`、page size 2、`total=1`/row ID 唯一，0/重复/total>1 不得 fallback；重复刷新幂等 | `tests/test_oa_attachment_refresh_request_service.py`、`tests/test_oa_projection_sync_service.py`、`tests/test_runtime_queue.py`、`tests/test_oa_manual_import_service.py`、`tests/test_oa_manual_import_api.py`、`tests/test_oa_attachment_invoice_promotion_service.py`、`web/src/test/WorkbenchApi.test.ts`、`web/src/test/SettingsOaManualSearchImportTable.test.tsx` |
...
| 2. Service-layer tests | 适用 | `tests/test_target_oa_applicant_token_provider.py`、`tests/test_oa_applicant_credentials_service.py`、`tests/test_oa_attachment_refresh_request_service.py`、`tests/test_oa_projection_sync_service.py`、`tests/test_oa_projection_sql_runtime.py`、`tests/test_oa_pending_payment_postgres_integration.py`、`tests/test_rehydrate_workbench_read_models.py`、`tests/test_oa_pending_payment_canonical_query.py` | 保护 service/repository/worker 编排、精确刷新 request/worker 分界、双顺序附件身份桥、repair 指纹门禁、凭据脱敏、外部 OA 登录失败、canonical snapshot 和零页面 fan-out。 |
| 3. API contract tests | 适用 | `tests/test_oa_applicant_credentials_api.py`、`tests/test_oa_manual_import_api.py`、`tests/test_input_invoice_usage_api.py`、`tests/test_oa_pending_payment_api.py`、`tests/test_auth_guard.py`、`tests/test_session_api.py`、`tests/test_workbench_settings_sync_api.py` | 保护精确刷新 202/status/result/error shape、response shape、canonical ACL-only 权限、direct denial、502/503 sync/compensation、retired runtime 字段缺失与 version conflict。 |
| 4. Read model/cache/background job tests | 适用 | `tests/test_runtime_queue.py`、`tests/test_oa_projection_sync_service.py`、`tests/test_oa_projection_sql_runtime.py`、`tests/test_worker_oa_sync.py`、`tests/test_runtime_worker_registry.py`、`tests/test_app_status_overview_service.py`、`tests/test_mongo_oa_adapter.py` | 保护精确 `oa.sync` durable result、targeted worker、普通 OA sync worker、projection repository、Mongo read status/backoff、App Status worker/readiness 注册。 |
...
| 进行中 OA 附件归属不完整或重复扫描 | `tests/test_mongo_oa_adapter.py`、`tests/test_oa_projection_sync_service.py`、`tests/test_oa_attachment_refresh_request_service.py`、`tests/test_oa_pending_payment_source_snapshot_repository.py`、`tests/test_oa_pending_payment_postgres_integration.py`、`tests/test_oa_attachment_invoice_promotion_service.py`、`web/src/test/SettingsOaManualSearchImportTable.test.tsx` | 普通 month/all sync 与显式精确刷新都对唯一 `in_progress + expense_claim` 复用 completed 的 parser/cache/promotion/matching，来源边必须指向明确子付款项；精确刷新还必须证明唯一同 scope pending owner，防止延迟任务复活已失去全量准入资格的 OA。普通同步只消费 `pending_admission_changed_scopes`，支付状态单独变化不得重复 promotion。状态完成后 owner 原子迁移并继续复用同一 promotion。相同 canonical ID 的 completed 在解析前胜出，同胜出状态重复 fail closed，不同 ID 不合并。进行中支付申请仍拒绝；正式人工导入入口仍 completed-only。 |
...
| PDF 文本层把下载计数与 20 位发票号拼成 21 位 | `tests/test_oa_attachment_invoice_service.py::OAAttachmentInvoiceServiceTests::test_parse_file_result_ocr_recovers_only_unrecognized_pdf_page_and_dedupes_all_pages`、`tests/test_oa_attachment_invoice_service.py::OAAttachmentInvoiceServiceTests::test_parse_invoice_text_does_not_truncate_adjacent_counter_from_digital_invoice_number`、`tests/test_oa_attachment_invoice_service.py::OAAttachmentInvoiceServiceTests::test_parse_invoice_text_rejects_labeled_21_digit_invoice_number`、`tests/test_manual_invoice_entry_service.py::UploadedInvoiceRecognitionTests::test_pdf_with_unrecognized_text_ocr_stops_after_first_recognized_page`、`tests/test_manual_invoice_entry_service.py::UploadedInvoiceRecognitionTests::test_text_pdf_recognition_does_not_run_ocr` | 带或不带“发票号码”标签的连续 21 位号码均不得截断猜测；只 OCR 未形成正式 identity 的同页。OA 收集并去重全部页面，人工上传首张成功后停止，文本成功页 OCR 为 0。 |
| PDF 金额行与下一行数字字段被压平 | `tests/test_oa_attachment_invoice_service.py::OAAttachmentInvoiceServiceTests::test_parse_file_result_ocr_recovers_only_unrecognized_pdf_page_and_dedupes_all_pages`、`tests/test_oa_attachment_invoice_service.py::OAAttachmentInvoiceServiceTests::test_parse_invoice_text_stops_total_before_following_numeric_field`、`tests/test_oa_attachment_invoice_service.py::OAAttachmentInvoiceServiceTests::test_parse_invoice_text_does_not_truncate_incomplete_decimal_total`、`tests/test_manual_invoice_entry_service.py::UploadedInvoiceRecognitionTests::test_pdf_with_unrecognized_text_ocr_stops_after_first_recognized_page` | OCR 后必须保真得到 net/tax/total，`28.80` 后的 `13%` 或税号不得被拼入金额；残缺小数与下一行税率形成伪两位金额时，必须因 `net + tax != total` fail closed。 |
| OCR 运行时故障被缓存为无发票 | `tests/test_oa_attachment_invoice_service.py::OAAttachmentInvoiceServiceTests::test_parse_file_result_does_not_cache_ocr_runtime_failure_as_no_evidence`、`tests/test_mongo_oa_adapter.py::MongoOAAdapterTests::test_worker_sync_does_not_save_cache_when_ocr_runtime_fails` | RapidOCR 初始化/推理失败必须显式中止并保持 cache miss，禁止写入 current parser version 的 `no_evidence`。 |
...
| 幂等人工附件刷新无法修复历史关联 | `tests/test_oa_attachment_invoice_promotion_service.py::OAAttachmentInvoicePromotionServiceTests::test_manual_refresh_reconciles_matching_when_canonical_invoices_are_unchanged`、`tests/test_oa_projection_sync_service.py::OaProjectionSyncServiceTests::test_targeted_attachment_refresh_updates_only_selected_completed_rows`、`tests/test_oa_projection_sync_service.py::OaProjectionSyncServiceTests::test_targeted_attachment_refresh_promotes_in_progress_expense_claim` | 精确 `refresh-attachments` 必须保持零重复 invoice write，同时为所选 completed 或 `in_progress + expense_claim` OA 补发既有五个月 matching window；普通自动 sync 不获得该强制语义。 |
...
## 2026-08-26 - OA PDF representation 回归

- Business core：适用。严格保护 20 位数电发票 identity，连续 21 位文本不得截断或猜测；按正式 identity 去重 OA 多页结果。
- Service layer：适用。OA 主链与人工上传共用同一按页 parser；OA 解析全部页面，人工上传首条成功即停止。
- Read model/cache/background job：适用既有回归。parser version 进入 cache key，`tests/test_mongo_oa_adapter.py` 继续保护旧版本 cache 由 durable OA sync 重建且 API read 不 inline OCR。
