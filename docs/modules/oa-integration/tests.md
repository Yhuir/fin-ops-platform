...
| 进行中 OA 刷新越过正式发票准入 | `tests/test_mongo_oa_adapter.py`、`tests/test_oa_projection_sync_service.py`、`tests/test_oa_attachment_refresh_request_service.py`、`tests/test_oa_pending_payment_source_snapshot_repository.py`、`tests/test_oa_pending_payment_postgres_integration.py`、`web/src/test/SettingsOaManualSearchImportTable.test.tsx` | 普通 month/all sync 与显式精确刷新都对唯一 `in_progress + expense_claim` 复用 completed 的 parser/cache，但强制零 promotion/matching/统一发票池写入；精确刷新还必须证明唯一同 scope pending owner，防止延迟任务复活已失去全量准入资格的 OA。状态完成后 owner 原子迁移并只走 completed promotion。相同 canonical ID 的 completed 在解析前胜出，同胜出状态重复 fail closed，不同 ID 不合并。进行中支付申请仍拒绝；前端不得把解析成功复用为 `can_import`。 |
