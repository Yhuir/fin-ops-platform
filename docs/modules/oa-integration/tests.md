...
| 进行中 OA 通过人工刷新提前解析附件 | `tests/test_mongo_oa_adapter.py::MongoOAAdapterTests::test_manual_attachment_refresh_keeps_in_progress_attachment_as_metadata_only`、`tests/test_oa_projection_sync_service.py::OaProjectionSyncServiceTests::test_targeted_attachment_refresh_fails_for_in_progress_row` | completed-only 约束同时位于 adapter 构建入口和精确 worker operation；人工刷新不得下载/OCR/生成发票，任务明确失败，但保留附件引用供完成态后解析。 |
