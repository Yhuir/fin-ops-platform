...
- 决策：删除未安装到现有 root helper、因而不可达的通用 alias repair 工具，改由 `0153_oa_source_alias_attachment_identity_repair.sql` 对唯一已审阅案例做一次性修复。迁移精确限定旧/新 OA、旧/新两组 item、两张 canonical 发票及 row index `0/1`，用发票不可变旧 provenance 的附件键，经 indexed cache source bridge，连接到当前 OA 外键持有的物理附件；bridge item 允许等于同一行的旧 item 或当前 item，因为 item ID 含规范化内容指纹、跨 OA 版本并不稳定。每张发票必须唯一命中一份当前 owned attachment，当前 OA 其它附件/凭证 bridge 不参与该 alias 事实的计数；禁止退化为金额、名称或顺序猜测。
...
- 性能：cache bridge 禁止加入 Workbench 页面热查询；运行时继续走 indexed active alias。迁移发现、强证据条件和禁止更新/删除核心事实由 `tests/test_postgres_migrations.py` 覆盖；`tests/test_oa_source_alias_repair_postgres_integration.py` 在真实 PostgreSQL 中复现“发票为旧 item、bridge 为当前 item”的跨版本链路。
