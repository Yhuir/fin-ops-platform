...
| `OA-E2E-002` | `partial` | `tests/test_mongo_oa_adapter.py`、`tests/test_worker_oa_sync.py`、`tests/test_oa_projection_sql_runtime.py`。 | 真实 OA Mongo 字段漂移、大月份和 staging 后台任务/direct API 收敛未完整闭合。 |
| `OA-E2E-003` | `covered` | `web/e2e/oa-pending-payments-flow.spec.ts`、`oa-pending-payments-nonfresh-flow.spec.ts`、API/service/page tests。 | 真实 OA projection 输入归 `OA-E2E-002`；`nonfresh` 是既有测试文件名。 |
