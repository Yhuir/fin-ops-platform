...
- OA 同步通过 worker / durable queue 写入本系统投影；Workbench、Invoice Lifecycle、OA 待付款、进项使用、税金、成本等下游页面通过各自 direct API / operation projection / cache warmup 收敛；Search 通过 direct `/api/search` 重新读取业务结果，不再由 Search 页面刷新 worker 驱动。
...
| OA 手动搜索/导入 | 设置页、Workbench、Search、历史 OA 补录 | 未完成单据误导入、附件刷新失败、手动 marker 删除后旧投影未清理 |
...
| Input invoice OA reverse | 进项使用、OA 关系、审计、direct API refetch | preview hash 过期、version conflict、idempotency、人工 submitted/not_submitted |
...
- API contract、DTO shape、错误字段、权限校验、状态值或响应同步诊断字段变化。
- 业务状态、UI 状态、OA projection/worker 状态或状态流转变化。
