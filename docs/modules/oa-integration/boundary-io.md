...
- `OA_PROJECTION_SYNC_VERSION=2026-08-17-expense-category-enum-v6` 触发一次存量重投，确保历史日常报销的 item/source binding、`category/feeType -> expense_type` 枚举映射与 `fee_content` / `fee_description` 保真字段符合当前合同；重投仍由 durable `oa.sync` worker 执行且必须幂等。
