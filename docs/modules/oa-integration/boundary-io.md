...
- `OAAttachmentInvoiceService` 使用 PDF 页面位置顺序、OCR 同行坐标恢复字段，票号只接受“发票号码”标签直接对应的完整值；不再从全文任选 20 位数字，不再将长数字截成税号。无法确定身份返回既有 `no_evidence`/未识别结果，原生文本未识别或普通税票财务字段不完整时运行一次该页 OCR；铁路客票仅票价结果保持原行为。
...
正式税票主体解析只读取带标签的名称和税号；明确购销方标签/区域优先，历史标准增值税票的两个无角色名称槽位继续按其固定购销方版式读取。铁路票的购买方标签不能因缺销方而退回全文单位名扫描；未识别主体保留空值。保留完整公司/分公司名称，不在“集团/公司/站/店”等后缀处截断。删除正式税票调用全文猜名称及可疑公司名重试的路径，机打票独立扫描保持原合同。税号明确角色优先，缺失值不前移。

parser version 升为 `2026-09-23-labelled-invoice-parties-v6`，复用 adapter 版本失效和 promotion repository 的版本过滤，不新增缓存或全历史重放。识别结果仍通过既有 recognition/normalization/promotion 创建或关联；财务不完整可按强身份关联已有票，不能改写已证实主体与财务值。

## 2026-09-23 普通税票部分财务证据
