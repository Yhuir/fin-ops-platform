...
- `OA_PROJECTION_SYNC_VERSION=2026-08-18-form-specific-expense-type-v8` 触发一次存量重投，确保支付申请 `category -> expense_type`、历史日常报销 `purposeType -> expense_type`、item/source binding 与 `fee_content` / `fee_description` 符合当前合同；重投仍由 durable `oa.sync` worker 执行且必须幂等，不允许直接修补 PostgreSQL normalized payload。
