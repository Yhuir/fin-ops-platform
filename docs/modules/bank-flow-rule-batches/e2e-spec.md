...
1. 在流水规则批量处理页筛选该标签，确认三栏标题路径和流水表列顺序。
2. 通过逐行 checkbox 选择 1 条，确认表头进入半选；再通过表头 checkbox 全选 4 条。
3. 确认行内不重复显示银行账户或主/子标签，并提交。
4. command 成功后确认页面立即清空选择，并通过当前页正常 GET 收敛；不得等待 operation barrier。
5. 打开关联台。

验收：

- 提交成功创建 active `relation_mode=bank_flow_rule_batch`。
- 表头与逐行 checkbox 均有可见控件；顶部显示选择数和“清空选择”，批次标题不再保留第二套全选/清空按钮。
- 交易时间为普通表格文本且紧随对方户名；金额列只显示收支 chip 与金额；关联信息独占按需列；摘要列只显示摘要、用途和备注。
- 关联台已配对区出现该 relation。
- 因有效批次成员数 `>=3`，默认以折叠摘要展示。
- 展开后可看到 4 条原始银行流水。
