...
- 持久化 I/O 使用 `save_bank_flow_rule_batch_mutation(...)` / `PostgresWorkbenchRepository.save_bank_flow_rule_batches*` 命名入口；提交/撤回等 mutation 只同步保存 relation fact 和 bank-flow 受影响月份 scope 的批次快照，禁止同步读取或写入 Workbench read model snapshot。没有月份 scope 时才允许 fallback `all` batch snapshot。禁止通过 no-OA persistence port、no-OA 物理表、Workbench read model broad snapshot 或逐行 projection fallback 写入新模块。
