...
| 页面查询 | `BankFlowRuleBatchPage.tsx` | 查询候选/已提交/已撤回批次，默认 page size 50；服务端通过专属 paged read port 执行 `LIMIT/OFFSET`、过滤范围 total 与 summary 范围聚合，查询数不随批次数增长，并携带 read model freshness/status/version。未提交主/子标签只由 active 且 OA/发票双 false 的规则集合构造；已提交/历史只由实际聚合中对应状态 count > 0 的冻结标签构造。页面状态/version/手工刷新变化必须使已显示的 Audit 结果失效。 |
| 页面只读 Audit | `PageBusinessAuditIcon` / AppHealth operations API | admin-only 调用 `page-audit?page=bank-flow-rule-batches`；non-deleted batch 及其精确银行成员集是 canonical expected-set，批次总金额/笔数必须从 canonical 银行流水按批次规则重算：普通批次求成员金额合计，内部转账一收一支只计单边金额，不能把两边绝对值重复相加；canonical batch、fresh page payload 和 `bank_flow_rule_batch` active relation 必须按 batch/case id 与完整 bank member set 相等，submitted 缺 relation、非 submitted 残留 relation、relation orphan，以及 draft 成员与其它 active relation 的跨 case overlap 均阻断；同时要求共享 relation 双向 edge equality 与只读一致性快照；不得借 audit 绕过规则/提交/撤回 service 边界。 |
| 批量提交 | `POST /api/bank-flow-rule-batches/submit-selection` | `transaction_ids` 必填、非空、去重。提交前重查银行流水身份、月份、账户、标签、canonical active relation 占用和当前规则版本；占用冲突返回 `409 bank_flow_rule_batch_selection_occupied`。关系行和 source versions 必须来自一次 canonical PostgreSQL source bundle，不能读取 Workbench relation read model 投影。 |
...
| Access-time convergence | 前端页面 / read-model gate | 写成功返回业务 receipt 与信息性 `affected_months` / scope，`freshness_targets` 与 `operation_barrier_targets` 为空。当前页面只通过自身正常 GET reconcile；其它页面/tab 不自动刷新。route 进入/重进、查询变化、浏览器手动刷新或明确重试时才重校验自身 read model；focus/visibility/BFCache/旧业务事件零页面 I/O。 |
...
- 未提交资格的 source version 是排序后合格 tag code 集合的稳定签名 `bank_flow_rule_batch_eligibility_version`，不是原始规则版本。OA-only 改为 invoice-only 等资格集合不变的保存不得让 bank-flow read model 失效。
...
- `submit_batch(batch_id)` 热路径不得为了 relation command、rollback 或响应组装读取完整 Workbench relation snapshot；`WorkbenchRelationCommandService.confirm_relation(...)` 必须通过 `load_workbench_pair_relations_for_row_ids(row_ids, case_ids=[case_id])` 读取 row/case scoped snapshot。bank-flow service 只能通过 `persist_mutation(...)` 保存当前 bank-flow batch facts，并通过 `snapshot_case_ids([case_id])` 读取变更 case 的 relation payload 和相关 history；已删除的 `after_mutation(...)` 假 callback、`workbench_rebuild_queued` 响应字段和同步 Workbench read-model snapshot 不得恢复。submit/withdraw/reset 不得调用派生 lifecycle、WorkBench scope 枚举或写任何页面 dirty/outbox。
...
- 列表 presentation 在单次请求内只允许读取一次银行标签字典，并把同一份 definition index 用于当前页标签和完整 summary categories；禁止按 batch/category 重复 deep-copy 整份字典。列表 freshness 读取本模块 repository 的 durable dirty/readiness/source-consistency proof，并比较稳定 canonical category/rule version；writer 不提供 dirty/outbox 兜底。fresh 月份 scope 若同时存在多个 `source_versions` 必须返回 `schema_mismatch` 并由访问 gate 入队修复，不能伪装 fresh。worker/refresh precheck 使用 `BankTransactionCategoryService.snapshot_version()`；该值与完整 snapshot SHA-256 合同完全一致，只在分类或标签字典真实变更时失效，不能引入 TTL、跨进程业务缓存或绕过 durable readiness。
...
| Integration | `workbench_relation_grouping.py`、Workbench display policy/decorator、relation command metadata mapping |
| Tests | `tests/test_bank_flow_rule_batch*.py`、`tests/test_no_oa_bank_batch_tag_selection_api.py`、`web/src/test/RelationGroupGrid.test.tsx`、affected no-OA regression tests |
...
- Service tests：规则保存零 enqueue、existing relation 非改写、批次提交、relation command payload、零 dirty/outbox、旧历史重算页面链路不可达。
...
- 本轮关闭旧残留：当前 `workbench_relation_grouping.py` 的 bank-flow 折叠摘要只接受 `bank_flow_rule_batch_summary`；`postgres_repositories/read_models.py` 将该 source kind 纳入 summary display-only 集合；`web/src/features/workbench/api.ts`、`ReconciliationWorkbenchPage.tsx` 使用 bank-flow source kind/relation metadata 和“流水规则批次”文案。已删除的 candidate-grouping/route legacy translation 链路不得恢复。
