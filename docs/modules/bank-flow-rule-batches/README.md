...
流水规则批量处理处理所有可批量提交的银行流水，不再只处理免 OA 候选。页面右侧抽屉以紧凑 xlsx/grid 方式维护每个银行明细标签的 OA/发票业务闭环要求；新 relation 创建时冻结该要求，关联台据此判定展示区。
...
- 页面提交的是银行流水批量关系事实。active relation 决定 ownership；冻结要求满足时进入 paired，未满足时同 case 留在 unpaired。规则保存不追溯改写既有 relation metadata。
