...
| --- | --- | --- | --- |
| BRB-E2E-001 | 标签规则抽屉跟随银行明细标签 | `web/e2e/bank-flow-rule-batches-flow.spec.ts`、`web/src/test/BankFlowRuleBatchPage.test.tsx` | 已覆盖新 route、标题、xlsx-like grid、OA/发票 checkbox、保存 `rules` 且不发送 `selected_tag_codes`；浏览器 E2E 已覆盖银行明细自动标签保存后本抽屉同步更新；抽屉打开、checkbox 勾选和保存已记录 operation latency。 |
