...
状态：covered-transition。当前实现使用 `web/e2e/bank-flow-rule-batches-flow.spec.ts` 覆盖流水规则入口；真实 pending-invoice/invoice attach 跨页补票入口和独立 read model 拆分仍列为后续风险。

| Spec ID | 业务场景 | 当前覆盖 | 缺口 |
| --- | --- | --- | --- |
| BRB-E2E-001 | 标签规则抽屉跟随银行明细标签 | `web/e2e/bank-flow-rule-batches-flow.spec.ts`、`web/src/test/NoOaBankBatchPage.test.tsx` | 已覆盖新 route、标题、xlsx-like grid、OA/发票 checkbox、保存 `rules` 且不发送 `selected_tag_codes`；浏览器 E2E 已覆盖银行明细自动标签保存后本抽屉同步更新。 |
