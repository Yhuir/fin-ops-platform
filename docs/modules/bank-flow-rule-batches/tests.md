...
| 5. Frontend component and interaction tests | 适用 | xlsx/grid 抽屉、左侧只读、OA/发票 checkbox、保存失败、标签变化后 grid 同步、选择清空、批量提交 loading/error/empty/stale 状态、单批内部往来提交 command 成功后立即本地更新列表、清空选择并禁止自动触发下一笔 detail GET，把 `bank_flow_rule_batch` freshness/reload 移到后台 reconcile、mutation 后后台等待只发送 `bank_flow_rule_batch` 自身 target 且事件仍保留完整跨页 targets、关联台 bank-flow 折叠行不显示旧计数文案且保留“展开 N 条明细”。 |
