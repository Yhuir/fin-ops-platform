...
## 2026-09-01 - 三栏流水表信息层级与选择控件修复

- 根因：页面使用 HeroUI Checkbox 时只渲染 root/hidden input，没有渲染 `Checkbox.Control` 与 `Checkbox.Indicator`；选中状态仍存在，但用户看不到表头和逐行选择框。
- 页面保留既有主标签、子标签、账户批次/流水三栏，没有新增状态层或数据请求。右栏表格收敛为选择、对方户名、交易时间、金额、按需关联、摘要/用途/备注；删除行内重复银行账户、主/子标签和批次标题第二套全选/清空按钮。
- 表头 checkbox 是唯一批量选择入口，支持未选/半选/全选；顶部只在有选择时显示选择数和“清空选择”。右栏流水列表是唯一纵向滚动 owner，表格只保留必要的横向溢出。
- canonical API、提交/撤回协议、权限、数据库、read model、worker 与部署拓扑均未改变，因此 `boundary-io.md` 不变。
