...
`read-models` 负责派生投影、freshness、refresh 和 operation barrier；`canonical-facts` 负责记录源业务事实的 owner、I/O、禁止路径和旧生产事实源删除规则。
...
- 写操作影响 read model dirty scope、domain event、operation barrier target 或 audit 行为。
- 将 legacy snapshot、local pickle、`state:*` JSON、Mongo app snapshot、GridFS fallback 或 direct SQL fallback 移入或移出 production 主路径。
