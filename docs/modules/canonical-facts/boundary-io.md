...
- OA 附件解析 cache 不是正式发票池。只有 `OAAttachmentInvoicePromotionService` 及发票导入确认的 transaction-scoped promotion 编排可按强发票身份和显式 OA/expense-item/attachment source context 调用 canonical invoice repository；同批身份一次查询、批量保存，重复输入不得刷新 `app.invoices.updated_at`。20 位纯数字 `invoice_no` 与显式 `digital_invoice_no` 使用同一强身份。发票写入必须先按稳定顺序锁定并批量加载当前 canonical rows，再以来源身份去重追加 `source_links[]`；禁止用 stale persistence object 或整列 last-write-wins 覆盖既有 OA 附件、显式明细归属及导入 provenance。经过校验的正式导入字段维护 canonical 表头事实，OA OCR 只补空；强身份或关键事实冲突 fail closed。`source_unique_key` 唯一索引继续阻止并发或漏加载产生重复事实。
