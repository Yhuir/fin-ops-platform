...
- `test_cash_api.py`：三个普通历史入口的 all 成功/日期互斥/未知 scope/重复 key/缺失范围/旧长范围拒绝，以及个人年、截止视图、项目候选拒绝新 scope；沿用权限/no-store/隐私测试。
- `test_cash_queries.py`：真实 PG 跨 2020–2026 历史、今天之后记录隔离、页码与全范围汇总、旧记录/无匹配搜索不改变账户余额、真实开账金额、周转和票据结算按今天截止；原范围/父对象/个人账/截止报表仍回归。
- `CashFlows/CashBooks/CashFlowCorrections`：section 重入第一请求为 all、隐藏普通 tab 同步清日期、非日期偏好保留、本次 Tab/刷新/抽屉关闭保持日期、all 与日期互斥、项目候选用 date_to、纠错候选全历史；全现金组件回归继续覆盖写后重读、验证、权限及失败状态。
- `test_cash_http_integration.py` 与 `cash-real-api-flow.spec.ts`：真实 HTTP→服务→PG 的旧年流水及保存/删除重读、首屏 all 后切自定义期间，原个人任务和票据链继续执行。
- `test_cash_query_performance.py`：原测量入口扩为七个业务年份，增加 all 流水/单账户/多项目分类/深页/周转/票据；10k/100k、并发 1/4、每组至少 100 次，输出 p50/p95/p99、失败率、语句数和独立 EXPLAIN ANALYZE 的计划节点/块读取/临时落盘。仅专属 disposable 数据库写合成数据，不放宽资源限制或加性能门禁。

七类覆盖：1 输入及业务日期边界；2 查询 service/repository；3 API；4 同快照直接查询（不存在缓存/read model/后台任务新增）；5 前端；6 真实 HTTP/浏览器链；7 旧范围/权限/任务年与截止/写后重读回归。执行命令和结果见[实施记录](implementation-notes.md)。
...
执行细节补充测试：跨年事项/任务关联流水默认不发本年期间且不漏记录；独立流水现发显式 all，主动选择期间后仍发有界日期；切视图恢复条件但重新取数，撤权卸载条件状态；删末页最后一行调整至最近有效页，重读失败不重发delete；录入Select仅启用项并保留已保存原值，停用项仍可历史筛选；任务/设置多区块能滚动到最后一表，无每表满屏叠加。候选49+null分页、真实搜索差异防抖、完整query应用前校验都有专项测试。
