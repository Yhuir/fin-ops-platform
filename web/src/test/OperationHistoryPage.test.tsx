...
describe("OperationHistoryPage", () => {
  test("only loads for an administrator and shows the durable event detail", async () => {
    ...
    await userEvent.click(within(table).getByRole("button", { name: "查看确认关联" }));
    ...
    expect(drawer).toHaveTextContent('"status": "unpaired"');
    expect(drawer).toHaveTextContent('"status": "paired"');
    expect(drawer).not.toHaveTextContent("internal-relation-1");
    await waitFor(() => {
      expect(fetchMock.mock.calls.some(([url]) => String(url).includes("/api/operations/audit-events/10000000"))).toBe(true);
    });
  });
  ...
  test("redirects a non-admin away before loading audit data", async () => {
    ...
    expect(fetchMock.mock.calls.some(([url]) => String(url).includes("/api/operations/audit-events"))).toBe(false);
  });
});
