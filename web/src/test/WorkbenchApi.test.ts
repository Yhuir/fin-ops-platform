...
describe("workbench api bank amount mapping", () => {
  ...
  test.each([
    ...
    ["oa_role_configuration_invalid", 502, "OA 入口角色配置不完整，访问权限未保存。请联系管理员修复角色配置。"],
    ["oa_role_sync_uncertain", 503, "OA 入口权限保存结果无法确认，请联系管理员核对后再操作。"],
    ["oa_access_accounts_invalid", 502, "账户已停用、已删除或不存在。请核对异常账户并移除无效的访问授权。"],
