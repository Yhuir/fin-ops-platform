...
vi.mock("../components/cash/CashFlowTable", () => ({ default: () => <p>完整现金流水视图</p> }));
...
describe("cash books", () => {
  test("defaults to the 17-column total, preserving each event's remaining balance and row category", () => {
    ...
    expect(mocks.query.mock.calls.filter(([path]) => path !== null).map(([path]) => path)).toEqual(["/reports/turnover"]);
  ...
  test("error does not display stale successful totals or a made-up zero", () => {
    ...
    expect(screen.queryByRole("grid", { name: "往来账总表" })).not.toBeInTheDocument();
    expect(screen.queryByText("5,500.00")).not.toBeInTheDocument();
  });
  test("cash-flow tab renders the complete shared flow owner, without adding another pool", async () => {
    render(<CashBooks />);
    await userEvent.click(screen.getByRole("tab", { name: "现金流水" }));
    expect(screen.getByText("完整现金流水视图")).toBeInTheDocument(); expect(screen.queryByRole("grid", { name: "往来账总表" })).not.toBeInTheDocument();
  });
  test("personal unconfigured coverage remains unknown and only fetches the active view", async () => {
    ...
    await userEvent.click(screen.getByRole("tab", { name: "无票报销冲抵" }));
