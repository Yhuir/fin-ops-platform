      ...
      <Popover.Dialog aria-label="选择成本标签">
        {!open ? null : loading ? <p role="status">正在读取标签…</p> : error ? <div role="alert">{error}<button type="button" onClick={onLoad}>重试</button></div> : !tags.length ? <p>暂无可选标签</p> : <div className="cost-manual-tag-columns" onKeyDownCapture={event => {
          ...
          <ListBox aria-label="主标签" selectionMode="single" selectedKeys={primary ? [primary] : []} onSelectionChange={keys => {
            ...
            {[...groups].map(([label, items]) => <ListBox.Item id={label} key={label} textValue={label} className="cost-manual-tag-option">
          ...
          {groups.has(primary) ? <ListBox key={primary} aria-label="子标签" selectionMode="single" selectedKeys={value ? [value] : []} onSelectionChange={keys => { if (keys === 'all' || !keys.size) return; choose(groups.get(primary)!.find(tag => tag.code === [...keys][0])!); }}>
            {groups.get(primary)!.map(tag => <ListBox.Item id={tag.code} key={tag.code} textValue={tag.sub_label || tag.primary_label} className="cost-manual-tag-option">{tag.sub_label || `${tag.primary_label}（主标签本身）`}</ListBox.Item>)}
          </ListBox> : <div className="cost-manual-tag-empty">选择主标签</div>}
        </div>}
      </Popover.Dialog>
    </Popover.Content>
  </Popover>;
