...
export default memo(function CostSourcePicker({ value, options, label, focusKey, lineId, invalid, disabled, isSourceDisabled, onChange }: {
  value: string; options: CostSourceOption[]; label: string; focusKey: string; lineId: number; invalid: boolean; disabled: boolean; isSourceDisabled: (lineId: number, sourceId: string) => boolean; onChange: (lineId: number, sourceId: string) => void;
}) {
  ...
  return <Popover isOpen={open} onOpenChange={next => { if (!disabled) setOpen(next); }}>
    ...
    <Popover.Content className="cost-source-picker-popover" placement="bottom start" offset={4}>
      <Popover.Dialog aria-label={label}>
        {open ? <ListBox aria-label={label} selectionMode="single" selectedKeys={value ? [value] : []} disabledKeys={options.filter(option => isSourceDisabled(lineId, option.id)).map(option => option.id)} onSelectionChange={keys => { if (keys === 'all') return; const key = [...keys][0]; if (key !== undefined) onChange(lineId, String(key)); setOpen(false); }}>
