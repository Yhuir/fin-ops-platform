      ...
      <div className="cost-source-table-scroll"><table className="cost-source-table" aria-label="成本分配明细">
        ...
        {task.units.map((unit, unitIndex) => {
          ...
          return <tbody key={unit.unitId}>
            {unit.costEligible === false ? <tr>{identity}<td colSpan={4}>待审批</td></tr> : lines.length ? lines.map((line, index) => <tr key={line.id}>{index === 0 ? identity : null}{lineCells('costLines', line, index, index === 0)}</tr>) : <tr className={zero ? 'cost-source-zero' : 'cost-source-unallocated'}>{identity}<td colSpan={3}><span>{zero ? '零成本' : '未分配'}</span>{!draft.oaAmountLocks[unit.unitId] && !task.allowsPartial ? <button type="button" className="cost-source-link cost-source-zero-button" disabled={disabled} onClick={() => onChange({ ...draft, zeroUnitIds: zero ? draft.zeroUnitIds.filter(id => id !== unit.unitId) : [...draft.zeroUnitIds, unit.unitId] })}>{zero ? '取消零成本' : '设为零成本'}</button> : null}</td><td>{addButton('costLines', unit.unitId)}</td></tr>}
          </tbody>;
        })}
