...
type Item = { key: string; label: string; amount: string; description: string };
...
export default function CostStatisticsHierarchy({ lanes, children }: { lanes: CostHierarchyLane[]; children: ReactNode }) {
  ...
  return <div className="cost-hierarchy" style={{ '--cost-lane-count': lanes.length } as React.CSSProperties}>
    ...
    <div className="cost-hierarchy-grid">
      {lanes.map((lane, index) => <div key={lane.title} className={`cost-hierarchy-column${activeLane === index ? ' is-current' : ''}`}>
        <CostExplorerList<Item> title={lane.title} count={lane.items.length} items={lane.items} emptyLabel={index > 0 && !lanes[index - 1].selectedKey ? `请先选择${lanes[index - 1].title}` : '当前范围暂无数据'} getKey={item => item.key} getPrimaryText={item => item.label} isActive={item => item.key === lane.selectedKey} onSelect={item => { lane.onSelect(item.key); setFocusedLane(index + 1); }} renderSecondary={item => item.description} renderMeta={item => <span className="cost-source-money">{formatCostAmount(item.amount)}</span>} />
      </div>)}
      <div className={`cost-hierarchy-detail${activeLane === lanes.length ? ' is-current' : ''}`}>{children}</div>
    </div>
  </div>;
}
