...
import { createSourceDraft, sourceSaveRequest, type SourceDraft } from '../../features/cost-statistics/sourceAllocation';
import type { CostStatisticsManualAllocationSummary, CostStatisticsManualAllocationTask } from '../../features/cost-statistics/types';
import './costSourceAllocation.css';

type Props = { canSave: boolean; pendingCount?: number; onSaved: () => void };
type TaskState = { task?: CostStatisticsManualAllocationTask; draft?: SourceDraft; dirty?: boolean; loading?: boolean; saving?: boolean; error?: string; notice?: string };
export default function CostStatisticsManualAllocationDrawer({ canSave, pendingCount, onSaved }: Props) {
  ...
  const loadDetail = async (id: string, force = false) => {
    ...
    try {
      ...
      setCase(id, { task, draft: createSourceDraft(task), dirty: false, loading: false });
    } catch (caught) {
      if (!controller.signal.aborted) setCase(id, { loading: false, error: caught instanceof Error ? caught.message : '任务读取失败' });
  ...
  const load = async (targetStatus = status, targetQuery = query, cursor?: string) => {
    ...
    } catch (caught) { if (!controller.signal.aborted) setError(caught instanceof Error ? caught.message : '人工分配任务加载失败'); }
  ...
  const save = async (id: string) => {
    const current = states[id]; if (!current.task || !current.draft || current.saving || !canSave || !current.task.canSave) return;
    const request = sourceSaveRequest(current.task, current.draft);
    setCase(id, { saving: true, error: undefined, notice: undefined });
    try {
      const saved = await saveCostStatisticsManualAllocation(request);
      setCase(id, { task: saved, draft: createSourceDraft(saved), dirty: false, saving: false, notice: saved.status === 'allocated' ? '分配已保存' : '来源分配已保存，仍有银行信息待完善' });
      if (saved.status !== current.task.status) setCounts(value => value ? { pending: value.pending + (saved.status === 'pending' ? 1 : -1), allocated: value.allocated + (saved.status === 'allocated' ? 1 : -1) } : null);
      if (saved.status !== status) { setItems(list => list.filter(item => item.relationCaseId !== id)); setExpanded(null); }
      onSaved();
    } catch (caught) { setCase(id, { saving: false, error: caught instanceof Error ? caught.message : '保存失败，请重试；当前草稿已保留' }); }
  };
  const close = () => {
    if (saving) return;
  ...
  return <>
    ...
    <AppDrawer open={open} title="成本人工分配" width="min(880px, 100vw)" className="cost-source-drawer" onClose={close} closeDisabled={saving}>
      <div className="cost-source-body">
        <ToggleButtonGroup aria-label="成本人工分配状态" selectedKeys={new Set([status])} selectionMode="single" onSelectionChange={keys => { const next = [...keys][0]; if (saving || (next !== 'pending' && next !== 'allocated')) return; setStatus(next); void load(next); }}>
          <ToggleButton id="pending">待分配 {counts?.pending ?? pendingCount ?? '—'}</ToggleButton><ToggleButton id="allocated">已完成 {counts?.allocated ?? '—'}</ToggleButton>
        </ToggleButtonGroup>
        <form className="cost-source-search" onSubmit={event => { event.preventDefault(); if (saving) return; setQuery(queryDraft.trim()); void load(status, queryDraft.trim()); }}>
          <Input aria-label="搜索人工分配任务" placeholder="搜索项目、费用或申请人" value={queryDraft} onChange={event => setQueryDraft(event.target.value)} /><Button aria-label="查询人工分配任务" type="submit" isIconOnly size="sm" variant="secondary" isDisabled={saving}><Search size={16} /></Button>
        </form>
        ...
        {items.map(item => {
          ...
          return <article className="cost-source-task" key={id}>
            <button type="button" className="cost-source-task-heading" aria-expanded={active} onClick={() => { setExpanded(active ? null : id); if (!active) void loadDetail(id); }}>
              <ChevronRight size={15} className={active ? 'is-expanded' : ''} /><span><strong>{item.projectNames.join('、') || '项目未填写'}</strong><small>{id}</small><span>净支出 <b className="cost-source-money">{item.netOutflowTotal}</b> · OA 原额 {item.oaTotal}</span></span>
              <span className="cost-source-task-meta"><Chip size="sm" color={item.status === 'pending' ? 'warning' : 'success'} variant="soft">{item.status === 'pending' ? '待分配' : '已完成'}</Chip><small>{item.bankEventCount} 条流水 · {item.unitCount} 个 OA 单元{state?.dirty ? ' · 未保存' : ''}</small></span>
            </button>
            {active ? <>
              {state?.loading ? <p role="status">正在读取该关联的来源…</p> : null}
              {state?.task && state.draft ? <CostSourceAllocationForm task={state.task} draft={state.draft} disabled={!canSave || !state.task.canSave || !!state.saving} saving={!!state.saving} error={state.error} notice={state.notice} onChange={draft => setCase(id, { draft, dirty: true, notice: undefined })} onSave={() => void save(id)} /> : state?.error ? <p className="cost-source-error" role="alert">{state.error}</p> : null}
              {state?.error ? <Button size="sm" variant="secondary" onPress={() => { if (!state.dirty || window.confirm('重新读取会替换当前草稿，是否继续？')) void loadDetail(id, true); }}>重新读取当前事实</Button> : null}
            </> : null}
          </article>;
        })}
