...
export type FlowPart = {
  id: string; mode: "loan" | "expense" | "settlement";
...
export function flowCompositionPayload(parts: FlowPart[], date: string, projectId: string | null, kind: CashFlowKind) {
  const related_items: Record<string, unknown>[] = []; const allocations: Record<string, unknown>[] = [];
  for (const part of parts) {
    if (part.mode === "loan" || part.mode === "expense") {
      ...
      allocations.push({ id: part.id, item_id: part.item.id, target_is_new: false, expected_item_version: part.item.version, kind: part.kind, amount: cashMoneyInput(part.amount) });
    }
  }
  return { related_items, origin_items: [], allocations };
}

export function CashFlowComposition({ parts, onChange, kind, existingItem, defaultAmount, defaultContent, disabled }: {
  parts: FlowPart[]; onChange: (parts: FlowPart[]) => void; kind: CashFlowKind; existingItem?: CashItem; defaultAmount: string; defaultContent: string; disabled: boolean;
}) {
  const [picking, setPicking] = useState<string | null>(null);
  function update(id: string, patch: Partial<FlowPart>) { onChange(parts.map(part => part.id === id ? { ...part, ...patch } : part)); }
  function add(mode: "loan" | "expense") {
    const part = newFlowPart(mode, kind);
    onChange([...parts, { ...part, ...(parts.length === 0 ? { amount: defaultAmount, content: defaultContent } : {}) }]);
  }
  if (kind === "transfer") return null;
  return <section className="cash-composition">
    <div className="cash-toolbar"><h3>借款、费用与还款（按需登记）</h3>
      {!existingItem && <><Button size="sm" variant="secondary" isDisabled={disabled} onPress={() => add("loan")}>新增借款 / 代付</Button>
        {kind === "payment" && <Button size="sm" variant="secondary" isDisabled={disabled} onPress={() => add("expense")}>登记实际费用</Button>}
      </>}
    ...
    {parts.map((part, index) => <div className="cash-composition-row" key={part.id}>
      ...
      {part.mode === "loan" || part.mode === "expense" ? <div className="cash-form-grid">
        ...
        {part.mode === "loan" && <>
          <CashInput label="往来对象" value={part.counterparty} onChange={counterparty => update(part.id, { counterparty })} required disabled={disabled || part.group === "personal"} />
          <CashSelect label="账簿分类" value={part.group} onChange={group => update(part.id, { group, counterparty: part.group === "personal" || group === "personal" ? "" : part.counterparty, direction: group === "personal" ? "receivable" : part.direction })} required disabled={disabled} options={[{ value: "company", label: "公司" }, { value: "external_person", label: "外部人员" }, { value: "personal", label: "个人借款 / 代付" }]} />
        ...
        <CashSelect label="处理类型" value={part.kind} onChange={value => update(part.id, { kind: value as CashSettlementKind, item: null })}
          disabled={disabled || Boolean(existingItem && index === 0)} options={(kind === "receipt" ? ["cash_repayment", "company_collection", "expense_refund"] : ["cash_repayment", "expense_payment"]).map(value => ({ value, label: value === "cash_repayment" ? kind === "receipt" ? "收回借款" : "归还借款" : settlementLabels[value as CashSettlementKind] }))} />
        ...
        {picking === part.id && <CashItemPicker label="选择要关联的事项" params={{ purpose: "settlement_target", settlement_kind: part.kind }} onCancel={() => setPicking(null)} onSelect={item => { update(part.id, { item }); setPicking(null); }} />}
