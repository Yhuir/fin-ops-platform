...
type Props = { open: boolean; onClose: () => void; kind?: CashFlowKind; onSaved?: () => void } & (
  | { originItem: CashItem; flowId?: never; task?: never; existingItem?: never; settlementKind?: never }
  | { originItem?: undefined; flowId?: string; task?: TaskContext; existingItem?: CashItem; settlementKind?: CashSettlementKind }
);
...
function CashFlowEditor({ onClose, onSaved, kind: initialKind = "receipt", task, existingItem, originItem, settlementKind, flow }: Props & { flow?: CashFlow }) {
  const [completed, setCompleted] = useState(false);
  const formId = useId();
  const [id] = useState(() => flow?.id ?? crypto.randomUUID());
  const [expectedVersion] = useState(flow?.version);
  const [kind, setKind] = useState<CashFlowKind>(flow?.kind ?? task?.kind ?? (originItem ? originItem.obligation_direction === "receivable" ? "payment" : "receipt" : initialKind));
  const [date, setDate] = useState(flow?.occurred_on ?? originItem?.origin_date ?? cashToday());
  const [amount, setAmount] = useState(flow?.amount ?? originItem?.original_amount ?? "");
  ...
  const [content, setContent] = useState(flow?.content ?? originItem?.content ?? ""); const [person, setPerson] = useState(flow?.person_name ?? ""); const [remark, setRemark] = useState(flow?.remark ?? "");
  ...
  const hasPersonal = parts.some(part => part.mode === "loan" && part.group === "personal");
  const personal = useCashQuery<CashPersonalSetting>(hasPersonal ? "/settings/personal-opening" : null);
  const composedParts = parts.map(part => part.mode === "loan" && part.group === "personal" ? { ...part, direction: "receivable", counterparty: personal.data?.counterparty ?? "" } : part);
  ...
  function changeKind(value: CashFlowKind) {
    setKind(value); setCategory(""); setParts([]);
    setError(null); setPendingKind(null); setDirty(true);
  }
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (hasPersonal && (personal.loading || personal.error)) throw new Error("个人专账配置尚未成功读取，请读取成功后再保存。");
      if (hasPersonal && (!personal.data?.counterparty || !personal.data.opening_date)) throw new Error("请先在基础设置确认个人专账归属人和起算日期。");
      if (!date || date > cashToday()) throw new Error("请填写不晚于今天的实际收付日期。");
      if (!content.trim()) throw new Error("请填写内容说明。");
      ...
      else if (originItem) body = { ...body, id, project_mode: "selection", oa_project_id: originItem.oa_project_id, related_items: [], allocations: [], origin_items: [{ item_id: originItem.id, expected_item_version: originItem.version }] };
  ...
  return <AppDrawer completion={completed ? "现金流水已保存" : undefined} open title={flow ? "编辑现金流水" : task ? `办理任务 · ${task.title}` : "新增现金流水"} onClose={() => !completed && (dirty || !correctionsValid) ? setClosing(true) : onClose()}
    ...
    <form id={formId} onSubmit={save} onChange={() => setDirty(true)} className="cash-form">
      {flow ? <CashSelect label="方向" value={kind} disabled={mutation.busy} onChange={value => changeKind(value as CashFlowKind)} required options={Object.entries(cashFlowLabels).map(([value, label]) => ({ value, label }))} /> : task || existingItem || originItem ?
      ...
      <div className="cash-form-grid">
        <CashInput label="实际发生日" type="date" value={date} onChange={setDate} required disabled={mutation.busy || Boolean(originItem)} />
        ...
        <CashInput label="内容说明" value={content} onChange={setContent} required disabled={mutation.busy} />
    ...
    {hasPersonal && <section className="cash-settings-subsection"><CashNotice error={personal.error?.message} />{personal.error && <Button size="sm" variant="tertiary" onPress={personal.reload}>重新读取个人专账配置</Button>}{personal.loading ? <p role="status">正在读取个人专账设置…</p> : !personal.error && (personal.data?.counterparty && personal.data.opening_date ? <p className="cash-hint">个人借款 / 代付归属：{personal.data.counterparty}。替个人还卡增加其应归还金额。</p> : <p className="cash-notice">请在基础设置确认个人归属人和起算日期。</p>)}</section>}
    <section className="cash-settings-subsection"><h3>项目</h3>
      <p>{originItem ? originItem.project_name_snapshot ?? "无项目" : existingItem ? existingItem.project_name_snapshot === null ? "无项目" : existingItem.project_name_snapshot : project === null ? "无项目" : project.name_snapshot}</p>
      {originItem ? <p className="cash-hint">补记“{originItem.content}”的现金来源，沿用事项日期和项目。</p> : existingItem ? <p className="cash-hint">沿用已有事项的项目，不改写 OA 状态。已结束的项目仍可办理其真实结算。</p> : <div className="cash-row-actions">
    ...
    {flow ? <CashFlowCorrections flowId={flow.id} mode="edit" onValidityChange={setCorrectionsValid} onChange={value => { setCorrections(value); if (value.source_corrections.length || value.settlement_changes.length || value.item_reference_changes.length) setDirty(true); }} /> : !originItem &&
      <CashFlowComposition parts={composedParts} kind={kind} existingItem={existingItem} defaultAmount={amount} defaultContent={content} disabled={mutation.busy} onChange={value => { setParts(value); setDirty(true); }} />}
  </AppDrawer>;
}
...
function CashFlowDetailDrawer({ flowId, onClose, onSaved }: Props & { flowId: string }) {
  ...
  const [originFlow, setOriginFlow] = useState<CashItem | null>(null);
  ...
  if (originFlow) return <CashFlowDrawer open originItem={originFlow} onClose={() => setOriginFlow(null)} />;
  ...
  if (itemId) return <CashItemDetail itemId={itemId} onClose={() => setItemId(null)} onFlow={setLinkedFlow} onOriginFlow={item => { setItemId(item.id); setOriginFlow(item); }} onActualFlow={(item, kind) => setActualFlow({ item, kind })} />;
      ...
      {mode === "delete" ? <><p>删除 {flow.occurred_on} 的{cashFlowLabels[flow.kind]} {cashAmount(flow.amount)} 元。现金流水及其对应账簿处理会一并移除。</p>
        ...
        <CashFlowCorrections flowId={flowId} mode="delete" onChange={setCorrections} onValidityChange={setCorrectionsValid} /></> : mode === "unlink" ? <p>仅解除这笔手动流水与“{flow.task?.title}”的错误关联，保留真实现金流水和已有账目处理，任务金额重新计算。</p> : <>
          ...
          {mode === "details" ? <dl className="cash-detail-grid">
            ...
            <dt>人员</dt><dd>{flow.person_name === null ? "—" : flow.person_name}</dd><dt>内容说明</dt><dd>{flow.content}</dd>
