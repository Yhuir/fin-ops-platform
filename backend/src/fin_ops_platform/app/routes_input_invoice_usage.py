...
class InputInvoiceUsageApiRoutes:
    ...
    def route(
        ...
        if method == "GET" and route_path == "/api/input-invoice-usage/payment-status-rules":
            return self.payment_status_rules(headers)
        ...
        if method == "GET" and route_path.startswith("/api/input-invoice-usage/invoices/") and route_path.endswith("/detail"):
            ...
            return self.invoice_detail(invoice_id, headers)
        if method == "GET" and route_path.startswith("/api/input-invoice-usage/bank-transactions/") and route_path.endswith("/detail"):
            ...
            return self.bank_transaction_detail(bank_transaction_id, headers)
    ...
    def rows(
        ...
        if auth_error is not None:
            return auth_error
        try:
            payload = self._query_service.rows(
                query,
    ...
    def invoice_detail(
        ...
        if auth_error is not None:
            return auth_error
        try:
            payload = self._query_service.invoice_detail(
                invoice_id,
                tenant_id=_tenant_id(session),
            )
        except InputInvoiceUsageError as exc:
    ...
    def bank_transaction_detail(
        ...
        if auth_error is not None:
            return auth_error
        try:
            payload = self._query_service.bank_transaction_detail(
                bank_transaction_id,
                tenant_id=_tenant_id(session),
            )
        except InputInvoiceUsageError as exc:
    ...
    def oa_detail(
        ...
        if auth_error is not None:
            return auth_error
        try:
            payload = self._query_service.oa_detail(
                oa_id,
                tenant_id=_tenant_id(session),
            )
        except InputInvoiceUsageError as exc:
    ...
    def _read_session(
        ...
        return self._resolve_read_session(
            headers,
            denied_message="当前账户没有访问进项发票使用情况页面权限。",
        )


def _tenant_id(session: Any | None) -> str:
