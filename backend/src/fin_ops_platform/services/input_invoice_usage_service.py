...
class InputInvoiceUsageQueryService:
    ...
    def _oa_summary(
        ...
        return {
            ...
            "workflowStatus": str(getattr(record, "workflow_status", "") or "") if record is not None else "",
    ...
    def _parse_filters(self, filters: str | list[dict[str, Any]] | None) -> list[dict[str, Any]]:
        try:
            return parse_input_invoice_usage_filters(filters)
        except InputInvoiceUsageQueryContractError as exc:
            raise _input_invoice_usage_query_contract_error(exc) from exc

    def _parse_sort(self, sort_field: str | None, sort_direction: str | None) -> tuple[str, str]:
        try:
            return parse_input_invoice_usage_sort(sort_field, sort_direction)
        except InputInvoiceUsageQueryContractError as exc:
            raise _input_invoice_usage_query_contract_error(exc) from exc
...
def _input_invoice_usage_query_contract_error(exc: InputInvoiceUsageQueryContractError) -> InputInvoiceUsageError:
    return InputInvoiceUsageError(exc.error_code, str(exc), details=exc.details)
