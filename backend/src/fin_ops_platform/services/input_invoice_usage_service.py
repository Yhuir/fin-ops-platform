...
class InputInvoiceUsageQueryService:
    ...
    def oa_reverse_preview(self, request: dict[str, Any] | None) -> dict[str, Any]:
        ...
        source = str(payload.get("source") or "currentFilters").strip() or "currentFilters"
        target_code = str(payload.get("targetApplicantCode") or "chen_xiuyun").strip()
        ...
        if invoice_ids:
            ...
            rows = list_payload["rows"]
            missing_ids = []

        candidate_rows: list[dict[str, Any]] = []
        rejected: list[dict[str, str]] = []
        for missing_id in missing_ids:
            rejected.append({"invoiceId": missing_id, "reasonCode": "invoice_not_found", "reason": "发票不存在"})
        for row in rows:
            if int(row["oa"].get("relationCount") or 0) > 0:
                rejected.append(
                    {
                        "invoiceId": row["invoiceId"],
        ...
        total = sum((_decimal(row["invoice"]["totalWithTax"]) for row in candidate_rows), start=ZERO)
        preview_id = "oa_reverse_preview_" + sha1(
            json.dumps([row["invoiceId"] for row in candidate_rows], sort_keys=True).encode("utf-8")
        ).hexdigest()[:16]
        return {
            "previewId": preview_id,
            "source": source,
            "invoiceCount": len(candidate_rows),
            ...
            "groups": [
                {
                    ...
                    "invoiceCount": len(candidate_rows),
                    "totalWithTax": _money(total),
                    "candidateInvoiceIds": [row["invoiceId"] for row in candidate_rows],
