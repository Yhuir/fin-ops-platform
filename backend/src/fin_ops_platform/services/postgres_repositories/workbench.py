        ...
        def write(connection: Any) -> None:
            ...
            if batch_ids:
                ...
                connection.execute(
                    ...
                    where bank_flow_rule_batch_id in (
                        select id
            ...
            self._replace_bank_flow_rule_batch_events(
                connection,
                snapshot.get("audit_log") if isinstance(snapshot, dict) else None,
            )
    ...
    def save_bank_flow_rule_batches_scope(
        ...
        *,
        ...
        batch_items = [
            ...
            for batch_id, payload in list(iter_mapping(batches))
            if month_start(payload.get("scope_month") or payload.get("month")) == scope_month
            and _no_oa_batch_relation_mode(payload) == BANK_FLOW_RULE_BATCH_RELATION_MODE
        ]
        batch_ids = [
            str(batch_id).strip()
            for batch_id, _payload in batch_items
        ...
        def write(connection: Any) -> None:
            if batch_ids:
                ...
                connection.execute(
                    "delete from app.bank_flow_rule_batches where scope_month = %s::date",
                    (scope_month,),
                )
            self._upsert_bank_flow_rule_batch_items(connection, batch_items)
            scoped_batch_ids = set(batch_ids)
