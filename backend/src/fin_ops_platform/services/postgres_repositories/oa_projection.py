...
class PostgresOAProjectionRepository:
    ...
    def list_etc_oa_detection_candidates(
        ...
        *,
        ...
        normalized_business_batch_id = text(business_batch_id)
        normalized_external_batch_id = text(external_etc_batch_id)
        marker_texts = [
            marker
            for marker in (
        ...
        rows = self._connection.fetch_all(
            ...
            from app.oa_applications oa
            where (
                    oa.normalized_payload::text ilike any(%s::text[])
        ...
        return [
            ...
            for row in rows
            if self._row_contains_any_etc_marker(row, marker_texts)
        ]
    ...
    @classmethod
    def _etc_oa_detection_candidate_from_row(cls, row: dict[str, Any]) -> dict[str, Any]:
        payload = row_payload(row, "normalized_payload", "payload", "raw_payload")
        if not isinstance(payload, dict):
            payload = {}
        ...
        reason = cls._first_text(
            ...
            payload.get("cause"),
            payload.get("remark"),
            detail_fields.get("费用内容"),
            detail_fields.get("事由"),
        )
        return {
            "oa_row_id": cls._first_text(payload.get("oa_row_id"), payload.get("oaRowId"), payload.get("id"), row.get("row_id")),
            "form_id": cls._etc_detection_form_id(row=row, payload=payload, detail_fields=detail_fields),
            "amount": decimal_text(payload.get("amount") or row.get("amount")),
            "invoice_count": cls._etc_detection_invoice_count(payload=payload, detail_fields=detail_fields),
            "applicant": cls._first_text(payload.get("applicant"), row.get("applicant")),
            "applicant_user_id": cls._first_text(
                payload.get("applicant_user_id"),
                payload.get("owner_user_id"),
                payload.get("userId"),
                payload.get("applicantUserId"),
                detail_fields.get("申请人ID"),
                detail_fields.get("用户ID"),
                payload.get("applicant"),
                row.get("applicant"),
            ),
            "owner_org_id": cls._first_text(
                payload.get("owner_org_id"),
                payload.get("org_id"),
                payload.get("orgId"),
                payload.get("department_id"),
                payload.get("departmentId"),
                detail_fields.get("部门ID"),
                detail_fields.get("组织ID"),
            ),
            "organization": cls._first_text(
                payload.get("organization"),
                payload.get("org_name"),
                payload.get("orgName"),
                detail_fields.get("部门"),
                detail_fields.get("组织"),
            ),
            "created_at": cls._etc_detection_created_at(row=row, payload=payload, detail_fields=detail_fields),
            "process_status": cls._first_text(
                payload.get("process_status"),
                payload.get("processStatus"),
                detail_fields.get("流程状态"),
                row.get("status"),
            ),
            "reason": reason,
            "cause": reason,
    ...
    @staticmethod
    def _row_contains_any_etc_marker(row: dict[str, Any], markers: list[str]) -> bool:
        search_text = "\n".join(
            str(serialize_value(value))
            for value in (
                row.get("normalized_payload"),
                row.get("raw_payload"),
                row.get("row_id"),
            )
            if value not in (None, "")
        ).lower()
        return any(marker.lower() in search_text for marker in markers)

    @staticmethod
    def _etc_detection_form_id(
        *,
        row: dict[str, Any],
    ...
    @staticmethod
    def _etc_detection_invoice_count(
        *,
        payload: dict[str, Any],
        detail_fields: dict[str, Any],
    ) -> int | None:
    ...
    def _etc_detection_created_at(
        ...
        *,
        ...
        return cls._format_datetime(
            cls._first_text(
                payload.get("created_at"),
                payload.get("createdAt"),
                payload.get("applicationDate"),
                payload.get("ApplicationDate"),
...
class PostgresOAProjectionAdapter:
    ...
    def list_etc_oa_detection_candidates(
        ...
        *,
        ...
        candidate_loader = getattr(self._repository, "list_etc_oa_detection_candidates", None)
