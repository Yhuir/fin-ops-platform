...
class PostgresCoreRepository:
    ...
    def _invoice_from_row(self, row: dict[str, Any]) -> Invoice:
        ...
        return Invoice(
            ...
            specific_business_type=self._text(payload.get("specific_business_type")),
            taxable_item_name=self._text(payload.get("taxable_item_name")),
            specification_model=self._text(payload.get("specification_model")),
            ...
            project_id=self._text(payload.get("project_id")),
    ...
    def _transaction_from_row(self, row: dict[str, Any]) -> BankTransaction:
        ...
        return BankTransaction(
            ...
            source_unique_key=self._text(row.get("source_unique_key")),
            data_fingerprint=self._text(row.get("data_fingerprint")),
            written_off_amount=Decimal(str(payload.get("written_off_amount") or row.get("written_off_amount") or "0")),
    ...
    def _file_item_from_row(self, row: dict[str, Any], payload: dict[str, Any]) -> FileImportPreviewItem:
        ...
        return FileImportPreviewItem(
            ...
            header_signature=self._text(payload.get("header_signature")),
            mapping_candidates=list(payload.get("mapping_candidates") or []),
            mapping_fields=list(payload.get("mapping_fields") or []),
            field_mapping={str(key): str(value) for key, value in dict(payload.get("field_mapping") or {}).items()},
            mapping_source=self._text(payload.get("mapping_source")),
