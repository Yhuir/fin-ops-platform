...
from fin_ops_platform.services.bank_batch_read_model_repository import BankBatchReadModelRepositoryPort
...
class BankBatchPersistenceError(RuntimeError):
    error_code = "no_oa_bank_batch_persistence_failed"


class BankBatchRelationMutationError(ValueError):
    def __init__(
        self,
        error_code: str,
...
class BankBatchPairRelationSnapshotPort:
...
class BankBatchApplicationService:
    def __init__(
        self,
        *,
        import_service: Any,
        ...
        bank_batch_read_model_repository: Any | None = None,
        ...
        self._bank_batch_read_model_repository = bank_batch_read_model_repository
        if self._bank_batch_read_model_repository is None and workbench_sql_read_repository is not None:
            self._bank_batch_read_model_repository = BankBatchReadModelRepositoryPort(
    ...
    def list_batches_payload(
        ...
        *,
        ...
        if relation_mode == BANK_FLOW_RULE_BATCH_RELATION_MODE and self._bank_batch_read_model_repository is None:
        ...
        summary_batches = self._bank_batch_service.list_batches(summary_filters)
        read_batches = self._bank_batch_service.list_batches(filters)
    ...
    def submit_internal_transfer_rows_from_workbench(
        ...
        *,
        ...
        try:
            ...
            submitted = self._bank_batch_service.submit_batch(
                str(batch["batch_id"]),
                actor=actor,
    ...
    def withdraw_batch(
        ...
        *,
        ...
        try:
            ...
            batch = self._bank_batch_service.withdraw_batch(
                batch_id,
                actor=actor,
                expected_version=expected_version,
                reason=reason,
            ...
            if not already_withdrawn:
                self._cancel_relation_for_batch(
                    ...
                    history_operation_type=(
                        ...
                        if relation_mode == BANK_FLOW_RULE_BATCH_RELATION_MODE
                        else "no_oa_bank_batch_withdraw"
                    ),
        ...
        try:
            for candidate in candidates:
                ...
                if not already_withdrawn:
                    ...
                    except BankBatchRelationMutationError as exc:
    ...
    def _positive_int(value: object, field: str, *, maximum: int | None = None) -> int:
        try:
            ...
            raise BankBatchRelationMutationError("invalid_paging", f"{field} must be a positive integer.") from exc
        if number < 1:
            raise BankBatchRelationMutationError("invalid_paging", f"{field} must be a positive integer.")
        if maximum is not None and number > maximum:
            raise BankBatchRelationMutationError("invalid_paging", f"{field} must be <= {maximum}.")
        return number
    ...
    def refresh_batches_from_prepared_rows(
        ...
        *,
        ...
        migration_result = self._bank_batch_service.last_legacy_migration_result()
    ...
    def persist_mutation(self, *, changed_case_ids: list[str], changed_scope_keys: list[str]) -> None:
        ...
        try:
            ...
            save_mutation(
                ...
                no_oa_bank_batch_snapshot=self._bank_batch_public_snapshot(),
    ...
    def _bank_batch_public_snapshot(self) -> dict[str, object]:
        public_snapshot = getattr(self._bank_batch_service, "public_snapshot", None)
    ...
    def _validate_internal_transfer_selection(
        ...
        *,
        ...
        refreshed = self._bank_batch_service.build_batches(
            bank_rows,
            categories_by_transaction_id,
            self._workbench_relation_active_relations_for_bank_rows(bank_rows),
    ...
    def _serialize_value(value: object) -> object:
        if is_dataclass(value):
            return {key: BankBatchApplicationService._serialize_value(val) for key, val in asdict(value).items()}
        if isinstance(value, dict):
            return {str(key): BankBatchApplicationService._serialize_value(val) for key, val in value.items()}
        if isinstance(value, list):
            return [BankBatchApplicationService._serialize_value(item) for item in value]
