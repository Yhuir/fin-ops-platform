...
def validate_manual_items(value: Any, options: dict[str, Any], previous: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    projects = {p["name"]: p for p in options["projects"]}
    tags = {t["code"]: t for t in options["tags"]}
    old = {item["unit_id"]: item for item in previous}
    seen, result = set(), []
    for item in value:
        if not isinstance(item, dict) or set(item) != {"unit_id", "project_name", "expense_content", "cost_tag_code"}:
        ...
        if project is None and not (prior and prior["project_name"] == item["project_name"]):
        ...
        result.append({**item, "expense_content": content,
                       "project_id": project["id"] if project else prior["project_id"],
