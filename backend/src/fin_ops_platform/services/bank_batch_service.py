...
BANK_FLOW_RULE_BATCH_SCHEMA_VERSION = "2026-07-bank-flow-rule-batch-v2"
...
class BankBatchRelationRepairReadPort:
...
class BankBatchService:
    def __init__(
        ...
        *,
        ...
        self._relation_read_port = relation_read_port or BankBatchRelationRepairReadPort(
    ...
    def _build_batches_for_month_scope(
        ...
        *,
        ...
        scoped_service = BankBatchService(
    ...
    def _normalize_batch(batch: dict[str, Any]) -> dict[str, Any]:
        ...
        normalized["relation_mode"] = BankBatchService._batch_relation_mode(normalized)
        ...
        normalized["total_amount"] = BankBatchService._format_amount(
            BankBatchService._decimal(normalized.get("total_amount")) or ZERO
        )
        ...
        normalized["row_tag_snapshot"] = BankBatchService._normalize_row_tag_snapshot(
        ...
        normalized["created_at"] = str(normalized.get("created_at") or BankBatchService._timestamp())
        ...
        normalized["status_bucket"] = BankBatchService._status_bucket(normalized["status"])
        normalized["can_submit"] = normalized["status"] == "draft"
        normalized["can_withdraw"] = False
        normalized["blocked_reason"] = BankBatchService._blocked_reason(normalized)
        return normalized
    ...
    def _normalize_audit_entry(entry: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "created_at": str(entry.get("created_at") or BankBatchService._timestamp()),
        }

    @staticmethod
    def _category_code(row: dict[str, Any], categories: dict[str, dict[str, Any]]) -> str:
        row_id = BankBatchService._row_id(row)
        category = categories.get(row_id) if isinstance(categories, dict) else None
    ...
    def _account_key(row: dict[str, Any]) -> str:
        ...
        if explicit:
            return explicit
        bank_name = BankBatchService._bank_name(row)
        last4 = BankBatchService._account_last4(row)
    ...
    def _account_payload(row: dict[str, Any]) -> dict[str, str]:
        return {
            "account_key": BankBatchService._account_key(row),
            "bank_name": BankBatchService._bank_name(row),
            "account_last4": BankBatchService._account_last4(row),
        }

    @staticmethod
    def _direction(row: dict[str, Any]) -> str:
        debit = BankBatchService._decimal(row.get("debit_amount"))
        credit = BankBatchService._decimal(row.get("credit_amount"))
        ...
        if value in {"outflow", "expense", "支", "出"}:
            return "outflow"
        signed = BankBatchService._decimal(row.get("signed_amount"))
    ...
    def _amount(row: dict[str, Any]) -> Decimal | None:
        ...
        if credit is not None and credit > ZERO:
            return credit
        amount = BankBatchService._decimal(row.get("amount"))
