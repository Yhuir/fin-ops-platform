-- Purpose units are a read projection. Original bank financial facts remain unchanged.
create view app.bank_transaction_units as
select
...
comment on view app.bank_transaction_units is
    'Canonical purpose units: unsplit bank row or persisted split children. Never use for bank balances or import identity.';
do $$
begin
    if exists (select 1 from pg_roles where rolname = 'fin_ops_api') then
        grant select on app.bank_transaction_units to fin_ops_api;
    end if;
    if exists (select 1 from pg_roles where rolname = 'fin_ops_app_runtime') then
        grant select on app.bank_transaction_units to fin_ops_app_runtime;
    end if;
    if exists (select 1 from pg_roles where rolname = 'fin_ops_worker') then
        grant select on app.bank_transaction_units to fin_ops_worker;
    end if;
    if exists (select 1 from pg_roles where rolname = 'fin_ops_readonly') then
        grant select on app.bank_transaction_units to fin_ops_readonly;
    end if;
end $$;
