-- Retry only the failed 0146 convergence job after exact bank-month scope recovery.
-- Fresh installations leave 0145 queued while migrations run with workers stopped,
-- so neither 0146 nor this migration creates a redundant replacement job.
with current_rules as (
    ...
    select
        'bank-relation-requirements-rollout-v3-' || current_rules.rule_version::text as job_id,
        'bank-relation-requirements-rollout-v2-' || current_rules.rule_version::text as supersedes_job_id,
        current_rules.rule_version,
        current_rules.tag_codes
    from current_rules
    join job.background_jobs old_job
      on old_job.job_id = 'bank-relation-requirements-rollout-v2-' || current_rules.rule_version::text
    ...
    select
        ...
        jsonb_build_object(
            'normalized_payload', jsonb_build_object(
                ...
                'owner_user_id', 'system:migration:0147',
                'visibility', 'system',
                'status', 'queued',
                'phase', 'queued',
                'current', 0,
