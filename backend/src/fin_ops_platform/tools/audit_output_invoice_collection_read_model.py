...
OUTPUT_INVOICE_PREDICATE = """
    i.status <> 'deleted'
    and (
        i.invoice_type = 'output'
        or i.invoice_type = 'output_invoice'
        or i.invoice_type like '销项%%'
    )
"""

ACTIVE_OUTPUT_INVOICES_CTE = f"""
active_output_invoices as (
    select
        coalesce(
    ...
    where {OUTPUT_INVOICE_PREDICATE}
)
"""

OUTPUT_INVOICE_LOOKUP_CTE = f"""
{ACTIVE_OUTPUT_INVOICES_CTE},
output_invoice_relation_lookup as (
    select invoice_id, invoice_id as relation_row_id, scope_key
    from active_output_invoices
    union
...
read_model_invoice_members as (
    select distinct
        ...
        coalesce(nullif(member.value->>'relatedInvoiceId', ''), nullif(member.value->>'invoiceId', ''), row.invoice_id) as invoice_id,
        row.total_with_tax as row_total_with_tax,
        row.payload,
        row.generated_at::text as generated_at
    from read_model.output_invoice_collection_rows row
...
ACTIVE_RELATION_OUTPUT_MEMBERS_CTE = f"""
{OUTPUT_INVOICE_LOOKUP_CTE},
active_relation_output_members as (
    select distinct
        relation.case_id,
    ...
    join output_invoice_relation_lookup lookup
...
{READ_MODEL_MEMBERS_CTE}
select
    (select count(*)::integer from active_output_invoices) as active_output_invoice_count,
    (select coalesce(sum(total_with_tax), 0)::numeric from active_output_invoices) as active_output_invoice_total_with_tax,
    (select count(distinct invoice_id)::integer from read_model_invoice_members) as read_model_invoice_member_count,
    (select count(*)::integer from read_model.output_invoice_collection_rows where cache_status = 'fresh') as read_model_row_count,
    (select count(*)::integer from read_model.output_invoice_collection_scopes) as output_invoice_collection_scope_count,
    ...
    *,
    ...
    try:
        ...
        report = postgres_configuration_missing_report(
            tool="audit_output_invoice_collection_read_model",
            message=str(exc),
        )
        report["required_env"] = [
        ...
        return 2
    report = audit_output_invoice_collection_read_model(
        active_connection,
...
def audit_output_invoice_collection_read_model(
    ...
    *,
    ...
    checks = (
        _noncanonical_output_invoice_type_issues,
        _dirty_scope_issues,
        _missing_output_scope_issues,
        _output_scope_row_count_mismatch_issues,
        ...
        _candidate_relation_in_output_collection_issues,
...
def _fetch_summary(connection: Any, *, tenant_id: str) -> dict[str, Any]:
    ...
    return {
        "active_output_invoice_count": _int(row.get("active_output_invoice_count")),
        "active_output_invoice_total_with_tax": _text(row.get("active_output_invoice_total_with_tax")) or "0",
        ...
        "output_invoice_collection_scope_count": _int(row.get("output_invoice_collection_scope_count")),
...
def _noncanonical_output_invoice_type_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    _ = tenant_id
    rows = connection.fetch_all(
        f"""
        /* check: noncanonical_output_invoice_type */
        with {ACTIVE_OUTPUT_INVOICES_CTE}
    ...
    return [
        AuditIssue(
            severity="error",
            code="noncanonical_output_invoice_type",
            message="Output invoice facts use a noncanonical invoice_type that the page service does not read directly.",
...
def _dirty_scope_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    rows = connection.fetch_all(
        ...
        where tenant_id = %s
          and scope_type in ('output_invoice_collection', 'workbench_relation')
    ...
    return [
        AuditIssue(
            ...
            message="Output invoice collection cannot be guaranteed while a required read model scope is pending, processing, or failed.",
...
def _missing_output_scope_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    _ = tenant_id
    rows = connection.fetch_all(
        f"""
        /* check: missing_output_invoice_collection_scope */
        with {ACTIVE_OUTPUT_INVOICES_CTE}
        select invoices.scope_key, count(*)::integer as invoice_count
        from active_output_invoices invoices
        left join read_model.output_invoice_collection_scopes scope
    ...
    return [
        AuditIssue(
            severity="error",
            code="missing_output_invoice_collection_scope",
            message="A month with output invoice facts has no output_invoice_collection scope row.",
            scope_key=_text(row.get("scope_key")),
...
def _output_scope_row_count_mismatch_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    _ = tenant_id
    rows = connection.fetch_all(
        """
        /* check: output_scope_row_count_mismatch */
        select
            scope.scope_key,
            scope.row_count::integer as scope_row_count,
            count(row.row_id)::integer as actual_row_count
        from read_model.output_invoice_collection_scopes scope
        left join read_model.output_invoice_collection_rows row
    ...
    return [
        AuditIssue(
            severity="error",
            code="output_scope_row_count_mismatch",
            message="output_invoice_collection scope row_count does not match the stored row count.",
            scope_key=_text(row.get("scope_key")),
...
def _missing_workbench_relation_scope_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    ...
    return [
        AuditIssue(
            ...
            message="A month with output invoice facts has no workbench_relation scope proof.",
            scope_key=_text(row.get("scope_key")),
            details=_details(row, "invoice_count"),
        )
        for row in rows
...
def _source_version_mismatch_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    rows = connection.fetch_all(
        ...
        select
            output_scope.scope_key,
            output_scope.source_versions->'workbench_relation_source_versions' as embedded_relation_versions,
            relation_scope.source_versions as current_relation_versions
        from read_model.output_invoice_collection_scopes output_scope
        join read_model.workbench_relation_scopes relation_scope
          on relation_scope.tenant_id = %s
         and relation_scope.scope_key = output_scope.scope_key
        where output_scope.scope_key <> 'all'
          and exists (
              select 1
              from read_model.output_invoice_collection_rows row
              where row.scope_key = output_scope.scope_key
          )
          and coalesce(output_scope.source_versions->'workbench_relation_source_versions', '{}'::jsonb)
              <> coalesce(relation_scope.source_versions, '{}'::jsonb)
        order by output_scope.scope_key
        limit %s
        """,
        (tenant_id, limit),
    )
    return [
        AuditIssue(
            severity="error",
            code="output_collection_relation_source_versions_mismatch",
            message="output_invoice_collection scope was built with stale workbench_relation source_versions.",
            scope_key=_text(row.get("scope_key")),
...
def _missing_read_model_member_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    ...
    return [
        AuditIssue(
            severity="error",
            code="missing_output_invoice_collection_member",
            message="An active output invoice is missing from read_model.output_invoice_collection_rows.",
            subject_id=_text(row.get("invoice_id")),
            scope_key=_text(row.get("scope_key")),
...
def _orphan_read_model_member_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    ...
    rows = connection.fetch_all(
        ...
        left join active_output_invoices invoices
    ...
    return [
        AuditIssue(
            severity="error",
            code="orphan_output_invoice_collection_member",
            message="A read model row references an invoice that is not an active output invoice fact.",
            subject_id=_text(row.get("invoice_id")),
            scope_key=_text(row.get("scope_key")),
...
def _duplicate_invoice_member_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    ...
    return [
        AuditIssue(
            severity="error",
            code="duplicate_output_invoice_collection_member",
            message="The same output invoice appears in multiple read model rows within one scope.",
            subject_id=_text(row.get("invoice_id")),
            scope_key=_text(row.get("scope_key")),
...
def _amount_mismatch_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    ...
    rows = connection.fetch_all(
        ...
        row_amounts as (
            ...
            from read_model_invoice_members member
            join active_output_invoices invoices
              on invoices.invoice_id = member.invoice_id
    ...
    return [
        AuditIssue(
            severity="error",
            code="output_invoice_collection_amount_mismatch",
...
def _active_relation_missing_workbench_row_issues(
    ...
    *,
    ...
    rows = connection.fetch_all(
        ...
        with {ACTIVE_RELATION_OUTPUT_MEMBERS_CTE}
        select
            member.case_id,
            member.invoice_id,
        ...
        from active_relation_output_members member
    ...
    return [
        AuditIssue(
            ...
            message="An active Workbench relation output invoice member is not projected as a linked workbench_relation row.",
    ...
    *,
    ...
    rows = connection.fetch_all(
        ...
        left join read_model.workbench_relation_groups relation_group
         ...
         and (
                member.relation_row_id = any(relation_group.output_invoice_ids)
             or member.invoice_id = any(relation_group.output_invoice_ids)
         )
    ...
    return [
        AuditIssue(
            ...
            message="An active Workbench relation output invoice member is not projected in a linked relation group.",
            subject_id=_text(row.get("case_id")),
            scope_key=_text(row.get("scope_key")),
...
def _cross_scope_relation_distribution_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    rows = connection.fetch_all(
        ...
        with
        {ACTIVE_RELATION_OUTPUT_MEMBERS_CTE},
        relation_scopes as (
            select distinct case_id, invoice_scope_key as scope_key
            from active_relation_output_members
...
def _relation_member_split_row_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    ...
    rows = connection.fetch_all(
        ...
        from active_relation_output_members relation_member
    ...
    return [
        AuditIssue(
            severity="error",
            code="active_relation_members_split_across_output_collection_rows",
            message="Output invoices from the same active relation are split across multiple page rows in one scope.",
            subject_id=_text(row.get("case_id")),
            scope_key=_text(row.get("scope_key")),
...
def _candidate_relation_in_output_collection_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    _ = tenant_id
    ...
    rows = connection.fetch_all(
        f"""
        /* check: candidate_relation_in_output_collection */
    ...
    return [
        AuditIssue(
            severity="error",
            code="candidate_relation_projected_into_output_collection",
            message="A candidate relation appears inside output invoice collection relation summaries.",
            subject_id=_text(row.get("row_id")),
            scope_key=_text(row.get("scope_key")),
...
def _candidate_workbench_relation_issues(connection: Any, *, tenant_id: str, limit: int) -> list[AuditIssue]:
    rows = connection.fetch_all(
        ...
        with {OUTPUT_INVOICE_LOOKUP_CTE}
        select
    ...
    issues = [
        AuditIssue(
            severity="error",
            code="candidate_workbench_relation_for_output_invoice",
            message="A candidate workbench_relation row exists for an output invoice relation lookup id.",
            subject_id=_text(row.get("row_id")),
            scope_key=_text(row.get("scope_key")),
    ...
    group_rows = connection.fetch_all(
        ...
        select scope_key, group_id, output_invoice_ids
        from read_model.workbench_relation_groups
        where tenant_id = %s
          and relation_status = 'candidate'
          and coalesce(array_length(output_invoice_ids, 1), 0) > 0
    ...
    issues.extend(
        AuditIssue(
            severity="error",
            code="candidate_workbench_relation_group_for_output_invoice",
            message="A candidate workbench_relation group contains output invoice members.",
            subject_id=_text(row.get("group_id")),
            scope_key=_text(row.get("scope_key")),
            details=_details(row, "output_invoice_ids"),
        )
