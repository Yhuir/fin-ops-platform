...
def build_etc_state_rows(record: StagingRecord, domain: str, p: dict[str, Any]) -> list[TargetRow]:
    ...
    return child_rows(record, domain, "app", "etc_business_batches", p, "business_batches", lambda c, legacy: {"legacy_mongo_id": legacy, "business_batch_id": one(c, legacy, "business_batch_id", "batch_id", "id"), "task_id": text(first(c, "task_id")), "status": text(first(c, "status", default="unknown")), "scope_month": month_start(first(c, "scope_month", "month")), "invoice_count": integer(first(c, "invoice_count", default=0)), "total_amount": decimal_value(first(c, "total_amount", default=0)), "import_attempts": JsonValue(first(c, "import_attempts", default=[])), "audit_events": JsonValue(first(c, "audit_events", default=[])), "version": integer(first(c, "version", default=1))})
