...
`.planning/` 保存 GSD 执行状态、阶段计划、debug 记录、page baseline、重构规划和历史工作流产物。它是协作和恢复上下文的工作区，不作为当前需求、架构、API 或验收事实源，也不是当前产品、read model、worker 或部署事实源。
...
- `.planning/refactors/` 中的记录只用于跨模块重构规划、状态跟踪、风险登记和 prompt 模板。进入实现前，必须重新读取当前代码、目标模块文档和测试。
