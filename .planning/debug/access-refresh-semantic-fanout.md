...
## 2026-07-24 production generation-set race and staged-access tail

- evidence: release `main-c4edf63a-20260724201915` confirm/withdraw returned in `347.125ms` / `233.145ms` and emitted zero forbidden write fan-out. Cost bank-tag reached fresh in `1.148s` / `2.284s`, but Cost project/all took `12.645s` / `11.229s`, Turnover took `3.281s` / `3.335s`, and Workbench took `4.393s` / `5.957s`. All business assertions passed; confirm checkpoint System Audit failed, recovery Audit passed, queue drained, and the test-owned fixture ended inactive.
- measured_root: Cost handlers were only about `78–328ms`; Workbench sibling handlers ran concurrently at about `3.25–3.33s`. The repository used per-month advisory locks even though each month transaction activated its own generation and republished all-scope generation-set stats. Concurrent transactions could each publish stats for an intermediate active set, leaving the final set digest without matching stats and creating save/statistics contention. Cost query meanwhile waited for Workbench before registering its own exact scope, serializing Workbench→Cost child→parent behind page polling.
- superseded_fix: this candidate preserved parallel payload calculation and serialized only the short Workbench activation/all-stats transaction with one `workbench_generation_set` advisory lock. It staged only the requested Cost parent and enforced mutation-response-to-visible; round 9 below proved both choices retained avoidable measurement/staging delay.
- verification: focused repository/service/runner tests pass. Candidate commit, deploy, strict `<3s` fixture, both checkpoint System Audits, queue/worker drain and final release consistency remain open.

## 2026-07-24 production round 9 measurement and parent staging

- evidence: release `main-8ff87f7c-20260724205906` preserved zero forbidden write fan-out and safely recovered the test-owned fixture. Confirm/recovery Cost bank-tag reached visible in `1.696s` / `1.346s`; Cost project/all took `15.675s` / `11.401s`; Turnover took `5.904s` / `2.543s`; Workbench took `6.153s` / `5.180s`. Queue and workers drained after recovery.
- measurement_root: the runner executed the write-time zero-fan-out audit before the first consumer GET, then enforced mutation-response-to-visible as the page SLO. That counted the runner's own pre-access work against an architecture whose refresh trigger is page access. `operation_commit_to_visible_ms` remains useful observation but is not the access SLO.
