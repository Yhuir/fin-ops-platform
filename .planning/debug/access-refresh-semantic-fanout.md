...
- fix: the single shared bank-flow SQL now maps canonical `direction` to the Cost `支出|收入` contract; the Cost page Audit uses the identical mapping. No cache, queue, worker, endpoint or fallback was added.
- verification: focused Cost SQL/Audit/API/App Health tests pass; a disposable PostgreSQL using realistic `支|收` rows proves expense/income summaries and parent statistics. Production deployment, `<3s` fixture, System Audit and queue-drain proof remain open.
