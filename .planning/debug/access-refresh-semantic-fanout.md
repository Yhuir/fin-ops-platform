...
- fix: preserve parallel payload calculation but serialize the short Workbench activation/all-stats transaction with one `workbench_generation_set` advisory lock. On a Workbench-stale Cost access, ensure exact Workbench first and stage only the requested Cost scope; the existing manifest dependency gate defers it until exact dependencies are fresh. The production runner now fails when mutation-response-to-first-visible exceeds consumer `target_ms`, even if the final GET itself is fast.
