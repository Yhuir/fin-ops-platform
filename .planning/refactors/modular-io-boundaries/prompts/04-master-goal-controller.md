...
Start with `server-py:etc-reconciliation-import-cleanup-service-extraction`.
...
- Row313 is the next local implementation boundary: extract the shared ETC reconciliation import/submission/business-batch cleanup cluster into an explicit service while keeping HTTP response/error mapping stable.
...
- Read `analysis/server-py-etc-reconciliation-task-delete-side-effect-service-audit-2026-06-25.md`, `STATE.md`, `MODULE-QUEUE.md`, `JOURNAL.md`, `NEXT-PROMPT.md`, and `12-PARALLEL-ORCHESTRATION.md`.
...
- Implement only the Row313 cleanup service extraction slice unless the audit proves a narrower safe implementation is required first.
