...
Start with `server-py:etc-legacy-batch-route-owner-audit`.
...
- Row316 is the next local boundary: audit legacy `/api/etc/batches*` compatibility route ownership and decide whether it should move to a compat-only route owner, be partially delegated to existing `EtcBusinessBatchApiRoutes`, or require another side-effect service boundary first.
...
- Read `analysis/server-py-etc-import-route-owner-audit-2026-06-25.md`, `STATE.md`, `MODULE-QUEUE.md`, `JOURNAL.md`, `NEXT-PROMPT.md`, and `12-PARALLEL-ORCHESTRATION.md`.
...
- Implement only the Row316 audit/selection slice unless the audit proves a narrower safe implementation can be completed and verified immediately.
- Keep business-batch v2 route behavior out of Row316.
