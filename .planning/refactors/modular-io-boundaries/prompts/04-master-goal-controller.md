...
- `production:read-model-controlled-production-api-browser-runbook` is complete as `production-evidence-deferred` in `analysis/production-read-model-controlled-production-api-browser-runbook-2026-06-25.md`: target OA applicant credentials provided a real full-access user session without printing tokens, 30/37 initial user-scope API probes passed, focused retry left pending invoice rows/filter-options and no-OA bank batches non-fresh/stale, GET fresh gates triggered bounded read-model refresh enqueues that converged to done/fresh, and final health/dirty/readiness/outbox/dead-letter postchecks were clean. Browser/admin/write closure remains open.
- `production:pending-invoice-no-oa-api-freshness-mismatch-read-only-diagnosis` is complete as `production-diagnosis-closed` in `analysis/production-pending-invoice-no-oa-api-freshness-mismatch-read-only-diagnosis-2026-06-25.md`: direct deployed PostgreSQL read-only metadata proved pending invoice `expense:all` repository payload is fresh/populated but API expected-source gate reports source-version mismatches, explaining rows HTTP 200/refreshing and filter-options HTTP 202/refreshing; no-OA dirty/outbox/readiness are clean, but API stale is explained by application-level row source-version comparison being stricter than App Status aggregate readiness. No production API endpoint call or mutation occurred.
- The next pending boundary is `production:pending-invoice-no-oa-source-version-contract-deep-diagnosis`.
