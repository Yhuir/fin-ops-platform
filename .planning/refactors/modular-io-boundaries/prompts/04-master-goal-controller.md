...
Copy the full prompt below into Codex to start or resume the autonomous run. Current local-first boundary state is `server-py:turnover-ledger-bank-row-tags-route-callback-collapse-selected` after turnover ledger tag-selection write route callback collapse.
