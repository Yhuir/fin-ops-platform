...
- The next pending boundary is `production:postgres-shared-memory-read-only-diagnosis`.
...
For `production:postgres-shared-memory-read-only-diagnosis`:
- Read `analysis/production-readiness-worker-status-controlled-read-only-2026-06-25.md`.
- Write a controlled production runbook/evidence file under `analysis/` before any SSH command.
- Use only non-secret read-only root SSH checks for PostgreSQL service status, process/resource state, filesystem/memory/shared-memory status and sanitized logs.
- Do not deploy, restart PostgreSQL or app services, requeue, mutate readiness, consume queues, write DB, source env files or print secrets.
