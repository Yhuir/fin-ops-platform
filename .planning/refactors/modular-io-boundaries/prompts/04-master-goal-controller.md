...
Copy the full prompt below into Codex to start or resume the autonomous run. Current local-first boundary state is `server-py:no-oa-bank-batch-post-refresh-producer-local-closure-audit-selected` after no-OA refresh producer extraction moved scope normalization and durable enqueue out of `Application`.
...
- Branch: dev.
- Last completed controller step: controlled write-flow evidence scenario selection.
...
- `production:read-model-full-user-scope-api-metadata-smoke-after-turnover-fixes` is complete as `production-controlled` in `analysis/production-read-model-full-user-scope-api-metadata-smoke-after-turnover-fixes-2026-06-25.md`: all 37 non-admin user-scope default API probes passed through target OA applicant credentials with 0 failed probes, 0 non-fresh probes, 0 refresh-enqueued probes and unchanged aggregate dirty/readiness/outbox/dead-letter counts.
- `planning:post-full-user-api-smoke-browser-admin-write-evidence-selection` is complete as `planning-closed` in `analysis/planning-post-full-user-api-smoke-browser-admin-write-evidence-selection-2026-06-25.md`: selected a read-only authenticated production browser page smoke runbook as the next lowest-risk evidence boundary; admin evidence remains deferred without a proven non-secret admin seam and write-flow evidence remains deferred until a separate operation-specific rollback/idempotency/audit/convergence runbook exists.
...
- `planning:controlled-write-flow-evidence-scenario-selection` is complete as `planning-closed` in `analysis/planning-controlled-write-flow-evidence-scenario-selection-2026-06-25.md`: selected a read-only sanitized write-flow scenario discovery runbook; controlled write apply remains forbidden without explicit approval, a reviewed reversible business object, rollback/idempotency/audit acceptance and suitable auth.
- `production:write-flow-scenario-discovery-read-only-runbook` is complete as `production-controlled` in `analysis/production-write-flow-scenario-discovery-read-only-runbook-2026-06-25.md`: committed/pushed runbook before execution, then ran read-only sanitized production discovery. Candidate counts were `turnover_manual_closure_or_withdraw=6`, `workbench_pair_withdraw_context=10`, `no_oa_bank_batch_withdraw_context=10`, with `scenario_count=26`; no identifiers, scenario file, HTTP write, `--apply`, browser/admin probe, secret output or mutation occurred, and pre/post production aggregates stayed unchanged.
- `planning:post-write-flow-discovery-closure-selection` is complete as `planning-closed` in `analysis/planning-post-write-flow-discovery-closure-selection-2026-06-25.md`: reconciled remaining browser/admin/write apply gates, rejected final/global closure, write apply and admin probe, and selected dedicated production browser smoke ops runner design as the next safe owned boundary.
- `deployment:production-browser-smoke-ops-runner-design` is complete as `analysis-closed` in `analysis/deployment-production-browser-smoke-ops-runner-design-2026-06-25.md`: designed a dedicated controlled browser runner outside the normal app release and production app host; the design keeps Playwright/browser runtime out of app releases, requires in-memory target OA token handoff, emits sanitized metadata only and found one pre-run contract gap in `production-route-shell.spec.ts` body text samples.
- `frontend:production-route-shell-sanitized-output-contract` is complete as `implementation-closed` in `analysis/frontend-production-route-shell-sanitized-output-contract-2026-06-25.md`: removed production route-shell page body samples from failure output and added static guard coverage; targeted verification passed with 8/8 tests.
- `deployment:production-browser-smoke-runner-bundle-contract` is complete as `analysis-closed` in `analysis/deployment-production-browser-smoke-runner-bundle-contract-2026-06-25.md`: defined separate runner bundle file/manifest/runtime/artifact contract, but deferred bundle implementation because `production-route-shell.spec.ts` still imports `strictTest.ts` raw console/pageerror/requestfailed diagnostics.
- `frontend:production-strict-diagnostics-sanitized-output-contract` is complete as `implementation-closed` in `analysis/frontend-production-strict-diagnostics-sanitized-output-contract-2026-06-25.md`: added production-only strict diagnostics redaction while preserving local raw diagnostics; targeted guard passed 9/9 and frontend build passed.
...
- `server-py:etc-reconciliation-task-route-owner-facade-extraction` is complete as `local-implementation-closed` in `analysis/server-py-etc-reconciliation-task-route-owner-facade-extraction-2026-06-25.md`; it added `EtcReconciliationTaskApiRoutes` and left task delete/imported-invoice delete side effects as the next audit target.
- `server-py:etc-reconciliation-task-delete-side-effect-service-audit` is complete as `analysis-closed` in `analysis/server-py-etc-reconciliation-task-delete-side-effect-service-audit-2026-06-25.md`; it selected `server-py:etc-reconciliation-import-cleanup-service-extraction`.
- `server-py:etc-reconciliation-import-cleanup-service-extraction` is complete as `local-implementation-closed` in `analysis/server-py-etc-reconciliation-import-cleanup-service-extraction-2026-06-25.md`; cleanup behavior is service-owned while HTTP callbacks remain in `Application`.
- `server-py:etc-reconciliation-delete-route-callback-audit` is complete as `local-implementation-closed` in `analysis/server-py-etc-reconciliation-delete-route-callback-audit-2026-06-25.md`; task delete and imported-invoice delete HTTP ownership moved into `EtcReconciliationTaskApiRoutes`.
...
- `server-py:etc-legacy-batch-read-payload-facade-audit` is complete as `analysis-closed` in `analysis/server-py-etc-legacy-batch-read-payload-facade-audit-2026-06-25.md`; it selected `server-py:etc-legacy-batch-read-facade-extraction`.
- `server-py:etc-legacy-batch-read-facade-extraction` is complete as `local-implementation-closed` in `analysis/server-py-etc-legacy-batch-read-facade-extraction-2026-06-25.md`; legacy list/detail/count/filter payload ownership moved into `EtcLegacyBatchReadFacade`.
- `server-py:etc-legacy-batch-route-callback-collapse-audit` is complete as `local-implementation-closed` in `analysis/server-py-etc-legacy-batch-route-callback-collapse-audit-2026-06-25.md`; remaining legacy batch list/detail/delete/draft/confirm/reopen HTTP callbacks moved into `EtcLegacyBatchApiRoutes`.
- `server-py:etc-invoice-route-owner-audit` is complete as `local-implementation-closed` in `analysis/server-py-etc-invoice-route-owner-audit-2026-06-25.md`; ETC invoice list/revoke-submitted HTTP mapping moved into `EtcInvoiceApiRoutes`.
...
- `server-py:etc-reconciliation-source-upload-parser-boundary-audit` is complete as `analysis-closed` in `analysis/server-py-etc-reconciliation-source-upload-parser-boundary-audit-2026-06-25.md`; selected source upload service extraction.
- `server-py:etc-reconciliation-source-upload-service-extraction` is complete as `local-implementation-closed` in `analysis/server-py-etc-reconciliation-source-upload-service-extraction-2026-06-25.md`; selected ticket-root text callback audit.
...
- `server-py:etc-business-route-owner-local-closure-audit` is complete as `analysis-closed` in `analysis/server-py-etc-business-route-owner-local-closure-audit-2026-06-25.md`; ETC business-batch route-owner local support is accounted for, without claiming whole ETC/global closure.
- `server-py:input-invoice-usage-oa-reverse-route-owner-audit` is complete as `analysis-closed` in `analysis/server-py-input-invoice-usage-oa-reverse-route-owner-audit-2026-06-25.md`; selected OA reverse route-owner facade extraction for lightweight preview/history/staged/batch create/get paths.
- `server-py:input-invoice-usage-oa-reverse-route-owner-facade-extraction` is complete as `local-implementation-closed` in `analysis/server-py-input-invoice-usage-oa-reverse-route-owner-facade-extraction-2026-06-25.md`; lightweight OA reverse preview/history/staged/batch create/get HTTP mapping now lives in `InputInvoiceUsageOaReverseApiRoutes`, and remaining draft mutation callbacks are selected for audit.
- `server-py:input-invoice-usage-oa-reverse-draft-mutation-callback-audit` is complete as `analysis-closed` in `analysis/server-py-input-invoice-usage-oa-reverse-draft-mutation-callback-audit-2026-06-25.md`; selected route-owner callback collapse for one-step draft, batch draft, revoke, status refresh and manual status with explicit OA provider/client and parse ports.
- `server-py:input-invoice-usage-oa-reverse-draft-mutation-route-callback-collapse` is complete as `local-implementation-closed` in `analysis/server-py-input-invoice-usage-oa-reverse-draft-mutation-route-callback-collapse-2026-06-25.md`; all `/api/input-invoice-usage/oa-reverse*` HTTP mapping now lives in `InputInvoiceUsageOaReverseApiRoutes`.
- `server-py:input-invoice-usage-oa-reverse-route-owner-local-closure-audit` is complete as `analysis-closed` in `analysis/server-py-input-invoice-usage-oa-reverse-route-owner-local-closure-audit-2026-06-25.md`; OA reverse route-owner local support is accounted for without claiming whole input-invoice-usage/global closure.
- `server-py:input-invoice-usage-core-route-owner-audit` is complete as `analysis-closed` in `analysis/server-py-input-invoice-usage-core-route-owner-audit-2026-06-25.md`; selected read route-owner facade extraction for rows/filter/detail/relation-detail/payment GET while deferring export and payment PUT.
- `server-py:input-invoice-usage-read-route-owner-facade-extraction` is complete as `local-implementation-closed` in `analysis/server-py-input-invoice-usage-read-route-owner-facade-extraction-2026-06-25.md`; read HTTP mapping now lives in `InputInvoiceUsageApiRoutes`, while export and payment PUT remain in `Application`.
- `server-py:input-invoice-usage-export-route-owner-audit` is complete as `analysis-closed` in `analysis/server-py-input-invoice-usage-export-route-owner-audit-2026-06-25.md`; selected export route callback collapse with explicit export/auth/audit/XLSX response ports.
...
- `server-py:etc-reconciliation-task-route-owner-facade-extraction` is complete in `analysis/server-py-etc-reconciliation-task-route-owner-facade-extraction-2026-06-25.md`.
- `server-py:etc-reconciliation-task-delete-side-effect-service-audit` is complete in `analysis/server-py-etc-reconciliation-task-delete-side-effect-service-audit-2026-06-25.md`.
- `server-py:etc-reconciliation-import-cleanup-service-extraction` is complete in `analysis/server-py-etc-reconciliation-import-cleanup-service-extraction-2026-06-25.md`.
- `server-py:etc-reconciliation-delete-route-callback-audit` is complete in `analysis/server-py-etc-reconciliation-delete-route-callback-audit-2026-06-25.md`.
...
- `server-py:etc-legacy-batch-read-payload-facade-audit` is complete in `analysis/server-py-etc-legacy-batch-read-payload-facade-audit-2026-06-25.md`.
- `server-py:etc-legacy-batch-read-facade-extraction` is complete in `analysis/server-py-etc-legacy-batch-read-facade-extraction-2026-06-25.md`.
- `server-py:etc-legacy-batch-route-callback-collapse-audit` is complete in `analysis/server-py-etc-legacy-batch-route-callback-collapse-audit-2026-06-25.md`.
- `server-py:etc-invoice-route-owner-audit` is complete in `analysis/server-py-etc-invoice-route-owner-audit-2026-06-25.md`.
...
- `server-py:etc-reconciliation-source-upload-parser-boundary-audit` is complete in `analysis/server-py-etc-reconciliation-source-upload-parser-boundary-audit-2026-06-25.md`.
- `server-py:etc-reconciliation-source-upload-service-extraction` is complete in `analysis/server-py-etc-reconciliation-source-upload-service-extraction-2026-06-25.md`.
...
- `server-py:etc-business-route-owner-local-closure-audit` is complete in `analysis/server-py-etc-business-route-owner-local-closure-audit-2026-06-25.md`.
- `server-py:input-invoice-usage-oa-reverse-route-owner-audit` is complete in `analysis/server-py-input-invoice-usage-oa-reverse-route-owner-audit-2026-06-25.md`.
- `server-py:input-invoice-usage-oa-reverse-route-owner-facade-extraction` is complete in `analysis/server-py-input-invoice-usage-oa-reverse-route-owner-facade-extraction-2026-06-25.md`.
- `server-py:input-invoice-usage-oa-reverse-draft-mutation-callback-audit` is complete in `analysis/server-py-input-invoice-usage-oa-reverse-draft-mutation-callback-audit-2026-06-25.md`.
- `server-py:input-invoice-usage-oa-reverse-draft-mutation-route-callback-collapse` is complete in `analysis/server-py-input-invoice-usage-oa-reverse-draft-mutation-route-callback-collapse-2026-06-25.md`.
- `server-py:input-invoice-usage-oa-reverse-route-owner-local-closure-audit` is complete in `analysis/server-py-input-invoice-usage-oa-reverse-route-owner-local-closure-audit-2026-06-25.md`.
- `server-py:input-invoice-usage-core-route-owner-audit` is complete in `analysis/server-py-input-invoice-usage-core-route-owner-audit-2026-06-25.md`.
- `server-py:input-invoice-usage-read-route-owner-facade-extraction` is complete in `analysis/server-py-input-invoice-usage-read-route-owner-facade-extraction-2026-06-25.md`.
- `server-py:input-invoice-usage-export-route-owner-audit` is complete in `analysis/server-py-input-invoice-usage-export-route-owner-audit-2026-06-25.md`.
...
- Read `analysis/server-py-etc-reconciliation-route-owner-local-closure-audit-2026-06-25.md`, `STATE.md`, `MODULE-QUEUE.md`, `JOURNAL.md`, `NEXT-PROMPT.md`, and `12-PARALLEL-ORCHESTRATION.md`.
...
- Execute only the Row333 ETC reconciliation task payload facade audit unless a narrower safe analysis step is required first.
