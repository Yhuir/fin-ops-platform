...
Start with `server-py:etc-reconciliation-task-mutation-callback-audit`.
...
- Row323 is the next local boundary: audit residual ETC reconciliation task mutation callbacks still injected from `server.py`.
...
- Read `analysis/server-py-etc-invoice-route-owner-audit-2026-06-25.md`, `STATE.md`, `MODULE-QUEUE.md`, `JOURNAL.md`, `NEXT-PROMPT.md`, and `12-PARALLEL-ORCHESTRATION.md`.
...
- Implement only the Row323 ETC reconciliation task mutation callback audit/selection slice unless a narrower safe implementation can be completed immediately.
