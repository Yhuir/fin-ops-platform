...
Copy the full prompt below into Codex to start or resume the autonomous run. Current local-first boundary state is `server-py:no-oa-bank-batch-route-callback-collapse-selected` after no-OA bank batch route-owner audit classified the remaining callbacks as thin HTTP wrappers.
