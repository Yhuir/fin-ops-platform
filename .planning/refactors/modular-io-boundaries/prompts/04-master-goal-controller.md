...
Start with `server-py:etc-invoice-route-owner-audit`.
...
- Row322 is the next local boundary: audit residual `/api/etc/invoices` and `/api/etc/invoices/revoke-submitted` ownership in `server.py`.
...
- Read `analysis/server-py-etc-legacy-batch-route-callback-collapse-audit-2026-06-25.md`, `STATE.md`, `MODULE-QUEUE.md`, `JOURNAL.md`, `NEXT-PROMPT.md`, and `12-PARALLEL-ORCHESTRATION.md`.
...
- Implement only the Row322 ETC invoice route-owner audit/selection slice unless a narrower safe implementation can be completed immediately.
