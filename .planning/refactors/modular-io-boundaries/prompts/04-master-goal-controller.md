...
Start with `server-py:input-invoice-usage-payment-rules-route-callback-collapse`.
...
- Row353 is the next local boundary: move payment-status-rules PUT HTTP mapping into `InputInvoiceUsageApiRoutes` with explicit settings/body/error/refresh ports.
