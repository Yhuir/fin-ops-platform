...
Continue the autonomous modular IO refactor after the `server-py:workbench-oa-bank-exception-route-owner-extraction` slice.
...
- `WorkbenchActionApiRoutes` now owns `/api/workbench/exception/preview`, `/api/workbench/exception/apply`, `/api/workbench/actions/confirm-link/preview`, `/api/workbench/actions/confirm-link`, `/api/workbench/actions/mark-exception`, `/api/workbench/actions/cancel-link`, `/api/workbench/actions/withdraw-link`, cash special action mapping/delegation boundaries, `/api/workbench/actions/update-bank-exception`, and `/api/workbench/actions/oa-bank-exception`.
- `/api/workbench/actions/confirm-personal-advance-repayment` and the other remaining `/api/workbench/actions/*` wrappers still live in `server.py` and delegate to `WorkbenchWriteFacade`.
...
- Preserve current behavior exactly:
  ...
  - Existing `WorkbenchWriteFacade.confirm_personal_advance_repayment(...)` remains the delegate through the route owner.
...
- Do not move cancel exception, ignore, unignore, exception preview/apply, confirm-link preview/submit, mark-exception, cancel-link, withdraw-link, cash special, update-bank-exception or OA-bank exception routes in this slice.
...
- `WorkbenchActionApiRoutes` method that owns personal advance repayment facade delegation while preserving app-level gates and request-id forwarding.
