...
Continue the autonomous modular IO refactor after the `server-py:workbench-confirm-link-submit-route-owner-extraction` slice.

## Current State

- Branch: `dev`
- Last completed boundary: `server-py:workbench-confirm-link-submit-route-owner-extraction`
...
- `WorkbenchActionApiRoutes` now owns `/api/workbench/exception/preview`, `/api/workbench/exception/apply`, `/api/workbench/actions/confirm-link/preview`, and `/api/workbench/actions/confirm-link` mapping/delegation boundaries.
- Modern `/api/workbench/actions/mark-exception` and the other remaining `/api/workbench/actions/*` wrappers still live in `server.py` and delegate to `WorkbenchWriteFacade`.
...
5. Read target evidence:
   - `.planning/refactors/modular-io-boundaries/analysis/server-py-workbench-confirm-link-submit-route-owner-extraction.md`
...
- Move `/api/workbench/actions/mark-exception` facade delegation behind `WorkbenchActionApiRoutes` without changing behavior.
- Preserve current behavior exactly:
  ...
  - `WorkbenchWriteFacade.mark_exception(...)` remains the delegate through the existing live helper semantics.
...
- Add or update a static guard proving mark-exception facade delegation is no longer app-owned once extracted.

Do not:

- Do not move cancel, withdraw, cash special, bank exception, OA-bank exception, personal advance repayment, cancel exception, ignore, unignore, exception preview/apply or confirm-link preview/submit routes in this slice.
...
- `WorkbenchActionApiRoutes` method that owns mark-exception facade delegation while preserving app-level gates.
