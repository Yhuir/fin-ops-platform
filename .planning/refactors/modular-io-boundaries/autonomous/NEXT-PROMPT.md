...
- Latest completed wave: `main-read-model-closure:wave-3-remaining-read-model-owner-split:search-workbench-relation`.
...
`main-read-model-closure:wave-3-remaining-read-model-owner-split:summary-read-models`

Recommended next family: summary and residual non-Workbench read models.
...
- Prefer coherent family-level owner extraction when safe; do not split every method one-by-one if a summary owner can be extracted with tests.
- Preserve the already-completed owner split:
...
- Workbench itself remains the active-generation exception and must not be mechanically converted.
...
- Update `analysis/read-model-main-wave-3-remaining-read-model-owner-split-2026-06-25.md` with summary read model PSCIP movement.
...
PYTHONPATH=backend/src python3 -m pytest tests/test_cost_statistics_read_model.py tests/test_tax_offset_read_model.py tests/test_no_oa_bank_batch_read_model.py tests/test_turnover_ledger_read_model.py -q
