...
Continue the autonomous modular IO refactor after the `server-py:workbench-update-bank-exception-route-owner-extraction` slice.
...
- Preserve current behavior exactly:
  ...
  - Existing `WorkbenchWriteFacade.oa_bank_exception(...)` remains the delegate through the route owner.
...
- Do not move personal advance repayment, cancel exception, ignore, unignore, exception preview/apply, confirm-link preview/submit, mark-exception, cancel-link, withdraw-link, cash special or update-bank-exception routes in this slice.
...
- `WorkbenchActionApiRoutes` method that owns OA-bank exception facade delegation while preserving app-level gates.
