...
| 18 | `read-models:pilot-gap-audit-and-contract-selection` | pending | implementation-pending | Choose and lock the first read model implementation pilot from `bank_detail`, `workbench_relation`, `pending_invoice`, or `oa_pending_payment`; produce gap audit, IO contract delta, tests plan and exact next implementation boundary. No Go work. |
| 19 | `read-models:repository-port-extraction-pilot` | pending | implementation-pending | Implement one selected read model repository port split or wrapper boundary with tests; do not split all `read_models.py` at once. |
| 20 | `read-models:refresh-freshness-operation-barrier-pilot` | pending | implementation-pending | Implement one selected write-to-read freshness/force-refresh/operation-barrier path with affected scopes and tests. |
| 21 | `read-models:legacy-contamination-removal-pilot` | pending | implementation-pending | Remove or quarantine one selected legacy read/refresh/write path and prove new paths cannot call it. |
| 22 | `read-models:pilot-verification-and-template-revision` | pending | implementation-pending | Verify the pilot module, update templates/runbook based on evidence, and decide the second batch only after the pilot is actually verified or production-evidence-deferred. |
