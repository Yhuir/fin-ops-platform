...
| 24 | `read-models:bank-detail-category-side-effect-port-extraction` | pending | implementation-pending | Extract or quarantine `Application._after_bank_category_confirmation_mutation(...)` and related suggestion/callback wiring behind an explicit side-effect port so category writes do not depend on unclassified Application callbacks. |
| 25 | `server-py:legacy-handler-extraction-implementation` | pending | implementation-pending | Continue route/service extraction only after pilot read model implementation evidence is available; keep `server.py` thin and test route ownership. |
| 26 | `batch-accounting:legacy-route-implementation` | pending | implementation-pending | Convert one batch-accounting route/legacy boundary from guard-only to actual service/repository/read-model contract implementation. |
| 27 | `go-hot-path:workbench-compute-admission` | blocked-by-prerequisite | go-admission-not-started | Admission review only for `workbench:matching-grouping-check`; blocked until Workbench/read model IO contracts, legacy isolation, freshness proof, tests and performance evidence are available. |
| 28 | `go-hot-path:workbench-read-model-builder-admission` | blocked-by-prerequisite | go-admission-not-started | Admission review only for active generation / scoped incremental Go Worker candidate after read model implementation contracts are stable. |
| 29 | `go-hot-path:import-parser-admission` | blocked-by-prerequisite | go-admission-not-started | Admission review only for parse/normalize/preview Go Processor candidate after import IO contract and performance evidence are available. |
| 30 | `go-hot-path:cost-summary-rollup-admission` | blocked-by-prerequisite | go-admission-not-started | Admission review only for summary/rollup candidate after cost read model implementation contract is stable. |
