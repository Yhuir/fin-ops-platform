...
| 23 | `read-models:bank-detail-server-helper-quarantine` | pending | implementation-pending | Classify remaining `server.py` bank detail helper/callback dependencies, remove or migrate the smallest safe unsafe helper, and register any retained helper as `compat-only` or `gateway-backed wrapper` with deletion conditions and contamination tests. |
| 24 | `server-py:legacy-handler-extraction-implementation` | pending | implementation-pending | Continue route/service extraction only after pilot read model implementation evidence is available; keep `server.py` thin and test route ownership. |
| 25 | `batch-accounting:legacy-route-implementation` | pending | implementation-pending | Convert one batch-accounting route/legacy boundary from guard-only to actual service/repository/read-model contract implementation. |
| 26 | `go-hot-path:workbench-compute-admission` | blocked-by-prerequisite | go-admission-not-started | Admission review only for `workbench:matching-grouping-check`; blocked until Workbench/read model IO contracts, legacy isolation, freshness proof, tests and performance evidence are available. |
| 27 | `go-hot-path:workbench-read-model-builder-admission` | blocked-by-prerequisite | go-admission-not-started | Admission review only for active generation / scoped incremental Go Worker candidate after read model implementation contracts are stable. |
| 28 | `go-hot-path:import-parser-admission` | blocked-by-prerequisite | go-admission-not-started | Admission review only for parse/normalize/preview Go Processor candidate after import IO contract and performance evidence are available. |
| 29 | `go-hot-path:cost-summary-rollup-admission` | blocked-by-prerequisite | go-admission-not-started | Admission review only for summary/rollup candidate after cost read model implementation contract is stable. |
