...
Current state: `autonomous-continue-after-read-models-pending-invoice-repository-port-extraction`
...
Completed `read-models:pending-invoice-repository-port-extraction`. `PendingInvoiceReadModelRepositoryPort` now narrows pending invoice rows, filter-options, source-version and projection save/mark repository access. `pending_invoice` remains implementation-gap-open; the next executable boundary audits freshness, force-refresh, special scope and operation barrier behavior.
