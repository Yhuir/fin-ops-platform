...
Current state: `autonomous-continue-after-read-models-output-invoice-collection-refresh-freshness-operation-barrier-audit`
...
Completed `read-models:output-invoice-collection-refresh-freshness-operation-barrier-audit`. Output collection mutation responses now expose `read_model_scope_keys` and `freshness_targets`, frontend lifecycle/receipt/red relation flows wait on concrete month operation barrier targets before refetching, and unused app-level output projection helpers were removed from `Application` with a regression guard. The next executable boundary is `read-models:output-invoice-collection-local-implementation-closure-audit`. Go hot-path admission remains blocked.
