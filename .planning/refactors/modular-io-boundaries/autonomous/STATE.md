...
Current state: `autonomous-continue-after-read-models-search-production-repository-unavailable-fail-closed`
...
Completed `read-models:search-production-repository-unavailable-fail-closed`. Production PostgreSQL `/api/search` now fails closed when the SQL read repository is unavailable, enqueues `api_sql_repository_unavailable`, and does not live scan local state. The next executable boundary is `read-models:search-post-fail-closed-local-implementation-closure-audit`. Go hot-path admission remains blocked.
...
`read-models:search-production-repository-unavailable-fail-closed`
