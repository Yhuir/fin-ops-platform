...
Current state: `autonomous-continue-after-read-models-tax-offset-full-state-read-model-snapshot-quarantine`
...
Completed `read-models:tax-offset-full-state-read-model-snapshot-quarantine`. Broad `Application._persist_state(...)` no longer serializes `tax_offset_read_models`; explicit runtime/executor persistence through `_persist_tax_offset_read_models_best_effort(...)` remains intact, and a static guard prevents the broad full-state writer from returning. `tax_offset` remains implementation-gap-open until a post-quarantine local closure audit re-runs against the current code. The next executable boundary is `read-models:tax-offset-post-full-state-local-implementation-closure-audit`. Go hot-path admission remains blocked.
