...
Current state: `autonomous-continue-after-read-models-bank-account-balance-all-only-scope-contract`
...
Completed `read-models:bank-account-balance-all-only-scope-contract`. `bank_account_balance` gateway scope policy now accepts only `all`, matching the producer/worker/storage all-only contract. The next executable boundary is `read-models:bank-account-balance-operation-barrier-regression`. Go hot-path admission remains blocked.
