...
Current state: `autonomous-continue-after-server-py-workbench-withdraw-link-route-owner-extraction`
...
Completed `server-py:workbench-withdraw-link-route-owner-extraction` as `implementation-closed`. `WorkbenchActionApiRoutes` now owns `/api/workbench/actions/withdraw-link` facade delegation while `Application` keeps HTTP dispatch, JSON body parsing, freshness guard, auth context and response serialization. The next executable boundary is `server-py:workbench-cash-special-route-owner-extraction`.
