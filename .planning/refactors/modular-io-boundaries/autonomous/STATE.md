...
Current state: `autonomous-continue-after-read-models-invoice-lifecycle-derived-lifecycle-executor-port-extraction`
...
Completed `read-models:invoice-lifecycle-derived-lifecycle-executor-port-extraction`. `InvoiceLifecycleDerivedLifecycleExecutor` now owns invoice lifecycle derived lifecycle refresh execution, `Application` only assembles the gateway-backed enqueue callback, and a static guard prevents the old app-owned helper from returning. The next executable boundary is `read-models:invoice-lifecycle-local-implementation-closure-audit`. Go hot-path admission remains blocked.
