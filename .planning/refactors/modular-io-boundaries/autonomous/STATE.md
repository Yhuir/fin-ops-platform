...
Current state: `autonomous-continue-after-read-models-tax-offset-local-implementation-closure-audit`
...
Completed `read-models:tax-offset-local-implementation-closure-audit` as analysis/accounting only. The audit found a remaining local implementation gap: `Application.rebuild_tax_offset_read_model_scope(...)` still owns tax offset worker rebuild, persistence and fresh Redis cache publish behavior. `tax_offset` cannot move to production-evidence-deferred yet. The next executable boundary is `read-models:tax-offset-worker-rebuild-executor-port-extraction`. Go hot-path admission remains blocked.
...
- `tax_offset` is now the eighth non-Go read model implementation pilot after `bank_detail`, `workbench_relation`, `pending_invoice`, `oa_pending_payment`, `input_invoice_usage`, `output_invoice_collection` and `invoice_lifecycle`. Repository port extraction is implemented: `TaxOffsetReadModelRepositoryPort` exposes only manifest-listed load/get/save methods, state-store tax read/write wiring uses the port, the SQL read repository property returns the port over the optional read connection, and tax projection save paths go through the port. Freshness/barrier audit is also implemented: SQL fresh gate, force refresh scope policy, `all` fan-out/month shard proof, plan-save/certified-import operation barrier and legacy/app-owned wrappers are accounted for; OA attachment invoice evidence fallback now promotes formal invoice payloads with `invoice_type` and no `evidence_type`. Local closure audit found a remaining app-owned worker rebuild/cache publish surface in `Application.rebuild_tax_offset_read_model_scope(...)`, so `tax_offset` remains implementation-gap-open. `cost_statistics`, `turnover_ledger`, `no_oa_bank_batch`, `search` and `bank_account_balance` remain implementation-gap-open candidates for later slices.
- The next pending boundary is `read-models:tax-offset-worker-rebuild-executor-port-extraction`, which must move app-owned worker rebuild/persist/fresh-cache publish behavior into an explicit executor/service boundary without changing tax business/API/worker event semantics.
