...
Current state: `autonomous-continue-after-read-models-pending-invoice-refresh-freshness-operation-barrier-audit`
...
Completed `read-models:pending-invoice-refresh-freshness-operation-barrier-audit` as analysis/accounting only. The audit confirmed existing pending invoice fresh gate, source-version stale enqueue, page-first force-refresh scope and worker base-scope expansion coverage, and found that pending invoice scope policy still lacks direction-specific filter allowlist validation. `pending_invoice` remains implementation-gap-open; the next executable boundary is scope policy filter allowlist enforcement.
...
- `pending_invoice` is selected as the next non-Go read model implementation pilot. Repository port extraction is implemented: PostgreSQL runtime read path and pending invoice projection save/mark paths now use `PendingInvoiceReadModelRepositoryPort`, while search index behavior remains on the search repository. Freshness/barrier audit is analysis-closed: rows/filter-options fresh gate, source-version stale enqueue, manifest page-first force refresh and worker base-scope expansion are accounted for, but scope policy filter allowlist enforcement and mutation freshness target contract work remain open.
