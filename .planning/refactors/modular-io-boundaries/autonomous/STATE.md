...
Current state: `autonomous-continue-after-read-models-bank-account-balance-refresh-producer-extraction`
...
Completed `read-models:bank-account-balance-refresh-producer-extraction`. `BankAccountBalanceReadModelRefreshProducer` now owns gateway-backed all-only refresh enqueue, and Application, Bank Details service injection, runtime import-state fan-out, runtime derived lifecycle fan-out and backfill enqueue route through it. The next executable boundary is `read-models:bank-account-balance-derived-lifecycle-executor-extraction`. Go hot-path admission remains blocked.
