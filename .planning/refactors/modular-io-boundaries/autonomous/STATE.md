...
Current state: `autonomous-continue-after-workbench-relations-pending-invoice-unused-pair-service-removal`
...
Completed `workbench-relations:pending-invoice-unused-pair-service-removal`. `workbench_relation` remains implementation-gap-open. Pending invoice query/application services no longer receive pair service injection; reads use `relation_facade` and writes use `relation_command_service`. The next boundary is `workbench-relations:no-oa-pair-service-boundary-audit`.
