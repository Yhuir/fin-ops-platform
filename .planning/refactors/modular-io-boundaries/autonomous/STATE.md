...
Current state: `autonomous-continue-after-workbench-relations-server-workbench-payload-relation-read-port-extraction`
...
Completed `workbench-relations:server-workbench-payload-relation-read-port-extraction`. `workbench_relation` remains implementation-gap-open. Workbench payload/live-row active relation reads now go through `WorkbenchPayloadRelationReadPort`. Source-version, transaction-persist, rollback, repair/precondition and whole-state persistence surfaces remain separate later slices; source-version relation snapshot provider extraction is next because it affects read model freshness.
