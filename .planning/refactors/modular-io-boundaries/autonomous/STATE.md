...
Current state: `turnover-ledger-closure-confirm-route-callback-collapse-selected`
...
Completed `server-py:turnover-ledger-confirm-route-callback-collapse` as `local-implementation-closed` in `analysis/server-py-turnover-ledger-confirm-route-callback-collapse-2026-06-25.md`. `POST /api/turnover-ledger/relations/confirm` now lives in `TurnoverLedgerApiRoutes.route(...)` with an explicit confirm request-boundary port, and the app callback was removed. T0 selected closure confirm route callback collapse next.
