...
Current state: `autonomous-continue-after-read-models-next-pilot-selection-after-cost-statistics`
...
Completed `read-models:next-pilot-selection-after-cost-statistics`. `turnover_ledger` is selected as the tenth non-Go read model implementation pilot because it has high user-visible stale grouped-ledger risk, direct Workbench/cost/search fan-out impact and a narrow manifest-listed repository-port first slice. The next executable boundary is `read-models:turnover-ledger-repository-port-extraction`. Go hot-path admission remains blocked.
...
- `turnover_ledger` is now the tenth non-Go read model implementation pilot. The first narrow implementation boundary is `read-models:turnover-ledger-repository-port-extraction`, which must add a `TurnoverLedgerReadModelRepositoryPort` for manifest-listed `list_turnover_ledger_view`, `save_turnover_ledger_rows` and `clear_turnover_ledger_rows`, wire query/projection paths through it and guard against unrelated read model method exposure.
- `no_oa_bank_batch`, `search` and `bank_account_balance` remain implementation-gap-open candidates for later slices.
