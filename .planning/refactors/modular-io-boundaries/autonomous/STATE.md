...
Current state: `autonomous-continue-after-read-models-tax-offset-cache-warmup-executor-port-extraction`
...
Completed `read-models:tax-offset-cache-warmup-executor-port-extraction`. `TaxOffsetCacheWarmupExecutor` now owns optional cache warmup scheduling/job execution, env gating, month normalization, idempotent background job creation, progress/success reporting, read model upsert and snapshot persistence. `Application._schedule_tax_offset_cache_warmup(...)` remains only a thin delegate, while `_run_tax_offset_cache_warmup_job(...)` and `_tax_offset_cache_warmup_enabled(...)` are removed and guarded from returning. `tax_offset` remains implementation-gap-open until the next final local closure audit re-runs against the current code. The next executable boundary is `read-models:tax-offset-final-local-implementation-closure-audit`. Go hot-path admission remains blocked.
