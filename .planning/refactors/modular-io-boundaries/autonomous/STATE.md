...
Current state: `autonomous-continue-after-read-models-search-refresh-producer-invalidation-service-extraction`
...
Completed `read-models:search-refresh-producer-invalidation-service-extraction`. Search refresh enqueue and invalidation scope normalization are now owned by `SearchReadModelRefreshProducer`; app-owned refresh producer/invalidation helpers were removed and guarded. The next executable boundary is `read-models:search-local-implementation-closure-audit`. Go hot-path admission remains blocked.
...
`read-models:search-refresh-producer-invalidation-service-extraction`
