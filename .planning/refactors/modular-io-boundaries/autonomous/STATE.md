...
Current state: `autonomous-continue-after-read-models-turnover-ledger-repository-port-extraction`
...
Completed `read-models:turnover-ledger-repository-port-extraction`. `TurnoverLedgerReadModelRepositoryPort` now owns the manifest-listed turnover read model list/save/clear methods, PostgreSQL state-store read wiring returns the port, app query injection uses the turnover-specific port instead of the broad workbench SQL read repository, and worker projection save paths receive the port. `turnover_ledger` remains implementation-gap-open. The next executable boundary is `read-models:turnover-ledger-refresh-freshness-operation-barrier-audit`. Go hot-path admission remains blocked.
