...
## 2026-07-02 Pending invoice production closure evidence

- Evidence directory: `.planning/refactors/read-model-performance-optimization/evidence/20260702T083207Z/pending-normalize-release/`.
- Release `pscip-l4-pending-normalize-f9220f5bb6` was uploaded and activated from commit `f9220f5bb6`; the standard deploy script again failed only at the raw `install runtime worker ensure helper` sudo step, after release layout/check-release passed. Activation completed through `sudo -n /usr/local/sbin/finops-deploy-control activate pscip-l4-pending-normalize-f9220f5bb6`.
- Active API/dispatcher/worker WorkingDirectory moved to `/opt/fin-ops/releases/pscip-l4-pending-normalize-f9220f5bb6/src`; local/public `/health/ready` returned ready, and `read-model-scope-contract` exited successfully with no failures.
- Authenticated pending diagnostic SLO passed `6/6`, `30` measured samples, max p95 `420.978ms`. Former failing probes now pass: `pending_rows_expense_page_size_50` p95 `339.341ms`, `pending_rows_expense_page_size_200` p95 `420.978ms`, `pending_rows_expense_requires_invoice` p95 `290.189ms`, and `pending_rows_income_page_size_50` p95 `270.652ms`. All row probes returned status `200`, `read_model_status=fresh`, and `refresh_enqueued_count=0`.
- Authenticated core API SLO passed `12/12`, `60` measured samples, max p95 `876.645ms`. The original blocker `pending_invoices_rows` now passes with p50 `234.893ms`, p95/p99 `307.310ms`, status `200`, `read_model_status=fresh`, and no refresh enqueue.
