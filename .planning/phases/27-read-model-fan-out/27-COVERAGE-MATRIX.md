...
| `workbench/api.ts#printWorkbenchReceipt` | `/api/workbench/actions/print-receipt`; Workbench paired relation | `fact-write` | relation-scoped immutable receipt snapshot, file object, source fingerprint and actor audit; never mutates canonical invoice/relation membership | none; current relation is read directly | idempotent snapshot reuse for the same fingerprint; zero dirty scope, enqueue, barrier or read-model fan-out | `test_workbench_relation_receipt_*`、`WorkbenchApi.test.ts`、`RelationGroupGrid.test.tsx`; production PDF/Browser probe |
