...
| `enqueue-bank-import-withdrawal` | `backend/src/fin_ops_platform/services/bank_import_withdrawal_service.py` | `.enqueue_read_model_refreshes_in_transaction(` | `1` | `retain` | 仅当撤回解除 active Workbench relation 时，在同一事务内提交实际受影响月份的 `workbench_relation` 精确 scope；无关系则零 enqueue |
