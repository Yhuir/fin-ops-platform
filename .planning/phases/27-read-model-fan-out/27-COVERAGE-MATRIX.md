...
| `imports.bank-transactions` | `/imports/bank-transactions` | import session service; `none` | file preview/retry/confirm、manual bank preview/confirm import | preview read-like；confirm explicit batch，受影响页面访问时精确收敛 | `p27-page-import-bank` |
...
| `appHealth/api.ts#withdrawBankTransactionImport` | `/api/imports/bank-transaction-batches/{batch_id}/withdraw`; App Health 导入历史 | `fact-write` | 单批次独占创建的银行流水、工作台关系、导入状态与审计，同一事务提交 | 仅关系实际变化时写 `workbench_relation` 精确月份 | 保留管理员显式撤回；禁止 broad fan-out 和并行旧删除路径 | `test_bank_import_withdrawal_service.py`、`test_app_health_api.py`；生产目标批次撤回 probe |
...
| `etc/api.ts#previewEtcZipFiles` | `/api/etc/import/preview`; import preview | `read-like-command` | transient preview/session only; no canonical business fact | none | no dirty/enqueue/barrier | `test_etc_*`; `p27-op-etc-preview` |
| `etc/api.ts#discardEtcImportSession` | `/api/etc/import/discard`; ETC import clear | `explicit-batch` | current-user unconfirmed ETC preview session only; no canonical business fact | none | atomic preview cleanup; no archive reload、dirty、enqueue or barrier | `test_etc_import_*`; `p27-op-etc-import-discard` |
...
| `imports/api.ts#previewManualInvoices`<br>`imports/api.ts#previewManualInvoicesAtEndpoint`<br>`imports/api.ts#recognizeManualInvoice` | `/imports/invoices/manual/preview|recognize`、`/api/workbench/oa-invoice-supplements/manual/preview`; manual invoice / Workbench invoice entry Drawer | `read-like-command` | OCR prefill and current-user multi-invoice transient import preview session only; endpoint helper is shared but creates no canonical invoice | none until existing confirm path | no dirty/enqueue/barrier; preview reuses the canonical import validator and rejects the whole invalid batch | `test_manual_invoice_entry_*`、`test_workbench_invoice_supplement_*`; `p27-op-manual-invoice-preview` |
| `imports/api.ts#previewManualBankTransactions` | `/imports/bank-transactions/manual/preview`; manual bank transaction entry Drawer | `read-like-command` | current-user multi-transaction transient import preview session only; no canonical bank fact | none until existing confirm path | no dirty/enqueue/barrier; preview reuses canonical bank import validation and rejects the whole invalid batch | `test_manual_bank_transaction_entry_*`、`test_import_file_api.py`、`web/src/test/ImportsApi.test.ts`; `p27-op-manual-bank-preview` |
| `imports/api.ts#discardImportSession` | `/imports/files/discard`; import pages | `explicit-batch` | current-user unconfirmed preview session/batch lifecycle only; no canonical business fact | none | atomic preview cleanup; no dirty/enqueue/barrier | `test_import_*`; `p27-op-import-discard` |
...
| `workbench/api.ts#previewWorkbenchConfirmLink`<br>`workbench/api.ts#previewWorkbenchWithdrawLink` | `/api/workbench/actions/**/preview`; Workbench dialogs | `read-like-command` | preview/candidate/conflict DTO only | none | no dirty/enqueue | `test_workbench_*`; `p27-op-workbench-preview` |
| `workbench/api.ts#cancelWorkbenchCashSpecial`<br>`workbench/api.ts#cancelWorkbenchLink`<br>`workbench/api.ts#confirmWorkbenchCashPassThrough`<br>`workbench/api.ts#confirmWorkbenchCashTicketPurchase`<br>`workbench/api.ts#confirmWorkbenchLink`<br>`workbench/api.ts#confirmWorkbenchPersonalAdvanceRepayment`<br>`workbench/api.ts#reviewWorkbenchAnomaly`<br>`workbench/api.ts#withdrawWorkbenchLink` | `/api/workbench/actions/**`, `/api/workbench/exceptions/review`; Workbench | `fact-write` | canonical relation/review/cash facts + case/row ids/months/generation precondition | `workbench` visible scope; all other consumers on access | UoW commit + exact active generation reconcile；delete downstream ordinary fan-out | `test_workbench_*`; `p27-op-workbench-fact` |
| `workbench/api.ts#confirmWorkbenchManualInvoiceSupplement` | `/api/workbench/oa-invoice-supplements/manual`; Workbench invoice entry Drawer | `fact-write` | complete manual invoice batch + canonical invoice facts + exact OA expense-item source link + canonical relation delta in one transaction | Workbench direct query and invoice consumers on access | one atomic import/relation commit; current page reruns canonical GET; zero read-model enqueue or cross-page fan-out | `test_workbench_invoice_supplement_*`; production workbench read probe |
| `workbench/api.ts#assignWorkbenchInvoiceExpenseItems` | `/api/workbench/actions/assign-invoice-expense-items`; Workbench invoice assignment Drawer | `fact-write` | one relation invoice's explicit OA expense-item source edges + idempotency/audit; relation topology and canonical amounts unchanged | Workbench direct query only; other consumers observe canonical source links on access | relation/member/item/fingerprint validation + source-links CAS in one UoW; current page performs one canonical GET; zero read-model enqueue or cross-page fan-out | `test_workbench_invoice_expense_item_assignment_*`、`test_workbench_query_postgres_integration.py`; Workbench Browser flow |
| `workbench/api.ts#uploadWorkbenchOaSupportingDocuments`<br>`workbench/api.ts#deleteWorkbenchOaSupportingDocument` | `/api/workbench/oa-invoice-supplements/documents**`; Workbench invoice entry Drawer | `fact-write` | exact OA row/expense-item supporting document metadata + object storage; explicitly outside canonical invoice pool | Workbench direct query only | upload/delete compensates partial storage failure and reruns current canonical GET; zero invoice import, read-model enqueue or cross-page fan-out | `test_workbench_oa_supporting_document_*`; production workbench read probe |
| `workbench/api.ts#fetchWorkbenchReceiptDraft` | `/api/workbench/actions/receipt-draft`; Workbench receipt Drawer | `read-like-command` | current active relation、receipt groups、source fingerprint and exact reversal diagnostics only；no durable mutation | none; current relation is read directly | permission-gated direct read；zero audit/dirty/enqueue/barrier/read-model fan-out | `test_workbench_relation_receipt_*`、`WorkbenchApi.test.ts`、`WorkbenchReceiptDrawer.test.tsx` |
| `workbench/api.ts#printWorkbenchReceipt` | `/api/workbench/actions/print-receipt`; Workbench paired/unpaired active relation | `fact-write` | edited relation-scoped immutable receipt snapshot, file object, source/document fingerprints and actor audit; never mutates canonical invoice/relation membership | none; current relation is read directly | relation version/source/amount revalidation and idempotent snapshot reuse for the same edited document; zero dirty scope, enqueue, barrier or read-model fan-out | `test_workbench_relation_receipt_*`、`WorkbenchApi.test.ts`、`RelationGroupGrid.test.tsx`、`WorkbenchReceiptDrawer.test.tsx`; production PDF/Browser probe |
...
| `web/src/components/imports/ManualBankTransactionEntryDrawer.tsx` | bank transaction import | `mixed` | `previewManualBankTransactions` is read-like; `discardImportSession`/`confirmImportFiles` use existing explicit import-session/batch boundaries | read-export hides the opener；full access previews transient rows and confirms only through the unified bank pool |
...
| `web/src/components/workbench/WorkbenchInvoiceAssignmentDrawer.tsx` | workbench | `writable` | explicit selection of one or more same-relation OA expense items for one unassigned invoice | opener is permission-gated; submit uses stable idempotency key and one canonical reread, with no local row movement |
| `web/src/components/workbench/WorkbenchInvoiceEntryDrawer.tsx` | workbench | `mixed` | supporting document upload/delete + multi-invoice preview/atomic confirm-and-associate | one Drawer with internal mode switch; supporting documents stay outside invoice pool, manual invoices enter the pool and exact OA expense-item relation atomically |
| `web/src/components/workbench/WorkbenchReceiptDrawer.tsx` | workbench | `mixed` | receipt draft read-like + edited PDF snapshot/print-request write | opener is permission-gated；draft has no mutation/audit；print revalidates relation/source/amount and writes only immutable file/snapshot/audit, never relation/invoice/read-model facts |
...
这里登记当前保留的显式 derived-data lifecycle 入口。所有 read model enqueue 与 frontend freshness barrier 已删除。
