...
| `cost-statistics/api.ts#saveCostStatisticsManualAllocation` | `/api/cost-statistics/manual-allocations`; 配对金额人工分配 Popover | `fact-write` | relation-scoped project allocation rows + CAS version | none；成本统计直接查询 allocation fact | 保存后仅重取当前成本归因视图，不 rebuild read model | `test_cost_statistics_*`; `p27-op-cost-manual-allocation` |
