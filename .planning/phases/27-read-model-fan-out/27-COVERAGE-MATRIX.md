...
| `imports/api.ts#previewManualInvoice`<br>`imports/api.ts#recognizeManualInvoice` | `/imports/invoices/manual/preview|recognize`; manual invoice Drawer | `read-like-command` | OCR prefill and current-user transient import preview session only; no canonical invoice | none until existing confirm path | no dirty/enqueue/barrier; preview reuses the canonical import validator | `test_manual_invoice_entry_*`; `p27-op-manual-invoice-preview` |
