...
- 自动候选/decision 不再是持久化、API 或 UI 状态；旧表、service/store/engine、repair CLI、前端组件和分支已删除。
...
- migration 0104 forward-drop `read_model.workbench_candidate_matches`、`read_model.workbench_reconciliation_decisions` 与精确旧 app-setting；不修改 canonical facts 或 active relations。
...
- 当前环境未设置 `FIN_OPS_TEST_DATABASE_URL`，因此没有执行真实 disposable PostgreSQL 0001–0104 migration/catalog/data-hash 集成；相关 opt-in tests 属于本次 full suite 的 skip 集合。
- 共享 worktree 含大量与本任务重叠或无关的既有改动，不能从当前 dirty main 安全部署或证明 release 只包含本任务变化。
...
3. 仅通过 `./scripts/deploy-oa.sh` 发布并执行 migration 0104；确认旧表/catalog objects 消失且 canonical/relation hashes 不变。
...
5. 运行对象 identity、Workbench page Audit 和 System Audit；证明 520 paired、13 identities 各自恰好一次 unpaired、`P=R`、`U=C-R`、无 overlap/omission、下游 linked/unlinked 一致。
6. 只有上述证据全部通过，才能把 Phase 21 与用户任务标记为 complete。
