...
def build_deploy_control_contract_check() -> str:
    return "\n".join(
        [
            ...
            "      exit 68",
            "    fi",
            "    if ! grep -q 'install_workbench_generation_retention' \"$DEPLOY_CONTROL\"; then",
            "      printf '%s\\n' 'deploy-control helper does not install versioned Workbench generation retention; install deploy/oa/bin/finops-deploy-control.sh before activating releases' >&2",
            "      exit 68",
            "    fi",
