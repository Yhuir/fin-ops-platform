...
def build_deploy_control_contract_check() -> str:
    return "\n".join(
        [
            ...
            "      printf '%s\\n' 'deploy-control helper does not install versioned OA sync enqueue timer; install deploy/oa/bin/finops-deploy-control.sh before activating releases' >&2",
            "      exit 68",
            "    fi",
...
def build_runtime_worker_ensure_contract_check() -> str:
    return "\n".join(
        [
            ...
            "    exit 68",
            "  fi",
            "  if ! grep -q 'migrate_retired_worker_runtime_env' \"$RUNTIME_WORKER_ENSURE\"; then",
            "    printf '%s\\n' 'runtime worker ensure helper does not retire per-worker RabbitMQ and Redis overrides; install deploy/oa/bin/finops-ensure-runtime-workers.sh before activating releases' >&2",
            "    exit 68",
            "  fi",
